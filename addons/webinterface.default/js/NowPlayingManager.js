...
NowPlayingManager.prototype = {
  ...
  updatePlayer: function() {
    xbmc.rpc.request({
      ...
      'params': '{"playerid":' + this.activePlayerId + ', "properties":["playlistid","speed","position","totaltime","time"]}',
      'success': function(data) {
        if (data && data.result)
        {
  ...
  nextTrack: function() {
    if (this.activePlayer) {
      xbmc.rpc.request({
        'method': 'Player.GoTo',
        'params': '{ "playerid": ' + this.activePlayerId + ', "to": "next" }',
        'success': function() {}
      });
    }
  },
  prevTrack: function() {
    if (this.activePlayer) {
      xbmc.rpc.request({
        'method': 'Player.GoTo',
        'params': '{ "playerid": ' + this.activePlayerId + ', "to": "previous" }',
        'success': function() {}
      });
    }
  },
  stopTrack: function() {
    if (this.activePlayer) {
      xbmc.rpc.request({
        'context': this,
        'method': 'Player.Stop',
        'params': '{ "playerid": ' + this.activePlayerId + ' }',
        'success': function(data) {
  ...
  playPlaylistItem: function(sender) {
    ...
    if (!this.activePlaylistItem || (this.activePlaylistItem !== undefined && sequenceId != this.activePlaylistItem.seq)) {
      xbmc.rpc.request({
        'method': 'Player.GoTo',
        'params': '{ "playerid": ' + this.activePlayerId + ', "to": ' + sequenceId + '}',
        'success': function() {}
      });
    }
    this.hidePlaylist();
  ...
  updateAudioPlaylist: function() {
    xbmc.rpc.request({
      ...
      'params': '{"playlistid":' + this.playlistid + ', "properties":["title","album","artist","duration","thumbnail"]}',
      'success': function(data) {
  ...
  updateVideoPlaylist: function() {
    xbmc.rpc.request({
      'context': this,
      'method': 'Playlist.GetItems',
      'params': '{"playlistid":' + this.playlistid + ', "properties":["title","season","episode","plot","runtime","showtitle","thumbnail"]}',
      'success': function(data) {
        if (data && data.result && data.result.items && data.result.items.length > 0 && data.result.limits.total > 0) {
          ...
          xbmc.rpc.request({
			...
			'params': '{"playerid":' + this.playlistid + ', "properties":["title","season","episode","plot","runtime","showtitle","thumbnail"]}',
            'success': function(data) {
