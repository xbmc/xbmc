...
MediaLibrary.prototype = {
    ...
    if (player >= 0)
    {
      switch(keyPressed) {
        ...
        case 'next':
          return xbmc.rpc.request({
            'method': 'Player.GoTo',
            'params': '{"playerid":' + player + ', "to":"next"}'
          });
        case 'previous':
          return xbmc.rpc.request({
            'method': 'Player.GoTo',
            'params': '{"playerid":' + player + ', "to":"previous"}'
          });
        case 'forward':
          return xbmc.rpc.request({
            'method': 'Player.SetSpeed',
            'params': '{"playerid":' + player + ', "speed":"increment"}'
          });
        case 'rewind':
          return xbmc.rpc.request({
            'method': 'Player.SetSpeed',
            'params': '{"playerid":' + player + ', "speed":"decrement"}'
          });
      }
    }
  },
  musicLibraryOpen: function(event) {
    ...
    if (!libraryContainer || libraryContainer.length == 0) {
      ...
      xbmc.rpc.request({
        ...
        'params': '{ "limits": { "start": 0 }, "properties": ["description", "theme", "mood", "style", "type", "albumlabel", "artist", "genre", "rating", "title", "year", "thumbnail"], "sort": { "method": "artist" } }',
        'success': function(data) {
  ...
  displayAlbumDetails: function(event) {
    ...
    if (!albumDetailsContainer || albumDetailsContainer.length == 0) {
      ...
      xbmc.rpc.request({
        ...
        'params': '{ "properties": ["title", "artist", "genre", "track", "duration", "year", "rating", "playcount"], "filter": { "albumid" : ' + event.data.album.albumid + ' } }',
        'success': function(data) {
  ...
  displayTVShowDetails: function(event) {
    ...
    if (!tvshowDetailsContainer || tvshowDetailsContainer.length == 0) {
      ...
      xbmc.rpc.request({
        ...
        'params': '{ "properties": [ "season", "showtitle", "playcount", "episode", "thumbnail","fanart" ], "tvshowid" : ' + event.data.tvshow.tvshowid + ' }',
        'success': function(data) {
  ...
  displaySeasonListings: function(event) {
    ...
    xbmc.rpc.request({
      ...
      'params': '{ "properties": [ "title", "thumbnail","episode","plot","season"], "season" : ' + seasons[selectedVal].season + ', "tvshowid" : ' + event.data.tvshow + ' }',
      'success': function(data) {
  ...
  playTVShow: function(event) {
    xbmc.rpc.request({
      'context': this,
      'method': 'Player.Open',
      'params': '{ "item": { "episodeid": ' + event.data.episode.episodeid + ' } }',
      'success': function(data) {
        this.hideOverlay();
      }
    });
  ...
  playMovie: function(event) {
    xbmc.rpc.request({
      'context': this,
      'method': 'Player.Open',
      'params': '{ "item": { "movieid": ' + event.data.movie.movieid + ' } }',
      'success': function(data) {
        this.hideOverlay();
      }
    });
  ...
  playTrack: function(event) {
    xbmc.rpc.request({
      ...
      'params': '{ "playlistid": ' + this.playlists["audio"] + ' }',
      'success': function(data) {
        xbmc.rpc.request({
          'context': this,
          'method': 'Playlist.Add',
          'params': '{ "playlistid": ' + this.playlists["audio"] + ', "item": { "albumid": ' + event.data.album.albumid + ' } }',
          'success': function(data) {
            xbmc.rpc.request({
              'method': 'Player.Open',
              'params': '{ "item": { "playlistid": ' + this.playlists["audio"] + ', "position": '+ event.data.itmnbr + ' } }',
              'success': function() {}
            });
          }
        });
  ...
  movieLibraryOpen: function() {
    ...
    if (!libraryContainer || libraryContainer.length == 0) {
      ...
      xbmc.rpc.request({
        ...
        'params': '{ "limits": { "start": 0 }, "properties": [ "genre", "director", "trailer", "tagline", "plot", "plotoutline", "title", "originaltitle", "lastplayed", "runtime", "year", "playcount", "rating", "thumbnail", "file" ], "sort": { "method": "sorttitle", "ignorearticle": true } }',
        'success': function(data) {
  ...
  tvshowLibraryOpen: function() {
    ...
    if (!libraryContainer || libraryContainer.length == 0) {
      ...
      xbmc.rpc.request({
        ...
        'params': '{ "properties": ["genre", "plot", "title", "lastplayed", "episode", "year", "playcount", "rating", "thumbnail", "studio", "mpaa", "premiered"] }',
        'success': function(data) {
  ...
  startSlideshow: function(event) {
    xbmc.rpc.request({
      'method': 'Player.Open',
      'params': '{ "item": { "recursive" : "true", "random": "true", "path" : "' + this.replaceAll(event.data.directory.file, "\\", "\\\\") + '" } }',
      'success': function() {}
    });
  },
  showDirectory: function(event) {
    ...
    if (!libraryContainer || libraryContainer.length == 0) {
      ...
      xbmc.rpc.request({
        ...
        'params': '{ "media" : "pictures", "directory": "' + jsonDirectory + '" }',
        'success': function(data) {
