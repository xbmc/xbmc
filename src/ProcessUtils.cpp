...
int ProcessUtils::runSync(const std::string& executable, const std::list<std::string>& args)
{
#ifdef PLATFORM_UNIX
...
#ifdef PLATFORM_UNIX
int ProcessUtils::runSyncUnix(const std::string& executable, const std::list<std::string>& args)
{
...
void ProcessUtils::runAsync(const std::string& executable, const std::list<std::string>& args)
{
#ifdef PLATFORM_WINDOWS
...
int ProcessUtils::runElevated(const std::string& executable, const std::list<std::string>& args,
...
bool ProcessUtils::waitForProcess(PLATFORM_PID pid)
{
  ...
  {
    LOG(Error, "Unable to get process handle for pid " + intToStr(pid) + " last error " +
...
int ProcessUtils::runElevatedLinux(const std::string& executable,
                                   const std::list<std::string>& args, const std::string& _task)
{
  ...
  for (unsigned int i = 0; i < sudos.size(); i++)
  {
    ...
    if (sudoBinary == "kdesudo")
    {
      ...
      std::string sudoMessage =
      task + " needs administrative privileges.  Please enter your password.";
...
#ifdef PLATFORM_MAC
int ProcessUtils::runElevatedMac(const std::string& executable, const std::list<std::string>& args)
{
...
std::string quoteArgs(const std::list<std::string>& arguments)
{
  ...
  for (std::list<std::string>::const_iterator iter = arguments.begin(); iter != arguments.end();
       iter++)
  {
    std::string arg = *iter;

    bool isQuoted = !arg.empty() && arg.at(0) == '"' && arg.at(arg.size() - 1) == '"';
...
#ifdef PLATFORM_WINDOWS
int ProcessUtils::runWindows(const std::string& _executable, const std::list<std::string>& _args,
                             RunMode runMode)
{
  ...
  executable.c_str(), commandLineStr, 0 /* process attributes */, 0 /* thread attributes */,
  false /* inherit handles */, NORMAL_PRIORITY_CLASS /* creation flags */, 0 /* environment */,
  0 /* current directory */, &startupInfo /* startup info */, &processInfo /* process information */
  );

  if (!result)
  {
    LOG(Error, "Failed to start child process. " + executable + " Last error: " +
               intToStr(GetLastError()));
    return RunFailed;
  }
  else
...
void ProcessUtils::convertWindowsCommandLine(LPCWSTR commandLine, int& argc, char**& argv)
{
  ...
  for (int i = 0; i < argc; i++)
  {
    ...
    int length = WideCharToMultiByte(CP_ACP, 0 /* flags */, argvUnicode[i],
                                     -1, /* argvUnicode is null terminated */
                                     buffer, BUFFER_SIZE, 0, false);
