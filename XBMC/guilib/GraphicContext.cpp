...
int CGraphicContext::BlitToScreen(SDL_Surface *src, float *x, float *y, float *u, float *v, DWORD *c)
{
  // renders the surface from u[0],v[0] -> u[1],v[1] into the parallelogram defined by x[0],y[0]...x[3],y[3]
  // run through the surface, and map our surface coordinates back to texture coordinates within our texture
  ...
  int minX = (int)x[0], maxX = (int)x[0]+1;
  int minY = (int)y[0], maxY = (int)y[0]+1;
  for (int i = 1; i < 4; ++i)
  {
    if (x[i] < minX) minX = (int)x[i];
    if (x[i]+1 > maxX) maxX = (int)x[i]+1;
    if (y[i] < minY) minY = (int)y[i];
    if (y[i]+1 > maxY) maxY = (int)y[i]+1;
  }
  if (minY < 0) minY = 0;
  if (minX < 0) minX = 0;
  if (maxY >= m_screenSurface->h) maxY = m_screenSurface->h - 1;
  if (maxX >= m_screenSurface->w) maxX = m_screenSurface->w - 1;
  ...
  for (int sy = minY; sy <= maxY; ++sy)
  {
    for (int sx = minX; sx <= maxX; ++sx)
    {
      ...
      float tu = m[0][0] * (sx - x[0] + 0.5f) + m[0][1] * (sy - y[0] + 0.5f) + u[0];
      float tv = m[1][0] * (sx - x[0] + 0.5f) + m[1][1] * (sy - y[0] + 0.5f) + v[0];
      if (tu > u[0]-0.5f && tu < u[1]+0.5f && tv > v[0]-0.5f && tv < v[1]+0.5f)
      { // in the texture - render it to screen
        BYTE *screen = (BYTE *)m_screenSurface->pixels + sy * m_screenSurface->pitch + sx*4;
#ifdef SDL_BILINEAR_FILTER
...
#else
        // grab nearest pixel instead
        int pu = (int)(tu + 0.5f);
        int pv = (int)(tv + 0.5f);
        CLAMP(pu,0,src->w-1);
        CLAMP(pv,0,src->h-1);
        BYTE *tex = (BYTE *)src->pixels + pv * src->pitch + pu * 4;
#endif
        BYTE *diffuse = (BYTE *)c;
        BYTE alpha = MODULATE(tex[3], diffuse[3]);
        screen[0] = (BYTE)ALPHA_BLEND(MODULATE(tex[0], diffuse[0]), screen[0], alpha);
        screen[1] = (BYTE)ALPHA_BLEND(MODULATE(tex[1], diffuse[1]), screen[1], alpha);
        screen[2] = (BYTE)ALPHA_BLEND(MODULATE(tex[2], diffuse[2]), screen[2], alpha);
        screen[3] = 255;
      }
    }
  }
