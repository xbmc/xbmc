...
void CVideoDatabase::UpdateFanart(const CFileItem &item, VIDEODB_CONTENT_TYPE type)
{
  ...
  if (type == VIDEODB_CONTENT_TVSHOWS)
    exec = FormatSQL("UPDATE tvshow set c%02d=%i WHERE idshow=%i", VIDEODB_ID_TV_FANART, item.GetVideoInfoTag()->m_fanart.m_xml, item.GetVideoInfoTag()->m_iDbId);
  else if (type == VIDEODB_CONTENT_MOVIES)
    exec = FormatSQL("UPDATE movie set c%02d=%i WHERE idmovie=%i", VIDEODB_ID_FANART, item.GetVideoInfoTag()->m_fanart.m_xml, item.GetVideoInfoTag()->m_iDbId);
