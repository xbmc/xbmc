/*
 *	emfdb.cpp -- EMF database compatability functions for WebServer.
 *	@copy	default.g
 *	
 *	Copyright (c) Mbedthis Software LLC, 2003-2007. All Rights Reserved.
 *	Portions Copyright (c) GoAhead Software, 1995-2000. All Rights Reserved.
 *	
 *	Use of this software is only permissible under the terms of the Mbedthis 
...
#if BLD_FEATURE_GACOPMPAT_MODULE
#include	"http.h"
#include	"compatApi.h"
 ...
 */
	dbListPtr = &dbListTables;
	tid = hAllocEntry((void***) dbListPtr, &dbMaxTables, sizeof(dbTable_t));
 ...
 */
	if (pTable->nColumns > 0) {
		int i;
		pTable->columnNames = 
			(char**) balloc(B_L, sizeof(char_t *) * pTable->nColumns);
		pTable->columnTypes = 
			(int*) balloc(B_L, sizeof(int *) * pTable->nColumns);
 ...
 */
	fmtAlloc(&tmpFile, MPR_MAX_FNAME, T("%s/data.tmp"), basicGetProductDir());
	if ((fd = gopen(tmpFile, 
 ...
 */
	if (nRet == 0) {
		fmtAlloc(&path, MPR_MAX_FNAME, T("%s/%s"), basicGetProductDir(), 
			filename);
...
int dbLoad(int did, char_t *filename, int flags)
{
	...
	char_t		*tablename, *lastTok;
	int			fd, tid, row;
	dbTable_t	*pTable;

	a_assert(did >= 0);

	fmtAlloc(&path, MPR_MAX_FNAME, T("%s/%s"), basicGetProductDir(), filename);
 ...
 */
	buf = (char*) balloc(B_L, sbuf.st_size + 1);
#if CE
