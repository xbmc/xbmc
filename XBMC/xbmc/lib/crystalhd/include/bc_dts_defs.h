...
/*------------------------------------------------------*
 *    Video Algorithm for DtsSetVideoParams()			*
 *------------------------------------------------------*/
...
typedef struct _BC_PIC_INFO_BLOCK {
   /* Common fields. */
	...
	U32     picture_number;		/* Ordinal display number  */
	U32     width;				/* pixels            */
	U32     height;				/* pixels            */
	...
	U32		sess_num;
	U32     ycom;
    U32     custom_aspect_ratio_width_height;
    U32     n_drop;             /* number of non-reference frames remaining to be dropped*/
...
#else
typedef BC_STATUS (*dts_pout_callback)(void  *shnd, U32	width,U32 height, U32 stride, struct _BC_DTS_PROC_OUT *pOut);
#endif
...
typedef struct _BC_DTS_PROC_OUT{
	...
	U8						*UVbuff;		/* Caller Supplied buffer for UV data */
	U32						UVbuffSz;		/* Caller Supplied UV buffer size */
	...
	//User Data
	U32						UserDataSz;
	U8						UserData[MAX_UD_SIZE];

	void					*hnd;
	dts_pout_callback		AppCallBack;
	U8						DropFrames;
	U8						b422Mode;		/* Picture output Mode */
	U8						bPibEnc;			/* PIB encrypted */
	U8						bRevertScramble;
...
typedef struct _BC_DTS_STATUS {
    ...
    U8          FreeListCount;              /* Number of frame buffers free.  (reported by driver) */
    U8          PowerStateChange;	    /* Number of active state power transitions (reported by driver) */
    U8          reserved_[1];

    U32         FramesDropped;              /* Number of frames dropped.  (reported by DIL) */
    U32         FramesCaptured;             /* Number of frames captured. (reported by DIL) */
    U32         FramesRepeated;             /* Number of frames repeated. (reported by DIL) */

    U32         InputCount;                 /* Times compressed video has been sent to the HW.
                                               i.e. Successful DtsProcInput() calls (reported by DIL) */
    U64         InputTotalSize;             /* Amount of compressed video that has been sent to the HW.
                                               (reported by DIL) */
    U32         InputBusyCount;             /* Times compressed video has attempted to be sent to the HW
                                               but the input FIFO was full. (reported by DIL) */

    U32         PIBMissCount;               /* Amount of times a PIB is invalid. (reported by DIL) */

	U32			cpbEmptySize;				/* supported only for H.264, specifically changed for Adobe. Report size of CPB buffer available.
												Reported by DIL */
	U64			NextTimeStamp;				/* TimeStamp of the next picture that will be returned by a call to ProcOutput. Added for Adobe. Reported
