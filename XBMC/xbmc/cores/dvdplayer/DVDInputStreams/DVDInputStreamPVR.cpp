...
bool CDVDInputStreamPVR::Open(const char* strFile, const std::string& content)
{
...
int CDVDInputStreamPVR::Read(BYTE* buf, int buf_size)
{
  if(!m_pFile) return -1;
...
__int64 CDVDInputStreamPVR::Seek(__int64 offset, int whence)
{
  if(!m_pFile) return -1;
...
__int64 CDVDInputStreamPVR::GetLength()
{
  if(!m_pFile) return -1;
...
int CDVDInputStreamPVR::GetStartTime()
{
  if (m_pLiveTV)
...
bool CDVDInputStreamPVR::NextChannel()
{
  if (m_pLiveTV)
...
bool CDVDInputStreamPVR::PrevChannel()
{
  if (m_pLiveTV)
...
bool CDVDInputStreamPVR::UpdateItem(CFileItem& item)
{
  if (m_pLiveTV)
...
bool CDVDInputStreamPVR::SeekTime(int iTimeInMsec)
{
  return false;
}
...
bool CDVDInputStreamPVR::CanRecord()
{
  if (m_pRecordable)
...
bool CDVDInputStreamPVR::IsRecording()
{
  if (m_pRecordable)
...
bool CDVDInputStreamPVR::Record(bool bOnOff)
{
  if (m_pRecordable)
