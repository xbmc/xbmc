...
timemapdone:
  ...
  /* find cell the time belongs too */
  for(cell_nr = first_cell_nr; (cell_nr <= last_cell_nr); cell_nr ++) {
    ...
    if (time >= length) {
      ...
      if( target < 0 )
      { /* sector wasn't found using time map, use cell interpolation */
  ...
  if(found) {
    ...
    /* based from the cell found from adding up time, make sure sector is within it */
    if( cell->first_sector < target || target > cell->last_sector )
    {
      #ifdef LOG_DEBUG    
      fprintf(MSG_OUT, "libdvdnav: time_search - sector is not within cell");
      #endif
    }
