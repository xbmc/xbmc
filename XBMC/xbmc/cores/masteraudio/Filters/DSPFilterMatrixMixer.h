...
#ifndef __DSPFILTER_MATRIX_MIXER_H__
#define __DSPFILTER_MATRIX_MIXER_H__
...
class CDSPFilterMatrixMixer : public CDSPFilterLPCM
{
public:
  CDSPFilterMatrixMixer();
  virtual ~CDSPFilterMatrixMixer();
...
protected:
  struct MixRoute
  {
    unsigned char m_FromIndex;
    unsigned char m_ToIndex;
    float m_Coeff;
  };

  void UpdateRouteList();
  int* GetChannelLayout(CStreamDescriptor* pDesc);
  int* m_pInputLayout;
  int* m_pOutputLayout;
  MixRoute* m_pRouteList;
  unsigned int m_RouteCount;
  ma_audio_container* m_pInputContainer;
};

#endif // __DSPFILTER_MATRIX_MIXER_H__