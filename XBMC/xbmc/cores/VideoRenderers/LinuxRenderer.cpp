...
void CLinuxRenderer::ReleaseImage(int source, bool preserve)
{
  ...
  unsigned char_t *src[] = { m_image.plane[0], m_image.plane[1], m_image.plane[2] };
  int     srcStride[] = { m_image.stride[0], m_image.stride[1], m_image.stride[2] };
  unsigned char_t *dst[] = { (unsigned char_t*)m_backbuffer->pixels, 0, 0 };
...
for (int n=0; n<720*90;n++) {
   *(((unsigned char_t*)m_backbuffer->pixels) + (720*10) + n) = 70;
}
