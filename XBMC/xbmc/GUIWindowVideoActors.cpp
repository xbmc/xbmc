    ...
    case GUI_MSG_CLICKED:
    {
      ...
      if (iControl==CONTROL_BTNVIEWASICONS)
      {
        bool bLargeIcons(false);
		    if ( m_strDirectory.IsEmpty() )
        {
		      g_stSettings.m_iMyVideoActorRootViewAsIcons++;
          if (g_stSettings.m_iMyVideoActorRootViewAsIcons > VIEW_AS_LARGEICONS) g_stSettings.m_iMyVideoActorRootViewAsIcons=VIEW_AS_LIST;
        }
		    else
        {
		      g_stSettings.m_iMyVideoActorViewAsIcons++;
          if (g_stSettings.m_iMyVideoActorViewAsIcons > VIEW_AS_LARGEICONS) g_stSettings.m_iMyVideoActorViewAsIcons=VIEW_AS_LIST;
        }
        ShowThumbPanel();
...
void CGUIWindowVideoActors::UpdateButtons()
{
  ...
  int iString;
	if ( m_strDirectory.IsEmpty() ) 
  {
		switch (g_stSettings.m_iMyVideoActorRootViewAsIcons)
    {
      case VIEW_AS_LIST:
  ...
  {
		switch (g_stSettings.m_iMyVideoActorViewAsIcons)
    {
      case VIEW_AS_LIST:
        iString=100; // view as icons
      break;
