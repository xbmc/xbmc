...
template<class TheDll, typename TheStruct, typename Props>
void CAddonDll<TheDll, TheStruct, Props>::UpdateSetting(const CStdString &key, const void* value)
{
  try
  {
    m_pDll->SetSetting(key.c_str(), value);
  }
  catch (std::exception &e)
  {
    CLog::Log(LOGERROR, "ADDON:: %s - exception '%s' during SetSetting occurred, contact Developer '%s' of this AddOn", Name().c_str(), e.what(), Author().c_str());
  }
}
...
ADDON_STATUS CAddonDll<TheDll, TheStruct, Props>::TransferSettings()
{
  ...
  CLog::Log(LOGDEBUG, "Calling TransferAddonSettings for: %s", Name().c_str());

  TiXmlElement *setting = m_addonXmlDoc.FirstChildElement("setting");
  while (setting)
  {
