...
// benchmark:
double video_time_usage=0.0f;
double vout_time_usage=0.0f;
double audio_time_usage=0.0f;
...
static __int64 seek_to_byte=0;
static __int64 step_sec=0;
...
static void exit_sighandler(int x)
{
    ...
    ++sig_count;
    if(sig_count==5 || (inited_flags==0 && sig_count>1))
        exit(1);
    if(sig_count>5)
    {
        // can't stop :(
...
static int libmpdemux_was_interrupted(int iEOF)
{
    mp_cmd_t* cmd;
    if((cmd = mp_input_get_cmd(0,0)) != NULL)
    {
        ...
        case MP_CMD_PLAY_TREE_STEP:
            {
                iEOF = (cmd->args[0].v.i > 0) ? PT_NEXT_ENTRY : PT_PREV_ENTRY;
            }
            break;
        case MP_CMD_PLAY_TREE_UP_STEP:
            {
                iEOF = (cmd->args[0].v.i > 0) ? PT_UP_NEXT : PT_UP_PREV;
            }
            break;
        case MP_CMD_PLAY_ALT_SRC_STEP:
            {
                iEOF = (cmd->args[0].v.i > 0) ?  PT_NEXT_SRC : PT_PREV_SRC;
            }
            break;
        }
        mp_cmd_free(cmd);
                         ...
                         )
{
    ...
    if (use_gui)
    {
        ...
        {
            import_playtree_playlist_into_gui(entry
                                              , mconfig);
            play_tree_free_list(entry
                                ,1);
        }
    }
    else
#endif
    {
          ...
          )
        {
            ...
            play_tree_remove(entry
                             ,1,1);
            return PT_NEXT_SRC;
        }
        play_tree_insert_entry
        (playtree_iter->tree,entry
        );
        play_tree_set_params_from(entry
                                  ,playtree_iter->tree);
        entry
        = playtree_iter->tree;
        if(play_tree_iter_step(playtree_iter,1,0) != PLAY_TREE_ITER_ENTRY)
...
play_next_file:
    ...
    { // error...
        ...
        iEOF = libmpdemux_was_interrupted(PT_NEXT_ENTRY);
    ...
    if(file_format == DEMUXER_TYPE_PLAYLIST)
    {
        ...
        entry
        = parse_playtree(stream,0);
        iEOF=playtree_add_playlist(entry
                                  );
        return 0; // goto goto_next_file;
    }
...
goto_enable_cache:
    ...
    if(stream_cache_size>0)
    {
        ...
        if(!stream_enable_cache(stream,stream_cache_size*1024,stream_cache_size*1024/5,stream_cache_size*1024/20))
            if((iEOF = libmpdemux_was_interrupted(PT_NEXT_ENTRY)))
    ...
    if (demuxer)
    {
        //printf("step10 demux type:%i\n",demuxer->type);
    }
    else
    {
        //printf("step10 no demux\n");
    }
    ...
    if (demuxer && demuxer->type==DEMUXER_TYPE_PLAYLIST)
    {
        ...
        ;
        play_tree_t *list = NULL, *entry
                            = NULL;

        current_module="handle_demux_playlist";
        while (ds_get_packet(demuxer->video,&playlist_entry
                            )>0)
        {
            char *temp, *bname;
            ...
            if (filename && !strcmp(mp_basename(playlist_entry
                                               ),playlist_entry
                                   )) // add reference path of current file
            {
                temp=malloc((strlen(filename)-strlen(mp_basename(filename))+strlen(playlist_entry
                                                                                  )+1)*sizeof(char));
                if (temp)
                {
    ...
    if(sh_video)
    {
        ...
        if(sub_auto)
        {
          printf("auto load sub file\n");
            // auto load sub file ...
            char **tmp = sub_filenames(NULL, filename);
            char **tmp2 = tmp;
            while (*tmp2)
            {
    ...
    if(!sh_video->inited)
    {
        ...
        if(!sh_audio)
            return 0;//goto goto_next_file;
        sh_video = d_video->sh = NULL;
        goto main; // return (MSGTR_Exit_error);
    }
...
main:
    ...
    { // save OSD level only once
        osd_level_saved = osd_level;
        osd_level = 0;
    }
    else if (osd_level_saved > -1)
    { // if there is a saved OSD level, restore it
    ...
    {
        if ( sh_audio )
            guiIntfStruct.AudioType=sh_audio->channels;
        else
        ...
        if ( guiGetEvent( guiSetValues,(char *)sh_video ) )
            return 0;//goto goto_next_file;
    ...
    if(!iEOF)
    {
        ...
        while(sh_audio)
        {
            ...
            if(playsize>MAX_OUTBURST)
                playsize=MAX_OUTBURST; // we shouldn't exceed it!
            ...
            if(time_frame>0.001 && !(vo_flags&256))
            {
...
#endif
                {
                    ...
                    if(softsleep)
                    {
                        ...
                        if(time_frame<0)
                            printf("Warning! Softsleep underflow!\n");
                        while(time_frame>0)
...
#define	FRAME_LAG_WARN	0.2
                    ...
                    t2=GetTimer();
                    tmpTimer=t2-lastframeout_ts;
                    j = ((float)tmpTimer) / 1000000.0;
            ...
            if(sh_audio)
            {
                ...
                if(delay_corrected)
                {
                    ...
                    if (v_pts >= voldpts+1.0)
                    {
                        ...
                        voldpts=v_pts;
                        /*
                        printf("A:%6.1f V:%6.1f A-V:%7.3f ct:%7.3f  %3d/%3d  v:%2d%% v:%2d%% a:%2d%% df:%d q:%d c:%d%% tf:%d ts:%d %7.3f  \n",
                               a_pts-audio_delay-delay,
                        v_pts,
                        AV_delay,
                        c_total,
                               (int)sh_video->num_frames,(int)sh_video->num_frames_decoded,
                               (sh_video->timer>0.5)?(int)((100.0f*video_time_usage*playback_speed/(double)sh_video->timer)):0,
                               (sh_video->timer>0.5)?(int)((100.0f*vout_time_usage *playback_speed/(double)sh_video->timer)):0,
                               (sh_video->timer>0.5)?(int)((100.0f*audio_time_usage*playback_speed/(double)sh_video->timer)):0
                               ,drop_frame_cnt
                               ,output_quality
                               ,cache_fill_status,
                               too_fast_frame_cnt,too_slow_frame_cnt,fFPS
                              );*/
                        fFPS=0.0f;
                    }
                }
        ...
        else if(!sh_audio)
        {
            if (d_audio)
            {
                if (d_audio->sh != NULL) //incase video had no audio in beginning
                    sh_audio=d_audio->sh;
            }
        }
                ...
                case MP_CMD_SEEK :
                    {
                        ...
                        if(abs==2)
                        {
                            /* Absolute seek to a specific timestamp in seconds */
                            abs_seek_pos = 1;
                            if(sh_video)
                                osd_function= (v > sh_video->timer) ? OSD_FFW : OSD_REW;
                            rel_seek_secs = v;
                            //printf("seek absolute:%03.3f", rel_seek_secs);
                        }
                        else if(abs)
                        {
                            ...
                            //printf("seek percentage:%03.3f", rel_seek_secs);
                        }
                        else
                        {
                            rel_seek_secs+= v;
                            ...
                            printf("seek relative:%03.3f", rel_seek_secs);
                        }
                        brk_cmd = 1;
                    }
                    break;
...
#endif
                        {
                            if(!force && playtree_iter)
                            {
                                ...
                                if(play_tree_iter_step(i,n,0) == PLAY_TREE_ITER_ENTRY)
                                    iEOF = (n > 0) ? PT_NEXT_ENTRY : PT_PREV_ENTRY;
                ...
                case MP_CMD_PLAY_TREE_UP_STEP :
                    {
                        ...
                        if(!force && playtree_iter)
                        {
                            ...
                            if(play_tree_iter_up_step(i,n,0) == PLAY_TREE_ITER_ENTRY)
                                iEOF = (n > 0) ? PT_UP_NEXT : PT_UP_PREV;
                            play_tree_iter_free(i);
                        }
                        else
                            iEOF = (n > 0) ? PT_UP_NEXT : PT_UP_PREV;
...
dvdnav_stream_change_event_t *stream_change = (dvdnav_stream_change_event_t*)(dvdnav_event->details);
                                ...
                                if (aid_temp>=0)
                                    aid_temp+=128; // FIXME: is this sane?
                        ...
                        case DVDNAV_SPU_CLUT_CHANGE:
                            {
                                ...
                                if (vo_spudec)
                                    spudec_update_palette(vo_spudec,new_clut);
        ...
        if(iEOF==1 && loop_times>=0)
        {
            ...
            iEOF=0;
            abs_seek_pos=3;
            rel_seek_secs=0; // seek to start of movie (0%)
            loop_seek = 1;
        }
...
if( 1 )
        ...
        if(use_gui)
        {
            ...
            {
                __int64 len = ( demuxer->movi_end - demuxer->movi_start );
                __int64 pos = ( demuxer->file_format == DEMUXER_TYPE_AUDIO?stream->pos:demuxer->filepos );
                guiIntfStruct.Position=(len <= 0? 0.0f : ( pos - demuxer->movi_start ) * 100.0f / len );
            }
            if ( sh_video )
                guiIntfStruct.TimeSec=sh_video->pts;
            else if ( sh_audio )
                guiIntfStruct.TimeSec=sh_audio->delay;
