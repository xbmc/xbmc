...
static long total_time_usage_start=0;
...
void parse_cfgfiles( m_config_t* conf )
{
    ...
    {
        printf("couldnt parse config file\n");
        return;
    }
...
double vdecode_time;
unsigned long lastframeout_ts=0;
...
int mplayer_init(int argc, char* argv[])
{
    ...
    d_video=NULL;
    iFramesBlitted=0;
    video_time_usage=0.0f;
    vout_time_usage=0.0f;
    audio_time_usage=0.0f;
    ...
    time_frame=0; // Timer
    grab_frames=0;
    drop_frame=0;     // current dropping status
    dropped_frames=0; // how many frames dropped since last non-dropped frame
    too_slow_frame_cnt=0;
    too_fast_frame_cnt=0;
    AV_delay=0; // average of A-V timestamp differences
    lastframeout_ts=0;
    next_frame_time=0;
...
mp_input_add_key_fd(-1,0,mplayer_get_key,NULL);
 ...
 if(use_menu) {
  ...
  if(use_gui){
    ...
    orgplayback_speed=playback_speed;
}

void mplayer_setcache_size(int iCacheSize)
{
    stream_cache_size=iCacheSize;
}
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
int mplayer_open_file(char* pszFilename)
{
    printf("mplayer_open_file(%s)\n",pszFilename);
    voldpts=0;
    m_iPlaySpeed=1;
    sh_video=NULL;
    d_video=NULL;
...
if(!use_stdin && !slave_mode){
    ...
    else
    {
        ...
        if (vobsub_name)
        {
            printf("open vobsub:%s", vobsub_name);
            vo_vobsub=vobsub_open(vobsub_name,spudec_ifo,1,&vo_spudec);
            if(vo_vobsub==NULL)
                printf("cannot open:%s",vobsub_name);
            else
                printf("opened %s",vobsub_name);
        }
        else if(sub_auto && filename && (strlen(filename)>=5))
        {
            /* try to autodetect vobsub from movie filename ::atmos */
            char *buf = malloc(128+(strlen(filename)-3) * sizeof(char));
            ...
            if (vo_vobsub) printf("auto open %s succeeded", buf);
            free(buf);
        }
        if(vo_vobsub) {
            printf("got vo_vobsub\n");
...
#ifdef _XBOX
		entry = entry->child;
		return mplayer_open_file(entry->files[0]);
#endif
#endif
#ifndef _XBOX
		goto goto_next_file;
...
#endif
        return 0;//goto goto_next_file; // return (MSGTR_Exit_error);
    }
...
if (spudec_ifo) {
  ...
  current_module="spudec_init_vobsub";
            ...
            if (vo_spudec) printf("got vo_spudec");
        }
...
total_frame_cnt=0; drop_frame_cnt=0; // fix for multifile fps benchmark
    ...
    orgplayback_speed=playback_speed;
    printf("mplayer_open_file(%s) done %4.4f\n",pszFilename,playback_speed);
    return 1;
}
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
int mplayer_process( /*int iPAL, int iPAL60, int iFrameRateAdjust*/)
{
    if (eof)
        return -1;
/*
    if (sh_video)
    {
    //  printf("pal:%i pal60:%i adj:%i fps:%2f %2f", iPAL,iPAL60,iFrameRateAdjust,sh_video->fps,playback_speed);
      if (!iPAL60)
      {
        float fFPS=sh_video->fps;
        if (iPAL)
        {
          if (fFPS>=23.0 && fFPS<=24.5)
          {
            float fSpeed=25.0f/23.976f;
            if (orgplayback_speed!=fSpeed)
            {
              orgplayback_speed=fSpeed;
              ...
              sh_video->frametime=1.0f/sh_video->fps;
            vo_fps = sh_video->fps;
              printf("switch from 23.976->25fps (%2f)",playback_speed);
            }
          }
        }
        else
        {
          if (fFPS>=24.5 && fFPS<=25.5)
          {
            float fSpeed=23.976f/25.0f;
            if (orgplayback_speed!=fSpeed)
            {
              orgplayback_speed=fSpeed;
              playback_speed=orgplayback_speed;
              ...
              sh_video->frametime=1.0f/sh_video->fps;
            vo_fps = sh_video->fps;
              printf("switch from 25fps->23.976 (%2f)",playback_speed);
            }
          }
        }
      }
...
*/
    if (!osd_level)
    {
        if(osd_visible || vo_osd_progbar_type>=0 || osd_function!= OSD_PLAY)
        {
           osd_visible =0;
...
#ifdef USE_OSD
                        if(osd_level && sh_video){
                            osd_visible=100;//sh_video->fps; // 1 sec
  ...
  if(demux_seek(demuxer,rel_seek_secs,abs_seek_pos)){
	...
	if( !edl_decision ) {
      ...
      if(sh_video) {
                    ...
                    osd_visible=100;//sh_video->fps; // to rewert to PLAY pointer after 1 sec
		    audio_time_usage=0; video_time_usage=0; vout_time_usage=0;
                    drop_frame_cnt=0;
                    too_slow_frame_cnt=0;
                    too_fast_frame_cnt=0;
...
if(vo_config_count && vo_spudec) {
  ...
  while(1) {
	...
	if(len > 0) {
                            ...
                            printf("VOB sub: len=%d v_pts=%5.3f v_timer=%5.3f sub=%5.3f ts=%d \n",len,sh_video->pts,sh_video->timer,timestamp / 90000.0,timestamp);
                        }
                    }
    } else {
                    // DVD sub
      ...
      if(len > 0) {
                        ...
                        printf("DVD sub: len=%d  v_pts=%5.3f  s_pts=%5.3f  ts=%d \n",len,sh_video->pts,d_dvdsub->pts,timestamp);
                    }
                }
...
void mplayer_GetAudioInfo(char* strFourCC,char* strAudioCodec, long* bitrate, long* samplerate, int* channels, int* bVBR)
{
    ...
    *channels=0;
    if (sh_audio)
    {
        ...
        if (sh_audio->codec)
        {
            strFourCC[0]=sh_audio->format&0xff;
            strFourCC[1]=(sh_audio->format>>8)&0xff;
            strFourCC[2]=(sh_audio->format>>16)&0xff;
            strFourCC[3]=(sh_audio->format>>24)&0xff;
            strFourCC[4]=0;
...
{
    *tooearly=too_fast_frame_cnt;
    *toolate=too_slow_frame_cnt;
    strcpy(strVideoCodec,"");
    *fps=0;
    if (sh_video)
    {
        if (sh_video->codec)
        {
            strFourCC[0]=sh_video->format&0xff;
            strFourCC[1]=(sh_video->format>>8)&0xff;
            strFourCC[2]=(sh_video->format>>16)&0xff;
            strFourCC[3]=(sh_video->format>>24)&0xff;
            strFourCC[4]=0;
            strcpy(strVideoCodec,sh_video->codec->info);
            *fps=sh_video->fps;
            *iWidth=sh_video->disp_w;
            *iHeight=sh_video->disp_h;
        }
    }
}
...
{
    *lFramesDropped=drop_frame_cnt;
    *iQuality=output_quality;
    *iCacheFilled=cache_fill_status;
    *fTotalCorrection=c_total;
    *fAVDelay = AV_delay;
}
...
void mplayer_setPercentage(int iPercent)
{
    ...
    osd_show_percentage = 25;
    abs_seek_pos = 3;
    rel_seek_secs = ((float)iPercent)/100.0f;
}
...
int mplayer_getPercentage()
{
    if (demuxer)
    {
        return demuxer_get_percent_pos(demuxer);
    }
    return 0;
}
...
void mplayer_showosd(int bonoff)
{
  if (bonoff) printf("enable mplayer osd");
  else printf("disable mplayer osd");
  if (bonoff)
  {
    osd_level=1;
  }
...
//
//int mplayer_getSubtitleName(int iStream, char* sName)
//{
//	if(stream->type == STREAMTYPE_DVD)
//	{
//		dvd_priv_t *d=stream->priv;
//		char lang[3];
//		lang[2]=0;
//		lang[1]=(d->language&255);
//		lang[0]=(d->language>>8);
//		strupr(lang);
//		sprintf(sName, "ID:%s", lang);
//		return 1;
//	}
//	else if(demuxer->type == DEMUXER_TYPE_OGG)
//	{
//		vo_sub->
//	}
//
...
int mplayer_getSubtitleStreamInfo(int iStream, stream_language_t* stream_info)
{
	...
	//	{
	//		if(stream_info)
	//			stream_info->id = oggsub_ids[iStream];
	//		return oggsub_ids[iStream];
	//	}
	//
	//if(vo_spudec)
	//{
	...
	//		dvd_priv_t *d=stream->priv;
	//		if(stream_info)
	//			memcpy(stream_info, d->subtitles+iStream,sizeof(stream_language_t));
	//		return d->subtitles[iStream].id;
	//	}
	//	else //Any other type is handled by a more difficult aproach
	//	{
	//		stream_info->channels = 0;
	//		stream_info->language= 0;
	//		stream_info->type = 0;
...
mplayer_SetAudioStream(int iStream)
{
	int aid_temp = mplayer_getAudioStreamInfo(iStream, NULL);
    if (d_audio && audio_id!=aid_temp)
    {
        mp_msg(MSGT_INPUT,MSGL_DBG2,"d_audio->id change: was %d is now %d\n",
                d_audio->id,aid_temp);
        // FIXME: need a bettery way to change audio stream id
        d_audio->id=audio_id=aid_temp;
        if(sh_audio)
...
{
	xbmc_sub_count=0;
	memset(xbmc_subtitles, 0, sizeof(xbmc_subtitle)*32);
	if(!stream || !demuxer) return;
	if(stream->type == STREAMTYPE_DVD)
	{
		printf("Building sub database for DVD");
		dvd_priv_t *d=stream->priv;
		int i;
	...
	if(demuxer->type == DEMUXER_TYPE_OGG) //Needs some modification to add name support
	{
		printf("Building sub database for OGM");
		int i;
    ...
    {
		printf("Building sub database for VOBSUBS");
		int i,c;
