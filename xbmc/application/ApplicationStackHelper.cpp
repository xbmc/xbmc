...
std::optional<std::string> CApplicationStackHelper::UpdateStackAndItem(const CFileItem& playedFile)
{
  ...
  std::vector<std::chrono::milliseconds> times;
  const std::string path{m_stackMap.begin()->second->stackItem->GetDynPath()};
  bool haveTimes{db.GetStackTimes(path, times)};

  // See if new part played
  if (GetKnownStackParts() - 1 == static_cast<int>(times.size()))
  {
    // Update filename for this part in stack://
    std::string stackedPath{path};
    const int partNumber{GetStackPartNumber(playedFile)};
    std::vector<std::string> paths{};
    CStackDirectory::GetPaths(stackedPath, paths);
    paths[partNumber] = playedFile.GetDynPath();
    ...
    times.emplace_back(totalTime);
    db.SetStackTimes(stackedPath, times);
    db.Close();
...
void CApplicationStackHelper::SetStackPartDynPath(const CFileItem& item) const
{
  // Need to build stack://
  std::vector<std::string> paths{};
