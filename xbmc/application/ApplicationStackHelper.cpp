...
void CApplicationStackHelper::SetNextPartBookmark(const std::string& path) const
{
...
void CApplicationStackHelper::SetStackPartStreamDetails(const CFileItem& item) const
{
  CVideoInfoTag* tag{GetStackPartInformation(item.GetPath())->stackItem->GetVideoInfoTag()};
