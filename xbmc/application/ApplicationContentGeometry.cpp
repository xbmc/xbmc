...
EffectiveGeometry AtRestGeometry()
{
  ...
  gui.aspect = ContentGeometryAtRestFromSettings();
...
{
  {
    ...
    m_inputs.live.varies = varies || (m_inputs.cached.HasRecord() && m_inputs.cached.record.varies);
...
{
  return m_inputs.cached.HasRecord() ? EnvelopeAspect(m_inputs.cached.record) : 0.0f;
}
