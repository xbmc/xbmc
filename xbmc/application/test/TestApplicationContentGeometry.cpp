...
/*!
 * A title that mixes ratios - a scope body with taller sequences in it, as an IMAX release has -
 * measures an envelope of the whole coded frame, because the union of a wide shape and a tall
 * one is the frame that contains both.
 *
 * Masking to that envelope pins every scope section inside a 16:9 opening, where it is drawn
 * shorter than the 16:9 sections around it: the picture changes height at every boundary and
 * never reaches the sides of a scope screen. The opening belongs at the widest shape, which
 * leaves every section the same height and varies only their width - what a masked room does.
 */
