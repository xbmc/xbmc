 ...
 *
 * The scan keeps the dominant ratio; what live adds is the set of shapes present and the extent
 * that has to be masked to. Only a row the running detector produced is touched.
 */
void RecordDiscoveredGeometry(const CFileItem& fileItem)
{
  ...
  // Re-read rather than trusting what was loaded when the file opened, which verifies the file
  // is still the one that was measured.
                                                   ...
                                                   const CBookmark& bookmarkParam)
{
  ...
  // Before the bookmark rules below, which return early for a playback too short to resume
  // from; what was learned about the file does not depend on whether it is worth resuming.
...
void CApplicationPlayerCallback::RequestVideoSettings(const CFileItem& fileItem)
{
  ...
  if (dbs.Open())
  {
    ...
    // Loaded here rather than by the resolver, because it needs the database and a stat of the
    // file, and this already runs off the playback thread. Keyed the same way the settings above
    // are, so a declaration and the measurement it overrides cannot describe different files.
