...
bool CAutorun::PlayDisc(const std::string& path, bool bypassSettings, bool startFromBeginning)
{
  ...
  int nSize = pl ? pl->GetPlaylist( PLAYLIST_MUSIC ).size() : 0;
...
//    bAllowPictures = !profileManager.GetCurrentProfile().picturesLocked();
      ...
      if (pItem->m_bIsFolder && pItem->GetPath() != "." && pItem->GetPath() != "..")
      {
        ...
        && (bypassSettings || CServiceBroker::GetSettings().GetBool(CSettings::SETTING_DVDS_AUTORUN)))
        {
          ...
          if (pl)
          {
            ...
            pl->SetShuffle (PLAYLIST_VIDEO, false);
            pl->Add(PLAYLIST_VIDEO, item);
  ...
  if (!nAddedToPlaylist && !bPlaying && (bypassSettings || CServiceBroker::GetSettings().GetBool(CSettings::SETTING_DVDS_AUTORUN)))
  {
    ...
    if (itemlist.Size())
    {
      ...
      {
        pl->ClearPlaylist(PLAYLIST_VIDEO);
        pl->Add(PLAYLIST_VIDEO, itemlist);
        pl->SetCurrentPlaylist(PLAYLIST_VIDEO);
        pl->Play(0, "");
      }
    }
  ...
  {
    auto pl = SERVICES::CServiceManager::GetInstance().GetService<CPlayListPlayer>();
    for (int i = 0; i < vecItems.Size() && pl; i++)
    {
      CFileItemPtr pItem = vecItems[i];
      if (!pItem->m_bIsFolder && pItem->IsAudio())
      {
        nAddedToPlaylist++;
        pl->Add(PLAYLIST_MUSIC, pItem);
      }
    }
  }
  /* Probably want this if/when we add some automedia action dialog...
