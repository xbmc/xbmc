...
{
  SaveLeaderboardEntries(7, "Sonikku", {});

  // Kept as a record that the answer was none, not as an absence
  std::vector<LeaderboardEntry> back;
  EXPECT_TRUE(LoadLeaderboardEntries(7, "Sonikku", back));
  EXPECT_TRUE(back.empty());

  // Still nothing for a board never looked at, or another account
  std::vector<LeaderboardEntry> other;
  EXPECT_FALSE(LoadLeaderboardEntries(8, "Sonikku", other));
  EXPECT_FALSE(LoadLeaderboardEntries(7, "Someone Else", other));
}
