...
#include "system.h"
#include "GUIWindowContacts.h"
#include "URL.h"
#include "Util.h"
#include "Application.h"
#include "GUIPassword.h"
#include "dialogs/GUIDialogMediaSource.h"
#include "GUIDialogContactInfo.h"
...
CGUIWindowContacts::CGUIWindowContacts(void)
...
void CGUIWindowContacts::OnInitWindow()
{
...
bool CGUIWindowContacts::OnMessage(CGUIMessage& message)
{
...
void CGUIWindowContacts::UpdateButtons()
{
...
void CGUIWindowContacts::OnPrepareFileItems(CFileItemList& items)
{
...
bool CGUIWindowContacts::Update(const CStdString &strDirectory, bool updateFilterPath /* = true */)
{
  if (m_thumbLoader.IsLoading())
    m_thumbLoader.StopThread();
  ...
  m_vecItems->SetArt("thumb", CContactThumbLoader::GetCachedImage(*m_vecItems, "thumb"));

  return true;
}

bool CGUIWindowContacts::OnClick(int iItem)
{
  ...
  if (pItem->IsCBZ() || pItem->IsCBR())
  {
    ...
    OnShowContactRecursive(strComicPath);
    return true;
  }
...
bool CGUIWindowContacts::GetDirectory(const CStdString &strDirectory, CFileItemList& items)
{
...
bool CGUIWindowContacts::OnPlayMedia(int iItem)
{
  if (m_vecItems->Get(iItem)->IsVideo())
    return CGUIMediaWindow::OnPlayMedia(iItem);

  return ShowContact(iItem, false);
}

bool CGUIWindowContacts::ShowContact(int iItem, bool startSlideShow)
{
  if ( iItem < 0 || iItem >= (int)m_vecItems->Size() ) return false;
  CFileItemPtr pItem = m_vecItems->Get(iItem);
  CStdString strContact = pItem->GetPath();
...
/*
  ...
  for (int i = 0; i < (int)m_vecItems->Size();++i)
  {
    ...
    if (!pItem->m_bIsFolder && !(URIUtils::IsRAR(pItem->GetPath()) || 
          URIUtils::IsZIP(pItem->GetPath())) && (pItem->IsContact() || (
...
void CGUIWindowContacts::OnShowContactRecursive(const CStdString& strPath)
{
  /*
  CGUIWindowSlideShow *pSlideShow = (CGUIWindowSlideShow *)g_windowManager.GetWindow(WINDOW_SLIDESHOW);
  if (pSlideShow)
...
void CGUIWindowContacts::OnSlideShowRecursive(const CStdString &strContact)
{
  /*
  CGUIWindowSlideShow *pSlideShow = (CGUIWindowSlideShow *)g_windowManager.GetWindow(WINDOW_SLIDESHOW);
  if (pSlideShow)
  {   
    ...
    m_slideShowStarted = true;
    pSlideShow->RunSlideShow(strContact, true,
...
void CGUIWindowContacts::OnSlideShowRecursive()
{
  CStdString strEmpty = "";
  OnSlideShowRecursive(m_vecItems->GetPath());
}

void CGUIWindowContacts::OnSlideShow()
{
  OnSlideShow(m_vecItems->GetPath());
}

void CGUIWindowContacts::OnSlideShow(const CStdString &strContact)
{

}

void CGUIWindowContacts::OnRegenerateThumbs()
{
...
void CGUIWindowContacts::OnScan(int iItem)
{
  CStdString strPath;
...
void CGUIWindowContacts::DoScan(const CStdString &strPath)
{
...
void CGUIWindowContacts::GetContextButtons(int itemNumber, CContextButtons &buttons)
{
  CFileItemPtr item;
...
bool CGUIWindowContacts::OnContextButton(int itemNumber, CONTEXT_BUTTON button)
{
...
void CGUIWindowContacts::OnItemLoaded(CFileItem *pItem)
{
//  CContactThumbLoader::ProcessFoldersAndArchives(pItem);
}

void CGUIWindowContacts::LoadPlayList(const CStdString& strPlayList)
{
  /*
  CLog::Log(LOGDEBUG,"CGUIWindowContacts::LoadPlayList()... converting playlist into slideshow: %s", strPlayList.c_str());
...
void CGUIWindowContacts::OnInfo(int itemNumber)
{
  CFileItemPtr item = (itemNumber >= 0 && itemNumber < m_vecItems->Size()) ? m_vecItems->Get(itemNumber) : CFileItemPtr();
  if (!item)
    return;
  ...
  CGUIDialogContactInfo *pictureInfo = (CGUIDialogContactInfo *)g_windowManager.GetWindow(WINDOW_DIALOG_PICTURE_INFO);
  if (pictureInfo)
  {
    pictureInfo->SetContact(item.get());
    pictureInfo->DoModal();
  }
}

CStdString CGUIWindowContacts::GetStartFolder(const CStdString &dir)
{
