...
CGUIDialogContactInfo::CGUIDialogContactInfo(void)
    : CGUIDialog(WINDOW_DIALOG_PICTURE_INFO, "DialogContactInfo.xml")
{
...
void CGUIDialogContactInfo::SetContact(CFileItem *item)
{
  g_infoManager.SetCurrentSlide(*item);
}

void CGUIDialogContactInfo::OnInitWindow()
{
  UpdateContactInfo();
  CGUIDialog::OnInitWindow();
}

bool CGUIDialogContactInfo::OnAction(const CAction& action)
{
  switch (action.GetID())
  {
...
void CGUIDialogContactInfo::FrameMove()
{
  if (g_infoManager.GetCurrentSlide().GetPath() != m_currentContact)
  {
    UpdateContactInfo();
    m_currentContact = g_infoManager.GetCurrentSlide().GetPath();
  }
  CGUIDialog::FrameMove();
}

void CGUIDialogContactInfo::UpdateContactInfo()
{
...
void CGUIDialogContactInfo::OnDeinitWindow(int nextWindowID)
{
  CGUIDialog::OnDeinitWindow(nextWindowID);
  CGUIMessage msgReset(GUI_MSG_LABEL_RESET, GetID(), CONTROL_PICTURE_INFO);
  OnMessage(msgReset);
  m_pictureInfo->Clear();
  m_currentContact.Empty();
}
