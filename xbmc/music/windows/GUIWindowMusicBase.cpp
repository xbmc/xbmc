...
void CGUIWindowMusicBase::ShowArtistInfo(const CFileItem *pItem, bool bShowInfo /* = true */)
{
  ...
  while (1)
  {
        ...
        !m_musicdatabase.GetArtistInfo(params.GetArtistId(), artistInfo.GetArtist()))
    {
      ...
      if (scanner.UpdateDatabaseArtistInfo(pItem->GetPath(), artistInfo, bShowInfo, m_dlgProgress) != INFO_ADDED || !artistInfo.Loaded())
      {
...
bool CGUIWindowMusicBase::ShowAlbumInfo(const CFileItem *pItem, bool bShowInfo /* = true */)
{
  ...
  while (1)
  {
        ...
        !m_musicdatabase.GetAlbum(params.GetAlbumId(), albumInfo.GetAlbum()))
    {
      ...
      CMusicInfoScanner scanner;
      if (scanner.UpdateDatabaseAlbumInfo(pItem->GetPath(), albumInfo, bShowInfo, m_dlgProgress) != INFO_ADDED || !albumInfo.Loaded())
      {
