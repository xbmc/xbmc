  ...
  for (ID3v2::FrameListMap::ConstIterator it = frameListMap.begin(); it != frameListMap.end(); ++it)
  {
    ...
    if (it->first == "CHAP")
    {
      ...
      for (auto* chapFrame : chapterFrames)
      {
        ...
        // Timing - all times are in Milliseocnds
        const unsigned int startTimeMs = chapFrame->startTime();
        unsigned int endTimeMs = chapFrame->endTime();

        // Make sure the very last chapter runs to EOF
        if (prevTimeMs == endTimeMs)
        ...
        CLog::Log(LOGDEBUG, "CTagLoaderTagLib - Chapter {}, {} start point {}, endpoint {}",
                  chapNumber + 1, chapter.name, startTimeMs, endTimeMs);

        prevTimeMs = endTimeMs;
        ++chapNumber;
      }
...
bool CTagLoaderTagLib::Load(const std::string& strFileName, CMusicInfoTag& tag, const std::string& fallbackFileExtension, EmbeddedArt *art /* = NULL */)
{
  ...
  unsigned int totalLenMs = 0; // time in ms to the end of the current file
  ...
  {
    if (file->audioProperties())
      totalLenMs = file->audioProperties()->lengthInMilliseconds();
