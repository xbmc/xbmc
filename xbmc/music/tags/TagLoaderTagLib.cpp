...
const std::vector<std::string> GetASFStringList(const List<ASF::Attribute>& list)
{
  std::vector<std::string> values;
...
static const std::vector<std::string> GetID3v2StringList(const ID3v2::FrameList& frameList)
{
...
template<>
bool TagParser<ASF::Tag>::ParseTag(ASF::Tag *asf, EmbeddedArt *art, CMusicInfoTag& tag)
{
  if (!asf)
    return false;
  ...
  {
    if (it->first == "Author")
      CTagLoaderTagLib::SetArtist(tag, GetASFStringList(it->second));
    else if (it->first == "WM/AlbumArtist")
      CTagLoaderTagLib::SetAlbumArtist(tag, GetASFStringList(it->second));
             ...
             it->first == "WM/Track")
    {
      ...
      CTagLoaderTagLib::SetGenre(tag, GetASFStringList(it->second));
    ...
    {} // Known unsupported, supress warnings
      ...
      tag.SetMusicBrainzArtistID(CTagLoaderTagLib::SplitMBID(GetASFStringList(it->second)));
      ...
      tag.SetMusicBrainzAlbumArtistID(CTagLoaderTagLib::SplitMBID(GetASFStringList(it->second)));
...
template<>
bool TagParser<ID3v1::Tag>::ParseTag(ID3v1::Tag *id3v1, EmbeddedArt *art, CMusicInfoTag& tag)
{
...
template<>
bool TagParser<ID3v2::Tag>::ParseTag(ID3v2::Tag *id3v2, MUSIC_INFO::EmbeddedArt *art, MUSIC_INFO::CMusicInfoTag& tag)
{
  ...
  for (ID3v2::FrameListMap::ConstIterator it = frameListMap.begin(); it != frameListMap.end(); ++it)
  {
    ...
    if      (it->first == "TPE1")   CTagLoaderTagLib::SetArtist(tag, GetID3v2StringList(it->second));
    else if (it->first == "TALB")   tag.SetAlbum(it->second.front()->toString().to8Bit(true));
    else if (it->first == "TPE2")   CTagLoaderTagLib::SetAlbumArtist(tag, GetID3v2StringList(it->second));
    else if (it->first == "TIT2")   tag.SetTitle(it->second.front()->toString().to8Bit(true));
    else if (it->first == "TCON")   CTagLoaderTagLib::SetGenre(tag, GetID3v2StringList(it->second));
      ...
      for (ID3v2::FrameList::ConstIterator ut = it->second.begin(); ut != it->second.end(); ++ut)
      {
        ...
        if      (desc == "MUSICBRAINZ ARTIST ID")
          tag.SetMusicBrainzArtistID(CTagLoaderTagLib::SplitMBID(StringListToVectorString(stringList)));
          ...
          tag.SetMusicBrainzAlbumArtistID(CTagLoaderTagLib::SplitMBID(StringListToVectorString(stringList)));
        else if (desc == "MUSICBRAINZ ALBUM ARTIST")
          CTagLoaderTagLib::SetAlbumArtist(tag, StringListToVectorString(stringList));
...
template<>
bool TagParser<APE::Tag>::ParseTag(APE::Tag *ape, EmbeddedArt *art, CMusicInfoTag& tag)
{
  if (!ape)
    return false;
  ...
  {
    if (it->first == "ARTIST")
      CTagLoaderTagLib::SetArtist(tag, StringListToVectorString(it->second.toStringList()));
      ...
      CTagLoaderTagLib::SetAlbumArtist(tag, StringListToVectorString(it->second.toStringList()));
      ...
      CTagLoaderTagLib::SetGenre(tag, StringListToVectorString(it->second.toStringList()));
    ...
    {}
      ...
      tag.SetMusicBrainzArtistID(CTagLoaderTagLib::SplitMBID(StringListToVectorString(it->second.toStringList())));
    else if (it->first == "MUSICBRAINZ_ALBUMARTISTID")
      tag.SetMusicBrainzAlbumArtistID(CTagLoaderTagLib::SplitMBID(StringListToVectorString(it->second.toStringList())));
...
template<>
bool TagParser<Ogg::XiphComment>::ParseTag(Ogg::XiphComment *xiph, EmbeddedArt *art, CMusicInfoTag& tag)
{
  if (!xiph)
    return false;
  ...
  {
    if (it->first == "ARTIST")
      CTagLoaderTagLib::SetArtist(tag, StringListToVectorString(it->second));
    else if (it->first == "ARTISTS")
      tag.SetMusicBrainzArtistHints(StringListToVectorString(it->second));
    else if (it->first == "ALBUMARTIST" || it->first == "ALBUM ARTIST")
      CTagLoaderTagLib::SetAlbumArtist(tag, StringListToVectorString(it->second));
    else if (it->first == "ALBUMARTISTS" || it->first == "ALBUM ARTISTS")
      ...
      CTagLoaderTagLib::SetGenre(tag, StringListToVectorString(it->second));
    else if (it->first == "COMMENT")
    ...
    {}
      ...
      tag.SetMusicBrainzArtistID(CTagLoaderTagLib::SplitMBID(StringListToVectorString(it->second)));
    else if (it->first == "MUSICBRAINZ_ALBUMARTISTID")
      tag.SetMusicBrainzAlbumArtistID(CTagLoaderTagLib::SplitMBID(StringListToVectorString(it->second)));
    else if (it->first == "MUSICBRAINZ_ALBUMARTIST")
      CTagLoaderTagLib::SetAlbumArtist(tag, StringListToVectorString(it->second));
    else if (it->first == "MUSICBRAINZ_ALBUMID")
...
template<>
bool TagParser<MP4::Tag>::ParseTag(MP4::Tag *mp4, EmbeddedArt *art, CMusicInfoTag& tag)
{
  if (!mp4)
    return false;
...
template<>
bool TagParser<Tag>::ParseTag(Tag *generic, EmbeddedArt *art, CMusicInfoTag& tag)
{
  if (!generic)
    return false;
...
bool CTagLoaderTagLib::Load(const std::string& strFileName, CMusicInfoTag& tag, const std::string& fallbackFileExtension, MUSIC_INFO::EmbeddedArt *art /* = NULL */)
{
  ...
  if (asf)
    TagParser<ASF::Tag>::ParseTag(asf, art, tag);
  if (id3v1)
    TagParser<ID3v1::Tag>::ParseTag(id3v1, art, tag);
  if (id3v2)
    TagParser<ID3v2::Tag>::ParseTag(id3v2, art, tag);
  if (generic)
    TagParser<Tag>::ParseTag(generic, art, tag);
  if (mp4)
    TagParser<MP4::Tag>::ParseTag(mp4, art, tag);
  if (xiph) // xiph tags override id3v2 tags in badly tagged FLACs
    TagParser<Ogg::XiphComment>::ParseTag(xiph, art, tag);
  if (ape && (!id3v2 || g_advancedSettings.m_prioritiseAPEv2tags)) // ape tags override id3v2 if we're prioritising them
    TagParser<APE::Tag>::ParseTag(ape, art, tag);
