...
template <typename T>
class TestTagParser : public ::testing::Test {
   public:
     T value_;
...
TYPED_TEST(TestTagParser, ParsesBasicTag) {
  ...
  CMusicInfoTag tag;
  EXPECT_TRUE(TagParser<TypeParam>::ParseTag(tg, NULL, tag));
...
TYPED_TEST(TestTagParser, HandleNullTag) {
  ...
  CMusicInfoTag tag;
  EXPECT_FALSE(TagParser<TypeParam>::ParseTag(NULL, NULL, tag));
}
...
template <typename T>
class EmptyTagParser : public ::testing::Test { 
  public:
    T value_;
};
...
TEST(TestTagLoaderTagLib, SetGenre)
{
