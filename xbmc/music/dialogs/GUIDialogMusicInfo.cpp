...
void CGUIDialogMusicInfo::SetAlbum(const CAlbum& album, const string &path)
{
  m_album = album;
...
void CGUIDialogMusicInfo::SetArtist(const CArtist& artist, const string &path)
{
  m_artist = artist;
  SetDiscography();
  *m_albumItem = CFileItem(path, true);
...
void CGUIDialogMusicInfo::SetLabel(int iControl, const string& strLabel)
{
  if (strLabel.empty())
  {
...
void CGUIDialogMusicInfo::OnGetFanart()
{
  ...
  {
    string strItemPath = StringUtils::Format("fanart://Remote%i",i);
    CFileItemPtr item(new CFileItem(strItemPath, false));
    string thumb = m_artist.fanart.GetPreviewURL(i);
  ...
  string strLocal = item.GetLocalFanart();
  if (!strLocal.empty())
  {
