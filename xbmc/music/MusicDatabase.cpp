                             ...
                             const CStdString& strArtist, const CStdString& strGenre, int year, bool bCompilation)
{
  ...
  try
  {
    ...
    else
    {
      ...
      ClearAlbumLastScrapedTime(idAlbum);
      m_pDS->close();
      return idAlbum;
    }
  }
...
bool CMusicDatabase::GetAlbum(int idAlbum, CAlbum& album)
{
  try
  {
    ...
    CStdString strSQL=PrepareSQL("SELECT albumview.*,albumartistview.*,songview.*,songartistview.*,albuminfosong.* "
                                 "  FROM albumview "
    ...
    while (!m_pDS->eof())
    {
      ...
      {
        album.infoSongs.push_back(GetAlbumInfoSongFromDataset(record, songOffset));
...
bool CMusicDatabase::SetAlbumLastScrapeTime(int idAlbum, const CDateTime& dtLastScraped)
{
  if (!dtLastScraped.IsValid())
    return false;
  
  CStdString strSQL = PrepareSQL("UPDATE album SET lastScraped = '%s' WHERE idAlbum = %i", dtLastScraped.GetAsDBDateTime().c_str(), idAlbum);
  return ExecuteQuery(strSQL);
}
...
bool CMusicDatabase::AddArtist(const CArtistCredit& artist)
{
  AddArtist(artist.m_strArtist,
            artist.m_strMusicBrainzArtistID);
  return true;
};
...
bool CMusicDatabase::GetArtist(int idArtist, CArtist &artist)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    CStdString strSQL=PrepareSQL("SELECT * FROM artistview LEFT JOIN discography ON artistview.idArtist = discography.idArtist WHERE artistview.idArtist = %i", idArtist);
    ...
    while (!m_pDS->eof())
    {
      ...
      artist.discography.push_back(make_pair(record->at(discographyOffset + 0).get_asString(), record->at(discographyOffset + 1).get_asString()));
      m_pDS->next();
    }
    m_pDS->close(); // cleanup recordset data
    return true;
...
bool CMusicDatabase::SetArtistLastScrapeTime(int idArtist, const CDateTime& dtLastScraped)
{
  if (!dtLastScraped.IsValid())
    return false;
  
  CStdString strSQL = PrepareSQL("UPDATE artist SET lastScraped = '%s' WHERE idArtist = %i", dtLastScraped.GetAsDBDateTime().c_str(), idArtist);
  return ExecuteQuery(strSQL);
}
...
{
  CStdString strSQL;
  strSQL=PrepareSQL("INSERT INTO discography (idArtist, strAlbum, strYear) values(%i, '%s', '%s')",
                    idArtist,
                    strAlbum.c_str(),
                    strYear.c_str());
  return ExecuteQuery(strSQL);
...
CSong CMusicDatabase::GetSongFromDataset(const dbiplus::sql_record* const record, int offset /* = 0 */, bool bWithMusicDbPath /* = false*/)
{
  CSong song;
  ...
  {
    CStdString strFileName = record->at(offset + song_strFileName).get_asString();
...
void CMusicDatabase::ExportToXML(const CStdString &xmlFile, bool singleFiles, bool images, bool overwrite)
{
  try
  {
    ...
    // find all albums
    set<int> albumIds;
    CStdString albumSQL = "select idAlbum FROM album";
    m_pDS->query(albumSQL.c_str());
    total = m_pDS->num_rows();
    current = 0;
    while (!m_pDS->eof())
    {
      int idAlbum = m_pDS->fv("idAlbum").get_asInt();
      albumIds.insert(idAlbum);
      m_pDS->next();
    }
    m_pDS->close();
    ...
    for (set<int>::iterator albumId = albumIds.begin(); albumId != albumIds.end(); ++albumId)
    {
      CAlbum album;
    ...
    // find all artists
    set<int> artistIds;
    CStdString artistSQL = "select idArtist FROM artist";
    ...
    while (!m_pDS->eof())
    {
      ...
      artistIds.insert(idArtist);
      m_pDS->next();
    }
    m_pDS->close();

    for (set<int>::iterator artistId = artistIds.begin(); artistId != artistIds.end(); ++artistId)
    {
      CArtist artist;
    ...
    while (entry)
    {
      ...
      if (strnicmp(entry->Value(), "artist", 6) == 0)
      {
        ...
        if (idArtist > -1)
        {
          ...
          artist.MergeScrapedArtist(importedArtist);
          UpdateArtist(artist);
        }
      ...
      else if (strnicmp(entry->Value(), "album", 5) == 0)
      {
        ...
        {
          CAlbum album;
          GetAlbum(idAlbum, album);
          album.MergeScrapedAlbum(importedAlbum);
          UpdateAlbum(album);
        }
...
void CMusicDatabase::SetPropertiesForFileItem(CFileItem& item)
{
  ...
  {
    CAlbum album;
    if (GetAlbum(idAlbum,album)) // true to force additional information
