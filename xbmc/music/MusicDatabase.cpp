...
{
  string strSQL = PrepareSQL("SELECT idAlbumInfoSong FROM albuminfosong WHERE idAlbumInfo = %i and iTrack = %i", idAlbum, song.iTrack);
...
int CMusicDatabase::AddSong(const int idAlbum,
                            const string& strTitle, const string& strMusicBrainzTrackID,
                            const string& strPathAndFileName, const string& strComment, const string& strThumb,
...
bool CMusicDatabase::GetSong(int idSong, CSong& song)
{
  try
  {
    ...
    string strSQL=PrepareSQL("SELECT songview.*,songartistview.* FROM songview "
...
int CMusicDatabase::AddAlbum(const string& strAlbum, const string& strMusicBrainzAlbumID,
                             const string& strArtist, const string& strGenre, int year, bool bCompilation)
{
  string strSQL;
  try
  {
...
int  CMusicDatabase::UpdateAlbum(int idAlbum,
                                 const string& strAlbum, const string& strMusicBrainzAlbumID,
                                 const string& strArtist, const string& strGenre,
                                 const string& strMoods, const string& strStyles,
                                 const string& strThemes, const string& strReview,
                                 const string& strImage, const string& strLabel,
...
{
  string strSQL = PrepareSQL("UPDATE album SET lastScraped = NULL WHERE idAlbum = %i", idAlbum);
  return ExecuteQuery(strSQL);
}

bool CMusicDatabase::HasAlbumBeenScraped(int idAlbum)
{
  string strSQL = PrepareSQL("SELECT idAlbum FROM album WHERE idAlbum = %i AND lastScraped IS NULL", idAlbum);
  return GetSingleValue(strSQL).empty();
}

int CMusicDatabase::AddGenre(const string& strGenre1)
{
  string strSQL;
  try
  {
    ...
    if (m_pDS->num_rows() == 0)
    {
      ...
      m_genreCache.insert(pair<string, int>(strGenre1, idGenre));
      return idGenre;
    }
    else
    {
...
bool CMusicDatabase::UpdateArtist(const CArtist& artist)
{
  ...
  for (std::vector<std::pair<string,string> >::const_iterator disc = artist.discography.begin();
...
int CMusicDatabase::AddArtist(const string& strArtist, const string& strMusicBrainzArtistID)
{
  string strSQL;
  try
  {
...
int  CMusicDatabase::UpdateArtist(int idArtist,
                                  const string& strArtist, const string& strMusicBrainzArtistID,
                                  const string& strBorn, const string& strFormed,
                                  const string& strGenres, const string& strMoods,
                                  const string& strStyles, const string& strInstruments,
                                  const string& strBiography, const string& strDied,
                                  const string& strDisbanded, const string& strYearsActive,
                                  const string& strImage, const string& strFanart)
{
  CScraperUrl thumbURL;
  CFanart fanart;
  std::vector<std::pair<string,string> > discography;
  if (idArtist < 0)
    return -1;
...
{
  string strSQL = PrepareSQL("SELECT idArtist FROM artist WHERE idArtist = %i AND lastScraped IS NULL", idArtist);
  return GetSingleValue(strSQL).empty();
}

bool CMusicDatabase::ClearArtistLastScrapedTime(int idArtist)
{
  string strSQL = PrepareSQL("UPDATE artist SET lastScraped = NULL WHERE idArtist = %i", idArtist);
  return ExecuteQuery(strSQL);
}

int CMusicDatabase::AddArtistDiscography(int idArtist, const string& strAlbum, const string& strYear)
{
  string strSQL=PrepareSQL("INSERT INTO discography (idArtist, strAlbum, strYear) values(%i, '%s', '%s')",
                               idArtist,
                               strAlbum.c_str(),
                               strYear.c_str());
  return ExecuteQuery(strSQL);
...
{
  string strSQL = PrepareSQL("DELETE FROM discography WHERE idArtist = %i", idArtist);
  return ExecuteQuery(strSQL);
}
...
bool CMusicDatabase::GetGenresByAlbum(int idAlbum, std::vector<int>& genres)
{
  try
  {
    string strSQL = PrepareSQL("select idGenre from album_genre where idAlbum = %i ORDER BY iOrder ASC", idAlbum);
    if (!m_pDS->query(strSQL.c_str()))
      return false;
    if (m_pDS->num_rows() == 0)
    {
...
bool CMusicDatabase::GetGenresBySong(int idSong, std::vector<int>& genres)
{
  try
  {
    string strSQL = PrepareSQL("select idGenre from song_genre where idSong = %i ORDER BY iOrder ASC", idSong);
    if (!m_pDS->query(strSQL.c_str()))
      return false;
    if (m_pDS->num_rows() == 0)
    {
...
int CMusicDatabase::AddPath(const string& strPath1)
{
  string strSQL;
  try
  {
    ...
    if (m_pDS->num_rows() == 0)
    {
      ...
      m_pathCache.insert(pair<string, int>(strPath, idPath));
      return idPath;
    }
    else
    {
...
void CMusicDatabase::GetFileItemFromDataset(const dbiplus::sql_record* const record, CFileItem* item, const CMusicDbUrl &baseUrl)
{
  ...
  string strRealPath = URIUtils::AddFileToFolder(record->at(song_strPath).get_asString(), record->at(song_strFileName).get_asString());
  ...
  else
  {
    ...
    string strFileName = record->at(song_strFileName).get_asString();
    string strExt = URIUtils::GetExtension(strFileName);
    string path = StringUtils::Format("%ld%s", record->at(song_idSong).get_asInt(), strExt.c_str());
...
bool CMusicDatabase::GetSongByFileName(const string& strFileName, CSong& song, int startOffset)
{
  song.Clear();
  ...
  {
    string strFile = URIUtils::GetFileName(strFileName);
    URIUtils::RemoveExtension(strFile);
    return GetSong(atol(strFile.c_str()), song);
  }

  string strPath = URIUtils::GetDirectory(strFileName);
  URIUtils::AddSlashAtEnd(strPath);
  ...
  string strSQL = PrepareSQL("select idSong from songview "
...
int CMusicDatabase::GetAlbumIdByPath(const string& strPath)
{
  try
  {
    string strSQL=PrepareSQL("select distinct idAlbum from song join path on song.idPath = path.idPath where path.strPath='%s'", strPath.c_str());
    m_pDS->query(strSQL.c_str());
    if (m_pDS->eof())
      return -1;
...
int CMusicDatabase::GetSongByArtistAndAlbumAndTitle(const string& strArtist, const string& strAlbum, const string& strTitle)
{
  try
  {
    string strSQL=PrepareSQL("select idSong from songview "
...
bool CMusicDatabase::SearchArtists(const string& search, CFileItemList &artists)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string strVariousArtists = g_localizeStrings.Get(340).c_str();
    string strSQL;
    ...
    string artistLabel(g_localizeStrings.Get(557)); // Artist
    while (!m_pDS->eof())
    {
      string path = StringUtils::Format("musicdb://artists/%ld/", m_pDS->fv(0).get_asInt());
      CFileItemPtr pItem(new CFileItem(path, true));
      string label = StringUtils::Format("[%s] %s", artistLabel.c_str(), m_pDS->fv(1).get_asString().c_str());
...
bool CMusicDatabase::GetTop100(const string& strBaseDir, CFileItemList& items)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string strSQL="select * from songview "
...
bool CMusicDatabase::GetTop100Albums(VECALBUMS& albums)
{
  try
  {
    ...
    string strSQL = "select albumview.* from albumview "
...
bool CMusicDatabase::GetTop100AlbumSongs(const string& strBaseDir, CFileItemList& items)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
...
bool CMusicDatabase::GetRecentlyPlayedAlbums(VECALBUMS& albums)
{
  try
  {
    ...
    string strSQL = StringUtils::Format("select distinct albumview.* from song join albumview on albumview.idAlbum=song.idAlbum where song.lastplayed IS NOT NULL order by song.lastplayed desc limit %i", RECENTLY_PLAYED_LIMIT);
    CLog::Log(LOGDEBUG, "%s query: %s", __FUNCTION__, strSQL.c_str());
    if (!m_pDS->query(strSQL.c_str())) return false;
    int iRowsFound = m_pDS->num_rows();
    if (iRowsFound == 0)
...
bool CMusicDatabase::GetRecentlyPlayedAlbumSongs(const string& strBaseDir, CFileItemList& items)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
...
bool CMusicDatabase::GetRecentlyAddedAlbums(VECALBUMS& albums, unsigned int limit)
{
  try
  {
    ...
    string strSQL = StringUtils::Format("select * from albumview where strAlbum != '' order by idAlbum desc limit %u", limit ? limit : g_advancedSettings.m_iMusicLibraryRecentlyAddedItems);
...
bool CMusicDatabase::GetRecentlyAddedAlbumSongs(const string& strBaseDir, CFileItemList& items, unsigned int limit)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
...
void CMusicDatabase::IncrementPlayCount(const CFileItem& item)
{
  try
  {
    ...
    string sql=PrepareSQL("UPDATE song SET iTimesPlayed=iTimesPlayed+1, lastplayed=CURRENT_TIMESTAMP where idSong=%i", idSong);
...
bool CMusicDatabase::GetSongsByPath(const string& strPath1, MAPSONGS& songs, bool bAppendToMap)
{
  string strPath(strPath1);
  try
  {
    ...
    string strSQL=PrepareSQL("select * from songview where strPath='%s'", strPath.c_str() );
    if (!m_pDS->query(strSQL.c_str())) return false;
    int iRowsFound = m_pDS->num_rows();
    if (iRowsFound == 0)
    {
...
bool CMusicDatabase::Search(const string& search, CFileItemList &items)
{
...
bool CMusicDatabase::SearchSongs(const string& search, CFileItemList &items)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string songLabel = g_localizeStrings.Get(179); // Song
    while (!m_pDS->eof())
    {
      CFileItemPtr item(new CFileItem);
      GetFileItemFromDataset(item.get(), baseUrl);
...
bool CMusicDatabase::SearchAlbums(const string& search, CFileItemList &albums)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string albumLabel(g_localizeStrings.Get(558)); // Album
    while (!m_pDS->eof())
    {
      CAlbum album = GetAlbumFromDataset(m_pDS.get());
      string path = StringUtils::Format("musicdb://albums/%ld/", album.idAlbum);
      CFileItemPtr pItem(new CFileItem(path, album));
      string label = StringUtils::Format("[%s] %s", albumLabel.c_str(), album.strAlbum.c_str());
      pItem->SetLabel(label);
...
bool CMusicDatabase::CleanupSongsByIds(const string &strSongIds)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string strSQL=PrepareSQL("select * from song join path on song.idPath = path.idPath where song.idSong in %s", strSongIds.c_str());
    if (!m_pDS->query(strSQL.c_str())) return false;
    int iRowsFound = m_pDS->num_rows();
    if (iRowsFound == 0)
    {
    ...
    { // get the full song path
      string strFileName = URIUtils::AddFileToFolder(m_pDS->fv("path.strPath").get_asString(), m_pDS->fv("song.strFileName").get_asString());
...
bool CMusicDatabase::CleanupSongs()
{
  try
  {
    ...
    {
      string strSQL=PrepareSQL("select song.idSong from song order by song.idSong limit %i offset %i",iLIMIT,i);
      if (!m_pDS->query(strSQL.c_str())) return false;
      int iRowsFound = m_pDS->num_rows();
...
bool CMusicDatabase::CleanupAlbums()
{
  try
  {
    ...
    string strSQL = "select * from album where album.idAlbum not in (select idAlbum from song)";
    if (!m_pDS->query(strSQL.c_str())) return false;
    int iRowsFound = m_pDS->num_rows();
    if (iRowsFound == 0)
    {
...
bool CMusicDatabase::CleanupPaths()
{
  try
  {
    ...
    string sql = "select * from path where idPath not in (select idPath from song)";
    if (!m_pDS->query(sql.c_str())) return false;
    int iRowsFound = m_pDS->num_rows();
    if (iRowsFound == 0)
    {
    ...
    while (!m_pDS->eof())
    {
      ...
      string path = m_pDS->fv("strPath").get_asString();
      string sql = PrepareSQL("select count(idPath) from songpaths where SUBSTR(strPath,1,%i)='%s'", StringUtils::utf8_strlen(path.c_str()), path.c_str());
...
bool CMusicDatabase::InsideScannedPath(const string& path)
{
  string sql = PrepareSQL("select idPath from path where SUBSTR(strPath,1,%i)='%s' LIMIT 1", path.size(), path.c_str());
...
bool CMusicDatabase::CleanupGenres()
{
  try
  {
    ...
    string strSQL = "delete from genre where idGenre not in (select idGenre from song_genre) and";
...
bool CMusicDatabase::LookupCDDBInfo(bool bRequery/*=false*/)
{
  ...
  if (bRequery)
  {
    string strFile = StringUtils::Format("%x.cddb", pCdInfo->GetCddbDiscId());
    ...
    if (!cddb.queryCDinfo(pCdInfo))
    {
      ...
      if (lasterror == E_WAIT_FOR_INPUT)
      {
        ...
        while (1)
        {
          ...
          string strArtist = cddb.getInexactArtist(i);
          if (!strArtist.empty())
            strTitle += " - " + strArtist;
      ...
      else
      {
        ...
        string strErrorText = StringUtils::Format("[%d] %s", cddb.getLastError(), cddb.getLastErrorText());
...
void CMusicDatabase::DeleteCDDBInfo()
{
  ...
  if (pDlg)
  {
    ...
    map<ULONG, string> mapCDDBIds;
    for (int i = 0; i < items.Size(); ++i)
    {
      if (items[i]->m_bIsFolder)
        continue;

      string strFile = URIUtils::GetFileName(items[i]->GetPath());
      ...
      string strDiskTitle, strDiskArtist;
      ...
      pDlg->Add(str);
      mapCDDBIds.insert(pair<ULONG, string>(lDiscId, str));
    }
    ...
    string strSelectedAlbum = pDlg->GetSelectedLabelText();
    map<ULONG, string>::iterator it;
    for (it = mapCDDBIds.begin();it != mapCDDBIds.end();it++)
    {
      if (it->second == strSelectedAlbum)
      {
        string strFile = StringUtils::Format("%x.cddb", it->first);
        CFile::Delete(URIUtils::AddFileToFolder(CProfilesManager::Get().GetCDDBFolder(), strFile));
        break;
      }
    }
...
bool CMusicDatabase::GetGenresNav(const string& strBaseDir, CFileItemList& items, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string strSQL = "SELECT %s FROM genre ";
...
bool CMusicDatabase::GetYearsNav(const string& strBaseDir, CFileItemList& items, const Filter &filter /* = Filter() */)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string strSQL = "SELECT DISTINCT albumview.iYear FROM albumview ";
    ...
    while (!m_pDS->eof())
    {
      ...
      CMusicDbUrl itemUrl = musicUrl;
      string strDir = StringUtils::Format("%ld/", m_pDS->fv(0).get_asInt());
      itemUrl.AppendPath(strDir);
      pItem->SetPath(itemUrl.ToString());
...
bool CMusicDatabase::GetAlbumsByYear(const string& strBaseDir, CFileItemList& items, int year)
{
  CMusicDbUrl musicUrl;
...
bool CMusicDatabase::GetCommonNav(const string &strBaseDir, const string &table, const string &labelField, CFileItemList &items, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  if (NULL == m_pDB.get()) return false;
  if (NULL == m_pDS.get()) return false;
  ...
  {
    Filter extFilter = filter;
    string strSQL = "SELECT %s FROM " + table + " ";
    ...
    while (!m_pDS->eof())
    {
      ...
      CMusicDbUrl itemUrl = musicUrl;
      string strDir = StringUtils::Format("%s/", labelValue.c_str());
      itemUrl.AppendPath(strDir);
      pItem->SetPath(itemUrl.ToString());
...
bool CMusicDatabase::GetAlbumTypesNav(const string &strBaseDir, CFileItemList &items, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  return GetCommonNav(strBaseDir, "albumview", "albumview.strType", items, filter, countOnly);
}

bool CMusicDatabase::GetMusicLabelsNav(const string &strBaseDir, CFileItemList &items, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  return GetCommonNav(strBaseDir, "albumview", "albumview.strLabel", items, filter, countOnly);
}

bool CMusicDatabase::GetArtistsNav(const string& strBaseDir, CFileItemList& items, bool albumArtistsOnly /* = false */, int idGenre /* = -1 */, int idAlbum /* = -1 */, int idSong /* = -1 */, const Filter &filter /* = Filter() */, const SortDescription &sortDescription /* = SortDescription() */, bool countOnly /* = false */)
{
  if (NULL == m_pDB.get()) return false;
  if (NULL == m_pDS.get()) return false;
  try
...
bool CMusicDatabase::GetArtistsByWhere(const string& strBaseDir, const Filter &filter, CFileItemList& items, const SortDescription &sortDescription /* = SortDescription() */, bool countOnly /* = false */)
{
  if (NULL == m_pDB.get()) return false;
  if (NULL == m_pDS.get()) return false;
  ...
  try
  {
    ...
    string strSQL = "SELECT %s FROM artistview ";
    ...
    for (DatabaseResults::const_iterator it = results.begin(); it != results.end(); it++)
    {
      ...
      try
      {
        ...
        CMusicDbUrl itemUrl = musicUrl;
        string path = StringUtils::Format("%ld/", artist.idArtist);
        itemUrl.AppendPath(path);
        pItem->SetPath(itemUrl.ToString());
...
bool CMusicDatabase::GetAlbumFromSong(int idSong, CAlbum &album)
{
  try
  {
    ...
    string strSQL = PrepareSQL("select albumview.* from song join albumview on song.idAlbum = albumview.idAlbum where song.idSong='%i'", idSong);
    if (!m_pDS->query(strSQL.c_str())) return false;
    int iRowsFound = m_pDS->num_rows();
    if (iRowsFound != 1)
    {
...
bool CMusicDatabase::GetAlbumsNav(const string& strBaseDir, CFileItemList& items, int idGenre /* = -1 */, int idArtist /* = -1 */, const Filter &filter /* = Filter() */, const SortDescription &sortDescription /* = SortDescription() */, bool countOnly /* = false */)
{
  CMusicDbUrl musicUrl;
  if (!musicUrl.FromString(strBaseDir))
    return false;
...
bool CMusicDatabase::GetAlbumsByWhere(const string &baseDir, const Filter &filter, CFileItemList &items, const SortDescription &sortDescription /* = SortDescription() */, bool countOnly /* = false */)
{
  ...
  try
  {
    ...
    string strSQL = "SELECT %s FROM albumview ";
    ...
    for (DatabaseResults::const_iterator it = results.begin(); it != results.end(); it++)
    {
      ...
      {
        CMusicDbUrl itemUrl = musicUrl;
        string path = StringUtils::Format("%ld/", record->at(album_idAlbum).get_asInt());
        itemUrl.AppendPath(path);
...
bool CMusicDatabase::GetSongsByWhere(const string &baseDir, const Filter &filter, CFileItemList &items, const SortDescription &sortDescription /* = SortDescription() */)
{
  if (m_pDB.get() == NULL || m_pDS.get() == NULL)
    return false;
  ...
  try
  {
    ...
    string strSQL = "SELECT %s FROM songview ";
...
bool CMusicDatabase::GetSongsByYear(const string& baseDir, CFileItemList& items, int year)
{
  CMusicDbUrl musicUrl;
...
bool CMusicDatabase::GetSongsNav(const string& strBaseDir, CFileItemList& items, int idGenre, int idArtist, int idAlbum, const SortDescription &sortDescription /* = SortDescription() */)
{
  CMusicDbUrl musicUrl;
  if (!musicUrl.FromString(strBaseDir))
    return false;
...
void CMusicDatabase::UpdateTables(int version)
{
  if (version < 19)
  {
    ...
    string sql = PrepareSQL("UPDATE song SET strExtraArtists=SUBSTR(strExtraArtists,%i), strExtraGenres=SUBSTR(strExtraGenres,%i)", len, len);
    m_pDS->exec(sql.c_str());
  ...
  if (version < 42)
  {
    ...
    string sql = "select idArtist,strArtist from artist";
...
unsigned int CMusicDatabase::GetSongIDs(const Filter &filter, vector<pair<int,int> > &songIDs)
{
  try
  {
    ...
    string strSQL = "select idSong from songview ";
...
int CMusicDatabase::GetSongsCount(const Filter &filter)
{
  try
  {
    ...
    string strSQL = "select count(idSong) as NumSongs from songview ";
    if (!CDatabase::BuildSQL(strSQL, filter, strSQL))
      return false;
...
bool CMusicDatabase::GetAlbumPath(int idAlbum, string& path)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string strSQL=PrepareSQL("select strPath from song join path on song.idPath = path.idPath where song.idAlbum=%ld", idAlbum);
...
bool CMusicDatabase::SaveAlbumThumb(int idAlbum, const string& strThumb)
{
  SetArtForItem(idAlbum, "album", "thumb", strThumb);
  // TODO: We should prompt the user to update the art for songs
  string sql = PrepareSQL("UPDATE art"
                              " SET url='-'"
...
bool CMusicDatabase::GetArtistPath(int idArtist, string &basePath)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string strSQL=PrepareSQL("SELECT strPath"
                                 "  FROM album_artist"
                                 "  JOIN song "
    ...
    while (!m_pDS2->eof())
    {
      string path = m_pDS2->fv("strPath").get_asString();
      if (basePath.empty())
        basePath = path;
      else
...
int CMusicDatabase::GetArtistByName(const string& strArtist)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string strSQL=PrepareSQL("select idArtist from artist where artist.strArtist like '%s'", strArtist.c_str());
...
int CMusicDatabase::GetAlbumByName(const string& strAlbum, const string& strArtist)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
...
bool CMusicDatabase::GetRandomSong(CFileItem* item, int& idSong, const Filter &filter)
{
  try
  {
    ...
    string strSQL = PrepareSQL("select %s from songview ", !filter.fields.empty() ? filter.fields.c_str() : "*");
    Filter extFilter = filter;
...
bool CMusicDatabase::GetCompilationAlbums(const string& strBaseDir, CFileItemList& items)
{
  CMusicDbUrl musicUrl;
  if (!musicUrl.FromString(strBaseDir))
    return false;
...
bool CMusicDatabase::GetCompilationSongs(const string& strBaseDir, CFileItemList& items)
{
  CMusicDbUrl musicUrl;
  if (!musicUrl.FromString(strBaseDir))
    return false;
...
void CMusicDatabase::SplitString(const string &multiString, vector<string> &vecStrings, string &extraStrings)
{
  vecStrings = StringUtils::Split(multiString, g_advancedSettings.m_musicItemSeparator);
  for (unsigned int i = 1; i < vecStrings.size(); i++)
    extraStrings += g_advancedSettings.m_musicItemSeparator + string(vecStrings[i]);
}

bool CMusicDatabase::SetPathHash(const string &path, const string &hash)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
...
bool CMusicDatabase::GetPathHash(const string &path, string &hash)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
...
bool CMusicDatabase::RemoveSongsFromPath(const string &path1, MAPSONGS& songs, bool exact)
{
  ...
  try
  {
    ...
    string sql = "select * from songview" + where;
    if (!m_pDS->query(sql.c_str())) return false;
    int iRowsFound = m_pDS->num_rows();
    if (iRowsFound > 0)
    {
...
bool CMusicDatabase::SetSongRating(const string &filePath, char rating)
{
  try
  {
    ...
    string sql = PrepareSQL("update song set rating='%c' where idSong = %i", rating, songID);
    m_pDS->exec(sql.c_str());
    return true;
  }
  catch (...)
...
int CMusicDatabase::GetSongIDFromPath(const string &filePath)
{
  // grab the where string to identify the song id
  CURL url(filePath);
  if (url.GetProtocol()=="musicdb")
  {
    string strFile=URIUtils::GetFileName(filePath);
    URIUtils::RemoveExtension(strFile);
  ...
  try
  {
    ...
    string strPath = URIUtils::GetDirectory(filePath);
    URIUtils::AddSlashAtEnd(strPath);
    DWORD crc = ComputeCRC(filePath);

    string sql = PrepareSQL("select idSong from song join path on song.idPath = path.idPath where song.dwFileNameCRC='%ul'and path.strPath='%s'", crc, strPath.c_str());
...
bool CMusicDatabase::SetScraperForPath(const string& strPath, const ADDON::ScraperPtr& scraper)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    // wipe old settings
    string strSQL = PrepareSQL("delete from content where strPath='%s'",strPath.c_str());
    m_pDS->exec(strSQL.c_str());
...
bool CMusicDatabase::GetScraperForPath(const string& strPath, ADDON::ScraperPtr& info, const ADDON::TYPE &type)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string strSQL = PrepareSQL("select * from content where strPath='%s'",strPath.c_str());
    m_pDS->query(strSQL.c_str());
    ...
    { // try and ascertain scraper for this path
      ...
      string scraperUUID = m_pDS->fv("content.strScraperPath").get_asString();
...
bool CMusicDatabase::ScraperInUse(const string &scraperID) const
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string sql = PrepareSQL("select count(1) from content where strScraperPath='%s'",scraperID.c_str());
...
bool CMusicDatabase::GetItems(const string &strBaseDir, CFileItemList &items, const Filter &filter /* = Filter() */, const SortDescription &sortDescription /* = SortDescription() */)
{
  CMusicDbUrl musicUrl;
  if (!musicUrl.FromString(strBaseDir))
    return false;
...
bool CMusicDatabase::GetItems(const string &strBaseDir, const string &itemType, CFileItemList &items, const Filter &filter /* = Filter() */, const SortDescription &sortDescription /* = SortDescription() */)
{
...
string CMusicDatabase::GetItemById(const string &itemType, int id)
{
  if (StringUtils::EqualsNoCase(itemType, "genres"))
...
void CMusicDatabase::ExportToXML(const string &xmlFile, bool singleFiles, bool images, bool overwrite)
{
  try
  {
    if (NULL == m_pDB.get()) return;
    ...
    // find all albums
    vector<int> albumIds;
    string sql = "select idAlbum FROM album WHERE lastScraped IS NOT NULL";
    m_pDS->query(sql.c_str());
    ...
    for (vector<int>::iterator albumId = albumIds.begin(); albumId != albumIds.end(); ++albumId)
    {
      ...
      if (singleFiles)
      {
        ...
        {
          string nfoFile = URIUtils::AddFileToFolder(strPath, "album.nfo");
    ...
    string artistSQL = "SELECT idArtist FROM artist where lastScraped IS NOT NULL";
    ...
    for (vector<int>::iterator artistId = artistIds.begin(); artistId != artistIds.end(); ++artistId)
    {
      ...
      if (singleFiles)
      {
        ...
        {
          string nfoFile = URIUtils::AddFileToFolder(strPath, "artist.nfo");
          if (overwrite || !CFile::Exists(nfoFile))
          {
            if (!xmlDoc.SaveFile(nfoFile))
          ...
          {
            string savedThumb = URIUtils::AddFileToFolder(strPath,"folder.jpg");
            string savedFanart = URIUtils::AddFileToFolder(strPath,"fanart.jpg");
...
void CMusicDatabase::ImportFromXML(const string &xmlFile)
{
  CGUIDialogProgress *progress = (CGUIDialogProgress *)g_windowManager.GetWindow(WINDOW_DIALOG_PROGRESS);
  try
  {
...
void CMusicDatabase::AddKaraokeData(int idSong, int iKaraokeNumber, DWORD crc)
{
  try
  {
    ...
    if (iKaraokeNumber > 0)
    {
      string strSQL = PrepareSQL("UPDATE karaokedata SET idSong=%i WHERE iKaraNumber=%i", idSong, iKaraokeNumber);
      m_pDS->exec(strSQL.c_str());
      return;
    }
...
bool CMusicDatabase::GetSongByKaraokeNumber(int number, CSong & song)
{
  try
  {
    ...
    string strSQL=PrepareSQL("SELECT * FROM karaokedata where iKaraNumber=%ld", number);
...
void CMusicDatabase::ExportKaraokeInfo(const string & outFile, bool asHTML)
{
  try
  {
    if (NULL == m_pDB.get()) return;
    ...
    string sql = "SELECT * FROM songview WHERE iKaraNumber > 0 ORDER BY strFileName";
    ...
    {
      CSong song = GetSongFromDataset();
      string songnum = StringUtils::Format("%06d", song.iKaraokeNumber);
...
void CMusicDatabase::ImportKaraokeInfo(const string & inputFile)
{
  CGUIDialogProgress *progress = (CGUIDialogProgress *)g_windowManager.GetWindow(WINDOW_DIALOG_PROGRESS);
...
bool CMusicDatabase::SetKaraokeSongDelay(int idSong, int delay)
{
  try
  {
    ...
    string strSQL = PrepareSQL("UPDATE karaokedata SET iKaraDelay=%i WHERE idSong=%i", delay, idSong);
    m_pDS->exec(strSQL.c_str());
...
bool CMusicDatabase::GetArtistArtForItem(int mediaId, const std::string &mediaType, std::map<std::string, std::string> &art)
{
  try
  {
    ...
    string sql = PrepareSQL("SELECT type,url FROM art WHERE media_id=(SELECT idArtist from %s_artist WHERE id%s=%i AND iOrder=0) AND media_type='artist'", mediaType.c_str(), mediaType.c_str(), mediaId);
    m_pDS2->query(sql.c_str());
    while (!m_pDS2->eof())
    {
      art.insert(make_pair(m_pDS2->fv(0).get_asString(), m_pDS2->fv(1).get_asString()));
...
bool CMusicDatabase::GetFilter(CDbUrl &musicUrl, Filter &filter, SortDescription &sorting)
{
  ...
  if (type == "artists")
  {
    ...
    string strSQL = "(artistview.idArtist IN ";
    if (idArtist > 0)
    ...
    if (!albumArtistsOnly)
    {
      string strVariousArtists = g_localizeStrings.Get(340);
