...
void CMusicDatabase::AddSong(CSong& song, bool bCheck)
{
  ...
  try
  {
    ...
    // with MBIDs we should always have an Album Artist - but send the MB album artist through anyway for the case where a user has no album
    // artist set but does have MBIDs set. in this case we can still fetch the artist data using MBIDs. this beats the alternative case which
    // is to assume no MBIDs because the AlbumArtist is empty and return the user no data at all even though they have MBIDs.
    else 
...
void CMusicDatabase::AddExtraSongArtists(const std::vector<std::string> &vecArtists, int idSong, bool bCheck)
{
  try
  {
    ...
    {
      // we only deal with the MBIDs at this point for the primary artist.
...
bool CMusicDatabase::UpdateOldVersion(int version)
{
  ...
  try
  {
    ...
    {
      int len = g_advancedSettings.m_musicItemSeparator.size() + 1;
      CStdString sql = PrepareSQL("ALTER TABLE artist ADD strMusicBrainzArtistID text");
      m_pDS->exec(sql.c_str());
      sql = PrepareSQL("ALTER TABLE album ADD strMusicBrainzAlbumID text");
      m_pDS->exec(sql.c_str());
      sql = PrepareSQL("ALTER TABLE album ADD strMusicBrainzAlbumArtistID text");
      m_pDS->exec(sql.c_str());
    }
