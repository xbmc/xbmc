...
bool CMusicInfoScanner::DoScan(const CStdString& strDirectory)
{
  ...
  // load subfolder
  CFileItemList items;
  CDirectory::GetDirectory(strDirectory, items, CMediaSettings::Get().GetMusicExtensions() + "|.jpg|.tbn|.lrc|.cdg");
...
//  CLog::Log(LOGDEBUG, __FUNCTION__" - processing dir: %s", strPath.c_str());
  CDirectory::GetDirectory(strPath, items, CMediaSettings::Get().GetMusicExtensions(), DIR_FLAG_NO_FILE_DIRS);
