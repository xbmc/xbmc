...
CSong::CSong(CFileItem& item)
{
  ...
  { // have musicbrainz artist info, so use it
    ...
    {
      string artistId = tag.GetMusicBrainzArtistID()[i];
      string artistName;
      /*
      ...
      if (artistName.empty())
        artistName = artistId;
      string strJoinPhrase = (i == tag.GetMusicBrainzArtistID().size()-1) ? "" : g_advancedSettings.m_musicItemSeparator;
  ...
  { // no musicbrainz info, so fill in directly
    ...
    {
      string strJoinPhrase = (it == --tag.GetArtist().end() ? "" : g_advancedSettings.m_musicItemSeparator);
