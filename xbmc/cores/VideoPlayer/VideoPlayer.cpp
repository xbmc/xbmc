...
void CVideoPlayer::Prepare()
{
  ...
  if (stackHelper->GetRegisteredStack(fileItem) != nullptr)
  {
    if (stackHelper->GetRegisteredStackPartNumber(fileItem) >= stackHelper->GetKnownStackParts())
    {
      ...
      {
        std::chrono::milliseconds length{m_pInputStream->GetDuration()};
        std::chrono::milliseconds time{stackHelper->GetStackTotalTimeMs()};
        ...
        fileItem.SetStartOffset(time.count());
        fileItem.SetEndOffset(time.count() + length.count());
      }
    }
  }
