...
public:
  ...
  bool operator()(const SelectionStream& ss) const
  {
    ...
    if ((isSameSubLang ||
         ((isExternal || isCC) && (ss.language.empty() || ss.language == "und"))) &&
...
public:
  ...
  bool operator()(const SelectionStream& lh, const SelectionStream& rh) const
  {
    ...
    // if all previous conditions do not match, allow fallback to external "unknown" language
    if (!m_isPrefForced && isLincluded && isLexternal &&
        (lh.language.empty() || lh.language == "und"))
    {
      return true;
    }
    return false;
