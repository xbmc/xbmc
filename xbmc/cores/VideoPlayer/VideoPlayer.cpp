...
public:
    ...
    : m_subStream(subStream)
  {
    ...
    // The subtitle setting, or where it names no language (none, original, forced_only) the audio
    // setting, or where that names none either (default, original, mediadefault) what is playing
...
{
private:
  // Resolves the settings this predicate ranks by, as well as answering whether a stream is
  // relevant at all
...
public:
  explicit PredicateSubtitlePriority(const CLanguageTag& playedAudioLang, int stream)
    : m_filter(playedAudioLang, stream), m_subStream(stream)
  {
  }
