...
public:
    ...
    : m_audiolang(lang), m_subStream(subStream)
  {
    ...
    // GetSubtitleLanguage fallback to locale or "eng" when SETTING_LOCALE_SUBTITLELANGUAGE is set:
    // none, original, forced_only, default, or the language conversion fails.
    // This lets you know if a language has been selected or if a special setting is used.
    m_isUserLangCodeSet = !m_isSubNone && !m_isPrefOriginal && !m_isPrefForced &&
                          !StringUtils::EqualsNoCase(subLangSetting, "default");
    m_subLang = g_langInfo.GetSubtitleLanguage();
  };
  ...
  bool operator()(const SelectionStream& ss) const
  {
    ...
    const bool isSameAudioLang = g_LangCodeExpander.CompareISO639Codes(ss.language, m_audiolang);
    const bool isSameSubLang = g_LangCodeExpander.CompareISO639Codes(ss.language, m_subLang);

    if (m_isPrefHearingImp)
    {
      const int chackFlags = FLAG_ORIGINAL | FLAG_HEARING_IMPAIRED;
      if ((ss.flags & chackFlags) == chackFlags)
        return false;

      const bool isSameLang = m_isUserLangCodeSet ? isSameSubLang : isSameAudioLang;

      if ((ss.flags & StreamFlags::FLAG_HEARING_IMPAIRED) && (isSameLang || ss.language == "cc"))
        return false;
    }
    ...
    {
      if ((ss.flags & StreamFlags::FLAG_FORCED) && isSameAudioLang)
        return false;
    }
...
public:
  explicit PredicateSubtitlePriority(const std::string& lang, int stream)
    : m_audioLang(lang), m_filter(lang, stream), m_subStream(stream)
  {
    auto settings = CServiceBroker::GetSettingsComponent()->GetSettings();
    const std::string subLangSetting =
        settings->GetString(CSettings::SETTING_LOCALE_SUBTITLELANGUAGE);

    bool m_isSubNone = StringUtils::EqualsNoCase(subLangSetting, "none");
    m_isPrefOriginal = StringUtils::EqualsNoCase(subLangSetting, "original");
    m_isPrefForced = StringUtils::EqualsNoCase(subLangSetting, "forced_only");
  ...
  bool operator()(const SelectionStream& lh, const SelectionStream& rh) const
  {
    ...
    const bool isLSameAudioLang = g_LangCodeExpander.CompareISO639Codes(lh.language, m_audioLang);
    const bool isRSameAudioLang = g_LangCodeExpander.CompareISO639Codes(rh.language, m_audioLang);
    ...
    if (m_isPrefHearingImp)
    {
      if (m_isPrefOriginal)
      {
        int chackFlags = FLAG_ORIGINAL | FLAG_HEARING_IMPAIRED | FLAG_DEFAULT;
        PREDICATE_RETURN((lh.flags & chackFlags) == chackFlags,
                         (rh.flags & chackFlags) == chackFlags);

        chackFlags = FLAG_ORIGINAL | FLAG_HEARING_IMPAIRED;
        PREDICATE_RETURN((lh.flags & chackFlags) == chackFlags,
                         (rh.flags & chackFlags) == chackFlags);
      }
      ...
      bool isLSameLang;
      bool isRSameLang;
      if (m_isUserLangCodeSet)
      {
        isLSameLang = isLSameSubLang;
        isRSameLang = isRSameSubLang;
      }
      else
      {
        isLSameLang = isLSameAudioLang;
        isRSameLang = isRSameAudioLang;
      }

      int chackFlags = FLAG_HEARING_IMPAIRED | FLAG_DEFAULT;
      PREDICATE_RETURN((lh.flags & chackFlags) == chackFlags && isLSameLang,
                       (rh.flags & chackFlags) == chackFlags && isRSameLang);

      chackFlags = FLAG_HEARING_IMPAIRED;
      PREDICATE_RETURN((lh.flags & chackFlags) == chackFlags && isLSameLang,
                       (rh.flags & chackFlags) == chackFlags && isRSameLang);
    }
    ...
    if (m_isPrefOriginal)
    {
      // disallow forced and impaired
      bool isLincluded = (lh.flags & FLAG_FORCED) == 0 && (lh.flags & FLAG_HEARING_IMPAIRED) == 0;
      bool isRincluded = (rh.flags & FLAG_FORCED) == 0 && (rh.flags & FLAG_HEARING_IMPAIRED) == 0;

      // try find original (default) in audio language
      int chackFlags = FLAG_ORIGINAL | FLAG_DEFAULT;
      PREDICATE_RETURN(isLincluded && (lh.flags & chackFlags) == chackFlags && isLSameAudioLang,
                       isRincluded && (rh.flags & chackFlags) == chackFlags && isRSameAudioLang);

      // try find original in audio language
      PREDICATE_RETURN(isLincluded && (lh.flags & FLAG_ORIGINAL) && isLSameAudioLang,
                       isRincluded && (rh.flags & FLAG_ORIGINAL) && isRSameAudioLang);

      // try find original (default) with any language
      chackFlags = FLAG_ORIGINAL | FLAG_DEFAULT;
      PREDICATE_RETURN(isLincluded && (lh.flags & chackFlags) == chackFlags,
                       isRincluded && (rh.flags & chackFlags) == chackFlags);
    ...
    if (m_isPrefForced)
    {
      int chackFlags = FLAG_FORCED | FLAG_DEFAULT;
      PREDICATE_RETURN((lh.flags & chackFlags) == chackFlags && isLSameAudioLang,
                       (rh.flags & chackFlags) == chackFlags && isRSameAudioLang);

      chackFlags = FLAG_FORCED;
      PREDICATE_RETURN((lh.flags & chackFlags) == chackFlags && isLSameAudioLang,
                       (rh.flags & chackFlags) == chackFlags && isRSameAudioLang);
    }

    PREDICATE_RETURN(lh.flags & FLAG_DEFAULT && isLSameSubLang,
                     rh.flags & FLAG_DEFAULT && isRSameSubLang);

    PREDICATE_RETURN(isLSameSubLang, isRSameSubLang);
