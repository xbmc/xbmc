  ...
  while (m_messenger.Get(pMsg, 0ms) == MSGQ_OK)
  {
             ...
             m_messenger.GetPacketCount(CDVDMsg::PLAYER_SEEK_CHAPTER) == 0)
    {
      ...
      if (m_pDemuxer)
      {
        ...
        const std::chrono::milliseconds position{m_pDemuxer->GetChapterPos(msg.GetChapter()) * 1000};
        ...
        if (hasEdit)
        {
          ...
          if (edit->action == EDL::Action::CUT)
          {
            ...
            const auto endTime{std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::duration<double, std::milli>(m_State.timeMax))};
  ...
  if (m_pDemuxer)
  {
    ...
    if (m_pDemuxer->GetChapterCount() > 0)
    {
      for (int i = 0, ie = m_pDemuxer->GetChapterCount(); i < ie; ++i)
      {
        ...
        state.chapters.emplace_back(name, m_Edl.GetTimeWithoutCuts(position).count() / 1000);
      }
    }
