...
//! \brief An open demux-and-decode session on a file's first video stream. The input stream is
//! held alongside the demuxer that references it, torn down in members-reversed order.
...
//! \brief Seek to a position in milliseconds and decode the first usable picture there. Takes
//! the session's parts, so a caller can walk N positions on one open stream, demuxer and codec.
