...
{
  const unsigned int level = GetQueueLevel(STREAM_AUDIO);
...
bool CMediaPipelineWebOS::HasAudioData() const
{
  ...
  return GetQueuedBytes(STREAM_AUDIO) > 0;
}
...
bool CMediaPipelineWebOS::HasVideoData() const
{
  ...
  return GetQueuedBytes(STREAM_VIDEO) > 0;
}
...
void CMediaPipelineWebOS::SendVideoMessage(const std::shared_ptr<CDVDMsg>& msg, const int priority)
{
  ...
  if (m_pipeline)
  {
    const unsigned int level = GetQueueLevel(STREAM_VIDEO);
...
{
  if (type == STREAM_AUDIO && m_audioHint.codec == AV_CODEC_ID_NONE)
    return 0;
