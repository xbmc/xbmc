...
DemuxPacket* CDVDDemuxFFmpeg::ReadInternal(bool keep)
{
    ...
    if (m_pFormatContext)
    {
      ...
      else
      {
        ...
        if (pPacket)
        {
          ...
          {
            logM(LOGDEBUG, "CDVDDemuxFFmpeg",
                 "video: pts [{}] dts [{}] den [{}] num [{}] avi [{}] codec [{}] [{}]",
                 m_pkt.pkt.pts, m_pkt.pkt.dts, stream->time_base.den, stream->time_base.num, m_bAVI,
                 stream->codecpar->codec_type, avcodec_get_name(stream->codecpar->codec_id));
  ...
  if (pPacket->iStreamId >= 0)
  {
    ...
    else if (stream->type == STREAM_VIDEO)
    {
      ...
      if (m_pFormatContext->streams[pPacket->iStreamId]->id == 0x1015 && m_dv_dual_stream &&
            ...
            if (bl_stream)
            {
              ...
              if (side_data && size)
                bl_video_stream->dovi =
                    *reinterpret_cast<AVDOVIDecoderConfigurationRecord*>(side_data);
...
StreamHdrType CDVDDemuxFFmpeg::DetermineHdrType(AVStream* pStream)
{
  ...
  if (av_stream_get_side_data(pStream, AV_PKT_DATA_DOVI_CONF, nullptr) ||
      convert_dual_stream) // DoVi
