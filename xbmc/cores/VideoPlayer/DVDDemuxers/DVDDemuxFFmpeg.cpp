...
CDemuxStream* CDVDDemuxFFmpeg::AddStream(int streamIdx)
{
  ...
  if (pStream && pStream->discard != AVDISCARD_ALL)
  {
    ...
    if (langTag)
    {
      //! @note A transport stream can carry several ISO 639 language descriptors for one track,
      //! which ffmpeg joins with commas ("deu,eng" for dual mono, or one entry per DVB subtitle
      //! page). A stream has a single language here, so the first is the one taken.
      ...
      //! @note ffmpeg does not read the Matroska v4 LanguageBCP47 element, which takes priority
      //! over Language when present, so every track is reported with the value of Language. A
      //! BCP 47 tag wrapped in curly braces in the title field is honoured here as a way to
      //! express a track's real language.
