...
const uint8_t rev_lut[32] = {0x00, 0x08, 0x04, 0x0c, /*  upper nibble */
                             0x02, 0x0a, 0x06, 0x0e, 0x01, 0x09, 0x05, 0x0d,
                             0x03, 0x0b, 0x07, 0x0f, 0x00, 0x80, 0x40, 0xc0, /*  lower nibble */
                             0x20, 0xa0, 0x60, 0xe0, 0x10, 0x90, 0x50, 0xd0,
                             0x30, 0xb0, 0x70, 0xf0};
...
signed int CDVDTeletextTools::deh24(unsigned char* p)
{
  ...
  int x = hamm24val[p[0]] + (p[1] & 127) * 16 + (p[2] & 127) * 2048;

  return (x ^ hamm24cor[e]) | hamm24err[e];
}

CDVDTeletextData::CDVDTeletextData(CProcessInfo& processInfo)
  : CThread("DVDTeletextData"), IDVDStreamPlayer(processInfo), m_messageQueue("teletext")
{
      ...
      if (Datai[0] >= 0x10 && Datai[0] <= 0x1F) /* Check we have a valid data identifier */
      {
        ...
        for (int row = 0; row < rows; row++)
        {
          ...
          if ((vtx_rowbyte[0] == 0x02 || vtx_rowbyte[0] == 0x03) && (vtx_rowbyte[1] == 0x2C))
          {
            ...
            if (packet_number == 0 && m_TXTCache->CurrentPage[magazine] != -1 &&
                m_TXTCache->CurrentSubPage[magazine] != -1)
              SavePage(m_TXTCache->CurrentPage[magazine], m_TXTCache->CurrentSubPage[magazine],
                       pagedata[magazine]);
            ...
            if (packet_number == 0)
            {
              ...
              m_TXTCache->CurrentPage[magazine] = m_TXTCache->PageReceiving =
                  magazine << 8 | b2 << 4 | b3;
              ...
              m_TXTCache->SubPageTable[m_TXTCache->CurrentPage[magazine]] =
                  m_TXTCache->CurrentSubPage[magazine];

              AllocateCache(magazine);
              LoadPage(m_TXTCache->CurrentPage[magazine], m_TXTCache->CurrentSubPage[magazine],
                       pagedata[magazine]);
              pageinfo_thread = &(m_TXTCache
                                      ->astCachetable[m_TXTCache->CurrentPage[magazine]]
                                                     [m_TXTCache->CurrentSubPage[magazine]]
                                      ->pageinfo);
              if (!pageinfo_thread)
                continue;

              if ((m_TXTCache->PageReceiving & 0xff) ==
                  0xfe) /* ?fe: magazine organization table (MOT) */
              ...
              if (dehamming[vtxt_row[7]] & 0x08) // subtitle page
              {
                ...
                if (use != -1)
                  m_TXTCache->SubtitlePages[use].language =
                      CountryConversionTable[pageinfo_thread->national];
              }

              /* check parity, copy line 0 to cache (start and end 8 bytes are not needed and used otherwise) */
              unsigned char* p = m_TXTCache
                                     ->astCachetable[m_TXTCache->CurrentPage[magazine]]
                                                    [m_TXTCache->CurrentSubPage[magazine]]
                                     ->p0;
              ...
              for (int i = 42 - 8; i < 42; i++)
                *p++ = deparity[vtxt_row[i]];
                     ...
                     dehamming[vtxt_row[2]] ==
                         0) /* packet 29/0 replaces 28/0 for a whole magazine */
            {
              Decode_p2829(vtxt_row, &(m_TXTCache->astP29[magazine]));
            }
            else if (m_TXTCache->CurrentPage[magazine] != -1 &&
                     m_TXTCache->CurrentSubPage[magazine] != -1)
              ...
              else if (packet_number == 27)
              {
                ...
                if (descode == 0) // reading FLOF-Pagelinks
                {
                  ...
                  if (b1 != 0xff)
                  {
                    ...
                    for (int i = 0; i < FLOFSIZE; i++)
                    {
                      ...
                      if (b2 != 0xff && b3 != 0xff)
                      {
                        b4 = ((b1 ^ (dehamming[vtxt_row[8 + i * 6]] >> 1)) & 6) |
                             ((b1 ^ (dehamming[vtxt_row[6 + i * 6]] >> 3)) & 1);
                        if (b4 == 0)
                          b4 = 8;
                        if (b2 <= 9 && b3 <= 9)
                          m_TXTCache->FlofPages[m_TXTCache->CurrentPage[magazine]][i] =
                              b4 << 8 | b2 << 4 | b3;
                      }
                    }
                    ...
                    if (pageinfo_thread->p24) /* packet 24 received */
                    {
                      ...
                      {
                        for (; l >= 2 &&
                               0 == m_TXTCache->FlofPages[m_TXTCache->CurrentPage[magazine]][l];
                             l--)
                          ; /* find used linkindex */
                        for (; e >= 1 && !isalnum(p[e]); e--)
                          ; /* find end */
                        for (a = a1 = e - 1; a >= 0 && p[a] >= ' '; a--) /* find start */
                          if (p[a] > ' ')
                        ...
                        if (a >= 0 && l >= 2)
                        {
                          ...
                          if (e - a1 < 11)
                            m_TXTCache
                                ->ADIPTable[m_TXTCache->FlofPages[m_TXTCache->CurrentPage[magazine]]
                                                                 [l]][e - a1 + 1] = '\0';
                        }
                        e = a - 1;
                        l--;
                ...
                else if (descode == 4) /* level 2.5 links (ignore level 3.5 links of /4 and /5) */
                {
                  ...
                  for (i = 0; i < 4; i++)
                  {
                    ...
                    p->l25 = !!(d1 & 0x04);
                    p->l35 = !!(d1 & 0x08);
                    p->page = (((d1 & 0x000003c0) >> 6) | ((d1 & 0x0003c000) >> (14 - 4)) |
                               ((d1 & 0x00003800) >> (11 - 8))) ^
                    ...
                    else if (p->page > 0x899)
                    {
                      ...
                      continue;
                                 ...
                                 ->astCachetable[p->page][0]) /* link valid && linked page cached */
                    {
                      TextPageinfo_t* pageinfo_link =
                          &(m_TXTCache->astCachetable[p->page][0]->pageinfo);
                      if (p->local)
            ...
            /* set update flag */
            if (m_TXTCache->CurrentPage[magazine] == m_TXTCache->Page &&
                m_TXTCache->CurrentSubPage[magazine] != -1)
            {
              SavePage(m_TXTCache->CurrentPage[magazine], m_TXTCache->CurrentSubPage[magazine],
                       pagedata[magazine]);
...
void CDVDTeletextData::ErasePage(int magazine) const
{
  ...
  TextCachedPage_t* pg =
      m_TXTCache
          ->astCachetable[m_TXTCache->CurrentPage[magazine]][m_TXTCache->CurrentSubPage[magazine]];
  if (pg)
  {
