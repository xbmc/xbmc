...
bool CDVDInputStreamNavigator::Open()
{
  ...
  {
    // Get title for later
    if (StringUtils::StartsWithNoCase(url.GetFileName(), "title"))
      title = std::stoi(URIUtils::GetFileName(url.GetFileName()));
  ...
  if (title > 0)
  {
    ...
    if (!m_dll.dvdnav_title_play(m_dvdnav, title))
      CLog::Log(LOGERROR, "Error on title_play(title {}): {}", title,
                m_dll.dvdnav_err_to_string(m_dvdnav));
  }
  ...
  if (result == DVDNAV_STATUS_ERR)
  {
    ...
    case DVDNAV_VTS_CHANGE:
      ...
      // accordingly.
      {
        ...
        {
          // If started playing a title directly then stop at end
          m_bEOF = true;
        ...
        else if(m_holdmode == HOLDMODE_NONE)
        {
