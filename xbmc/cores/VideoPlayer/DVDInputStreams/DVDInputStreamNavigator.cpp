...
void CDVDInputStreamNavigator::SetAudioStreamName(AudioStreamInfo &info, const audio_attr_t &audio_attributes)
{
  ...
  info.m_channelLayout = StreamUtils::GetLayoutXYZ(StreamUtils::GetDefaultMask(info.channels));

  info.codecDesc.append(" ");

  if (!info.m_channelLayout.empty())
    info.codecDesc.append(info.m_channelLayout);
  else
    info.codecDesc.append(StringUtils::Format("{:d} {}", info.channels,
