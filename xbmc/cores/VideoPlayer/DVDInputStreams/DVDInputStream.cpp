    ...
    bool& closed)
{
  ...
  // Update streamdetails for DVD titles
  ...
  // Need to update dynpath here (and not in OnPlayerCloseFile()) to save settings
  auto& components{CServiceBroker::GetAppComponents()};
  const auto stackHelper{components.GetComponent<CApplicationStackHelper>()};
  if (stackHelper->IsInStack(item))
    stackHelper->UpdateDiscStackAndItemTimes(item);
