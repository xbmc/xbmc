    ...
    bool& closed)
{
  ...
  const auto& it{std::ranges::find(playedPlaylists, true, &PlaylistInformation::inMenu)};
  if (it != playedPlaylists.end())
  {
    ...
    if (it2 != playedPlaylists.end())
    {
      ...
      std::erase_if(playedPlaylists, [](const PlaylistInformation& p) { return p.inMenu; });
    }
    else
    {
      ...
      else
      {
        ...
        if (std::next(it) == playedPlaylists.end() &&
            std::ranges::count(playedPlaylists, true, &PlaylistInformation::inMenu) == 1)
