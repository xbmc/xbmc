...
bool CDVDStreamInfo::Equal(const CDVDStreamInfo& right, bool withextradata)
{
  ...
  ||  height   != right.height
  ||  width    != right.width
  ||  cropBottom != right.cropBottom
  ||  cropTop  != right.cropTop
  ||  cropLeft != right.cropLeft
  ||  cropRight!= right.cropRight
  ||  stills   != right.stills
  ||  level    != right.level
...
void CDVDStreamInfo::Assign(const CDemuxStream& right, bool withextradata)
{
  ...
  else if(  right.type == STREAM_VIDEO )
  {
    ...
    width     = stream->iWidth;
    cropBottom= stream->iCropBottom;
    cropTop   = stream->iCropTop;
    cropLeft  = stream->iCropLeft;
    cropRight = stream->iCropRight;
