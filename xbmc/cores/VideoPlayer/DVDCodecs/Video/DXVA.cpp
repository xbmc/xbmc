...
bool DXVA::CVideoBuffer::ReduceForAnalysis(KODI::VIDEO::GEOMETRY::ReducedFrame& reduction,
                                           unsigned int sourceWidth,
                                           unsigned int sourceHeight,
                                           unsigned int targetWidth)
{
  // The decoder-device surface and its array slice, bypassing the shared and copy subclasses'
  // overrides: their resources live on the app device, whose immediate context belongs to the
  // render thread.
  ComPtr<ID3D11Resource> resource;
  if (FAILED(CVideoBuffer::GetResource(&resource)))
    return false;

  const auto pool = std::dynamic_pointer_cast<CVideoBufferPool>(m_pool);
  if (!pool)
    return false;

  const bool fullRange = m_pFrame && m_pFrame->color_range == AVCOL_RANGE_JPEG;
  return pool->GetReadback()->Reduce(resource.Get(), CVideoBuffer::GetIdx(), sourceWidth,
                                     sourceHeight, fullRange, reduction, targetWidth);
}
...
std::shared_ptr<CSurfaceReadback> CVideoBufferPool::GetReadback()
{
  std::unique_lock lock(m_section);
  if (!m_readback)
    m_readback = std::make_shared<CSurfaceReadback>();
  return m_readback;
}
