...
bool CDVDVideoCodecAmlogic::Open(CDVDStreamInfo& hints, CDVDCodecOptions& options)
{
  ...
  logM(LOGDEBUG, "CDVDVideoCodecAmlogic", "codec {:d} profile:{:d} extra_size:{:d} fps:{:d}/{:d}",
       m_hints.codec, m_hints.profile, m_hints.extradata.GetSize(), m_hints.fpsrate,
       m_hints.fpsscale);
  ...
  switch (m_hints.codec)
  {
    ...
    case AV_CODEC_ID_MPEG2VIDEO:
      if (m_hints.width <= CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
                               CSettings::SETTING_VIDEOPLAYER_USEAMCODECMPEG2))
        goto FAIL;

      switch (m_hints.profile)
      {
        case FF_PROFILE_MPEG2_422:
          logM(LOGDEBUG, "CDVDVideoCodecAmlogic", "MPEG2 unsupported hints.profile({:d})",
               m_hints.profile);
          goto FAIL;
      }
      ...
      if ((m_hints.width == 720 || m_hints.width == 544 || m_hints.width == 480) &&
          m_hints.height == 576 && m_hints.aspect == 0.0)
    ...
    case AV_CODEC_ID_H264:
      if (m_hints.width <= CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
                               CSettings::SETTING_VIDEOPLAYER_USEAMCODECH264))
      {
        logM(LOGDEBUG, "CDVDVideoCodecAmlogic", "h264 size check failed {:d}",
      ...
      switch (hints.profile)
      {
        ...
        case FF_PROFILE_H264_CAVLC_444:
          logM(LOGDEBUG, "CDVDVideoCodecAmlogic", "H264 unsupported hints.profile({:d})",
               m_hints.profile);
          goto FAIL;
      }
      if ((aml_support_h264_4k2k() == AML_NO_H264_4K2K) &&
          ((m_hints.width > 1920) || (m_hints.height > 1088)))
      {
        logM(LOGDEBUG, "CDVDVideoCodecAmlogic",
             "4K H264 is supported only on Amlogic S802 and S812 chips or newer");
        goto FAIL;
      }
      ...
      if (m_hints.extradata && m_hints.extradata.GetData()[0] == 1)
      {
        ...
        memcpy(m_hints.extradata.GetData(), m_bitstream->GetExtraData(),
      ...
      // correct aspect ratio will be detected later anyway
      if ((m_hints.width == 1440 || m_hints.width == 1280) && m_hints.height == 1080 &&
          m_hints.aspect == 0.0)
        m_hints.aspect = 16.0 / 9.0;
      ;
    ...
    case AV_CODEC_ID_MSMPEG4V3:
      if (m_hints.width <= CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
                               CSettings::SETTING_VIDEOPLAYER_USEAMCODECMPEG4))
        goto FAIL;
    ...
    case AV_CODEC_ID_VP9:
      ...
      {
        logM(LOGDEBUG, "CDVDVideoCodecAmlogic",
             "VP9 hardware decoder is not supported on current platform");
        goto FAIL;
      }
    ...
    case AV_CODEC_ID_AV1:
      ...
      {
        logM(LOGDEBUG, "CDVDVideoCodecAmlogic",
             "AV1 hardware decoder is not supported on current platform");
        goto FAIL;
      }
    ...
    case AV_CODEC_ID_HEVC:
      if (aml_support_hevc())
      {
        ...
        {
          logM(LOGDEBUG, "CDVDVideoCodecAmlogic",
               "8K HEVC hardware decoder is not supported on current platform");
          goto FAIL;
        }
        else if (!aml_support_hevc_4k2k() && ((m_hints.width > 1920) || (m_hints.height > 1088)))
        {
          logM(LOGDEBUG, "CDVDVideoCodecAmlogic",
               "4K HEVC hardware decoder is not supported on current platform");
          goto FAIL;
        }
      }
      else
      {
        logM(LOGDEBUG, "CDVDVideoCodecAmlogic",
             "HEVC hardware decoder is not supported on current platform");
        goto FAIL;
      }
      if ((hints.profile == FF_PROFILE_HEVC_MAIN_10) && !aml_support_hevc_10bit())
      {
        logM(LOGDEBUG, "CDVDVideoCodecAmlogic",
             "HEVC 10-bit hardware decoder is not supported on current platform");
        goto FAIL;
      }
      ...
      if (m_hints.extradata && !m_hints.cryptoSession && m_bitstream &&
          (aml_dv_mode() != DV_MODE::OFF))
      {
        auto settings = CServiceBroker::GetSettingsComponent()->GetSettings();

        bool dualPriorityHdr10Plus =
        ...
        if (m_hints.hdrType == StreamHdrType::HDR_TYPE_DOLBYVISION)
        {
          ...
          {
            logM(LOGINFO, "CDVDVideoCodecAmlogic",
                 "DV HEVC bitstream - if stream also contains HDR10+, native HDR10+ has priority.");
          ...
          else if (settings->GetBool(CSettings::SETTING_COREELEC_AMLOGIC_DV_HDR10PLUS_CONVERT))
          {
            ...
            if (preferConvertHdr10Plus)
              logM(LOGINFO, "CDVDVideoCodecAmlogic",
                   "DV HEVC bitstream - if stream also contains HDR10+, conversion will be "
                   "preferred over original Dolby Vision.");
          }

          if (m_hints.dovi.dv_profile == 7)
          {
            auto convertDovi =
                static_cast<DOVIMode>(settings->GetInt(CSettings::SETTING_VIDEOPLAYER_CONVERTDOVI));
            if (convertDovi)
            {
              logM(LOGINFO, "CDVDVideoCodecAmlogic",
                   "DV HEVC bitstream - user chooses to convert to mode [{:d}]", convertDovi);
        ...
        {
          auto peakBrightnessSource = static_cast<PeakBrightnessSource>(settings->GetInt(
              CSettings::SETTING_COREELEC_AMLOGIC_DV_HDR10PLUS_PEAK_BRIGHTNESS_SOURCE));
          logM(LOGINFO, "CDVDVideoCodecAmlogic",
               "HEVC bitstream - if also HDR10+ then will be considered for conversion to Dolby "
               "Vision P8.1 with brightness source [{:d}]",
        ...
        {
          unsigned int mode(
              aml_vs10_by_setting(CSettings::SETTING_COREELEC_AMLOGIC_DV_VS10_HDR10PLUS));
          if (mode < DOLBY_VISION_OUTPUT_MODE_BYPASS)
          {
            // for VS10 conversion need to remove the HDR10plus metadata.
            logM(LOGINFO, "CDVDVideoCodecAmlogic",
                 "HDR10 HEVC bitstream - if HDR10+ then metadata will be removed to allow correct "
                 "VS10 processing");
...
bool CDVDVideoCodecAmlogic::DualLayerConvert(uint8_t* pData,
                                             uint32_t iSize,
                                             const DemuxPacket& packet)
{
  ...
  logComponentM(LOGDEBUG, LOGVIDEO, "CDVDVideoCodecAmlogic",
                "DT-DL {} package with dts: {:.3f}, pts: {:.3f} and size {} arrived, list {} empty",
                packet.isELPackage ? "EL" : "BL", packet.dts / DVD_TIME_BASE,
                packet.pts / DVD_TIME_BASE, iSize, m_packages.empty() ? "is" : "is not");
    ...
    if (isELPackageBackup != packet.isELPackage)
    {
      ...
      if (packet.isELPackage)
        dual_layer_converted =
            m_bitstream->Convert(pDataBackup, iSizeBackup, pData, iSize, packet.pts);
      else
        dual_layer_converted =
            m_bitstream->Convert(pData, iSize, pDataBackup, iSizeBackup, packet.pts);
    }
  }

  if (!dual_layer_converted) // backup package and don't send to decoder yet
  {
    auto pDataBackup = static_cast<uint8_t*>(
        KODI::MEMORY::AlignedMalloc(packet.iSize + AV_INPUT_BUFFER_PADDING_SIZE, 16));
    memcpy(pDataBackup, packet.pData, packet.iSize);
    m_packages.emplace_back(pDataBackup, iSize, packet.isELPackage, packet.dts);

    logComponentM(LOGDEBUG, LOGVIDEO, "CDVDVideoCodecAmlogic",
                  "did add DT-DL {} package with dts: {:.3f}, pts: {:.3f} and size {} in list",
                  packet.isELPackage ? "EL" : "BL", packet.dts / DVD_TIME_BASE,
                  packet.pts / DVD_TIME_BASE, packet.iSize);
  ...
  {
    logComponentM(LOGDEBUG, LOGVIDEO, "CDVDVideoCodecAmlogic",
                  "converted DT-DL with dts: {:.3f}, pts: {:.3f}", packet.dts / DVD_TIME_BASE,
...
bool CDVDVideoCodecAmlogic::SingleLayerConvert(uint8_t* pData,
                                               uint32_t iSize,
                                               const DemuxPacket& packet) const
{
...
bool CDVDVideoCodecAmlogic::AddData(const DemuxPacket& packet)
{
  ...
  if (pData)
  {
    ...
    if (!m_opened)
    {
      ...
      logM(LOGINFO, "CDVDVideoCodecAmlogic", "Open decoder: fps:{:d}/{:d}", m_hints.fpsrate,
           m_hints.fpsscale);
  ...
  m_last_added =
      m_Codec->AddData(pData, iSize, packet.dts, m_hints.ptsinvalid ? DVD_NOPTS_VALUE : packet.pts);
...
CDVDVideoCodec::VCReturn CDVDVideoCodecAmlogic::GetPicture(VideoPicture* pVideoPicture)
{
  ...
  if (retVal == VC_PICTURE)
  {
    ...
    static_cast<CAMLVideoBuffer*>(pVideoPicture->videoBuffer)
        ->Set(this, m_Codec, m_Codec->GetOMXPts(), m_Codec->GetAmlDuration(),
...
void CDVDVideoCodecAmlogic::SetCodecControl(int flags)
{
  ...
  {
    logComponentM(LOGDEBUG, LOGVIDEO, "CDVDVideoCodecAmlogic", "{:x}->{:x}", m_codecControlFlags,
                  flags);
...
void CDVDVideoCodecAmlogic::FrameRateTracking(uint8_t* pData, int iSize, double dts, double pts)
{
  ...
  if (m_mpeg2_sequence)
  {
        ...
        (m_mpeg2_sequence->fps_rate > 0) && (m_mpeg2_sequence->fps_scale > 0))
    {
      ...
      logM(LOGDEBUG, "CDVDVideoCodecAmlogic", "fps:{:d}/{:d} mpeg2_fps:{:d}/{:d} options:0x{:2x}",
           m_hints.fpsrate, m_hints.fpsscale, m_mpeg2_sequence->fps_rate,
           m_mpeg2_sequence->fps_scale, m_hints.codecOptions);
  ...
  if (m_h264_sequence)
  {
    ...
    if (CBitstreamConverter::h264_sequence_header(pData, iSize, m_h264_sequence.get()))
    {
      ...
      logM(LOGDEBUG, "CDVDVideoCodecAmlogic", "detected h264 aspect ratio({:f})",
