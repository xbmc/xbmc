  ...
  switch(m_hints.codec)
  {
    ...
    case AV_CODEC_ID_HEVC:
    {
      ...
      if (!removeDovi && (isDvhe || isDvh1))
      {
        ...
        // respect the configured frame rate ceiling so content the display cannot show in
        // Dolby Vision falls back to the display's real capabilities.
    ...
    case AV_CODEC_ID_AV1:
    {
      ...
      if (m_hints.hdrType == StreamHdrType::HDR_TYPE_DOLBYVISION && m_hints.dovi.dv_profile == 10)
      {
        ...
        // As in the HEVC path above: honour the forced Dolby Vision frame rate ceiling.
        if (displaySupportsDovi &&
            CAndroidUtils::IsForcedDolbyVisionBlockedForFps(GetHintsFrameRate(m_hints)))
          displaySupportsDovi = false;
