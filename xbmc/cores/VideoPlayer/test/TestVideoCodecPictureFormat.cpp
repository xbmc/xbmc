...
DecodedPicture DecodeFirstPicture(const std::string& path)
{
  ...
  CDVDVideoCodec::VCReturn state = CDVDVideoCodec::VC_NONE;
  ...
  for (int attemptsLeft = demuxer->GetNrOfStreams() * 160; attemptsLeft >= 0; --attemptsLeft)
  {
    ...
    if (packet->iStreamId != videoStream)
    {
      CDVDDemuxUtils::FreeDemuxPacket(packet);
      continue;
    }

    codec->AddData(*packet);
    CDVDDemuxUtils::FreeDemuxPacket(packet);

    state = CDVDVideoCodec::VC_NONE;
    while (state == CDVDVideoCodec::VC_NONE)
      state = codec->GetPicture(&picture);

    if (state == CDVDVideoCodec::VC_PICTURE && !(picture.iFlags & DVP_FLAG_DROPPED))
      break;
  }

  if (state != CDVDVideoCodec::VC_PICTURE)
    return result;
