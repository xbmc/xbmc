...
//! \brief How often the settings store is consulted: short enough that a value changed while
//! watching takes effect while watching.
...
{
  // The message goes before the state: a served reading describes the stream that is going
  // away, and once the selector forgets it nothing remembers there is anything to withdraw.
                                         ...
                                         int speed)
{
  ...
  // Every frame is measured; only the rules are re-read on a cadence. Reading them is dearer
  // than the measurement they configure.
  ...
  {
    // Turned off mid-playback: withdraw what was served and go dormant.
    Withdraw();
    SetState("off");
    return InForce();
  }
  ...
  // A declared ratio pins, so there is nothing a reading could change - and measuring
  // anyway would spend the frame access on a value nothing serves.
      ...
      picture.stereoMode != m_stereoMode)
  {
    ...
    // The shape the room is in decides which of two entries a reading between them resolves to
    // - 2.3881 is within tolerance of both scope ratios - and the resolver already uses it for
  ...
  if (reduced)
  {
    // The reading is in the reduction's coordinates; the selector, and everything served
    // from it, lives in coded space.
  ...
  // Built only while something is reading it: this line carries the reading, the debounce run
  // and the drop counts, so it changes every frame and costs up to three formatted strings.
  if (TakeDebugRequest())
  {
    if (reduced)
    {
                                        ...
                                        bool& reduced)
{
  ...
  // The picture's own planes are not readable - a hardware decoder's surface. Its device can
  // still scale the picture to a small target and hand that back, for a fraction of a percent
  // of the full frame's readback and the same vocabulary entry.
  if (picture.videoBuffer)
  {
    // Timed because the copy blocks this thread until the GPU has produced it. The average is
    // surfaced on the debug overlay.
    const auto reduceStart = std::chrono::steady_clock::now();
    if (picture.videoBuffer->ReduceForAnalysis(m_reduction, picture.iWidth, picture.iHeight,
...
{
  // Read here rather than per frame: the two cache reads take a lock shared with the GUI and
  // render threads, and a shape is served a handful of times in a title.
...
void CLiveGeometryMonitor::SetState(std::string_view state)
{
  ...
  // Compared before assigning: most of these are constants restated on every decoded frame, and
  // the declared-ratio line is 26 characters, past the small-string buffer.
  if (m_state != state)
    m_state = state;
}
