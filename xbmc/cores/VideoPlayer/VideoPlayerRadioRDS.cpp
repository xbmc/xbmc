...
static const char* piCountryCodes_E[15][7] = {
    // 0   1    2    3    4    5    6
    {"DE", "GR", "MA", "__", "MD", "__", "__"}, {"DZ", "CY", "CZ", "IE", "EE", "__", "__"},
    {"AD", "SM", "PL", "TR", "KG", "__", "__"}, {"IL", "CH", "VA", "MK", "__", "__", "__"},
    {"IT", "JO", "SK", "TJ", "__", "__", "__"}, {"BE", "FI", "SY", "__", "UA", "__", "__"},
    {"RU", "LU", "TN", "__", "__", "__", "__"}, {"PS", "BG", "__", "NL", "PT", "__", "__"},
...
static const std::string piRDSLanguageCodes[128] = {
    ...
    "___", "alb", "bre", "cat", "hrv", "wel", "cze", "dan",
    "ger", "eng", "spa", "epo", "est", "baq", "fae", "fre", // 0
    "fry", "gle", "gla", "glg", "ice", "ita", "smi", "lat",
    "lav", "ltz", "lit", "hun", "mlt", "dut", "nor", "oci", // 1
    "pol", "por", "rum", "rom", "srp", "slo", "slv", "fin",
    "swe", "tur", "nld", "wln", "___", "___", "___", "___", // 2
    "___", "___", "___", "___", "___", "___", "___", "___",
    "___", "___", "___", "___", "___", "___", "___", "___", // 3
    "___", "___", "___", "___", "___", "zul", "vie", "uzb",
    "urd", "ukr", "tha", "tel", "tat", "tam", "tgk", "swa", // 4
    "srn", "som", "sin", "sna", "scc", "rue", "rus", "que",
    "pus", "pan", "per", "pap", "ori", "nep", "nde", "mar", // 5
    "mol", "mys", "mlg", "mkd", "_?_", "kor", "khm", "kaz",
    "kan", "jpn", "ind", "hin", "heb", "hau", "grn", "guj", // 6
    "gre", "geo", "ful", "prs", "chv", "chi", "bur", "bul",
    "ben", "bel", "bam", "aze", "asm", "arm", "ara", "amh" // 7
};

/* ----------------------------------------------------------------------------------------------------------- */

#define EntityChars 56
static const char* entitystr[EntityChars] = {
    "&apos;",   "&amp;",    "&quot;",  "&gt",      "&lt",      "&copy;",   "&times;", "&nbsp;",
...
static const char* entitychar[EntityChars] = {
    "'", "&", "\"", ">", "<", "c", "*", " ", "Ä", "ä", "Ö", "ö", "Ü", "ü", "ß", "°", "À", "Á", "Â",
    "Ã", "à", "á",  "â", "ã", "È", "É", "Ê", "Ë", "è", "é", "ê", "ë", "Ì", "Í", "Î", "Ï", "ì", "í",
    "î", "ï", "Ò",  "Ó", "Ô", "Õ", "ò", "ó", "ô", "õ", "Ù", "Ú", "Û", "Ñ", "ù", "ú", "û", "ñ"};
  ...
  for (unsigned int i = 0; i < len; ++i)
  {
    ...
    if (m_UECPDataStart)
    {
      ...
      if (m_UECPDatabStuff == true)
      {
        ...
        {
          case 0x00:
            m_UECPData[m_UECPDataIndex] = 0xfd;
            break;
          case 0x01:
            m_UECPData[m_UECPDataIndex] = 0xfe;
            break;
          case 0x02:
            m_UECPData[m_UECPDataIndex] = 0xff;
            break;
          default:
            m_UECPData[++m_UECPDataIndex] = data[i]; // should never be
        }
        m_UECPDatabStuff = false;
      }
      else
      ...
      else
      {
        ...
        else
        {
          ...
          do
          {
            uint8_t* msg =
                m_UECPData +
                ptr; //!< Current selected UECP message element (increased if more as one element is in frame)
            switch (msg[UECP_ME_MEC])
            {
              case UECP_RDS_PI:
                ret = DecodePI(msg);
                break; //!< Program Identification
              case UECP_RDS_PS:
                ret = DecodePS(msg);
                break; //!< Program Service name (PS)
              case UECP_RDS_DI:
                ret = DecodeDI(msg);
                break; //!< Decoder Identification and dynamic PTY indicator
              case UECP_RDS_TA_TP:
                ret = DecodeTA_TP(msg);
                break; //!< Traffic Announcement and Traffic Programme bits.
              case UECP_RDS_MS:
                ret = DecodeMS(msg);
                break; //!< Music/Speech switch
              case UECP_RDS_PTY:
                ret = DecodePTY(msg);
                break; //!< Program Type
              case UECP_RDS_PTYN:
                ret = DecodePTYN(msg);
                break; //!< Program Type Name
              case UECP_RDS_RT:
                ret = DecodeRT(msg, len);
                break; //!< RadioText
              case UECP_ODA_DATA:
                ret = DecodeODA(msg, len);
                break; //!< Open Data Application
              case UECP_OTHER_RASS:
                m_UECPDataDeadBreak = true;
                break; //!< Radio screen show (RaSS) (not present, before on SWR radio)
              case UECP_CLOCK_RTC:
                ret = DecodeRTC(msg);
                break; //!< Real time clock
              case UECP_TDC_TMC:
                ret = DecodeTMC(msg, len);
                break; //!< Traffic message channel
              case UECP_EPP_TM_INFO:
                ret = DecodeEPPTransmitterInfo(msg);
                break; //!< EPP transmitter information
              case UECP_SLOW_LABEL_CODES:
                ret = DecodeSlowLabelingCodes(msg);
                break; //!< Slow Labeling codes
              case UECP_DAB_DYN_LABEL_CMD:
                ret = DecodeDABDynLabelCmd(msg, len);
                break; //!< DAB: Dynamic Label command
              case UECP_DAB_DYN_LABEL_MSG:
                ret = DecodeDABDynLabelMsg(msg, len);
                break; //!< DAB: Dynamic Label message (DL)
              case UECP_RDS_AF:
                ret = DecodeAF(msg, len);
                break; //!< Alternative Frequencies list
              case UECP_RDS_EON_AF:
                ret = DecodeEonAF(msg, len);
                break; //!< EON Alternative Frequencies list
              case UECP_TDC_TDC:
                ret = DecodeTDC(msg, len);
                break; //!< Transparent Data Channel
              case UECP_LINKAGE_INFO:
                ret = 5;
                break; //!< Linkage information
              case UECP_TDC_EWS:
                ret = 6;
                break; //!< Emergency warning system
              case UECP_RDS_PIN:
                ret = 5;
                break; //!< Program Item Number
              case UECP_TDC_IH:
                ret = 7;
                break; //!< In-house applications (Should be ignored)
              case UECP_TDC_FREE_FMT_GROUP:
                ret = 7;
                break; //!< Free-format group (unused)
              case UECP_ODA_CONF_SHORT_MSG_CMD:
                ret = 8;
                break; //!< ODA Configuration and Short Message Command (unused)
              case UECP_CLOCK_RTC_CORR:
                ret = 3;
                break; //!< Real time clock correction (unused)
              case UECP_CLOCK_CT_ON_OFF:
                ret = 2;
                break; //!< Real time clock on/off (unused)
              default:
          ...
          {
            CServiceBroker::GetGUI()->GetInfoManager().SetCurrentItem(
                g_application.CurrentFileItem());
            m_currentFileUpdate = false;
          }
        }
      }
...
unsigned int CDVDRadioRDSData::DecodeTA_TP(const uint8_t* msgElement)
{
  ...
  if (traffic_announcement && !m_TA_TP_TrafficAdvisory && traffic_programme && dsn == 0 &&
      CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool("pvrplayback.trafficadvisory"))
  {
    CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Warning, g_localizeStrings.Get(19021),
                                          g_localizeStrings.Get(29930));
    m_TA_TP_TrafficAdvisory = true;
    auto& components = CServiceBroker::GetAppComponents();
    const auto appVolume = components.GetComponent<CApplicationVolumeHandling>();
    m_TA_TP_TrafficVolume = appVolume->GetVolumePercent();
    float trafAdvVol = (float)CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(
  ...
  if (!traffic_announcement && m_TA_TP_TrafficAdvisory &&
      CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool("pvrplayback.trafficadvisory"))
  {
    m_TA_TP_TrafficAdvisory = false;
    auto& components = CServiceBroker::GetAppComponents();
...
pty_skin_info pty_skin_info_table[32][2] = {{{"none", 29940}, {"none", 29940}},
...
unsigned int CDVDRadioRDSData::DecodePTY(const uint8_t* msgElement)
{
  ...
  if (pty >= 0 && pty < 32 && m_PTY != pty)
  {
    ...
    if (m_PTY == RDS_PTY_ALARM_TEST)
      CGUIDialogKaiToast::QueueNotification(CGUIDialogKaiToast::Info, g_localizeStrings.Get(29931),
                                            g_localizeStrings.Get(29970), TOAST_DISPLAY_TIME,
                                            false);

    if (m_PTY == RDS_PTY_ALARM)
    {
      CGUIDialogKaiToast::QueueNotification(
          CGUIDialogKaiToast::Warning, g_localizeStrings.Get(29931), g_localizeStrings.Get(29971),
...
unsigned int CDVDRadioRDSData::DecodeRT(uint8_t* msgElement, unsigned int len)
{
  ...
  else
  {
    ...
    {
      if (msgElement[UECP_ME_DATA + i] <=
          0xfe) // additional rds-character, see RBDS-Standard, Annex E
        temptext[ii++] = (msgElement[UECP_ME_DATA + i] >= 0x80)
                             ? sRDSAddChar[msgElement[UECP_ME_DATA + i] - 0x80]
                             : msgElement[UECP_ME_DATA + i];
    }
...
unsigned int CDVDRadioRDSData::DecodeRTC(uint8_t* msgElement)
{
  ...
  m_RTC_DateTime.SetDateTime(msgElement[UECP_CLOCK_YEAR], msgElement[UECP_CLOCK_MONTH],
                             msgElement[UECP_CLOCK_DAY], hours, minutes,
                             msgElement[UECP_CLOCK_SECONDS]);
...
unsigned int CDVDRadioRDSData::DecodeRTPlus(uint8_t* msgElement, unsigned int len)
{
  ...
  if ((msgElement[5] & 0x10) && (msgElement[5] & 0x08) && rtp_typ[0] == RTPLUS_INFO_URL &&
      rtp_typ[1] == RTPLUS_ITEM_ARTIST)
    return 10;
      ...
      switch (rtp_typ[i])
      {
        ...
        case RTPLUS_ITEM_TITLE: // Item-Title...
          if ((msgElement[5] & 0x08) > 0 && (m_RTPlus_iToggle & 0x01) == 0x01)
          {
            ...
            if (memcmp(m_RTPlus_Title, m_RTPlus_Temptext, RT_MEL) != 0 ||
                (msgElement[5] & 0x10) != m_RTPlus_ItemToggle)
            {
        ...
        case RTPLUS_ITEM_ARTIST: // Item-Artist..
          if ((msgElement[5] & 0x08) > 0 && (m_RTPlus_iToggle & 0x02) == 0x02)
          {
            ...
            if (memcmp(m_RTPlus_Artist, m_RTPlus_Temptext, RT_MEL) != 0 ||
                (msgElement[5] & 0x10) != m_RTPlus_ItemToggle)
            {
...
#ifdef RDS_IMPROVE_CHECK
    {
      ...
      if (m_RTPlus_iDiffs > 0)
        printf("  StartTime : %02d:%02d:%02d  (last Title elapsed = %d s)\n", ts->tm_hour,
               ts->tm_min, ts->tm_sec, m_RTPlus_iDiffs);
      else
