...
CRenderBufferShaders::CRenderBufferShaders(AVPixelFormat av_pix_format, unsigned width, unsigned height)
...
CRenderBufferShaders::~CRenderBufferShaders()
{
  CRenderBufferShaders::ReleasePicture();
}

void CRenderBufferShaders::AppendPicture(const VideoPicture& picture)
{
...
bool CRenderBufferShaders::IsLoaded()
{
  if (!videoBuffer)
    return false;
...
bool CRenderBufferShaders::UploadBuffer()
{
  if (!videoBuffer)
    return false;
...
unsigned CRenderBufferShaders::GetViewCount() const
{
  return m_viewCount;
}

ID3D11View* CRenderBufferShaders::GetView(unsigned viewIdx)
{
  if (videoBuffer->GetFormat() == AV_PIX_FMT_D3D11VA_VLD &&
      AV_PIX_FMT_D3D11VA_VLD == av_format)
  {
...
void CRenderBufferShaders::ReleasePicture()
{
...
bool CRenderBufferShaders::UploadFromGPU()
{
...
bool CRenderBufferShaders::UploadFromBuffer() const
{
  uint8_t* bufData[3];
  int srcLines[3];
...
CRenderBufferBase* CRendererShaders::CreateBuffer()
{
  return new CRenderBufferShaders(m_format, m_sourceWidth, m_sourceHeight);
}
