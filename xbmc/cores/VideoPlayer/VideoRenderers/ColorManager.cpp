...
bool CColorManager::Probe3dLut(const std::string& filename, int* clutSize)
{
  ...
  {
    CLog::Log(LOGERROR, "{}: Could not open 3DLUT file: {}", __FUNCTION__, filename.c_str());
    return false;
  }

  if (lutFile.Read(&header, sizeof(header)) < static_cast<ssize_t>(sizeof(header)))
  {
    CLog::Log(LOGERROR, "{}: Could not read 3DLUT header: {}", __FUNCTION__, filename.c_str());
    return false;
  }
  ...
  {
    CLog::Log(LOGERROR, "{}: Not a 3DLUT file: {}", __FUNCTION__, filename.c_str());
    return false;
  }
  ...
  {
    CLog::Log(LOGERROR, "{}: Unsupported 3DLUT file: {}", __FUNCTION__, filename.c_str());
    return false;
  }
