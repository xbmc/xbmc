...
// VideoPicture::color_range is a single bit, so an unspecified range has already been folded
// into limited by the time it reaches here. The hint still carries the real value, and the
// detector reports lower confidence when it had to assume.
                                             ...
                                             FrameRef& frame)
{
  ...
  // The stereo views split proportionally, so the view rectangle computed in the
  // reduction's coordinates is the reduction of the view rectangle.
 ...
 *
 * \param visited every point taken, kept across passes so an escalation can avoid them
 */
struct GeometrySampleRun
{
  ...
  //! \brief One pass over \p schedule: seek to each point, decode its pictures and read each
  //! one into the scan.
                                                         ...
                                                         const std::function<bool()>& cancelled)
{
  ...
  // Named on every per-point line: the background sweep and the measurement taken on the way
  // into playback run at once and write to the same log. The file name rather than the path,
  // which is on the two lines that bracket them.
  ...
  // A short pass cannot tell a fixed-ratio title from one whose changes it missed. Where it
  // looks inconclusive, densify on the open demuxer and judge the whole set together.
  ...
  // The clusters are the combining decision - which one won and by how much - and cannot be
  // recovered afterwards from the single rectangle that is served.
  ...
  // Every point is accounted for: usable and discarded are what the combiner saw, unreadable is
  // what never reached it, and the three sum to the points visited.
  CLog::LogF(LOGDEBUG,
