...
TEST_F(TestVideoBufferDMA, SetDimensionsSameValuesKeepsMemo)
{
...
TEST_F(TestVideoBufferDMA, AddonDoublePopulatePattern)
{
  auto buffer = std::make_unique<CVideoBufferDMA>(0, FOURCC_NV12, 2, 1920 * 1080 * 3 / 2);
  ASSERT_TRUE(buffer->Alloc());

  // CAddonVideoCodec populates dimensions once at GetFrameBuffer and again
  // at GetPicture, both times with identical values
  const int strides[YuvImage::MAX_PLANES] = {1920, 1920, 0};
  const int offsets[YuvImage::MAX_PLANES] = {0, 1920 * 1080, 0};
  buffer->SetDimensions(1920, 1080, strides, offsets);
  buffer->SetDimensions(1920, 1080, strides, offsets);
...
TEST_F(TestVideoBufferDMA, SetDimensionsChangedStrideInvalidates)
{
  auto buffer = std::make_unique<CVideoBufferDMA>(0, FOURCC_NV12, 2, 1920 * 1080 * 3 / 2);
  ASSERT_TRUE(buffer->Alloc());
...
TEST_F(TestVideoBufferDMA, SetDimensionsChangedGeometryInvalidates)
{
  auto buffer = std::make_unique<CVideoBufferDMA>(0, FOURCC_NV12, 2, 1920 * 1080 * 3 / 2);
  ASSERT_TRUE(buffer->Alloc());
  ...
  buffer->SetDimensions(1280, 720, strides, offsets);
  ASSERT_TRUE(buffer->GetIdentity(stat.Fn()));
  EXPECT_EQ(stat.calls, 2);
}
