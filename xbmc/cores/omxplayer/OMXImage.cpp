...
OMX_IMAGE_CODINGTYPE COMXImageFile::GetCodingType(unsigned int &width, unsigned int &height, int orientation)
{
  ...
  if(!m_image_size)
  {
    CLog::Log(LOGERROR, "%s::%s %s m_image_size unexpected (%lu)", CLASSNAME, __func__, m_filename, m_image_size);
...
bool COMXImageFile::ReadFile(const std::string& inputFile, int orientation)
{
  ...
  {
    CLog::Log(LOGERROR, "%s::%s %s not found", CLASSNAME, __func__, m_filename);
    return false;
  }
  ...
  if(!m_image_size)
  {
    CLog::Log(LOGERROR, "%s::%s %s m_image_size zero", CLASSNAME, __func__, m_filename);
    return false;
  }
  m_image_buffer = (uint8_t *)malloc(m_image_size);
  if(!m_image_buffer)
  {
    CLog::Log(LOGERROR, "%s::%s %s m_image_buffer null (%lu)", CLASSNAME, __func__, m_filename, m_image_size);
    return false;
  }
  ...
  {
    CLog::Log(LOGDEBUG, "%s::%s %s GetCodingType=0x%x (%dx%d)", CLASSNAME, __func__, m_filename, eCompressionFormat, m_width, m_height);
    return false;
  }
