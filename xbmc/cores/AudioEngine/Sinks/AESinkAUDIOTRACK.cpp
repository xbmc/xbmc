...
void CAESinkAUDIOTRACK::Deinitialize()
{
  ...
  if (diff > 0)
  {
    CLog::Log(LOGDEBUG, "Flushing might not be properly implemented, sleeping: {} ms", diff);
    usleep(diff * 1000);
  }
}
...
void CAESinkAUDIOTRACK::GetDelay(AEDelayStatus& status)
{
  ...
  {
    CLog::Log(LOGDEBUG, "You did it wrong man - fully wrong! offset {} head pos {}", m_offset, m_headPos);
    m_offset = 0;
  }
