...
bool CDVDDemuxFFmpeg::Open(CDVDInputStream* pInput)
{
  ...
  else
  {
    ...
    if( iformat == NULL )
    {
      ...
      if(context->is_streamed && !m_pInput->IsStreamType(DVDSTREAM_TYPE_PVRMANAGER))
  ...
  if (streaminfo)
  {
    if (m_pInput->IsStreamType(DVDSTREAM_TYPE_TV) || m_pInput->IsStreamType(DVDSTREAM_TYPE_PVRMANAGER))
    {
