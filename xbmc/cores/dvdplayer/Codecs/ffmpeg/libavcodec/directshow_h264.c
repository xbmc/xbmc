                                    ...
                                    DXVA_PicParams_H264 *pp)
{
    ...
    fill_picture_entry(&pp->CurrPic,
                       127,/*ff_dxva2_get_surface_index(ctx, current_picture),*/
    ...
    for (i = 0, j = 0; i < FF_ARRAY_ELEMS(pp->RefFrameList); i++) {
        ...
        if (r) {
            fill_picture_entry(&pp->RefFrameList[i],
                               127,/*ff_dxva2_get_surface_index(ctx, r),*/
                               r->long_ref != 0);
    ...
    /*pp->StatusReportFeedbackNumber    = 1 + ctx->report_id++;*/ /*  This is not set in ffmpeg*/
                                             ...
                                             DXVA2_DecodeBufferDesc *sc)
{
    ...
    struct dxva1_picture_context *ctx_pic = current_picture->data[3];
                       ...
                       av_unused uint32_t size)
{
    const H264Context *h = avctx->priv_data;
    struct dxva_context *ctx = avctx->hwaccel_context;
    struct dxva1_picture_context *ctx_pic = h->s.current_picture_ptr->data[3];

    /* this is only for dxva2 */
...
static int end_frame(AVCodecContext *avctx)
{
    ...
    struct dxva1_picture_context *ctx_pic =
        h->s.current_picture_ptr->data[3];
    
    if (ctx_pic->slice_count <= 0 || ctx_pic->bitstream_size <= 0)
        return -1;
    return 0;/*ff_dxva2_common_end_frame(avctx, s,
