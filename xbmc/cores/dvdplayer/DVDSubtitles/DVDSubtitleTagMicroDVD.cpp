...
void CDVDSubtitleTagMicroDVD::ConvertLine(CDVDOverlayText* pOverlay, const char* line, int len)
{
  ...
  while (machine_status > 0)
  {
    if (machine_status == 1)
    {
      ...
      {
        size_t pos2, pos3;
        if (((pos2 = strUTF8.find(':', pos)) != string::npos) && \
           ((pos3 = strUTF8.find('}', pos2)) != string::npos))
        {
          string tagName = strUTF8.substr(pos + 1, pos2 - pos - 1);
          string tagValue = strUTF8.substr(pos2 + 1, pos3 - pos2 - 1);
    ...
    {
      size_t pos4;
      if ((pos4= strUTF8.find('|', pos)) != string::npos)
      {
        pos = pos4;
