...
bool CDVDSubtitleParserVplayer::Open(CDVDStreamInfo &hints)
{
  ...
  while (m_pStream->ReadLine(line, sizeof(line)))
  {
    ...
    {
      std::string hour = reg.GetReplaceStdString("\\1");
      std::string min  = reg.GetReplaceStdString("\\2");
      std::string sec  = reg.GetReplaceStdString("\\3");
      std::string lines[3];
      lines[0] = reg.GetReplaceStdString("\\4");
      lines[1] = reg.GetReplaceStdString("\\6");
      lines[2] = reg.GetReplaceStdString("\\8");
