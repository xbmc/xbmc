...
void CDVDOverlayRenderer::Render(YV12Image* pImage, CDVDOverlay* pOverlay)
{
  if (pOverlay->IsOverlayType(DVDOVERLAY_TYPE_SPU))
  {
    Render_SPU_YUV(pImage, (CDVDOverlaySpu*)pOverlay, pOverlay->bForced);
  }
...
void CDVDOverlayRenderer::RenderI420(DVDVideoPicture* pPicture, CDVDOverlaySpu* pOverlay, bool bCrop)
{
  ...
  p_dest[0] = pPicture->data[0] + pOverlay->x + pPicture->iLineSize[0] * pOverlay->y;
  p_dest[1] = pPicture->data[1] + (pOverlay->x >> 1) + (pOverlay->y >> 1) * pPicture->iLineSize[1];
  ...
  for ( i_y = 0; i_y < pOverlay->height; i_y ++ )
  {
    ...
    p_dest[0] += pPicture->iLineSize[0];
    if (i_y & 1)
      continue;
    p_dest[1] += pPicture->iLineSize[1];
    p_dest[2] += pPicture->iLineSize[2];
  }
}

// render the parsed sub (parsed rle) onto the yuv image
void CDVDOverlayRenderer::Render_SPU_YUV(YV12Image* pImage, CDVDOverlaySpu* pOverlay, bool bCrop)
{
  unsigned __int8* p_dest[3];
  unsigned __int8* p_destptr;
  unsigned __int16* p_source = (unsigned __int16*)pOverlay->pData;
  ...
  p_dest[0] = pImage->plane[0] + pOverlay->y * pImage->stride[0];
  p_dest[1] = pImage->plane[1] + (pOverlay->y >> 1) * pImage->stride[1];
  p_dest[2] = pImage->plane[2] + (pOverlay->y >> 1) * pImage->stride[2];;
  ...
  for ( i_y = pOverlay->y; i_y < pOverlay->y + pOverlay->height; i_y ++ )
  {
    ...
    p_dest[0] += pImage->stride[0];
    if (i_y & 1)
      continue;
    p_dest[1] += pImage->stride[1];
    p_dest[2] += pImage->stride[2];
  }
}