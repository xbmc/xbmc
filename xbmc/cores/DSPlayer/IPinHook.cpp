...
{
	return DShowUtil::GetDXVAMode (&g_guidDXVADecoder);
}
...
static HRESULT STDMETHODCALLTYPE GetVideoAcceleratorGUIDsMine(IAMVideoAcceleratorC * This, LPDWORD pdwNumGuidsSupported, LPGUID pGuidsSupported)
{
	...
	if(pdwNumGuidsSupported)
	{
		...
		if(pGuidsSupported)
		{
			...
			{
				LOG(_T("[out] pGuidsSupported[%d] = %s"), i, DShowUtil::CStringFromGUID(pGuidsSupported[i]));
			}
		}
	}
...
static HRESULT STDMETHODCALLTYPE GetUncompFormatsSupportedMine(IAMVideoAcceleratorC * This, const GUID *pGuid, LPDWORD pdwNumFormatsSupported, LPDDPIXELFORMAT pFormatsSupported)
{
	...
	if(pGuid)
	{
		LOG(_T("[in] *pGuid = %s"), DShowUtil::CStringFromGUID(*pGuid));
	}
											 ...
											 LPVOID lpPrivateOutputData, DWORD cbPrivateOutputData, DWORD dwNumBuffers, const AMVABUFFERINFO *pamvaBufferInfo)
{
	...
	if(lpPrivateInputData)
	{
		...
		else if ((dwFunction == 0xfffff101) || (dwFunction == 0xfffff501))
		{
			...
			LOG(_T("	 ConfigRequested->guidConfigBitstreamEncryption	= %s"), DShowUtil::CStringFromGUID (ConfigRequested->guidConfigBitstreamEncryption));
			LOG(_T("	 ConfigRequested->guidConfigMBcontrolEncryption	= %s"), DShowUtil::CStringFromGUID (ConfigRequested->guidConfigMBcontrolEncryption));
			LOG(_T("	 ConfigRequested->guidConfigResidDiffEncryption	= %s"), DShowUtil::CStringFromGUID (ConfigRequested->guidConfigResidDiffEncryption));
		}
		else
...
static void LogDXVA2Config (const DXVA2_ConfigPictureDecode* pConfig)
{
	...
	LOG(_T("	- guidConfigBitstreamEncryption     %s"), DShowUtil::CStringFromGUID(pConfig->guidConfigBitstreamEncryption));
	LOG(_T("	- guidConfigMBcontrolEncryption     %s"), DShowUtil::CStringFromGUID(pConfig->guidConfigMBcontrolEncryption));
	LOG(_T("	- guidConfigResidDiffEncryption     %s"), DShowUtil::CStringFromGUID(pConfig->guidConfigResidDiffEncryption));
}
...
static void LogVideoCardCaps(IDirectXVideoDecoderService* pDecoderService)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {
			LOG (_T("=== New mode : %s"), DShowUtil::GetDXVAMode (&pDecoderGuids[iGuid]));
...
//	((DXVA2_VideoDesc*)pVideoDesc)->Format = (D3DFORMAT)0x3231564E;
	...
	LOG(_T("IDirectXVideoDecoderService::CreateVideoDecoder  %s  (%d render targets) hr = %08x"), DShowUtil::GetDXVAMode(&g_guidDXVADecoder), NumRenderTargets, hr);
