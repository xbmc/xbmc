...
public:
	...
	SmartAutoPtr< T >& operator=( SmartAutoPtr< TSrc >& p ) throw()
	{
		if(m_p==p.m_p)
		{
			// This means that two SmartAutoPtr of two different types had the same m_p in them
...
template <typename T>
  class SmartAutoPtrForList : public SmartAutoPtr<T>
  {
  public:
    SmartAutoPtrForList() throw()
    {
    }
    SmartAutoPtrForList(T* lp) throw() :
    SmartAutoPtr<T>(lp)

    {
    }
    SmartAutoPtrForList(const SmartAutoPtr<T>& lp) throw() :
    SmartAutoPtr<T>(lp)
    {
    }
    T* operator=(T* lp) throw()
    ...
    template <typename Q>
    T* operator=(const SmartAutoPtr<Q>& lp) throw()
    {
      if( !AreComObjectsEqual(*this, lp) )
      {
        return static_cast<T*>(SmartQIPtrAssign((IUnknown**)&m_ptr, lp.m_ptr, __uuidof(T)));
    ...
    T* operator=(const SmartAutoPtr<T>& lp) throw()
    {
      if( !AreComObjectsEqual(m_ptr, lp.m_ptr) )
      {
        return static_cast<T*>(SmartPtrAssign((IUnknown**)&m_ptr, lp.m_ptr));
