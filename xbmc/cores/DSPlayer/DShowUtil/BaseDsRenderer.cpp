...
#include <basedsrenderer.h>        // DirectShow base class definitions
...
// Implements the CBaseDsRenderer class

CBaseDsRenderer::CBaseDsRenderer(REFCLSID RenderClass, // CLSID for this renderer
...
// We will probably also have made an input pin based on CDsRendererInputPin
// that has to be deleted, it's created when an enumerator calls our GetPin

CBaseDsRenderer::~CBaseDsRenderer()
{
...
HRESULT CBaseDsRenderer::GetMediaPositionInterface(REFIID riid, __deref_out void **ppv)
{
...
STDMETHODIMP CBaseDsRenderer::NonDelegatingQueryInterface(REFIID riid, __deref_out void **ppv)
{
...
HRESULT CBaseDsRenderer::SourceThreadCanWait(BOOL bCanWait)
{
    if (bCanWait == TRUE) {
...
void CBaseDsRenderer::DisplayRendererState()
{
...
HRESULT CBaseDsRenderer::WaitForRenderTime()
{
...
void CBaseDsRenderer::WaitForReceiveToComplete()
{
    for (;;) {
        if (!m_bInReceive) {
            break;
...
FILTER_STATE CBaseDsRenderer::GetRealState() {
    return m_State;
}
...
STDMETHODIMP CBaseDsRenderer::GetState(DWORD dwMSecs,FILTER_STATE *State)
{
...
HRESULT CBaseDsRenderer::CompleteStateChange(FILTER_STATE OldState)
{
...
STDMETHODIMP CBaseDsRenderer::Stop()
{
...
STDMETHODIMP CBaseDsRenderer::Pause()
{
    CAutoLock cRendererLock(&m_InterfaceLock);
...
STDMETHODIMP CBaseDsRenderer::Run(REFERENCE_TIME StartTime)
{
    CAutoLock cRendererLock(&m_InterfaceLock);
    FILTER_STATE OldState = m_State;
...
int CBaseDsRenderer::GetPinCount()
{
    if (m_pInputPin == NULL) {
        //  Try to create it
        (void)GetPin(0);
...
CBasePin *CBaseDsRenderer::GetPin(int n)
{
    CAutoLock cObjectCreationLock(&m_ObjectCreationLock);
    ...
    if (m_pInputPin == NULL) {
        ...
        // CDsRendererInputPin's constructor only changes
        // hr's value if an error occurs.
        HRESULT hr = NOERROR;

        m_pInputPin = new CDsRendererInputPin(this,&hr,L"In");
        if (NULL == m_pInputPin) {
            return NULL;
        }
...
STDMETHODIMP CBaseDsRenderer::FindPin(LPCWSTR Id, __deref_out IPin **ppPin)
{
...
HRESULT CBaseDsRenderer::EndOfStream()
{
...
HRESULT CBaseDsRenderer::BeginFlush()
{
...
HRESULT CBaseDsRenderer::EndFlush()
{
...
HRESULT CBaseDsRenderer::CompleteConnect(IPin *pReceivePin)
{
...
HRESULT CBaseDsRenderer::Active()
{
    return NOERROR;
}
...
HRESULT CBaseDsRenderer::Inactive()
{
    if (m_pPosition) {
...
HRESULT CBaseDsRenderer::SetMediaType(const CMediaType *pmt)
{
    return NOERROR;
}
...
HRESULT CBaseDsRenderer::BreakConnect()
{
...
HRESULT CBaseDsRenderer::GetSampleTimes(IMediaSample *pMediaSample,
...
HRESULT CBaseDsRenderer::ShouldDrawSampleNow(IMediaSample *pMediaSample,
...
void CBaseDsRenderer::SignalTimerFired()
{
    m_dwAdvise = 0;
}
...
HRESULT CBaseDsRenderer::CancelNotification()
{
...
BOOL CBaseDsRenderer::ScheduleSample(IMediaSample *pMediaSample)
{
...
HRESULT CBaseDsRenderer::Render(IMediaSample *pMediaSample)
{
...
BOOL CBaseDsRenderer::HaveCurrentSample()
{
...
IMediaSample *CBaseDsRenderer::GetCurrentSample()
{
    CAutoLock cRendererLock(&m_RendererLock);
    if (m_pMediaSample) {
...
HRESULT CBaseDsRenderer::PrepareReceive(IMediaSample *pMediaSample)
{
...
HRESULT CBaseDsRenderer::Receive(IMediaSample *pSample)
{
    ASSERT(pSample);
...
HRESULT CBaseDsRenderer::ClearPendingSample()
{
    CAutoLock cRendererLock(&m_RendererLock);
    if (m_pMediaSample) {
...
HRESULT CBaseDsRenderer::SendEndOfStream()
{
...
HRESULT CBaseDsRenderer::NotifyEndOfStream()
{
    CAutoLock cRendererLock(&m_RendererLock);
...
HRESULT CBaseDsRenderer::ResetEndOfStream()
{
    ResetEndOfStreamTimer();
    CAutoLock cRendererLock(&m_RendererLock);
...
void CBaseDsRenderer::ResetEndOfStreamTimer()
{
...
HRESULT CBaseDsRenderer::StartStreaming()
{
    CAutoLock cRendererLock(&m_RendererLock);
...
HRESULT CBaseDsRenderer::StopStreaming()
{
    CAutoLock cRendererLock(&m_RendererLock);
    m_bEOSDelivered = FALSE;
...
void CBaseDsRenderer::SetRepaintStatus(BOOL bRepaint)
{
    CAutoLock cSampleLock(&m_RendererLock);
...
void CBaseDsRenderer::SendNotifyWindow(IPin *pPin,HWND hwnd)
{
...
void CBaseDsRenderer::SendRepaint()
{
    CAutoLock cSampleLock(&m_RendererLock);
    ASSERT(m_pInputPin);
...
BOOL CBaseDsRenderer::OnDisplayChange()
{
...
void CBaseDsRenderer::OnRenderStart(IMediaSample *pMediaSample)
{
#ifdef PERF
...
void CBaseDsRenderer::OnRenderEnd(IMediaSample *pMediaSample)
{
#ifdef PERF
...
CDsRendererInputPin::CDsRendererInputPin(__inout CBaseDsRenderer *pRenderer,
                                     __inout HRESULT *phr,
...
STDMETHODIMP CDsRendererInputPin::EndOfStream()
{
...
STDMETHODIMP CDsRendererInputPin::BeginFlush()
{
    CAutoLock cRendererLock(&m_pRenderer->m_InterfaceLock);
    {
        CAutoLock cSampleLock(&m_pRenderer->m_RendererLock);
...
STDMETHODIMP CDsRendererInputPin::EndFlush()
{
    CAutoLock cRendererLock(&m_pRenderer->m_InterfaceLock);
    CAutoLock cSampleLock(&m_pRenderer->m_RendererLock);
...
STDMETHODIMP CDsRendererInputPin::Receive(IMediaSample *pSample)
{
...
HRESULT CDsRendererInputPin::BreakConnect()
{
...
HRESULT CDsRendererInputPin::CompleteConnect(IPin *pReceivePin)
{
...
STDMETHODIMP CDsRendererInputPin::QueryId(__deref_out LPWSTR *Id)
{
...
HRESULT CDsRendererInputPin::CheckMediaType(const CMediaType *pmt)
{
...
HRESULT CDsRendererInputPin::Active()
{
...
HRESULT CDsRendererInputPin::Inactive()
{
...
HRESULT CDsRendererInputPin::SetMediaType(const CMediaType *pmt)
{
...
CBaseDsVideoRenderer::CBaseDsVideoRenderer(
      REFCLSID RenderClass, // CLSID for this renderer
      __in_opt LPCTSTR pName,         // Debug ONLY description
      __inout_opt LPUNKNOWN pUnk,       // Aggregated owner object
      __inout HRESULT *phr) :       // General OLE return code

    CBaseDsRenderer(RenderClass,pName,pUnk,phr),
    m_cFramesDropped(0),
    m_cFramesDrawn(0),
...
HRESULT CBaseDsVideoRenderer::ResetStreamingTimes()
{
...
HRESULT CBaseDsVideoRenderer::OnStartStreaming()
{
    ResetStreamingTimes();
    return NOERROR;
} // OnStartStreaming
...
HRESULT CBaseDsVideoRenderer::OnStopStreaming()
{
...
void CBaseDsVideoRenderer::OnWaitStart()
{
...
void CBaseDsVideoRenderer::OnWaitEnd()
{
#ifdef PERF
...
void CBaseDsVideoRenderer::PreparePerformanceData(int trLate, int trFrame)
{
    m_trLate = trLate;
    m_trFrame = trFrame;
...
void CBaseDsVideoRenderer::RecordFrameLateness(int trLate, int trFrame)
{
...
void CBaseDsVideoRenderer::ThrottleWait()
{
    if (m_trThrottle>0) {
...
void CBaseDsVideoRenderer::OnDirectRender(IMediaSample *pMediaSample)
{
    m_trRenderAvg = 0;
...
void CBaseDsVideoRenderer::OnRenderStart(IMediaSample *pMediaSample)
{
    RecordFrameLateness(m_trLate, m_trFrame);
...
void CBaseDsVideoRenderer::OnRenderEnd(IMediaSample *pMediaSample)
{
...
STDMETHODIMP CBaseDsVideoRenderer::SetSink( IQualityControl * piqc)
{
...
STDMETHODIMP CBaseDsVideoRenderer::Notify( IBaseFilter * pSelf, Quality q)
{
...
HRESULT CBaseDsVideoRenderer::SendQuality(REFERENCE_TIME trLate,
...
HRESULT CBaseDsVideoRenderer::ShouldDrawSampleNow(IMediaSample *pMediaSample,
...
BOOL CBaseDsVideoRenderer::ScheduleSample(IMediaSample *pMediaSample)
{
    // We override ShouldDrawSampleNow to add quality management

    BOOL bDrawImage = CBaseDsRenderer::ScheduleSample(pMediaSample);
    if (bDrawImage == FALSE) {
  ++m_cFramesDropped;
  return FALSE;
    }
...
STDMETHODIMP CBaseDsVideoRenderer::get_FramesDroppedInRenderer(__out int *pcFramesDropped)
{
...
STDMETHODIMP CBaseDsVideoRenderer::get_FramesDrawn( int *pcFramesDrawn)
{
...
STDMETHODIMP CBaseDsVideoRenderer::get_AvgFrameRate( int *piAvgFrameRate)
{
...
STDMETHODIMP CBaseDsVideoRenderer::get_AvgSyncOffset(__out int *piAvg)
{
...
//  statistics
//
HRESULT CBaseDsVideoRenderer::GetStdDev(
    int nSamples,
    __out int *piResult,
    LONGLONG llSumSq,
    LONGLONG iTot
...
STDMETHODIMP CBaseDsVideoRenderer::get_DevSyncOffset(__out int *piDev)
{
    // First frames have invalid stamps, so we get no stats for them
    // So we need 2 frames to get 1 datum, so N is cFramesDrawn-1
...
STDMETHODIMP CBaseDsVideoRenderer::get_Jitter(__out int *piJitter)
{
    // First frames have invalid stamps, so we get no stats for them
...
STDMETHODIMP
CBaseDsVideoRenderer::NonDelegatingQueryInterface(REFIID riid,__deref_out VOID **ppv)
{
    ...
    return CBaseDsRenderer::NonDelegatingQueryInterface(riid,ppv);
}
...
STDMETHODIMP
CBaseDsVideoRenderer::JoinFilterGraph(__inout_opt IFilterGraph *pGraph, __in_opt LPCWSTR pName)
{
