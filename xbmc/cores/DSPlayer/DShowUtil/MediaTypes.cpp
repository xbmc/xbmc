...
CStdString VIH2String(int i)
{
  CStdString ret = CStdString(GuidNames[*vihs[i].subtype]);
...
CStdString Subtype2String(const GUID& subtype)
{
  CStdString ret = CStdString(GuidNames[subtype]);
  if(!ret.Left(13).CompareNoCase(_T("MEDIASUBTYPE_"))) ret = ret.Mid(13);
