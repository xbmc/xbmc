...
HRESULT CFGLoader::LoadFilterRules(const CFileItem& _pFileItem)
{
    ...
    pFileItem.HasVideoInfoTag() && !pFileItem.GetVideoInfoTag()->HasStreamDetails())
  {
    ...
    CDVDFileInfo::GetFileStreamDetails(&pFileItem);
  }
  ...
  BeginEnumPins(pBF, pEP, pPin)
  {
    ...
    BeginEnumMediaTypes(pPin, pEMT, pMT)
    {
      ...
      if (! fourccfinded)
      {
        ...
        CLog::Log(LOGINFO, "%s \tVideo stream fourcc : %c%c%c%c", __FUNCTION__, s.fourcc >> 24 & 0xff, s.fourcc >> 16 & 0xff, s.fourcc >> 8 & 0xff, s.fourcc & 0xff);
