...
{
	{
		...
		if (m_pAllocator)
		{
      for (std::list<CDX9SubPic*>::iterator it; it != m_pAllocator->m_AllocatedSurfaces.end(); it++)
      {
        //POSITION ThisPos = pos;
				CDX9SubPic *pSubPic = (*it);
				if (pSubPic == this)
				{
          
					m_pAllocator->m_AllocatedSurfaces.remove(*it);//RemoveAt(ThisPos);
					break;
				}
      }
...
{
	{
		...
		CAutoLock Lock(&ms_SurfaceQueueLock);
    for (std::list<CDX9SubPic*>::iterator it; it != m_AllocatedSurfaces.end(); it++)
    {
      (*it)->m_pAllocator = NULL;
    }
		while (!m_AllocatedSurfaces.empty())
      m_AllocatedSurfaces.pop_back();
    while (!m_FreeSurfaces.empty())
      m_FreeSurfaces.pop_back();
	}
}
