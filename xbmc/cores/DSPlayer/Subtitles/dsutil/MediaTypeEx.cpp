...
CStdString CMediaTypeEx::GetVideoCodecName(const GUID& subtype, DWORD biCompression, DWORD* fourcc)
{
  ...
  if(biCompression)
  {
    ...
    std::map<DWORD, CStdString>::iterator it = names.find(MAKEFOURCC(b[3], b[2], b[1], b[0]));
    if (it == names.end())
    {
...
CStdString CMediaTypeEx::GetAudioCodecName(const GUID& subtype, WORD wFormatTag)
{
  ...
  static std::map<WORD, CStdString> names;
...
/*void CMediaTypeEx::Dump(CAtlList<CStdString>& sl)
{
	...
	CStdString major = DShowUtil::CStdStringFromGUID(majortype);
	CStdString sub = DShowUtil::CStdStringFromGUID(subtype);
	CStdString format = DShowUtil::CStdStringFromGUID(formattype);
	...
	else if(formattype == FORMAT_WaveFormatEx)
	{
		...
		if(wfe.wFormatTag != WAVE_FORMAT_PCM && wfe.cbSize > 0)
		{
			if(wfe.wFormatTag == WAVE_FORMAT_EXTENSIBLE && wfe.cbSize == sizeof(WAVEFORMATEXTENSIBLE)-sizeof(WAVEFORMATEX))
			{
				...
				sl.push_back(str);
				str.Format(_T("SubFormat: %s"), DShowUtil::CStdStringFromGUID(wfe.SubFormat));
				sl.push_back(str);
