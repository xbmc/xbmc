...
/*IPin* AppendFilter(IPin* pPin, CStdString DisplayName, IGraphBuilder* pGB)
{
  IPin* pRet = pPin;
  ...
  do
  {
    ...
    IMoniker* pMoniker;
    ULONG chEaten;
    if(S_OK != MkParseDisplayName(pBindCtx, LPOLESTR(CStdStringW(DisplayName).c_str()), &chEaten, &pMoniker))
      break;
...
/*IPin* InsertFilter(IPin* pPin, CStdString DisplayName, IGraphBuilder* pGB)
{
	do
	{
		if(!pPin || DisplayName.IsEmpty() || !pGB)
...
int Eval_Exception(int n_except)
{
	...
	{
	  MessageBox(NULL, _T("The property page of this filter has just caused a\nmemory access violation. The application will gently die now :)"), NULL, MB_ICONERROR | MB_OK);
	}
...
/*void ShowPPage(CStdString DisplayName, HWND hParentWnd)
{
	CComPtr<IBindCtx> pBindCtx;
	CreateBindCtx(0, &pBindCtx);
	...
	CComPtr<IMoniker> pMoniker;
	ULONG chEaten;
	if(S_OK != MkParseDisplayName(pBindCtx, CStdStringW(DisplayName), &chEaten, &pMoniker))
		return;
...
void ShowPPage(IUnknown* pUnk, HWND hParentWnd)
{
	...
	if(SUCCEEDED(pSPP->GetPages(&caGUID)))
	{
		...
		MyOleCreatePropertyFrame(
			hParentWnd, 0, 0, CStdStringW(str), 
			1, (IUnknown**)&lpUnk, 
...
/*void CStdStringToBin(CStdString str, std::vector<BYTE>& data)
{
	str.Trim();
...
/*CStdString BinToCStdString(BYTE* ptr, int len)
{
	CStdString ret;
...
/*cdrom_t GetCDROMType(TCHAR drive, std::list<CStdString>& files)
{
	files.clear();
	...
	if(GetDriveType(path + _T("\\")) == DRIVE_CDROM)
	{
		...
		if(files.size() > 0) return CDROM_VideoCD;

		// CDROM_DVDVideo
		FindFiles(path + _T("\\VIDEO_TS\\video_ts.ifo"), files);
		if(files.size() > 0) return CDROM_DVDVideo;
		...
		if(files.size() > 0) return CDROM_Audio;
...
/*bool GetKeyFrames(CStdString fn, std::vector<UINT>& kfs)
{
	kfs.clear();

	CStdString fn2 = CStdString(fn).MakeLower();
			...
			if(AVIFileGetStream(pfile, &pavi, streamtypeVIDEO, 0) == AVIERR_OK)
			{
				...
				else
				{
					for(int kf = 0; ; kf++)
					{
						...
						if(kf < 0 || kfs.size() > 0 && kfs[kfs.size()-1] >= (UINT)kf) break;
						kfs.push_back(kf);
					}

					if(kfs.size() > 0 && kfs[kfs.size()-1] < si.dwLength-1)
					{
						kfs.push_back(si.dwLength-1);
					}
				}
			}
...
/*bool CreateFilter(CStdStringW DisplayName, IBaseFilter** ppBF, CStdStringW& FriendlyName)
{
...
/*HRESULT LoadExternalObject(LPCTSTR path, REFCLSID clsid, REFIID iid, void** ppv)
{
  ...
  HRESULT hr = E_FAIL;
  if(hInst || (hInst = CoLoadLibrary(LPOLESTR(CStdStringW(fullpath).c_str()), true)))
  {
...
/*CStdString MakeFullPath(LPCTSTR path)
{
	CStdString full(path);
	full.Replace('/', '\\');

	CStdString fn;
	fn.ReleaseBuffer(GetModuleFileName(GetInstanceHandle(), fn.GetBuffer(MAX_PATH), MAX_PATH));
	CPath p(fn);

	if(full.GetLength() >= 2 && full[0] == '\\' && full[1] != '\\')
	{
		p.StripToRoot();
		full = CStdString(p) + full.Mid(1); 
	}
	else if(full.Find(_T(":\\")) < 0)
	{
		p.RemoveFileSpec();
		p.AddBackslash();
		full = CStdString(p) + full;
	}
...
/*CStdString GetMediaTypeName(const GUID& guid)
{
...
/*GUID GUIDFromCStdString(CStdString str)
{
	GUID guid = GUID_NULL;
...
HRESULT GUIDFromCStdString(CStdString str, GUID& guid)
{
	guid = GUID_NULL;
	return CLSIDFromString(CComBSTR(str), &guid);
}*/

CStdString CStdStringFromGUID(const GUID& guid)
{
	WCHAR null[128] = {0}, buff[128];
	StringFromGUID2(GUID_NULL, null, 127);
...
/*CStdStringA UTF16To8(LPCWSTR utf16)
{
	CStdStringA str;
...
/*CStdString ISO6391ToLanguage(LPCSTR code)
{
	CHAR tmp[2+1];
...
{
	CStdString null = CStdStringFromGUID(GUID_NULL);
	CStdString majortype = CStdStringFromGUID(MEDIATYPE_Stream);
	CStdString subtype = CStdStringFromGUID(subtype2);

	SetRegKeyValue(_T("Media Type\\") + majortype, subtype, _T("0"), chkbytes);
	SetRegKeyValue(_T("Media Type\\") + majortype, subtype, _T("Source Filter"), CStdStringFromGUID(clsid));
...
void RegisterSourceFilter(const CLSID& clsid, const GUID& subtype2, const std::list<CStdString>& chkbytes, LPCTSTR ext, ...)
{
	CStdString null = CStdStringFromGUID(GUID_NULL);
	CStdString majortype = CStdStringFromGUID(MEDIATYPE_Stream);
	CStdString subtype = CStdStringFromGUID(subtype2);
...
{
	DeleteRegKey(_T("Media Type\\") + CStdStringFromGUID(MEDIATYPE_Stream), CStdStringFromGUID(subtype));
}*/
