...
STDMETHODIMP_(bool) CSubPicQueue::LookupSubPic(REFERENCE_TIME rtNow, Com::SmartPtr<ISubPic> &ppSubPic)
{
  ...
  //find pBF2 in the list
		Com::SmartQIPtr<ISubPic> pSubPic = *it;
...
STDMETHODIMP CSubPicQueue::GetStats(int nSubPic, REFERENCE_TIME& rtStart, REFERENCE_TIME& rtStop)
{
	...
	{
    int i = 0;
    for (std::list<ISubPic*>::iterator it; it != m_Queue.end(); it++)
    {
      if (i == nSubPic)
      {
...
//						m_fBufferUnderrun = true;
				...
				if(rtNow < rtStop)
				{
					REFERENCE_TIME rtCurrent = dsmax(rtNow, rtStart);
					...
					while (rtCurrent < rtStop)
					{
						...
						if (bIsAnimated)
						{
							if (rtCurrent < m_rtNow + rtTimePerFrame)
								rtCurrent = dsmin(m_rtNow + rtTimePerFrame, rtStop-1);

							REFERENCE_TIME rtEndThis = dsmin(rtCurrent + rtTimePerFrame, rtStop);
