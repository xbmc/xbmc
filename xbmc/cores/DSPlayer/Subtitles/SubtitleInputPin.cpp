...
HRESULT CSubtitleInputPin::CompleteConnect(IPin* pReceivePin)
{
    ...
    pRTS->m_name = CStdString(DShowUtil::GetPinName(pReceivePin)) + _T(" (embeded)");
	...
	else if(m_mt.majortype == MEDIATYPE_Subtitle)
	{
      ...
      name = DShowUtil::ISO6392ToLanguage(psi->IsoLang);
      lcid = DShowUtil::ISO6392ToLcid(psi->IsoLang);
  ...
  pSample->GetTime(&tStart, &tStop);
	...
	/*if(m_mt.majortype == MEDIATYPE_Text)
	{
		...
		if(!strncmp((char*)pData, __GAB1__, strlen(__GAB1__)))
		{
			...
			while(ptr < end)
			{
				...
				{
          pRTS->Add(DShowUtil::AToW(&ptr[8]), false, *(int*)ptr, *(int*)(ptr+4));
					fInvalidate = true;
				}
		...
		else if(pData != 0 && len > 1 && *pData != 0)
		{
			...
			if(!str.IsEmpty())
			{
				pRTS->Add(DShowUtil::AToW(str), false, (int)(tStart / 10000), (int)(tStop / 10000));
				fInvalidate = true;
			}
		}
	}
	else if(m_mt.majortype == MEDIATYPE_Subtitle)
	{
		...
		if(m_mt.subtype == MEDIASUBTYPE_UTF8)
		{
			...
			CStdStringW str = DShowUtil::UTF8To16(CStdStringA((LPCSTR)pData, len)).Trim();
			if(!str.IsEmpty())
			{
        ...
        std::list<CStdStringW> sl;
				...
				if(sl.size() == fields)
				{
          CStdStringW tmpsl = sl.front();
          sl.pop_front();
					stse.readorder = wcstol(tmpsl, NULL, 10);
          tmpsl = sl.front();
          sl.pop_front();
					stse.layer = wcstol(tmpsl, NULL, 10);
          tmpsl = sl.front();
          sl.pop_front();
					stse.style = tmpsl;
          tmpsl = sl.front();
          ...
          tmpsl = sl.front();
          sl.pop_front();
					stse.marginRect.left = wcstol(tmpsl, NULL, 10);
          tmpsl = sl.front();
          sl.pop_front();
					stse.marginRect.right = wcstol(tmpsl, NULL, 10);
          tmpsl = sl.front();
          sl.pop_front();
					stse.marginRect.top = stse.marginRect.bottom = wcstol(tmpsl, NULL, 10);
          tmpsl = sl.front();
          sl.pop_front();
					if(fields == 10) stse.marginRect.bottom = wcstol(tmpsl, NULL, 10);
          tmpsl = sl.front();
          sl.pop_front();
					stse.effect = tmpsl;
          tmpsl = sl.front();
	...
	if(fInvalidate)
	{
    CLog::Log(LOGDEBUG,"InvalidateSubtitle(%I64d, ..)", tStart);
