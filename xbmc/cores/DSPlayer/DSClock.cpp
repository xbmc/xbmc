...
int64_t CDSClock::m_systemOffset;
int64_t CDSClock::m_systemFrequency;
CCriticalSection CDSClock::m_systemsection;
...
double CDSClock::GetAbsoluteClock()
{
  ...
  return DS_TIME_BASE * (double)current / m_systemFrequency;
}

double CDSClock::WaitAbsoluteClock(double target)
{
  CSingleLock lock(m_systemsection);
  ...
  systemtarget = (int64_t)(target / DS_TIME_BASE * (double)freq);
  systemtarget += offset;
  systemtarget = g_VideoReferenceClock.Wait(systemtarget);
  systemtarget -= offset;
  return (double)systemtarget / freq * DS_TIME_BASE;
}

double CDSClock::GetClock()
{
  ...
  return DS_TIME_BASE * (double)current / m_systemUsed + m_iDisc;
}

void CDSClock::SetSpeed(int iSpeed)
{
  ...
  int64_t current;
  int64_t newfreq = m_systemFrequency * DS_PLAYSPEED_NORMAL / iSpeed;
...
double CDSClock::DistanceToDisc()
{
...
bool CDSClock::SetMaxSpeedAdjust(double speed)
{
...
int CDSClock::UpdateFramerate(double fps)
{
