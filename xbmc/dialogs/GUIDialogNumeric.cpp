...
void CGUIDialogNumeric::FrameMove()
{
  ...
  { // format up the time
    strLabel =
        StringUtils::Format("{:2}:{:02}:{:02}", m_datetime.GetHour(), m_datetime.GetMinute(), m_datetime.GetSecond());
    start = m_block * 3;
    end = m_block * 3 + 2;
  }
  else if (m_mode == INPUT_DATE)
  { // format up the date
    strLabel =
        StringUtils::Format("{:2}/{:2}/{:4}", m_datetime.GetDay(), m_datetime.GetMonth(), m_datetime.GetYear());
    start = m_block * 3;
    end = m_block * 3 + 2;
    if (m_block == 2)
      end = m_block * 3 + 4;
...
std::string CGUIDialogNumeric::GetOutputString() const
{
  ...
  {
  case INPUT_DATE:
    return StringUtils::Format("{:02}/{:02}/{:04}", m_datetime.GetDay(), m_datetime.GetMonth(), m_datetime.GetYear());
  case INPUT_TIME:
...
bool CGUIDialogNumeric::ShowAndGetSeconds(std::string &timeString, const std::string &heading)
{
  ...
  timeString = StringUtils::SecondsToTimeString(std::chrono::duration_cast<std::chrono::seconds>(time.time_since_epoch()).count());
  return true;
}
