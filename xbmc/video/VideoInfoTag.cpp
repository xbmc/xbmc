...
void CVideoInfoTag::ParseNative(const tinyxml2::XMLElement* movie, bool prioritise)
{
  ...
  if (node)
  {
    for (const auto* child = node->FirstChildElement("rating"); child != nullptr;
  ...
  if (epbookmark)
  {
    ...
    if (playerstate)
    {
      tinyxml2::XMLPrinter epBookmarkprinter;
      const auto* value = playerstate->FirstChildElement();
      value->Accept(&epBookmarkprinter);
      if (value)
        m_EpBookmark.playerState = epBookmarkprinter.CStr();
    }
  }
  else
  ...
  while (thumb)
  {
    ...
    if (prioritise)
    {
      tinyxml2::XMLPrinter thumbPrinter;
      thumb->Accept(&thumbPrinter);
      std::string temp = thumbPrinter.CStr();
      xmlAdd = temp + xmlAdd;
    }
    thumb = thumb->NextSiblingElement("thumb");
  }
  ...
  while (node)
  {
    const auto* pNode = node->FirstChildElement("name");
  ...
  if (m_strEpisodeGuide.empty())
  {
    ...
    if (epguide)
    {
      ...
      {
        tinyxml2::XMLPrinter epgGuidePrinter;
        epguide->Accept(&epgGuidePrinter);
        m_strEpisodeGuide = epgGuidePrinter.CStr();
      }
    }
  }
  ...
  if (fanart)
  {
    tinyxml2::XMLPrinter fanartPrinter;
    fanart->Accept(&fanartPrinter);
    // we prioritise mixed-mode nfo's with fanart set
    if (prioritise)
    {
      const char* temp{fanartPrinter.CStr()};
      m_fanart.m_xml = temp + m_fanart.m_xml;
    }
    else
      m_fanart.m_xml.append(fanartPrinter.CStr());
    m_fanart.Unpack();
  }
  ...
  if (resume)
  {
    ...
    if (playerstate)
    {
      ...
      if (value)
      {
        tinyxml2::XMLPrinter resumePointPrinter;
        value->Accept(&resumePointPrinter);
        m_resumePoint.playerState = resumePointPrinter.CStr();
      }
    }
  }
...
void CVideoInfoTag::CAssetInfo::Save(tinyxml2::XMLNode* movie)
{
...
void CVideoInfoTag::CAssetInfo::ParseNative(const tinyxml2::XMLElement* movie)
{
  std::string value;
