...
CVideoInfoScanner::~CVideoInfoScanner() = default;
  ...
  try
  {
    ...
    for (const auto& rootPathToScan : m_pathsToScan)
    {
      ...
      const ScraperPtr info =
          m_database.GetScraperForPath(rootPathToScan, rootPathScraperScanSettings, foundDirectly);
                                        ...
                                        CONTENT_TYPE contentType)
{
  ...
  CDirectory::GetDirectory(strDirectory, nonRecursiveRemoteItems,
                           CServiceBroker::GetFileExtensionProvider().GetVideoExtensions(),
                           DIR_FLAG_DEFAULTS);

  for (const auto& remoteItem : nonRecursiveRemoteItems)
  {
    // Support scan cancellation
    if (m_bStop)
    {
    ...
    { // For other media types, it's the reverse: we only store files
      if (remoteItem->m_bIsFolder)
      {
                                             ...
                                             CONTENT_TYPE contentType)
{
  ...
  const std::vector<std::string>& regexpExcludeRules = contentType == CONTENT_TVSHOWS
  ...
  // Skip folders where .nomedia file is present
  if (HasNoMedia(strDirectory))
    return false;

  // Skip if path is dedicated to a plugin
  if (URIUtils::IsPlugin(strDirectory) &&
      !CPluginDirectory::IsMediaLibraryScanningAllowed(TranslateContent(contentType), strDirectory))
  {
    CLog::Log(LOGINFO,
          ...
          || !remoteItem->m_bIsFolder) // if item is not a folder, it's a file we want to scan
      {
        ...
        switch (pathContent.contentType)
        {
          ...
          case CONTENT_MUSICVIDEOS:
            isVideoAlreadyInDB = m_database.HasMusicVideoInfo(remoteItemPath);
            break;
          case CONTENT_TVSHOWS:
            isVideoAlreadyInDB = m_database.HasTvShowInfo(remoteItemPath);
            break;
        }
        ...
        if (!isVideoAlreadyInDB) // Video is not yet in the DB so we try to add it:
        {
          ...
          bool foundSomething = RetrieveVideoInfo(
              itemList, pathContent.scrapperScanSettings.parent_name_root, pathContent.contentType);
          if (foundSomething)
          {
        ...
        {
          CLog::Log(LOGDEBUG, "VideoInfoScanner: item '{}' is already in the DB, no need to rescan",
                    CURL::GetRedacted(remoteItemPath));
        }
      }
    }
                                          ...
                                          CGUIDialogProgress* pDlgProgress)
{
  ...
  for (int i = 0; i < items.Size(); ++i)
  {
    ...
    if (scraper->Content() == CONTENT_TVSHOWS)
      ...
      ret =
          RetrieveInfoForMusicVideo(pItem.get(), bDirNames, scraper, useLocal, pURL, pDlgProgress);
    else
    {
    ...
    else if (ret == INFO_ADDED || ret == INFO_HAVE_ALREADY)
      FoundSomeInfo = true;
    else if (ret == INFO_NOT_FOUND)
    {
      CLog::Log(LOGWARNING, "No information found for item '{}', it won't be added to the library.",
                CURL::GetRedacted(pItem->GetPath()));
      ...
      if (eventLog)
      {
        const std::string itemlogpath = (scraper->Content() == CONTENT_TVSHOWS)
                                            ? CURL::GetRedacted(pItem->GetPath())
  ...
  if (content == CONTENT_TVSHOWS && !seenPaths.empty())
  {
    ...
    for (std::vector<std::pair<int, std::string>>::iterator i = libPaths.begin();
         i < libPaths.end(); ++i)
    {
                                                                ...
                                                                CGUIDialogProgress* pDlgProgress)
{
  ...
  else if (pItem->IsPlugin() && pItem->HasVideoInfoTag() &&
           pItem->GetVideoInfoTag()->m_iIdShow >= 0)
  {
  ...
  {
    INFO_RET ret = RetrieveInfoForEpisodes(pItem, idTvShow, scraper, useLocal, pDlgProgress);
    if (ret == INFO_ADDED)
  ...
  if (scraper->IsPython() && CUtil::GetFilenameIdentifier(movieTitle, identifierType, identifier))
  {
    ...
    if (GetDetails(pItem, uniqueIDs, url, scraper,
                   (result == CInfoScanner::COMBINED_NFO || result == CInfoScanner::OVERRIDE_NFO)
                       ? loader.get()
                       : nullptr,
                   pDlgProgress))
    {
      if ((lResult = AddVideo(pItem, scraper->Content(), false, useLocal)) < 0)
        return INFO_ERROR;
  ...
  if (GetDetails(pItem, {}, url, scraper,
                 (result == CInfoScanner::COMBINED_NFO || result == CInfoScanner::OVERRIDE_NFO)
                     ? loader.get()
                     : nullptr,
                 pDlgProgress))
    ...
    CGUIDialogProgress* pDlgProgress)
{
  ...
  if (scraper->IsPython() && CUtil::GetFilenameIdentifier(movieTitle, identifierType, identifier))
  {
    ...
    if (GetDetails(pItem, uniqueIDs, url, scraper,
                   ...
                   pDlgProgress))
    {
      if (AddVideo(pItem, scraper->Content(), bDirNames, useLocal) < 0)
        return INFO_ERROR;
      return INFO_ADDED;
    }
  }
...
CInfoScanner::INFO_RET CVideoInfoScanner::RetrieveInfoForEpisodes(CFileItem* item,
                                                                  long showID,
                                                                  ...
                                                                  CGUIDialogProgress* progress)
{
  ...
  if (ret == INFO_ADDED)
  {
    ...
    for (std::map<int, std::map<std::string, std::string>>::const_iterator i = seasonArt.begin();
         i != seasonArt.end(); ++i)
    {
      if (i->second.empty())
      {
...
bool CVideoInfoScanner::EnumerateSeriesFolder(CFileItem* item, EPISODELIST& episodeList)
{
  ...
  if (item->m_bIsFolder)
  {
    ...
    if (item->IsPlugin())
    {
      ...
      hash = GetRecursiveFastHash(item->GetPath(), regexpExcludeRules);

    if (m_database.GetPathHash(item->GetPath(), dbHash) && (allowEmptyHash || !hash.empty()) &&
        StringUtils::EqualsNoCase(dbHash, hash))
    {
                             ...
                             [&](const CFileItemPtr& i) {
                               ...
                               return (std::count_if(foldersToRemove.begin(), foldersToRemove.end(),
                                                     [&](const std::string& removePath) {
                                                       return path.rfind(removePath, 0) == 0;
                                                     }) > 0) &&
                                      file != "VIDEO_TS.IFO" &&
                                      (file != "INDEX.BDMV" ||
  ...
  for (int i = 0; i < items.Size(); ++i)
  {
    ...
    if (CUtil::ExcludeFileOrFolder(items[i]->GetPath(), regexpExcludeRules))
      continue;
...
bool CVideoInfoScanner::ProcessItemByVideoInfoTag(const CFileItem* item, EPISODELIST& episodeList)
{
  ...
  if (tag->m_firstAired.IsValid())
  {
    ...
    CLog::Log(LOGDEBUG, "{} - found match for: '{}', firstAired: '{}' = '{}', title: '{}'",
              __FUNCTION__, CURL::GetRedacted(episode.strPath), tag->m_firstAired.GetAsDBDateTime(),
...
{
  SETTINGS_TVSHOWLIST expression =
  ...
  for (unsigned int i = 0; i < expression.size(); ++i)
  {
    ...
    if (!byDate && reg2.RegComp(CServiceBroker::GetSettingsComponent()
                                    ->GetAdvancedSettings()
                                    ->m_tvshowMultiPartEnumRegExp))
    {
      ...
      {
        if (((regexppos <= regexp2pos) && regexppos != -1) || (regexppos >= 0 && regexp2pos == -1))
        {
...
bool CVideoInfoScanner::GetEpisodeAndSeasonFromRegExp(CRegExp& reg,
                                                      EPISODE& episodeInfo,
                                                      int defaultSeason)
{
                                 ...
                                 bool libraryImport /* = false */)
{
  ...
  {
    strTitle = StringUtils::Format("{} - {}x{} - {}", showInfo->m_strTitle, movieDetails.m_iSeason,
                                   movieDetails.m_iEpisode, strTitle);
  }
  ...
  else if (content == CONTENT_TVSHOWS)
  {
    if (pItem->m_bIsFolder)
    {
      ...
      if (!URIUtils::IsMultiPath(pItem->GetPath()) ||
          !CMultiPathDirectory::GetPaths(pItem->GetPath(), multipath))
        multipath.push_back(pItem->GetPath());
      std::vector<std::pair<std::string, std::string>> paths;
      for (std::vector<std::string>::const_iterator i = multipath.begin(); i != multipath.end();
           ++i)
  ...
  if (!pItem->m_bIsFolder)
  {
    ...
    if ((libraryImport || advancedSettings->m_bVideoLibraryImportResumePoint) &&
      ...
      m_database.AddBookMarkToFile(pItem->GetPath(), movieDetails.GetResumePoint(),
                                   CBookmark::RESUME);
  }
                                   ...
                                   const std::string& actorArtPath)
{
  ...
  if (pItem->HasVideoInfoTag() && !pItem->GetVideoInfoTag()->m_coverArt.empty())
  {
    for (auto& it : pItem->GetVideoInfoTag()->m_coverArt)
    {
      ...
      {
        std::string thumb =
            CTextureUtils::GetWrappedImageURL(pItem->GetPath(), "video_" + it.m_type);
...
std::string CVideoInfoScanner::GetImage(const CScraperUrl::SUrlEntry& image,
                                        const std::string& itemPath)
{
...
CInfoScanner::INFO_RET CVideoInfoScanner::OnProcessSeriesFolder(
    EPISODELIST& files,
    const ADDON::ScraperPtr& scraper,
    bool useLocal,
...
bool CVideoInfoScanner::CanFastHash(const CFileItemList& items,
                                    const std::vector<std::string>& regexExcludeRules) const
{
  ...
  {
    if (items[i]->m_bIsFolder &&
        !CUtil::ExcludeFileOrFolder(items[i]->GetPath(), regexExcludeRules))
      return false;
  }
  return true;
}
...
std::string CVideoInfoScanner::GetRecursiveFastHash(
    const std::string& directory, const std::vector<std::string>& regexExcludeRules) const
{
  CFileItemList items;
...
void CVideoInfoScanner::GetSeasonThumbs(
