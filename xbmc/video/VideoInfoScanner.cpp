...
CVideoInfoScanner::~CVideoInfoScanner()
{
                                                                ...
                                                                CGUIDialogProgress* pDlgProgress)
  {
    ...
    if (result == InfoType::FULL)
    {
      ...
      if (item.GetVideoInfoTag()->IsDefaultVideoVersion())
        defaultVersionFileId = item.GetVideoInfoTag()->m_iFileId; // Updated in AddMovie()
      ...
      do
      {
        ...
        if (result == InfoType::FULL)
        {
          ...
          const int versionId{static_cast<int>(
              AddVideo(&item, nullptr, bDirNames, true, NULL, false, CONTENT_MOVIEVERSIONS))};
          if (versionId < 0)
            return InfoRet::INFO_ERROR;
      ...
      // Set default version
      m_database.SetDefaultVideoVersion(VideoDbContentType::MOVIES, movieId, defaultVersionFileId);
                                   ...
                                   const CONTENT_TYPE& contentOverride /* = CONTENT_NONE */)
  {
    ...
    const CONTENT_TYPE& content{!scraper || contentOverride != CONTENT_NONE ? contentOverride
                                                                            : scraper->Content()};
    const bool usingLocalScraper{scraper && scraper->ID() == "metadata.local"};
    const bool noRemoteArt{usingLocalScraper && CServiceBroker::GetSettingsComponent()
                                                    ->GetAdvancedSettings()
                                                    ->m_bNoRemoteArtWithLocalScraper};

    if (!libraryImport)
      GetArtwork(pItem, content, videoFolder, useLocal && !pItem->IsPlugin(),
                 showInfo ? showInfo->m_strPath : "", noRemoteArt);
      ...
      if (pItem->m_bIsFolder)
      {
        ...
        if (!libraryImport)
          GetSeasonThumbs(movieDetails, seasonArt, CVideoThumbLoader::GetArtTypes(MediaTypeSeason),
                          useLocal && !pItem->IsPlugin(), noRemoteArt);
                                     ...
                                     bool noRemoteArt /* = false */)
  {
        ...
        art.find("fanart") == art.end())
    {
      ...
      if (!fanart.empty() && !(noRemoteArt && URIUtils::IsRemote(fanart)))
    ...
    for (const auto& url : pItem->GetVideoInfoTag()->m_strPictureURL.GetUrls())
    {
          ...
          art.find(aspect) == art.end())
      {
        ...
        if (!image.empty() && !(noRemoteArt && URIUtils::IsRemote(image)))
      ...
      CGUIDialogProgress* pDlgProgress /* = NULL */)
  {
    ...
    for (EPISODELIST::iterator file = files.begin(); file != files.end(); ++file)
    {
      ...
      if (result == InfoType::FULL)
      {
        ...
        if (AddVideo(&item, info, file->isFolder, true, &showInfo, false, CONTENT_TVSHOWS) < 0)
          return InfoRet::INFO_ERROR;
        continue;
      }
      ...
      if (bFound)
      {
        ...
        if (AddVideo(&item, info, file->isFolder, useLocal, &showInfo, false, CONTENT_TVSHOWS) < 0)
          return InfoRet::INFO_ERROR;
      }
      else
      {
    ...
    for (std::vector<SActorInfo>::iterator i = actors.begin(); i != actors.end(); ++i)
    {
      if (i->thumb.empty())
      {
        ...
        if (!i->thumbUrl.GetFirstUrlByType().m_url.empty())
        {
          ...
          const bool notUsingThisRemoteArt{noRemoteArt && URIUtils::IsRemote(thumb)};
