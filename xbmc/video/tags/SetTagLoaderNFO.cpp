...
CSetTagLoaderNFO::CSetTagLoaderNFO(const CFileItem& item) : ISetInfoTagLoader(item)
{
  if (!m_item.GetVideoInfoTag()->m_set.title.empty() &&
      !VIDEO::CVideoInfoScanner::GetMovieSetInfoFolder(m_item.GetVideoInfoTag()->m_set.title)
           .empty())
...
CInfoScanner::INFO_TYPE CSetTagLoaderNFO::Load(CSetInfoTag& tag, const bool prioritise)
{
  ...
  CInfoScanner::INFO_TYPE result{nfoReader.Create(m_path, scraper)};

  if (result == CInfoScanner::FULL_NFO || result == CInfoScanner::COMBINED_NFO)
    nfoReader.GetDetails(tag, nullptr);

  // ** Get set name from originally passed
  if (tag.GetTitle().empty())
    tag.SetTitle(m_item.GetVideoInfoTag()->m_set.title);
