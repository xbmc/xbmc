...
//! \brief A strided run of samples through one plane, in 8-bit equivalent buckets. Templated
//! on sample width so the per-sample path has no depth branch.
template<typename T>
class CSampleLine
{
public:
...
LineOutcome CDetector<T>::Classify(const LineSpec& line)
{
  ...
  //! \brief The outermost above-threshold runs, admitting only runs of minLength.
  struct BrightRuns
  {
...
DetectionResult CDetector<T>::Run()
{
  ...
  //! \brief What survived the walks is too little to be content.
  const auto noContent = [this](unsigned int low, unsigned int high)
  { return high <= low || high - low < m_params.minContentExtent; };
  ...
  // A finding thinner than the floor is not a bar.
