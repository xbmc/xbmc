...
/*!
 * \brief The widest display-space ratio the record ever saw, built from the record's own frame
 *        and display ratio rather than from a playing stream.
 *
 * What the masking opens to, and so the one shape of a title that gets the whole raster. A
 * narrower section is drawn inside that opening at the same height, which is what a masked room
 * does; an opening narrower than a section scales that section down instead.
 *
