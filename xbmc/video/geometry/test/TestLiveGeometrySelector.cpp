...
/*!
 * Measured on a scope film's archive-footage finale (Disclosure Day, 2026-08-20): the disc
 * letterboxes its own inserts, so a 16:9 archive clip reads as 2828x1608 at 506,276 - a 16:9
 * shape composed entirely inside the scope band. Resolving that to the vocabulary and fitting
 * the ratio to the frame claimed the full 3840x2160, and the picture spent twenty seconds
 * shrunk into a 16:9 window. The reading never touched a row outside the band; a rectangle
 * that gains ground the reading itself never reached is not evidence of a wider picture.
 */
TEST_F(TestLiveGeometrySelector, AnInsertComposedInsideTheServedShapeIsNotAWiderPicture)
{
...
TEST_F(TestLiveGeometrySelector, AReadingBeyondTheServedShapeStillWidensAtOnce)
{
  // A real cut to 1.90 uses rows the scope band does not have, and that is the difference
  // between it and an insert: the reading itself reaches outside what is served.
  Served(SCOPE_READ);
  EXPECT_EQ((CRectInt{0, 69, WIDTH, 2091}), Served(CRectInt{0, 69, WIDTH, HEIGHT - 69}).rect);
}

TEST_F(TestLiveGeometrySelector, AnInsertNarrowerThanItsRatioClaimsIsStillComposition)
{
  // A 4:3-shaped insert inside the scope band resolves to the pillarbox entry, which is
  // narrower on one axis and taller on the other. The tall half is still ground the reading
  // never touched, so it is composition, not a section - the mixed direction must not fall
  // through to the narrowing guard and be served after three frames.
  const CRectInt FOUR_THREE_INSERT{848, 276, 848 + 2144, 276 + 1608};

  Served(SCOPE_READ);
  for (int i = 0; i < 10; ++i)
    Quiet(FOUR_THREE_INSERT);

  EXPECT_EQ(SCOPE, *m_selector.Published());
  EXPECT_EQ(1u, m_selector.Shapes().size());
}

TEST_F(TestLiveGeometrySelector, SeekIntoAnInsertKeepsTheServedShape)
{
  // Landing mid-insert after a jump: the reading is authoritative about where playback
  // landed, but it still lies inside what is served, so what is served still covers it.
  const CRectInt SIXTEEN_NINE_INSERT{506, 276, 506 + 2828, 276 + 1608};
