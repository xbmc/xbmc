...
/*!
 * A source already at the output size takes a straight copy rather than a box average over
 * boxes of one sample. It has to land on the same answer sample for sample - this is the whole
 * of the Direct3D path, where the staging texture arrives at the output size, and any
 * disagreement would move a boundary the detector then reads.
 *
 * The stride is padded past the width, as a mapped staging texture's row pitch is.
 */
TEST(TestFrameReduction, ASourceAtTheOutputSizeIsCopiedSampleForSample)
{
  constexpr unsigned int WIDTH = 64;
  constexpr unsigned int HEIGHT = 32;
  constexpr int PITCH = 96;

  std::vector<uint8_t> luma(static_cast<size_t>(PITCH) * HEIGHT, 0xEE);
  std::vector<uint8_t> chroma(static_cast<size_t>(PITCH) * HEIGHT / 2, 0xEE);
  for (unsigned int y = 0; y < HEIGHT; ++y)
  {
    for (unsigned int x = 0; x < WIDTH; ++x)
      luma[static_cast<size_t>(y) * PITCH + x] = static_cast<uint8_t>((y * 7 + x * 3) & 0xFF);
  }
  for (unsigned int y = 0; y < HEIGHT / 2; ++y)
  {
    for (unsigned int x = 0; x < WIDTH / 2; ++x)
    {
      chroma[static_cast<size_t>(y) * PITCH + x * 2] = static_cast<uint8_t>(100 + x);
      chroma[static_cast<size_t>(y) * PITCH + x * 2 + 1] = static_cast<uint8_t>(200 - y);
    }
  }
  ...
  source.uStrideBytes = PITCH;

  ReducedFrame out;
  ASSERT_TRUE(ReduceFrame(source, WIDTH, out));
  ASSERT_EQ(WIDTH, out.width);
  ...
  for (unsigned int y = 0; y < HEIGHT; ++y)
  {
    for (unsigned int x = 0; x < WIDTH; ++x)
    {
      ASSERT_EQ(luma[static_cast<size_t>(y) * PITCH + x], out.y[static_cast<size_t>(y) * WIDTH + x])
          << "luma at " << x << "," << y;
    }
  }

  // The interleaved plane splits into two planar ones, so the copy has to pick alternate
  // samples rather than consecutive ones.
  for (unsigned int y = 0; y < HEIGHT / 2; ++y)
  {
    for (unsigned int x = 0; x < WIDTH / 2; ++x)
    {
      const size_t at = static_cast<size_t>(y) * (WIDTH / 2) + x;
      ASSERT_EQ(static_cast<uint8_t>(100 + x), out.u[at]) << "Cb at " << x << "," << y;
      ASSERT_EQ(static_cast<uint8_t>(200 - y), out.v[at]) << "Cr at " << x << "," << y;
    }
  }
}
