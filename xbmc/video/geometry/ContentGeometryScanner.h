...
public:
   ...
   *        a sweep is already running, or the library has not been opted in to measurement.
   *
  ...
  /*!
   \brief A file is being opened for playback, so the sweep must get off the disk now.

   Distinct from StopSweep(), which ends a sweep: this suspends it, and it takes the file it
   abandoned again once the room is quiet. Set before the play path touches the media and
   cleared once the player owns it, because until the player reports playing there is nothing
   else for the sweep to notice - and that gap is long enough to seek a file across a share.
   */
