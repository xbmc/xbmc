...
//! \brief What a live reading has to clear before it is served, counted in frames.
struct LiveSelectorParams
{
  //! \brief Edges closer together than this are the same edge, in coded pixels. Must stay under
  //! the gap between neighbouring vocabulary entries, which is what it is compared between.
...
struct LiveGeometryReading
{
  ...
  bool varies{false}; //!< more than one shape has been served this playback
};

/*!
 * \brief Says which shape is on screen, this frame. Pure and clockless.
 *
 * Each reading resolves against the whole vocabulary and what is served is the entry's own
 * rectangle, so two frames of one section give the identical rectangle however much the
 * boundary wandered. A reading matching no real ratio is refused. A stream opening and a seek
 * both serve the first reading that follows them.
 */
...
public:
  ...
  //! \brief Describe the stream being read, discarding all state including what was served.
  //! \p coded is the analysed region, \p par the width of a coded pixel in display pixels.
  ...
  //! \brief Adopt new rules without touching the state.
...
private:
  ...
  bool GainsPicture(const CRectInt& rect) const;
  std::optional<CRectInt> Snap(const CRectInt& rect);

  //! \brief Serve \p rect, which is always a reading.
  LiveGeometryReading Publish(const CRectInt& rect);
  ...
  unsigned int m_inset{0}; //!< frames composed inside the served shape
};
