...
{
  // Non-live, so it answers to the same setting as the scan and the sweep: this holds the film
  // behind a busy dialog to sample it, which is not something live detection ever does.
  // RemeasureContentGeometry is deliberately not gated here - it is asked for by name.
...
{
  // Measuring away from the picture is opted into separately: a sweep is hours of work over
  // every title in the library, and the feature switch alone does not ask for it.
