...
void SerializeEffectiveGeometry(const EffectiveGeometry& geometry, CVariant& value)
{
  ...
  // The rectangle is omitted rather than zeroed when there is no frame behind it, zeros being a
  // rectangle a consumer can act on. The ratio is always present.
  ...
  // Always written, empty included: varies on its own says one rectangle does not describe the
  // title without saying what the others are, and an absent array would be a third state.
...
void ParseGeometryOverrides(const CVariant& geometry, GeometryOverrides& overrides)
{
  ...
  // Null rather than absence is what "not stated" looks like here: a request arriving over
  // JSON-RPC has had every omitted property filled in from the service description, so presence
