...
std::optional<LiveGeometryReading> CLiveGeometrySelector::Feed(const DetectionResult& sample)
{
  ...
  // An insert composed inside the served shape - archive footage the disc itself letterboxes -
  // has a real ratio, and that ratio's fitted rectangle claims ground the reading never
  // reached. Widening is only evidence when the reading itself extends past what is served.
