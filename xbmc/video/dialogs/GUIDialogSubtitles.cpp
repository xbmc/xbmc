...
void CGUIDialogSubtitles::Search(const std::string &search/*=""*/)
{
  ...
  if (StringUtils::EqualsNoCase(preferredLanguage, KODI::LANGINFO::subLanguageOriginal))
  {
    ...
    // Sent to the subtitle service addon as a url option below, so it stays English rather than
    // being localized for display, and names the language alone: an addon matches on names like
    // "English", and would not know what to do with "English (Australia)"
