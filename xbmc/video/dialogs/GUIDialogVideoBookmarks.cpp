...
int CGUIDialogVideoBookmarks::ItemToBookmarkIndex(int item)
{
  if (item < 0 || item >= (int)m_vecItems->Size())
    return -1;

  std::shared_ptr<CFileItem> fileItem = m_vecItems->Get(item);

  if (!fileItem->GetProperty("isbookmark").asBoolean(false))
    return -1;

  const int bookmarkIdx{fileItem->GetProperty("bookmark").asInteger32(-1)};
  if (bookmarkIdx < 0 || bookmarkIdx >= m_bookmarks.size())
  {
