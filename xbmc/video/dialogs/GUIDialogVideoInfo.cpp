                                                 ...
                                                 const std::string& artType)
{
  ...
  if (!embeddedArt.empty())
  {
    ...
    itemEmbedded->SetLabel(
        CServiceBroker::GetResourcesComponent()->GetLocalizeStrings().Get(13519));
...
bool CGUIDialogVideoInfo::UpdateVideoItemSortTitle(const std::shared_ptr<CFileItem>& pItem)
{
  ...
  if (!CGUIKeyboardFactory::ShowAndGetInput(
          currentTitle,
          CVariant{CServiceBroker::GetResourcesComponent()->GetLocalizeStrings().Get(16107)},
          false))
    return false;
