  ...
  case GUI_MSG_CLICKED:
    {
      ...
      {
        string strDirector = StringUtils::Join(m_movieItem->GetVideoInfoTag()->m_director, g_advancedSettings.m_videoItemSeparator);
      ...
      else if (iControl == CONTROL_LIST)
      {
        ...
        if (ACTION_SELECT_ITEM == iAction || ACTION_MOUSE_LEFT_CLICK == iAction)
        {
          ...
          if (iItem < 0 || iItem >= m_castList->Size())
            break;
          string strItem = m_castList->Get(iItem)->GetLabel();
          string strFind = StringUtils::Format(" %s ",g_localizeStrings.Get(20347).c_str());
          size_t iPos = strItem.find(strFind);
          if (iPos == std::string::npos)
            iPos = strItem.size();
          string tmp = strItem.substr(0, iPos);
          OnSearch(tmp);
        }
      }
    }
  ...
  case GUI_MSG_NOTIFY_ALL:
    {
      if (IsActive() && message.GetParam1() == GUI_MSG_UPDATE_ITEM && message.GetItem())
      {
        ...
        if (item && StringUtils::EqualsNoCase(m_movieItem->GetPath(), item->GetPath()))
  ...
  *m_movieItem = *item;
  ...
  { // music video
    ...
    for (std::vector<std::string>::const_iterator it = artists.begin(); it != artists.end(); ++it)
    {
      ...
      string thumb = database.GetArtForItem(idArtist, "artist", "thumb");
  ...
  { // movie/show/episode
    for (CVideoInfoTag::iCast it = m_movieItem->GetVideoInfoTag()->m_cast.begin(); it != m_movieItem->GetVideoInfoTag()->m_cast.end(); ++it)
    {
      ...
      { // backward compatibility
        string thumb = CScraperUrl::GetThumbURL(it->thumbUrl.GetFirstThumb());
        if (!thumb.empty())
        {
          item->SetArt("thumb", thumb);
    ...
    else if (type == VIDEODB_CONTENT_MOVIES)
    {
      ...
      {
        string localTrailer = m_movieItem->FindTrailer();
...
void CGUIDialogVideoInfo::Update()
{
  ...
  string strTmp = m_movieItem->GetVideoInfoTag()->m_strPlot;
...
void CGUIDialogVideoInfo::OnSearch(string& strSearch)
{
...
void CGUIDialogVideoInfo::DoSearch(string& strSearch, CFileItemList& items)
{
  CVideoDatabase db;
  if (!db.Open())
    return;
  ...
  {
    string label = movies[i]->GetVideoInfoTag()->m_strTitle;
  ...
  for (int i = 0; i < movies.Size(); ++i)
  {
    string label = movies[i]->GetVideoInfoTag()->m_strShowTitle;
    if (movies[i]->GetVideoInfoTag()->m_iYear > 0)
      label += StringUtils::Format(" (%i)", movies[i]->GetVideoInfoTag()->m_iYear);
    movies[i]->SetLabel(label);
  }
  ...
  for (int i = 0; i < movies.Size(); ++i)
  {
    string label = movies[i]->GetVideoInfoTag()->m_strTitle + " (" +  movies[i]->GetVideoInfoTag()->m_strShowTitle + ")";
    movies[i]->SetLabel(label);
  }
  ...
  for (int i = 0; i < movies.Size(); ++i)
  {
    string label = StringUtils::Join(movies[i]->GetVideoInfoTag()->m_artist, g_advancedSettings.m_videoItemSeparator) + " - " + movies[i]->GetVideoInfoTag()->m_strTitle;
    if (movies[i]->GetVideoInfoTag()->m_iYear > 0)
      label += StringUtils::Format(" (%i)", movies[i]->GetVideoInfoTag()->m_iYear);
    movies[i]->SetLabel(label);
  }
...
void CGUIDialogVideoInfo::Play(bool resume)
{
  ...
  {
    string strPath = StringUtils::Format("videodb://tvshows/titles/%i/",m_movieItem->GetVideoInfoTag()->m_iDbId);
    Close();
...
void CGUIDialogVideoInfo::OnGetArt()
{
  ...
  else
  {
    ...
    {
      string strItemPath = StringUtils::Format("thumb://Remote%i", i);
    ...
    string localThumb = CVideoThumbLoader::GetLocalArt(*m_movieItem, type);
    if (!localThumb.empty())
    {
...
void CGUIDialogVideoInfo::OnGetFanart()
{
  ...
  for (unsigned int i = 0; i < m_movieItem->GetVideoInfoTag()->m_fanart.GetNumFanarts(); i++)
  {
    ...
    CFileItemPtr item(new CFileItem(strItemPath, false));
    string thumb = m_movieItem->GetVideoInfoTag()->m_fanart.GetPreviewURL(i);
...
//    CTextureCache::Get().ClearCachedImage(thumb);
  ...
  g_mediaManager.GetLocalDrives(sources);
  bool flip=false;
  if (!CGUIDialogFileBrowser::ShowAndGetImage(items, sources, g_localizeStrings.Get(20437), result, &flip, 20445) || StringUtils::EqualsNoCase(result, "fanart://Current"))
...
void CGUIDialogVideoInfo::SetLabel(int iControl, const string &strLabel)
{
  if (strLabel.empty())
  {
...
void CGUIDialogVideoInfo::AddItemPathToFileBrowserSources(VECSOURCES &sources, const CFileItem &item)
{
  ...
  string itemDir = item.GetVideoInfoTag()->m_basePath;
...
bool CGUIDialogVideoInfo::DeleteVideoItem(const CFileItemPtr &item, bool unavailable /* = false */)
{
  ...
  {
    string strDeletePath = item->GetVideoInfoTag()->GetPath();
...
bool CGUIDialogVideoInfo::GetMoviesForSet(const CFileItem *setItem, CFileItemList &originalMovies, CFileItemList &selectedMovies)
{
  ...
  string strHeading = StringUtils::Format(g_localizeStrings.Get(20457));
  string baseDir = StringUtils::Format("videodb://movies/sets/%d", setItem->GetVideoInfoTag()->m_iDbId);
...
bool CGUIDialogVideoInfo::GetSetForMovie(const CFileItem *movieItem, CFileItemPtr &selectedSet)
{
  ...
  CFileItemList listItems;
  string baseDir = "videodb://movies/sets/";
  ...
  {
    // add clear item
    string strClear = StringUtils::Format(g_localizeStrings.Get(20467), currentSetLabel.c_str());
    CFileItemPtr clearItem(new CFileItem(strClear));
    clearItem->GetVideoInfoTag()->m_iDbId = -1; // -1 will be used to clear set
    listItems.AddFront(clearItem, 0);
    // add keep current set item
    string strKeep = StringUtils::Format(g_localizeStrings.Get(20469), currentSetLabel.c_str());
  ...
  string strHeading = StringUtils::Format(g_localizeStrings.Get(20466));
  dialog->Reset();
  dialog->SetHeading(strHeading);
  dialog->SetItems(&listItems);
  if (currentSetId >= 0)
...
bool CGUIDialogVideoInfo::GetItemsForTag(const string &strHeading, const std::string &type, CFileItemList &items, int idTag /* = -1 */, bool showAll /* = true */)
{
  CVideoDatabase videodb;
  if (!videodb.Open())
    return false;
...
bool CGUIDialogVideoInfo::ManageVideoItemArtwork(const CFileItemPtr &item, const std::string &type)
{
  ...
  if (type != "artist")
  {
    ...
    {
      string picturePath;
      string strThumb = URIUtils::AddFileToFolder(picturePath, "folder.jpg");
  ...
  {
    string strThumb = URIUtils::AddFileToFolder(artistPath, "folder.jpg");
    if (XFILE::CFile::Exists(strThumb))
    {
      CFileItemPtr pItem(new CFileItem(strThumb, false));
      pItem->SetLabel(g_localizeStrings.Get(13514));
