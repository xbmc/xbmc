                                   ...
                                   int idFile /* = -1 */)
{
  try
  {
    const std::string sql{PrepareSQL(
        "select * from episode_view where idShow=%i order by idSeason, idEpisode", idShow)};
    pDS->query(sql);
    ...
    while (!pDS->eof())
    {
      std::string file{URIUtils::AddFileToFolder(pDS->fv("strPath").get_asString(),
