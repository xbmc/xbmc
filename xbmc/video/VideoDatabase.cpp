...
void CVideoDatabase::CreateTables()
{
  ...
  m_pDS->exec("CREATE TABLE bookmark ( idBookmark integer primary key, idPath integer, idFile integer, timeInSeconds double, totalTimeInSeconds double, thumbNailImage text, strHash text, player text, playerState text, type integer)\n");
...
//********************************************************************************************************************************
bool CVideoDatabase::GetMovieInfo(const std::string& strFilenameAndPath, CVideoInfoTag& details, int idMovie /* = -1 */)
{
  try
  {
...
bool CVideoDatabase::GetEpisodeInfo(const std::string& strFilenameAndPath, CVideoInfoTag& details, int idEpisode /* = -1 */)
{
  try
  {
    // TODO: Optimize this - no need for all the queries!
...
bool CVideoDatabase::GetMusicVideoInfo(const std::string& strFilenameAndPath, CVideoInfoTag& details, int idMVideo /* = -1 */)
{
  try
  {
    // TODO: Optimize this - no need for all the queries!
...
//********************************************************************************************************************************
int CVideoDatabase::SetDetailsForItem(const CVideoInfoTag& details, const std::map<std::string, std::string> &artwork)
{
  return SetDetailsForItem(details.m_iDbId, details.m_type, details, artwork);
}

int CVideoDatabase::SetDetailsForItem(int id, const MediaType& mediaType, const CVideoInfoTag& details, const std::map<std::string, std::string> &artwork)
{
...
int CVideoDatabase::SetDetailsForMovie(const std::string& strFilenameAndPath, const CVideoInfoTag& details, const map<string, string> &artwork, int idMovie /* = -1 */)
{
  try
  {
    ...
    { // query DB for any movies matching imdbid and year
      std::string strSQL = PrepareSQL("select files.playCount, files.lastPlayed from movie,files where files.idFile=movie.idFile and movie.c%02d='%s' and movie.c%02d=%i and movie.idMovie!=%i and files.playCount > 0", VIDEODB_ID_IDENT, details.m_strIMDBNumber.c_str(), VIDEODB_ID_YEAR, details.m_iYear, idMovie);
      m_pDS->query(strSQL.c_str());
...
int CVideoDatabase::SetDetailsForEpisode(const std::string& strFilenameAndPath, const CVideoInfoTag& details, const map<string, string> &artwork, int idShow, int idEpisode)
{
  try
  {
    ...
    { // query DB for any episodes matching idShow, Season and Episode
      std::string strSQL = PrepareSQL("select files.playCount, files.lastPlayed from episode, files where files.idFile=episode.idFile and episode.c%02d=%i and episode.c%02d=%i AND episode.idShow=%i and episode.idEpisode!=%i and files.playCount > 0",VIDEODB_ID_EPISODE_SEASON, details.m_iSeason, VIDEODB_ID_EPISODE_EPISODE, details.m_iEpisode, idShow, idEpisode);
      m_pDS->query(strSQL.c_str());
  ...
  try
  {
    ...
    else
    {
      ...
      if (type == CBookmark::FOLDER_RESUME)
        idPath = GetPathId(strFilenameAndPath);
      else
        idFile = GetFileId(strFilenameAndPath);
      ...
      while (!m_pDS->eof())
      {
        ...
        {
          idFile = m_pDS->fv("idFile").get_asInt();
          m_pDS2->query(PrepareSQL("SELECT * FROM files WHERE idFile=%i", idFile).c_str());
          idPath = m_pDS2->fv("idPath").get_asInt();
          bookmark.item = strFilenameAndPath;
	  std::string strFilename = m_pDS2->fv("strFilename").get_asString();
          m_pDS2->close();
          m_pDS2->query(PrepareSQL("SELECT strPath FROM path WHERE idPath=%i", idPath));
...
bool CVideoDatabase::GetResumeBookMark(const std::string& strFilenameAndPath, CBookmark &bookmark)
{
  ...
  GetBookMarksForFile(strFilenameAndPath, bookmarks, bookmark.type);
  if (bookmarks.size() > 0)
  {
...
void CVideoDatabase::AddBookMarkToFile(const std::string& strFilenameAndPath, const CBookmark &bookmark, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  try
  {
    ...
    if (type == CBookmark::FOLDER_RESUME)
    {
      std::string strDirectory = g_playlistPlayer.GetFolderPath(PLAYLIST_VIDEO);
      if (CSettings::Get().GetBool("filelists.checkfolderhash"))
        hash = CFileUtils::GetFastHash(strDirectory);
      idPath = GetPathId(strDirectory);
      if (idPath == -1)
        idPath = AddPath(strDirectory);
      if (idPath < 0)
        return;
    }
    ...
    else if (type == CBookmark::FOLDER_RESUME)
    {
      strSQL = PrepareSQL("SELECT idBookmark FROM bookmark WHERE idPath=%i AND type=%i", idPath, (int)type);
    }
    ...
    if (idBookmark >= 0 )
      strSQL=PrepareSQL("UPDATE bookmark SET timeInSeconds = %f, totalTimeInSeconds = %f, thumbNailImage = '%s', player = '%s', playerState = '%s', idPath = %i, idFile = %i, strHash = '%s' WHERE idBookmark = %i", bookmark.timeInSeconds, bookmark.totalTimeInSeconds, bookmark.thumbNailImage.c_str(), bookmark.player.c_str(), bookmark.playerState.c_str(), idPath, idFile, hash.c_str(), idBookmark);
    else
      strSQL=PrepareSQL("INSERT INTO bookmark (idBookmark, idFile, timeInSeconds, totalTimeInSeconds, thumbNailImage, player, playerState, type, idPath, strHash) VALUES(NULL,%i,%f,%f,'%s','%s','%s', %i, %i, '%s')", idFile, bookmark.timeInSeconds, bookmark.totalTimeInSeconds, bookmark.thumbNailImage.c_str(), bookmark.player.c_str(), bookmark.playerState.c_str(), (int)type, idPath, hash.c_str());
    m_pDS->exec(strSQL.c_str());
  }
  catch (...)
  {
...
void CVideoDatabase::ClearBookMarkOfFile(const std::string& strFilenameAndPath, CBookmark& bookmark, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  try
  {
    ...
    int idPath = -1;
    if (type == CBookmark::FOLDER_RESUME)
      idPath = GetPathId(g_playlistPlayer.GetFolderPath(PLAYLIST_VIDEO));
    else
      idFile = GetFileId(strFilenameAndPath);
    if (idFile < 0 && idPath < 0) return ;
    if (NULL == m_pDB.get()) return ;
    ...
    std::string strSQL;
    if (type == CBookmark::FOLDER_RESUME)
      strSQL = PrepareSQL("SELECT idBookmark FROM bookmark WHERE idPath=%i AND type=%i AND playerState LIKE '%s' AND player LIKE '%s' AND (timeInSeconds BETWEEN %f AND %f)", idPath, type, bookmark.playerState.c_str(), bookmark.player.c_str(), mintime, maxtime);
    else
      strSQL = PrepareSQL("SELECT idBookmark FROM bookmark WHERE idFile=%i AND type=%i AND playerState LIKE '%s' AND player LIKE '%s' AND (timeInSeconds BETWEEN %f AND %f)", idFile, type, bookmark.playerState.c_str(), bookmark.player.c_str(), mintime, maxtime);
...
void CVideoDatabase::ClearBookMarksOfFile(const std::string& strFilenameAndPath, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  ...
  if (type == CBookmark::FOLDER_RESUME)
    ...
    idFile = GetFileId(strFilenameAndPath);
  if (idFile < 0 && idPath < 0) return;
  return ClearBookMarksOfFile(type == CBookmark::FOLDER_RESUME ? idPath : idFile, type);
}
...
void CVideoDatabase::ClearBookMarksOfFile(int idFile, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  ...
  try
  {
    ...
    std::string strSQL;
    if (type == CBookmark::FOLDER_RESUME)
      strSQL = PrepareSQL("DELETE FROM bookmark WHERE idPath=%i AND type=%i", idFile, (int)type);
    else
      strSQL = PrepareSQL("DELETE FROM bookmark WHERE idFile=%i AND type=%i", idFile, (int)type);
    m_pDS->exec(strSQL.c_str());
    if (type == CBookmark::EPISODE)
    {
...
void CVideoDatabase::UpdateTables(int iVersion)
{
  ...
  { // add seasons table
    ...
    while (!m_pDS->eof())
    {
      std::string sql = PrepareSQL("INSERT INTO seasons (idShow,season)"
                                  "  SELECT DISTINCT"
                                  "    idShow,c%02d"
                                  "  FROM"
                                  "    episode"
  ...
  { // add idShow to episode table
    ...
    while (!m_pDS->eof())
    {
      ...
      std::string update = PrepareSQL("UPDATE episode SET idShow=(SELECT idShow FROM tvshowlinkepisode WHERE idEpisode=%d) WHERE idEpisode=%d", idEpisode, idEpisode);
  ...
  { // add idSet to movie table
    ...
    while (!m_pDS->eof())
    {
      ...
      std::string sql = PrepareSQL("UPDATE movie SET idSet=(SELECT idSet FROM setlinkmovie WHERE idMovie = %d LIMIT 1) WHERE idMovie = %d", idMovie, idMovie);
      m_pDS2->exec(sql.c_str());
      m_pDS->next();
    }
  ...
  { // update the runtime columns
    ...
    for (vector< pair<string, int> >::iterator i = tables.begin(); i != tables.end(); ++i)
    {
      std::string sql = PrepareSQL("select id%s,c%02d from %s where c%02d != ''", i->first.c_str(), i->second, (i->first=="mvideo")?"musicvideo":i->first.c_str(), i->second);
      m_pDS->query(sql.c_str());
  ...
  if (iVersion < 92)
  {
    m_pDS->exec("ALTER TABLE bookmark ADD idPath INTEGER");
    m_pDS->exec("ALTER TABLE bookmark ADD strHash TEXT");
  }
}
