...
int CVideoDatabase::GetEpisodeId(const std::string& strFilenameAndPath, int idEpisode, int idSeason) // input value is episode/season number hint - for multiparters
{
  try
  {
    ...
    if (idReturnEpisode == -1 && idEpisode != -1 && idSeason != -1)
    {
      ...
      // In which case strFilenameAndPath is the basepath (in case of ISO) or
      // <basepath>/BDMV/index.bdmv (in case of files)
      const std::string strSQL{
          PrepareSQL("select strFileName, strPath from episode_view "
                     "where c%02d='%s' and c%02d=%i and c%02d=%i",
                     VIDEODB_ID_EPISODE_BASEPATH, URIUtils::GetDiscBase(strFilenameAndPath).c_str(),
                     VIDEODB_ID_EPISODE_SEASON, idSeason, VIDEODB_ID_EPISODE_EPISODE, idEpisode)};
...
void CVideoDatabase::GetEpisodesByFileId(int idFile, std::vector<CVideoInfoTag>& episodes)
{
  ...
  {
    const int idShow{
        GetSingleValueInt(PrepareSQL("SELECT idShow FROM episode WHERE idFile=%i", idFile), m_pDS)};
...
void CVideoDatabase::ExportArt(const CFileItem& item, const ArtMap& artwork, bool overwrite)
{
  for (const auto& [artType, artPath] : artwork)
  {
  ...
  try
  {
    ...
    while (movie)
    {
      ...
      if (StringUtils::CompareNoCase(movie->Value(), MediaTypeMovie, 5) == 0)
      {
        ...
        if (URIUtils::IsBlurayPath(lastPath) && URIUtils::IsBlurayPath(item.GetPath()) &&
            URIUtils::GetDirectory(lastPath) == URIUtils::GetDirectory(item.GetPath()) &&
            item.HasVideoVersions())
        {
          item.SetProperty("idMovie", lastMovieId);
          const int versionId{static_cast<int>(scanner.AddVideo(
              &item, nullptr, useFolders, true, NULL, true, CONTENT_MOVIEVERSIONS))};

          // Set default version
          if (item.GetVideoInfoTag()->IsDefaultVideoVersion())
            SetDefaultVideoVersion(VideoDbContentType::MOVIES, lastMovieId, versionId);
        }
        else
        {
          lastMovieId =
