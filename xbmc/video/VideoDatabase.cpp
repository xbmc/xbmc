...
void CVideoDatabase::CreateTables()
{
  ...
  string columns = "CREATE TABLE movie ( idMovie integer primary key, idFile integer";
...
void CVideoDatabase::CreateAnalytics()
{
  ...
  string createColIndex = StringUtils::Format("CREATE INDEX ix_episode_season_episode on episode (c%02d, c%02d)", VIDEODB_ID_EPISODE_SEASON, VIDEODB_ID_EPISODE_EPISODE);
...
void CVideoDatabase::CreateViews()
{
  ...
  string episodeview = PrepareSQL("CREATE VIEW episodeview AS SELECT "
                                      "  episode.*,"
  ...
  string tvshowview = PrepareSQL("CREATE VIEW tvshowview AS SELECT "
                                     "  tvshow.*,"
                                     "  path.strPath AS strPath,"
...
//********************************************************************************************************************************
int CVideoDatabase::GetPathId(const string& strPath)
{
  string strSQL;
  try
  {
...
bool CVideoDatabase::GetPaths(set<string> &paths)
{
  try
  {
...
int CVideoDatabase::RunQuery(const string &sql)
{
...
bool CVideoDatabase::GetSubPaths(const string &basepath, vector< pair<int,string> >& subpaths)
{
  string sql;
  try
  {
...
int CVideoDatabase::AddPath(const string& strPath, const string &strDateAdded /*= "" */)
{
  string strSQL;
  try
  {
...
bool CVideoDatabase::GetPathHash(const string &path, string &hash)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string strSQL=PrepareSQL("select strHash from path where strPath='%s'", path.c_str());
    m_pDS->query(strSQL.c_str());
    if (m_pDS->num_rows() == 0)
      return false;
...
//********************************************************************************************************************************
int CVideoDatabase::AddFile(const string& strFileNameAndPath)
{
  string strSQL = "";
  try
  {
    ...
    string strSQL=PrepareSQL("select idFile from files where strFileName='%s' and idPath=%i", strFileName.c_str(),idPath);
...
void CVideoDatabase::UpdateFileDateAdded(int idFile, const string& strFileNameAndPath)
{
  ...
  try
  {
    ...
    string file = strFileNameAndPath;
...
bool CVideoDatabase::SetPathHash(const string &path, const string &hash)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string strSQL=PrepareSQL("update path set strHash='%s' where idPath=%ld", hash.c_str(), idPath);
    m_pDS->exec(strSQL.c_str());
...
bool CVideoDatabase::LinkMovieToTvshow(int idMovie, int idShow, bool bRemove)
{
   try
  {
    ...
    {
      string strSQL=PrepareSQL("delete from movielinktvshow where idMovie=%i and idShow=%i", idMovie, idShow);
      m_pDS->exec(strSQL.c_str());
      return true;
    }

    string strSQL=PrepareSQL("insert into movielinktvshow (idShow,idMovie) values (%i,%i)", idShow,idMovie);
    m_pDS->exec(strSQL.c_str());
...
bool CVideoDatabase::IsLinkedToTvshow(int idMovie)
{
   try
  {
    ...
    string strSQL=PrepareSQL("select * from movielinktvshow where idMovie=%i", idMovie);
    m_pDS->query(strSQL.c_str());
    if (m_pDS->eof())
    {
      m_pDS->close();
...
//********************************************************************************************************************************
int CVideoDatabase::GetFileId(const string& strFilenameAndPath)
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
...
//********************************************************************************************************************************
int CVideoDatabase::GetMovieId(const string& strFilenameAndPath)
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
...
int CVideoDatabase::GetTvShowId(const string& strPath)
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
...
int CVideoDatabase::GetEpisodeId(const string& strFilenameAndPath, int idEpisode, int idSeason) // input value is episode/season number hint - for multiparters
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
    ...
    string strSQL=PrepareSQL("select idEpisode from episode where idFile=%i", idFile);
...
int CVideoDatabase::GetMusicVideoId(const string& strFilenameAndPath)
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
    ...
    string strSQL=PrepareSQL("select idMVideo from musicvideo where idFile=%i", idFile);
...
//********************************************************************************************************************************
int CVideoDatabase::AddMovie(const string& strFilenameAndPath)
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
    ...
    if (idMovie < 0)
    {
      ...
      string strSQL=PrepareSQL("insert into movie (idMovie, idFile) values (NULL, %i)", idFile);
      m_pDS->exec(strSQL.c_str());
...
int CVideoDatabase::AddTvShow(const string& strPath)
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
    ...
    string strSQL=PrepareSQL("select tvshowlinkpath.idShow from path,tvshowlinkpath where path.strPath='%s' and path.idPath=tvshowlinkpath.idPath",strPath.c_str());
    m_pDS->query(strSQL.c_str());
    if (m_pDS->num_rows() != 0)
...
//********************************************************************************************************************************
int CVideoDatabase::AddEpisode(int idShow, const string& strFilenameAndPath)
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
    ...
    string strSQL=PrepareSQL("insert into episode (idEpisode, idFile, idShow) values (NULL, %i, %i)", idFile, idShow);
    m_pDS->exec(strSQL.c_str());
...
int CVideoDatabase::AddMusicVideo(const string& strFilenameAndPath)
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
    ...
    if (idMVideo < 0)
    {
      ...
      if (idFile < 0)
        return -1;
      UpdateFileDateAdded(idFile, strFilenameAndPath);
      string strSQL=PrepareSQL("insert into musicvideo (idMVideo, idFile) values (NULL, %i)", idFile);
      m_pDS->exec(strSQL.c_str());
...
//********************************************************************************************************************************
int CVideoDatabase::AddToTable(const string& table, const string& firstField, const string& secondField, const string& value)
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
    ...
    string strSQL = PrepareSQL("select %s from %s where %s like '%s'", firstField.c_str(), table.c_str(), secondField.c_str(), value.c_str());
    m_pDS->query(strSQL.c_str());
    if (m_pDS->num_rows() == 0)
    {
      m_pDS->close();
...
int CVideoDatabase::AddSet(const string& strSet)
{
  if (strSet.empty())
    return -1;
...
int CVideoDatabase::AddGenre(const string& strGenre)
{
  return AddToTable("genre", "idGenre", "strGenre", strGenre);
}

int CVideoDatabase::AddStudio(const string& strStudio)
{
  return AddToTable("studio", "idStudio", "strStudio", strStudio);
}

//********************************************************************************************************************************
int CVideoDatabase::AddCountry(const string& strCountry)
{
  return AddToTable("country", "idCountry", "strCountry", strCountry);
}

int CVideoDatabase::AddActor(const string& strActor, const string& thumbURLs, const string &thumb)
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
    if (NULL == m_pDS.get()) return -1;
    int idActor = -1;
    string strSQL=PrepareSQL("select idActor from actors where strActor like '%s'", strActor.c_str());
    m_pDS->query(strSQL.c_str());
    if (m_pDS->num_rows() == 0)
    {
      m_pDS->close();
...
void CVideoDatabase::AddLinkToActor(const char *table, int actorID, const char *field, int secondID, const string &role, int order)
{
  try
  {
    if (NULL == m_pDB.get()) return ;
    ...
    string strSQL=PrepareSQL("select * from actorlink%s where idActor=%i and id%s=%i", table, actorID, field, secondID);
    m_pDS->query(strSQL.c_str());
    if (m_pDS->num_rows() == 0)
    {
      // doesnt exists, add it
...
void CVideoDatabase::AddToLinkTable(const char *table, const char *firstField, int firstID, const char *secondField, int secondID, const char *typeField /* = NULL */, const char *type /* = NULL */)
{
  try
  {
    ...
    string strSQL = PrepareSQL("select * from %s where %s=%i and %s=%i", table, firstField, firstID, secondField, secondID);
...
void CVideoDatabase::RemoveFromLinkTable(const char *table, const char *firstField, int firstID, const char *secondField, int secondID, const char *typeField /* = NULL */, const char *type /* = NULL */)
{
  try
  {
    ...
    string strSQL = PrepareSQL("DELETE FROM %s WHERE %s = %i AND %s = %i", table, firstField, firstID, secondField, secondID);
    if (typeField != NULL && type != NULL)
      strSQL += PrepareSQL(" AND %s='%s'", typeField, type);
    m_pDS->exec(strSQL.c_str());
  }
...
//********************************************************************************************************************************
bool CVideoDatabase::LoadVideoInfo(const string& strFilenameAndPath, CVideoInfoTag& details)
{
...
bool CVideoDatabase::HasMovieInfo(const string& strFilenameAndPath)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
...
bool CVideoDatabase::HasTvShowInfo(const string& strPath)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
...
bool CVideoDatabase::HasEpisodeInfo(const string& strFilenameAndPath)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
...
bool CVideoDatabase::HasMusicVideoInfo(const string& strFilenameAndPath)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
...
void CVideoDatabase::DeleteDetailsForTvShow(const string& strPath, int idTvShow /* = -1 */)
{
  try
  {
    if (NULL == m_pDB.get()) return ;
...
//********************************************************************************************************************************
void CVideoDatabase::GetMoviesByActor(const string& strActor, CFileItemList& items)
{
  Filter filter;
...
void CVideoDatabase::GetTvShowsByActor(const string& strActor, CFileItemList& items)
{
  Filter filter;
...
void CVideoDatabase::GetEpisodesByActor(const string& strActor, CFileItemList& items)
{
  Filter filter;
...
void CVideoDatabase::GetMusicVideosByArtist(const string& strArtist, CFileItemList& items)
{
  try
  {
    items.Clear();
...
//********************************************************************************************************************************
bool CVideoDatabase::GetMovieInfo(const string& strFilenameAndPath, CVideoInfoTag& details, int idMovie /* = -1 */)
{
  try
  {
    ...
    string sql = PrepareSQL("select * from movieview where idMovie=%i", idMovie);
    if (!m_pDS->query(sql.c_str()))
      return false;
...
//********************************************************************************************************************************
bool CVideoDatabase::GetTvShowInfo(const string& strPath, CVideoInfoTag& details, int idTvShow /* = -1 */)
{
  try
  {
    if (idTvShow < 0)
    ...
    string sql = PrepareSQL("SELECT * FROM tvshowview WHERE idShow=%i", idTvShow);
    if (!m_pDS->query(sql.c_str()))
      return false;
...
bool CVideoDatabase::GetSeasonInfo(int idSeason, CVideoInfoTag& details)
{
  ...
  try
  {
    ...
    string sql = PrepareSQL("SELECT idShow FROM seasons WHERE idSeason=%i", idSeason);
    if (!m_pDS->query(sql.c_str()))
      return false;
...
bool CVideoDatabase::GetEpisodeInfo(const string& strFilenameAndPath, CVideoInfoTag& details, int idEpisode /* = -1 */)
{
  try
  {
    // TODO: Optimize this - no need for all the queries!
    ...
    string sql = PrepareSQL("select * from episodeview where idEpisode=%i",idEpisode);
    if (!m_pDS->query(sql.c_str()))
      return false;
...
bool CVideoDatabase::GetMusicVideoInfo(const string& strFilenameAndPath, CVideoInfoTag& details, int idMVideo /* = -1 */)
{
  try
  {
    // TODO: Optimize this - no need for all the queries!
    ...
    string sql = PrepareSQL("select * from musicvideoview where idMVideo=%i", idMVideo);
    if (!m_pDS->query(sql.c_str()))
      return false;
...
bool CVideoDatabase::GetFileInfo(const string& strFilenameAndPath, CVideoInfoTag& details, int idFile /* = -1 */)
{
  try
  {
    if (idFile < 0)
    ...
    string sql = PrepareSQL("SELECT * FROM files "
    ...
    string strFileName = m_pDS->fv("files.strFilename").get_asString();
...
string CVideoDatabase::GetValueString(const CVideoInfoTag &details, int min, int max, const SDbTableOffsets *offsets) const
{
  ...
  for (int i = min + 1; i < max; ++i)
  {
    ...
    case VIDEODB_TYPE_STRING:
      conditions.push_back(PrepareSQL("c%02d='%s'", i, ((string*)(((char*)&details)+offsets[i].offset))->c_str()));
      break;
...
//********************************************************************************************************************************
int CVideoDatabase::SetDetailsForMovie(const string& strFilenameAndPath, const CVideoInfoTag& details, const map<string, string> &artwork, int idMovie /* = -1 */)
{
  try
  {
    BeginTransaction();
    ...
    string strSQL = PrepareSQL("select files.playCount, files.lastPlayed from movie,files where files.idFile=movie.idFile and movie.c%02d='%s' and movie.c%02d=%i and movie.idMovie!=%i and files.playCount > 0", VIDEODB_ID_IDENT, details.m_strIMDBNumber.c_str(), VIDEODB_ID_YEAR, details.m_iYear, idMovie);
    m_pDS->query(strSQL.c_str());
    ...
    string sql = "update movie set " + GetValueString(details, VIDEODB_ID_MIN, VIDEODB_ID_MAX, DbMovieOffsets);
    if (idSet > 0)
...
int CVideoDatabase::SetDetailsForMovieSet(const CVideoInfoTag& details, const std::map<std::string, std::string> &artwork, int idSet /* = -1 */)
{
  ...
  try
  {
    ...
    // and insert the new row
    string sql = PrepareSQL("UPDATE sets SET strSet='%s' WHERE idSet=%i", details.m_strTitle.c_str(), idSet);
    m_pDS->exec(sql.c_str());
    CommitTransaction();
...
int CVideoDatabase::SetDetailsForTvShow(const string& strPath, const CVideoInfoTag& details, const map<string, string> &artwork, const map<int, map<string, string> > &seasonArt, int idTvShow /*= -1 */)
{
  try
  {
    if (!m_pDB.get() || !m_pDS.get())
    ...
    // and insert the new row
    string sql = "update tvshow set " + GetValueString(details, VIDEODB_ID_TV_MIN, VIDEODB_ID_TV_MAX, DbTvShowOffsets);
...
int CVideoDatabase::SetDetailsForSeason(const CVideoInfoTag& details, const std::map<std::string, std::string> &artwork, int idShow, int idSeason /* = -1 */)
{
   ...
   try
  {
    ...
    // and insert the new row
    string sql = PrepareSQL("UPDATE seasons SET season=%i WHERE idSeason=%i", details.m_iSeason, idSeason);
    m_pDS->exec(sql.c_str());
    CommitTransaction();
...
int CVideoDatabase::SetDetailsForEpisode(const string& strFilenameAndPath, const CVideoInfoTag& details, const map<string, string> &artwork, int idShow, int idEpisode)
{
  try
  {
    BeginTransaction();
    ...
    string strSQL = PrepareSQL("select files.playCount, files.lastPlayed from episode, files where files.idFile=episode.idFile and episode.c%02d=%i and episode.c%02d=%i AND episode.idShow=%i and episode.idEpisode!=%i and files.playCount > 0",VIDEODB_ID_EPISODE_SEASON, details.m_iSeason, VIDEODB_ID_EPISODE_EPISODE, details.m_iEpisode, idShow, idEpisode);
    m_pDS->query(strSQL.c_str());
    ...
    // and insert the new row
    string sql = "update episode set " + GetValueString(details, VIDEODB_ID_EPISODE_MIN, VIDEODB_ID_EPISODE_MAX, DbEpisodeOffsets);
...
{
  string sql = PrepareSQL("idShow=%i AND season=%i", showID, season);
  string id = GetSingleValue("seasons", "idSeason", sql);
  if (id.empty())
    return -1;
...
int CVideoDatabase::SetDetailsForMusicVideo(const string& strFilenameAndPath, const CVideoInfoTag& details, const map<string, string> &artwork, int idMVideo /* = -1 */)
{
  try
  {
    BeginTransaction();
    ...
    if (!details.m_artist.empty())
    {
      ...
      {
        string artist = details.m_artist[i];
    ...
    // update our movie table (we know it was added already above)
    // and insert the new row
    string sql = "update musicvideo set " + GetValueString(details, VIDEODB_ID_MUSICVIDEO_MIN, VIDEODB_ID_MUSICVIDEO_MAX, DbMusicVideoOffsets);
...
void CVideoDatabase::SetStreamDetailsForFile(const CStreamDetails& details, const string &strFileNameAndPath)
{
...
void CVideoDatabase::SetStreamDetailsForFileId(const CStreamDetails& details, int idFile)
{
  ...
  try
  {
    ...
    if (details.GetVideoDuration())
    {
      ...
      {
        string sql = PrepareSQL("update %s set c%02d=%d where idFile=%d and c%02d=''",
...
//********************************************************************************************************************************
void CVideoDatabase::GetFilePathById(int idMovie, string &filePath, VIDEODB_CONTENT_TYPE iType)
{
  try
  {
    if (NULL == m_pDB.get()) return ;
    ...
    if (!m_pDS->eof())
    {
      if (iType != VIDEODB_CONTENT_TVSHOWS)
      {
        string fileName = m_pDS->fv("files.strFilename").get_asString();
...
//********************************************************************************************************************************
void CVideoDatabase::GetBookMarksForFile(const string& strFilenameAndPath, VECBOOKMARKS& bookmarks, CBookmark::EType type /*= CBookmark::STANDARD*/, bool bAppend, long partNumber)
{
  try
  {
    ...
    else
    {
      ...
      string strSQL=PrepareSQL("select * from bookmark where idFile=%i and type=%i order by timeInSeconds", idFile, (int)type);
      m_pDS->query( strSQL.c_str() );
      while (!m_pDS->eof())
      {
        CBookmark bookmark;
        ...
        {
          string strSQL2=PrepareSQL("select c%02d, c%02d from episode where c%02d=%i order by c%02d, c%02d", VIDEODB_ID_EPISODE_EPISODE, VIDEODB_ID_EPISODE_SEASON, VIDEODB_ID_EPISODE_BOOKMARK, m_pDS->fv("idBookmark").get_asInt(), VIDEODB_ID_EPISODE_SORTSEASON, VIDEODB_ID_EPISODE_SORTEPISODE);
...
bool CVideoDatabase::GetResumeBookMark(const string& strFilenameAndPath, CBookmark &bookmark)
{
...
void CVideoDatabase::DeleteResumeBookMark(const string &strFilenameAndPath)
{
  if (!m_pDB.get() || !m_pDS.get())
    return;
  ...
  try
  {
    string sql = PrepareSQL("delete from bookmark where idFile=%i and type=%i", fileID, CBookmark::RESUME);
    m_pDS->exec(sql.c_str());
  }
  catch(...)
  {
...
void CVideoDatabase::GetEpisodesByFile(const string& strFilenameAndPath, vector<CVideoInfoTag>& episodes)
{
  try
  {
    string strSQL = PrepareSQL("select * from episodeview where idFile=%i order by c%02d, c%02d asc", GetFileId(strFilenameAndPath), VIDEODB_ID_EPISODE_SORTSEASON, VIDEODB_ID_EPISODE_SORTEPISODE);
    m_pDS->query(strSQL.c_str());
    while (!m_pDS->eof())
    {
...
//********************************************************************************************************************************
void CVideoDatabase::AddBookMarkToFile(const string& strFilenameAndPath, const CBookmark &bookmark, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  try
  {
    int idFile = AddFile(strFilenameAndPath);
...
void CVideoDatabase::ClearBookMarkOfFile(const string& strFilenameAndPath, CBookmark& bookmark, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  try
  {
    int idFile = GetFileId(strFilenameAndPath);
    ...
    double mintime = bookmark.timeInSeconds - 0.5f;
    double maxtime = bookmark.timeInSeconds + 0.5f;
    string strSQL = PrepareSQL("select idBookmark from bookmark where idFile=%i and type=%i and playerState like '%s' and player like '%s' and (timeInSeconds between %f and %f)", idFile, type, bookmark.playerState.c_str(), bookmark.player.c_str(), mintime, maxtime);
...
//********************************************************************************************************************************
void CVideoDatabase::ClearBookMarksOfFile(const string& strFilenameAndPath, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  try
  {
    int idFile = GetFileId(strFilenameAndPath);
    ...
    string strSQL=PrepareSQL("delete from bookmark where idFile=%i and type=%i", idFile, (int)type);
    m_pDS->exec(strSQL.c_str());
    if (type == CBookmark::EPISODE)
    {
...
bool CVideoDatabase::GetBookMarkForEpisode(const CVideoInfoTag& tag, CBookmark& bookmark)
{
  try
  {
    string strSQL = PrepareSQL("select bookmark.* from bookmark join episode on episode.c%02d=bookmark.idBookmark where episode.idEpisode=%i", VIDEODB_ID_EPISODE_BOOKMARK, tag.m_iDbId);
    m_pDS->query( strSQL.c_str() );
    if (!m_pDS->eof())
    {
      bookmark.timeInSeconds = m_pDS->fv("timeInSeconds").get_asDouble();
...
void CVideoDatabase::AddBookMarkForEpisode(const CVideoInfoTag& tag, const CBookmark& bookmark)
{
  try
  {
    ...
    string strSQL = PrepareSQL("delete from bookmark where idBookmark in (select c%02d from episode where c%02d=%i and c%02d=%i and idFile=%i)", VIDEODB_ID_EPISODE_BOOKMARK, VIDEODB_ID_EPISODE_SEASON, tag.m_iSeason, VIDEODB_ID_EPISODE_EPISODE, tag.m_iEpisode, idFile);
    m_pDS->exec(strSQL.c_str());
...
void CVideoDatabase::DeleteBookMarkForEpisode(const CVideoInfoTag& tag)
{
  try
  {
    string strSQL = PrepareSQL("delete from bookmark where idBookmark in (select c%02d from episode where idEpisode=%i)", VIDEODB_ID_EPISODE_BOOKMARK, tag.m_iDbId);
    m_pDS->exec(strSQL.c_str());
...
void CVideoDatabase::DeleteMovie(const string& strFilenameAndPath, bool bKeepId /* = false */, int idMovie /* = -1 */)
{
  try
  {
    if (NULL == m_pDB.get()) return ;
...
void CVideoDatabase::DeleteTvShow(const string& strPath, bool bKeepId /* = false */, int idTvShow /* = -1 */)
{
  try
  {
    if (NULL == m_pDB.get()) return ;
    ...
    string strSQL=PrepareSQL("select episode.idEpisode,path.strPath,files.strFileName from episode,path,files where episode.idShow=%i and episode.idFile=files.idFile and files.idPath=path.idPath",idTvShow);
    m_pDS2->query(strSQL.c_str());
    while (!m_pDS2->eof())
    {
      string strFilenameAndPath;
      string strPath = m_pDS2->fv("path.strPath").get_asString();
      string strFileName = m_pDS2->fv("files.strFilename").get_asString();
...
void CVideoDatabase::DeleteEpisode(const string& strFilenameAndPath, int idEpisode /* = -1 */, bool bKeepId /* = false */)
{
  try
  {
    if (NULL == m_pDB.get()) return ;
...
void CVideoDatabase::DeleteMusicVideo(const string& strFilenameAndPath, bool bKeepId /* = false */, int idMVideo /* = -1 */)
{
  try
  {
    if (NULL == m_pDB.get()) return ;
...
void CVideoDatabase::GetDetailsFromDB(const dbiplus::sql_record* const record, int min, int max, const SDbTableOffsets *offsets, CVideoInfoTag &details, int idxOffset)
{
  for (int i = min + 1; i < max; i++)
  {
    ...
    {
    case VIDEODB_TYPE_STRING:
      *(string*)(((char*)&details)+offsets[i].offset) = record->at(i+idxOffset).get_asString();
      break;
    case VIDEODB_TYPE_INT:
    case VIDEODB_TYPE_COUNT:
...
bool CVideoDatabase::GetStreamDetails(CVideoInfoTag& tag) const
{
  ...
  try
  {
    string strSQL = PrepareSQL("SELECT * FROM streamdetails WHERE idFile = %i", tag.m_iFileId);
    pDS->query(strSQL.c_str());
...
bool CVideoDatabase::GetResumePoint(CVideoInfoTag& tag)
{
  ...
  try
  {
    ...
    {
      string strSQL=PrepareSQL("select timeInSeconds, totalTimeInSeconds from bookmark where idFile=%i and type=%i order by timeInSeconds", tag.m_iFileId, CBookmark::RESUME);
      m_pDS2->query( strSQL.c_str() );
      if (!m_pDS2->eof())
      {
...
CVideoInfoTag CVideoDatabase::GetDetailsForMovie(const dbiplus::sql_record* const record, bool getDetails /* = false */)
{
  ...
  string strFileName = record->at(VIDEODB_DETAILS_MOVIE_FILE).get_asString();
  ConstructPath(details.m_strFileNameAndPath,details.m_strPath,strFileName);
  ...
  if (getDetails)
  {
    ...
    // get tags
    string strSQL = PrepareSQL("SELECT tag.strTag FROM tag, taglinks WHERE taglinks.idMedia = %i AND taglinks.media_type = 'movie' AND taglinks.idTag = tag.idTag ORDER BY tag.idTag", idMovie);
    m_pDS2->query(strSQL.c_str());
    while (!m_pDS2->eof())
    {
    ...
    {
      string strSQL = PrepareSQL("select c%02d from tvshow where idShow=%i",
...
CVideoInfoTag CVideoDatabase::GetDetailsForTvShow(const dbiplus::sql_record* const record, bool getDetails /* = false */, CFileItem* item /* = NULL */)
{
  ...
  if (getDetails)
  {
    ...
    // get tags
    string strSQL = PrepareSQL("SELECT tag.strTag FROM tag, taglinks WHERE taglinks.idMedia = %i AND taglinks.media_type = 'tvshow' AND taglinks.idTag = tag.idTag ORDER BY tag.idTag", idTvShow);
    m_pDS2->query(strSQL.c_str());
    while (!m_pDS2->eof())
    {
      details.m_tags.push_back(m_pDS2->fv("tag.strTag").get_asString());
...
CVideoInfoTag CVideoDatabase::GetDetailsForEpisode(const dbiplus::sql_record* const record, bool getDetails /* = false */)
{
  ...
  string strFileName = record->at(VIDEODB_DETAILS_EPISODE_FILE).get_asString();
  ConstructPath(details.m_strFileNameAndPath,details.m_strPath,strFileName);
  ...
  if (getDetails)
  {
    ...
    castTime += XbmcThreads::SystemClockMillis() - time; time = XbmcThreads::SystemClockMillis();
    details.m_strPictureURL.Parse();
    string strSQL = PrepareSQL("select * from bookmark join episode on episode.c%02d=bookmark.idBookmark where episode.idEpisode=%i and bookmark.type=%i", VIDEODB_ID_EPISODE_BOOKMARK,details.m_iDbId,CBookmark::EPISODE);
    m_pDS2->query(strSQL.c_str());
    if (!m_pDS2->eof())
...
CVideoInfoTag CVideoDatabase::GetDetailsForMusicVideo(const dbiplus::sql_record* const record, bool getDetails /* = false */)
{
  ...
  string strFileName = record->at(VIDEODB_DETAILS_MUSICVIDEO_FILE).get_asString();
  ConstructPath(details.m_strFileNameAndPath,details.m_strPath,strFileName);
  ...
  {
    // get tags
    string strSQL = PrepareSQL("SELECT tag.strTag FROM tag, taglinks WHERE taglinks.idMedia = %i AND taglinks.media_type = 'musicvideo' AND taglinks.idTag = tag.idTag ORDER BY tag.idTag", idMVideo);
    m_pDS2->query(strSQL.c_str());
    while (!m_pDS2->eof())
    {
      details.m_tags.push_back(m_pDS2->fv("tag.strTag").get_asString());
...
void CVideoDatabase::GetCast(const string &table, const string &table_id, int type_id, vector<SActorInfo> &cast)
{
  try
  {
    if (!m_pDB.get()) return;
    ...
    string sql = PrepareSQL("SELECT actors.strActor,"
                                "  actorlink%s.strRole,"
                                "  actorlink%s.iOrder,"
                                "  actors.strThumb,"
                                "  art.url "
...
bool CVideoDatabase::GetVideoSettings(const string &strFilenameAndPath, CVideoSettings &settings)
{
  try
  {
    ...
    string strSQL=PrepareSQL("select * from settings, files, path where settings.idFile=files.idFile and path.idPath=files.idPath and path.strPath='%s' and files.strFileName='%s'", strPath.c_str() , strFileName.c_str());
#else
    int idFile = GetFileId(strFilenameAndPath);
    ...
    string strSQL=PrepareSQL("select * from settings where settings.idFile = '%i'", idFile);
#endif
    m_pDS->query( strSQL.c_str() );
    if (m_pDS->num_rows() > 0)
...
void CVideoDatabase::SetVideoSettings(const string& strFilenameAndPath, const CVideoSettings &setting)
{
  try
  {
    if (NULL == m_pDB.get()) return ;
    ...
    if (idFile < 0)
      return;
    string strSQL = StringUtils::Format("select * from settings where idFile=%i", idFile);
    m_pDS->query( strSQL.c_str() );
    if (m_pDS->num_rows() > 0)
    {
      m_pDS->close();
...
void CVideoDatabase::SetArtForItem(int mediaId, const string &mediaType, const string &artType, const string &url)
{
  try
  {
    ...
    string sql = PrepareSQL("SELECT art_id FROM art WHERE media_id=%i AND media_type='%s' AND type='%s'", mediaId, mediaType.c_str(), artType.c_str());
    m_pDS->query(sql.c_str());
    if (!m_pDS->eof())
    { // update
...
bool CVideoDatabase::GetArtForItem(int mediaId, const string &mediaType, map<string, string> &art)
{
  try
  {
    ...
    string sql = PrepareSQL("SELECT type,url FROM art WHERE media_id=%i AND media_type='%s'", mediaId, mediaType.c_str());
    m_pDS2->query(sql.c_str());
    while (!m_pDS2->eof())
    {
...
bool CVideoDatabase::GetTvShowSeasonArt(int showId, map<int, map<string, string> > &seasonArt)
{
  try
  {
    ...
    string sql = PrepareSQL("select idSeason,season from seasons where idShow=%i", showId);
    m_pDS2->query(sql.c_str());
...
bool CVideoDatabase::GetArtTypes(const std::string &mediaType, std::vector<std::string> &artTypes)
{
  try
  {
    ...
    string sql = PrepareSQL("SELECT DISTINCT type FROM art WHERE media_type='%s'", mediaType.c_str());
...
bool CVideoDatabase::GetStackTimes(const string &filePath, vector<int> &times)
{
  try
  {
    // obtain the FileID (if it exists)
    ...
    if (NULL == m_pDS.get()) return false;
    // ok, now obtain the settings for this file
    string strSQL=PrepareSQL("select times from stacktimes where idFile=%i\n", idFile);
    m_pDS->query( strSQL.c_str() );
    if (m_pDS->num_rows() > 0)
    { // get the video settings info
      int timeTotal = 0;
...
void CVideoDatabase::SetStackTimes(const string& filePath, vector<int> &times)
{
  try
  {
    if (NULL == m_pDB.get()) return ;
    ...
    // add the items
    string timeString = StringUtils::Format("%i", times[0]);
...
void CVideoDatabase::RemoveContentForPath(const string& strPath, CGUIDialogProgress *progress /* = NULL */)
{
  ...
  try
  {
    ...
    for (vector< pair<int, string> >::const_iterator i = paths.begin(); i != paths.end(); ++i)
    {
      ...
      {
        string strSQL = PrepareSQL("select files.strFilename from files join movie on movie.idFile=files.idFile where files.idPath=%i", i->first);
        m_pDS2->query(strSQL.c_str());
        if (m_pDS2->eof())
        {
...
void CVideoDatabase::SetScraperForPath(const string& filePath, const ScraperPtr& scraper, const VIDEO::SScanSettings& settings)
{
  ...
  try
  {
    ...
    {
      string content = TranslateContent(scraper->Content());
...
bool CVideoDatabase::ScraperInUse(const string &scraperID) const
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string sql = PrepareSQL("select count(1) from path where strScraper='%s'", scraperID.c_str());
...
void CVideoDatabase::UpdateTables(int iVersion)
{
  ...
  { // add seasons table
    ...
    while (!m_pDS->eof())
    {
      string sql = PrepareSQL("INSERT INTO seasons (idShow,season)"
                                  "  SELECT DISTINCT"
                                  "    idShow,c%02d"
                                  "  FROM"
                                  "    episode"
  ...
  { // add idShow to episode table
    ...
    while (!m_pDS->eof())
    {
      ...
      string update = PrepareSQL("UPDATE episode SET idShow=(SELECT idShow FROM tvshowlinkepisode WHERE idEpisode=%d) WHERE idEpisode=%d", idEpisode, idEpisode);
  ...
  { // add idSet to movie table
    ...
    while (!m_pDS->eof())
    {
      ...
      string sql = PrepareSQL("UPDATE movie SET idSet=(SELECT idSet FROM setlinkmovie WHERE idMovie = %d LIMIT 1) WHERE idMovie = %d", idMovie, idMovie);
      m_pDS2->exec(sql.c_str());
      m_pDS->next();
    }
  ...
  { // update the runtime columns
    ...
    for (vector< pair<string, int> >::iterator i = tables.begin(); i != tables.end(); ++i)
    {
      string sql = PrepareSQL("select id%s,c%02d from %s where c%02d != ''", i->first.c_str(), i->second, (i->first=="mvideo")?"musicvideo":i->first.c_str(), i->second);
      m_pDS->query(sql.c_str());
...
bool CVideoDatabase::LookupByFolders(const string &path, bool shows)
{
...
bool CVideoDatabase::GetPlayCounts(const string &strPath, CFileItemList &items)
{
  if(URIUtils::IsMultiPath(strPath))
  {
    vector<string> paths;
...
int CVideoDatabase::GetPlayCount(const CFileItem &item)
{
  ...
  try
  {
    ...
    string strSQL = PrepareSQL("select playCount from files WHERE idFile=%i", id);
    int count = 0;
...
void CVideoDatabase::UpdateMovieTitle(int idMovie, const string& strNewMovieTitle, VIDEODB_CONTENT_TYPE iType)
{
  try
  {
    if (NULL == m_pDB.get()) return ;
    ...
    else if (iType == VIDEODB_CONTENT_MOVIE_SETS)
    {
      ...
      string strSQL = PrepareSQL("UPDATE sets SET strSet='%s' WHERE idSet=%i", strNewMovieTitle.c_str(), idMovie );
      m_pDS->exec(strSQL.c_str());
    }
  ...
  {
    CLog::Log(LOGERROR, "%s (int idMovie, const string& strNewMovieTitle) failed on MovieID:%i and Title:%s", __FUNCTION__, idMovie, strNewMovieTitle.c_str());
  }
}

bool CVideoDatabase::UpdateVideoSortTitle(int idDb, const string& strNewSortTitle, VIDEODB_CONTENT_TYPE iType /* = VIDEODB_CONTENT_MOVIES */)
{
  try
  {
  ...
  {
    CLog::Log(LOGERROR, "%s (int idDb, const string& strNewSortTitle, VIDEODB_CONTENT_TYPE iType) failed on ID: %i and Sort Title: %s", __FUNCTION__, idDb, strNewSortTitle.c_str());
  }
...
bool CVideoDatabase::GetGenresNav(const string& strBaseDir, CFileItemList& items, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  return GetNavCommon(strBaseDir, items, "genre", idContent, filter, countOnly);
}

bool CVideoDatabase::GetCountriesNav(const string& strBaseDir, CFileItemList& items, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  return GetNavCommon(strBaseDir, items, "country", idContent, filter, countOnly);
}

bool CVideoDatabase::GetStudiosNav(const string& strBaseDir, CFileItemList& items, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  return GetNavCommon(strBaseDir, items, "studio", idContent, filter, countOnly);
}

bool CVideoDatabase::GetNavCommon(const string& strBaseDir, CFileItemList& items, const string &type, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    if (CProfilesManager::Get().GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      map<int, pair<string,int> > mapItems;
      map<int, pair<string,int> >::iterator it;
      while (!m_pDS->eof())
      {
        int id = m_pDS->fv(0).get_asInt();
        string str = m_pDS->fv(1).get_asString();
        ...
        {
          // check path
          if (g_passwordManager.IsDatabasePathUnlocked(string(m_pDS->fv(2).get_asString()),*CMediaSourceSettings::Get().GetSources("video")))
          {
            if (idContent == VIDEODB_CONTENT_MOVIES || idContent == VIDEODB_CONTENT_MUSICVIDEOS)
              mapItems.insert(pair<int, pair<string,int> >(id, pair<string,int>(str,m_pDS->fv(3).get_asInt()))); //fv(3) is file.playCount
            else if (idContent == VIDEODB_CONTENT_TVSHOWS)
              mapItems.insert(pair<int, pair<string,int> >(id, pair<string,int>(str,0)));
          }
        }
        m_pDS->next();
      }
      ...
      for (it = mapItems.begin(); it != mapItems.end(); ++it)
      {
        ...
        string path = StringUtils::Format("%ld/", it->first);
    ...
    else
    {
      while (!m_pDS->eof())
      {
        ...
        CVideoDbUrl itemUrl = videoUrl;
        string path = StringUtils::Format("%ld/", m_pDS->fv(0).get_asInt());
        itemUrl.AppendPath(path);
        pItem->SetPath(itemUrl.ToString());
...
bool CVideoDatabase::GetTagsNav(const string& strBaseDir, CFileItemList& items, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  string mediaType;
  if (idContent == VIDEODB_CONTENT_MOVIES)
    mediaType = "movie";
  ...
  try
  {
    ...
    string strSQL = "SELECT %s FROM taglinks ";
    ...
    while (!m_pDS->eof())
    {
      ...
      CVideoDbUrl itemUrl = videoUrl;
      string path = StringUtils::Format("%ld/", idTag);
      itemUrl.AppendPath(path);
      pItem->SetPath(itemUrl.ToString());
...
bool CVideoDatabase::GetSetsNav(const string& strBaseDir, CFileItemList& items, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool ignoreSingleMovieSets /* = false */)
{
  if (idContent != VIDEODB_CONTENT_MOVIES)
    return false;
...
bool CVideoDatabase::GetSetsByWhere(const string& strBaseDir, const Filter &filter, CFileItemList& items, bool ignoreSingleMovieSets /* = false */)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
...
bool CVideoDatabase::GetMusicVideoAlbumsNav(const string& strBaseDir, CFileItemList& items, int idArtist /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string strSQL = "select %s from musicvideoview ";
    Filter extFilter = filter;
    ...
    if (CProfilesManager::Get().GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      map<int, pair<string,string> > mapAlbums;
      map<int, pair<string,string> >::iterator it;
      while (!m_pDS->eof())
      {
        int lidMVideo = m_pDS->fv(1).get_asInt();
        string strAlbum = m_pDS->fv(0).get_asString();
        it = mapAlbums.find(lidMVideo);
        // was this genre already found?
        if (it == mapAlbums.end())
        {
          // check path
          if (g_passwordManager.IsDatabasePathUnlocked(string(m_pDS->fv("path.strPath").get_asString()),*CMediaSourceSettings::Get().GetSources("video")))
    ...
    else
    {
      while (!m_pDS->eof())
      {
        if (!m_pDS->fv(0).get_asString().empty())
        {
          ...
          CVideoDbUrl itemUrl = videoUrl;
          string path = StringUtils::Format("%ld/", m_pDS->fv(1).get_asInt());
          itemUrl.AppendPath(path);
          pItem->SetPath(itemUrl.ToString());
...
bool CVideoDatabase::GetWritersNav(const string& strBaseDir, CFileItemList& items, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  return GetPeopleNav(strBaseDir, items, "writer", idContent, filter, countOnly);
}

bool CVideoDatabase::GetDirectorsNav(const string& strBaseDir, CFileItemList& items, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  return GetPeopleNav(strBaseDir, items, "director", idContent, filter, countOnly);
}

bool CVideoDatabase::GetActorsNav(const string& strBaseDir, CFileItemList& items, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
...
bool CVideoDatabase::GetPeopleNav(const string& strBaseDir, CFileItemList& items, const string &type, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  if (NULL == m_pDB.get()) return false;
  if (NULL == m_pDS.get()) return false;
...
bool CVideoDatabase::GetYearsNav(const string& strBaseDir, CFileItemList& items, int idContent /* = -1 */, const Filter &filter /* = Filter() */)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    if (CProfilesManager::Get().GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      map<int, pair<string,int> > mapYears;
      map<int, pair<string,int> >::iterator it;
      while (!m_pDS->eof())
      {
        int lYear = 0;
        ...
        if (it == mapYears.end())
        {
          ...
          if (g_passwordManager.IsDatabasePathUnlocked(string(m_pDS->fv("path.strPath").get_asString()),*CMediaSourceSettings::Get().GetSources("video")))
          {
            string year = StringUtils::Format("%d", lYear);
            if (idContent == VIDEODB_CONTENT_MOVIES || idContent == VIDEODB_CONTENT_MUSICVIDEOS)
              mapYears.insert(pair<int, pair<string,int> >(lYear, pair<string,int>(year,m_pDS->fv(2).get_asInt())));
            else
              mapYears.insert(pair<int, pair<string,int> >(lYear, pair<string,int>(year,0)));
          }
        }
        m_pDS->next();
      }
    ...
    else
    {
      while (!m_pDS->eof())
      {
        ...
        CVideoDbUrl itemUrl = videoUrl;
        string path = StringUtils::Format("%ld/", lYear);
        itemUrl.AppendPath(path);
        pItem->SetPath(itemUrl.ToString());
...
bool CVideoDatabase::GetStackedTvShowList(int idShow, string& strIn) const
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    if (idShow == -1)
      return false;
    string strSQL = PrepareSQL("select idShow from tvshow where c00 like (select c00 from tvshow where idShow=%i) order by idShow", idShow);
...
bool CVideoDatabase::GetSeasonsNav(const string& strBaseDir, CFileItemList& items, int idActor, int idDirector, int idGenre, int idYear, int idShow, bool getLinkedMovies /* = true */)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string strIn = PrepareSQL("= %i", idShow);
    GetStackedTvShowList(idShow, strIn);

    string strSQL = PrepareSQL("SELECT episodeview.c%02d, "
                                          "path.strPath, "
    ...
    string showTitle = m_pDS->fv(2).get_asString();
    string showPlot = m_pDS->fv(3).get_asString();
    string showPremiered = m_pDS->fv(4).get_asString();
    string showStudio = m_pDS->fv(6).get_asString();
    string showMPAARating = m_pDS->fv(7).get_asString();
    ...
    if (CProfilesManager::Get().GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      ...
      for (it=mapSeasons.begin();it != mapSeasons.end();++it)
      {
        ...
        CVideoDbUrl itemUrl = videoUrl;
        string strDir = StringUtils::Format("%ld/", it->first);
    ...
    else
    {
      while (!m_pDS->eof())
      {
        ...
        CVideoDbUrl itemUrl = videoUrl;
        string strDir = StringUtils::Format("%ld/", iSeason);
        itemUrl.AppendPath(strDir);
        pItem->SetPath(itemUrl.ToString());
...
bool CVideoDatabase::GetSortedVideos(MediaType mediaType, const string& strBaseDir, const SortDescription &sortDescription, CFileItemList& items, const Filter &filter /* = Filter() */)
{
  if (NULL == m_pDB.get() || NULL == m_pDS.get())
    return false;
...
bool CVideoDatabase::GetItems(const string &strBaseDir, CFileItemList &items, const Filter &filter /* = Filter() */, const SortDescription &sortDescription /* = SortDescription() */)
{
  CVideoDbUrl videoUrl;
  if (!videoUrl.FromString(strBaseDir))
    return false;
...
bool CVideoDatabase::GetItems(const string &strBaseDir, const string &mediaType, const string &itemType, CFileItemList &items, const Filter &filter /* = Filter() */, const SortDescription &sortDescription /* = SortDescription() */)
{
...
bool CVideoDatabase::GetItems(const string &strBaseDir, VIDEODB_CONTENT_TYPE mediaType, const string &itemType, CFileItemList &items, const Filter &filter /* = Filter() */, const SortDescription &sortDescription /* = SortDescription() */)
{
...
string CVideoDatabase::GetItemById(const string &itemType, int id)
{
...
bool CVideoDatabase::GetMoviesNav(const string& strBaseDir, CFileItemList& items,
...
bool CVideoDatabase::GetMoviesByWhere(const string& strBaseDir, const Filter &filter, CFileItemList& items, const SortDescription &sortDescription /* = SortDescription() */)
{
  try
  {
    movieTime = 0;
    ...
    string strSQL = "select %s from movieview ";
    string strSQLExtra;
    ...
    for (DatabaseResults::const_iterator it = results.begin(); it != results.end(); it++)
    {
          ...
          g_passwordManager.IsDatabasePathUnlocked(movie.m_strPath, *CMediaSourceSettings::Get().GetSources("video")))
      {
        ...
        CVideoDbUrl itemUrl = videoUrl;
        string path = StringUtils::Format("%ld", movie.m_iDbId);
        itemUrl.AppendPath(path);
        pItem->SetPath(itemUrl.ToString());
...
bool CVideoDatabase::GetTvShowsNav(const string& strBaseDir, CFileItemList& items,
...
bool CVideoDatabase::GetTvShowsByWhere(const string& strBaseDir, const Filter &filter, CFileItemList& items, const SortDescription &sortDescription /* = SortDescription() */)
{
  try
  {
    movieTime = 0;
    ...
    string strSQL = "SELECT %s FROM tvshowview ";
    CVideoDbUrl videoUrl;
    string strSQLExtra;
    Filter extFilter = filter;
    SortDescription sorting = sortDescription;
    ...
    for (DatabaseResults::const_iterator it = results.begin(); it != results.end(); it++)
    {
          ...
          (!g_advancedSettings.m_bVideoLibraryHideEmptySeries || movie.m_iEpisode > 0))
      {
        ...
        CVideoDbUrl itemUrl = videoUrl;
        string path = StringUtils::Format("%ld/", record->at(0).get_asInt());
        itemUrl.AppendPath(path);
        pItem->SetPath(itemUrl.ToString());
  ...
  switch (type)
  {
    case VIDEODB_CONTENT_TVSHOWS:
    {
      ...
      while (i < items.Size())
      {
        ...
        string strTitle = pItem->GetVideoInfoTag()->m_strTitle;
        string strFanArt = pItem->GetArt("fanart");
    ...
    case VIDEODB_CONTENT_EPISODES:
    {
      ...
      {
        CFileItemPtr pItem = items.Get(i);
        string strPath = pItem->GetVideoInfoTag()->m_strPath;
        ...
        string strFileNameAndPath = pItem->GetVideoInfoTag()->m_strFileNameAndPath;
        ...
        while (j < items.Size())
        {
          ...
          string jFileNameAndPath = jTag->m_strFileNameAndPath;
...
bool CVideoDatabase::GetEpisodesNav(const string& strBaseDir, CFileItemList& items, int idGenre, int idYear, int idActor, int idDirector, int idShow, int idSeason, const SortDescription &sortDescription /* = SortDescription() */)
{
  CVideoDbUrl videoUrl;
  if (!videoUrl.FromString(strBaseDir))
    return false;
...
bool CVideoDatabase::GetEpisodesByWhere(const string& strBaseDir, const Filter &filter, CFileItemList& items, bool appendFullShowPath /* = true */, const SortDescription &sortDescription /* = SortDescription() */)
{
  try
  {
    movieTime = 0;
    ...
    string strSQL = "select %s from episodeview ";
    CVideoDbUrl videoUrl;
    string strSQLExtra;
    Filter extFilter = filter;
    SortDescription sorting = sortDescription;
...
bool CVideoDatabase::GetMusicVideosNav(const string& strBaseDir, CFileItemList& items, int idGenre, int idYear, int idArtist, int idDirector, int idStudio, int idAlbum, int idTag /* = -1 */, const SortDescription &sortDescription /* = SortDescription() */)
{
  CVideoDbUrl videoUrl;
  if (!videoUrl.FromString(strBaseDir))
    return false;
...
bool CVideoDatabase::GetRecentlyAddedMoviesNav(const string& strBaseDir, CFileItemList& items, unsigned int limit)
{
  Filter filter;
...
bool CVideoDatabase::GetRecentlyAddedEpisodesNav(const string& strBaseDir, CFileItemList& items, unsigned int limit)
{
  Filter filter;
...
bool CVideoDatabase::GetRecentlyAddedMusicVideosNav(const string& strBaseDir, CFileItemList& items, unsigned int limit)
{
  Filter filter;
...
int CVideoDatabase::GetTvShowForEpisode(int idEpisode)
{
  try
  {
    ...
    string strSQL=PrepareSQL("select idShow from episode where idEpisode=%i", idEpisode);
    m_pDS2->query( strSQL.c_str() );
...
int CVideoDatabase::GetSeasonForEpisode(int idEpisode)
{
  ...
  string id = GetSingleValue("episode", column, PrepareSQL("idEpisode=%i", idEpisode));
  if (id.empty())
    return -1;
...
int CVideoDatabase::GetMusicVideoCount(const string& strWhere)
{
  try
  {
    ...
    string strSQL = StringUtils::Format("select count(1) as nummovies from musicvideoview where %s",strWhere.c_str());
    m_pDS->query( strSQL.c_str() );
...
ScraperPtr CVideoDatabase::GetScraperForPath( const string& strPath )
{
  SScanSettings settings;
  return GetScraperForPath(strPath, settings);
}

ScraperPtr CVideoDatabase::GetScraperForPath(const string& strPath, SScanSettings& settings)
{
  bool dummy;
  return GetScraperForPath(strPath, settings, dummy);
}

ScraperPtr CVideoDatabase::GetScraperForPath(const string& strPath, SScanSettings& settings, bool& foundDirectly)
{
  foundDirectly = false;
  try
  {
    ...
    string strPath1 = URIUtils::GetDirectory(strPath2);
    int idPath = GetPathId(strPath1);

    if (idPath > -1)
    {
      string strSQL=PrepareSQL("select path.strContent,path.strScraper,path.scanRecursive,path.useFolderNames,path.strSettings,path.noUpdate,path.exclude from path where path.idPath=%i",idPath);
      m_pDS->query( strSQL.c_str() );
    }
    ...
    { // path is stored in db
      ...
      string strcontent = m_pDS->fv("path.strContent").get_asString();
      ...
      string scraperID = m_pDS->fv("path.strScraper").get_asString();
    ...
    { // this path is not saved in db
      ...
      while (URIUtils::GetParentPath(strPath1, strParent))
      {
        ...
        string strSQL=PrepareSQL("select path.strContent,path.strScraper,path.scanRecursive,path.useFolderNames,path.strSettings,path.noUpdate, path.exclude from path where strPath='%s'",strParent.c_str());
        m_pDS->query(strSQL.c_str());
...
string CVideoDatabase::GetContentForPath(const string& strPath)
{
  SScanSettings settings;
  bool foundDirectly = false;
  ...
  if (scraper)
  {
    ...
    { // check for episodes or seasons.  Assumptions are:
      ...
      string sql = PrepareSQL("select count(1) from episodeview where strPath = '%s' limit 1", strPath.c_str());
      m_pDS->query( sql.c_str() );
...
void CVideoDatabase::GetMovieGenresByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
  ...
  try
  {
    ...
    {
      if (CProfilesManager::Get().GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
        if (!g_passwordManager.IsDatabasePathUnlocked(string(m_pDS->fv("path.strPath").get_asString()),
      ...
      string strDir = StringUtils::Format("%ld/", m_pDS->fv("genre.idGenre").get_asInt());
...
void CVideoDatabase::GetMovieCountriesByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      string strDir = StringUtils::Format("%ld/", m_pDS->fv("country.idCountry").get_asInt());
      pItem->SetPath("videodb://movies/genres/"+ strDir);
      pItem->m_bIsFolder=true;
      items.Add(pItem);
      m_pDS->next();
...
void CVideoDatabase::GetTvShowGenresByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
...
void CVideoDatabase::GetMovieActorsByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      string strDir = StringUtils::Format("%ld/", m_pDS->fv("actors.idActor").get_asInt());
...
void CVideoDatabase::GetTvShowsActorsByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
...
void CVideoDatabase::GetMusicVideoArtistsByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
...
void CVideoDatabase::GetMusicVideoGenresByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
...
void CVideoDatabase::GetMusicVideoAlbumsByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      CFileItemPtr pItem(new CFileItem(m_pDS->fv(0).get_asString()));
      string strDir = StringUtils::Format("%ld", m_pDS->fv(1).get_asInt());
...
void CVideoDatabase::GetMusicVideosByAlbum(const string& strSearch, CFileItemList& items)
{
  string strSQL;
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      string strDir = StringUtils::Format("3/2/%ld",m_pDS->fv("musicvideo.idMVideo").get_asInt());
...
bool CVideoDatabase::GetMusicVideosByWhere(const string &baseDir, const Filter &filter, CFileItemList &items, bool checkLocks /*= true*/, const SortDescription &sortDescription /* = SortDescription() */)
{
  try
  {
    movieTime = 0;
...
unsigned int CVideoDatabase::GetMusicVideoIDs(const string& strWhere, vector<pair<int,int> > &songIDs)
{
  try
  {
    if (NULL == m_pDB.get()) return 0;
    ...
    string strSQL = "select distinct idMVideo from musicvideoview " + strWhere;
...
bool CVideoDatabase::GetRandomMusicVideo(CFileItem* item, int& idSong, const string& strWhere)
{
  try
  {
    idSong = -1;
    ...
    string strSQL = StringUtils::Format("select * from musicvideoview where %s", strWhere.c_str());
    ...
    *item->GetVideoInfoTag() = GetDetailsForMusicVideo(m_pDS);
    string path = StringUtils::Format("videodb://musicvideos/titles/%ld",item->GetVideoInfoTag()->m_iDbId);
    item->SetPath(path);
...
int CVideoDatabase::GetMatchingMusicVideo(const string& strArtist, const string& strAlbum, const string& strTitle)
{
  try
  {
    if (NULL == m_pDB.get()) return -1;
...
void CVideoDatabase::GetMoviesByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
...
void CVideoDatabase::GetTvShowsByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      CFileItemPtr pItem(new CFileItem(m_pDS->fv(1).get_asString()));
      string strDir = StringUtils::Format("tvshows/titles/%ld/", m_pDS->fv("tvshow.idShow").get_asInt());
...
void CVideoDatabase::GetEpisodesByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      string path = StringUtils::Format("videodb://tvshows/titles/%ld/%ld/%ld",m_pDS->fv("episode.idShow").get_asInt(),m_pDS->fv(2).get_asInt(),m_pDS->fv(0).get_asInt());
      pItem->SetPath(path);
      pItem->m_bIsFolder=false;
      items.Add(pItem);
      m_pDS->next();
...
void CVideoDatabase::GetMusicVideosByName(const string& strSearch, CFileItemList& items)
{
...
void CVideoDatabase::GetEpisodesByPlot(const string& strSearch, CFileItemList& items)
{
// Alternative searching - not quite as fast though due to
// retrieving all information
//  Filter filter;
...
void CVideoDatabase::GetMoviesByPlot(const string& strSearch, CFileItemList& items)
{
  string strSQL;
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      CFileItemPtr pItem(new CFileItem(m_pDS->fv(1).get_asString()));
      string path = StringUtils::Format("videodb://movies/titles/%ld", m_pDS->fv(0).get_asInt());
      pItem->SetPath(path);
      pItem->m_bIsFolder=false;
...
void CVideoDatabase::GetMovieDirectorsByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      string strDir = StringUtils::Format("%ld/", m_pDS->fv("directorlinkmovie.idDirector").get_asInt());
      CFileItemPtr pItem(new CFileItem(m_pDS->fv("actors.strActor").get_asString()));
...
void CVideoDatabase::GetTvShowsDirectorsByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      string strDir = StringUtils::Format("%ld/", m_pDS->fv("directorlinktvshow.idDirector").get_asInt());
      CFileItemPtr pItem(new CFileItem(m_pDS->fv("actors.strActor").get_asString()));
...
void CVideoDatabase::GetMusicVideoDirectorsByName(const string& strSearch, CFileItemList& items)
{
  string strSQL;
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      string strDir = StringUtils::Format("%ld/", m_pDS->fv("directorlinkmusicvideo.idDirector").get_asInt());
      CFileItemPtr pItem(new CFileItem(m_pDS->fv("actors.strActor").get_asString()));
...
void CVideoDatabase::CleanDatabase(CGUIDialogProgressBarHandle* handle, const set<int>* paths, bool showProgress)
{
  ...
  try
  {
    ...
    // find all the files
    string sql = "SELECT files.idFile, files.strFileName, path.strPath FROM files, path WHERE files.idPath = path.idPath";
    if (paths)
    {
      if (paths->empty())
      {
...
void CVideoDatabase::DumpToDummyFiles(const string &path)
{
  // get all tvshows
  CFileItemList items;
  GetTvShowsByWhere("videodb://tvshows/titles/", "", items);
  string showPath = URIUtils::AddFileToFolder(path, "shows");
  CDirectory::Create(showPath);
  for (int i = 0; i < items.Size(); i++)
  {
    // create a folder in this directory
    string showName = CUtil::MakeLegalFileName(items[i]->GetVideoInfoTag()->m_strShowTitle);
    string TVFolder = URIUtils::AddFileToFolder(showPath, showName);
    ...
    { // right - grab the episodes and dump them as well
      ...
      for (int i = 0; i < episodes.Size(); i++)
      {
        ...
        string episode = StringUtils::Format("%s.s%02de%02d.avi", showName.c_str(), tag->m_iSeason, tag->m_iEpisode);
        // and make a file
        string episodePath = URIUtils::AddFileToFolder(TVFolder, episode);
        CFile file;
  ...
  string moviePath = URIUtils::AddFileToFolder(path, "movies");
  CDirectory::Create(moviePath);
  for (int i = 0; i < items.Size(); i++)
  {
    CVideoInfoTag *tag = items[i]->GetVideoInfoTag();
    string movie = StringUtils::Format("%s.avi", tag->m_strTitle.c_str());
    CFile file;
...
void CVideoDatabase::ExportToXML(const string &path, bool singleFiles /* = false */, bool images /* = false */, bool actorThumbs /* false */, bool overwrite /*=false*/)
{
  CGUIDialogProgress *progress=NULL;
  try
  {
    ...
    string exportRoot = URIUtils::AddFileToFolder(path, "xbmc_videodb_" + CDateTime::GetCurrentDateTime().GetAsDBDate());
    string xmlFile = URIUtils::AddFileToFolder(exportRoot, "videodb.xml");
    string actorsDir = URIUtils::AddFileToFolder(exportRoot, "actors");
    string moviesDir = URIUtils::AddFileToFolder(exportRoot, "movies");
    string musicvideosDir = URIUtils::AddFileToFolder(exportRoot, "musicvideos");
    string tvshowsDir = URIUtils::AddFileToFolder(exportRoot, "tvshows");
    if (!singleFiles)
    {
      images = true;
      overwrite = false;
    ...
    progress = (CGUIDialogProgress *)g_windowManager.GetWindow(WINDOW_DIALOG_PROGRESS);
    // find all movies
    string sql = "select * from movieview";
    ...
    while (!m_pDS->eof())
    {
      ...
      if (singleFiles && CUtil::SupportsWriteFileOperations(movie.m_strFileNameAndPath))
      {
        ...
        {
          string nfoFile(URIUtils::ReplaceExtension(item.GetTBNFile(), ".nfo"));
      ...
      if (images && !bSkip)
      {
        if (!singleFiles)
        {
          string strFileName(movie.m_strTitle);
          if (movie.m_iYear > 0)
        ...
        for (map<string, string>::const_iterator i = artwork.begin(); i != artwork.end(); ++i)
        {
          string savedThumb = item.GetLocalArt(i->first, false);
    ...
    while (!m_pDS->eof())
    {
      ...
      if (images && !bSkip)
      {
        if (!singleFiles)
        {
          string strFileName(StringUtils::Join(movie.m_artist, g_advancedSettings.m_videoItemSeparator) + "." + movie.m_strTitle);
          if (movie.m_iYear > 0)
            strFileName += StringUtils::Format("_%i", movie.m_iYear);
          item.SetPath(GetSafeFile(moviesDir, strFileName) + ".avi");
        }
    ...
    while (!m_pDS->eof())
    {
      ...
      if (singleFiles && CUtil::SupportsWriteFileOperations(tvshow.m_strPath))
      {
        ...
        {
          string nfoFile = URIUtils::AddFileToFolder(tvshow.m_strPath, "tvshow.nfo");
      ...
      if (images && !bSkip)
      {
        ...
        for (map<string, string>::const_iterator i = artwork.begin(); i != artwork.end(); ++i)
        {
          string savedThumb = item.GetLocalArt(i->first, true);
          CTextureCache::Get().Export(i->second, savedThumb, overwrite);
        }
        ...
        for (map<int, map<string, string> >::const_iterator i = seasonArt.begin(); i != seasonArt.end(); ++i)
        {
          ...
          for (map<string, string>::const_iterator j = i->second.begin(); j != i->second.end(); j++)
          {
            string savedThumb(item.GetLocalArt(seasonThumb + "-" + j->first, true));
            if (!i->second.empty())
      ...
      while (!pDS->eof())
      {
        ...
        if (images && !bSkip)
        {
          if (!singleFiles)
          {
            string epName = StringUtils::Format("s%02ie%02i.avi", episode.m_iSeason, episode.m_iEpisode);
...
void CVideoDatabase::ExportActorThumbs(const string &strDir, const CVideoInfoTag &tag, bool singleFiles, bool overwrite /*=false*/)
{
  ...
  for (CVideoInfoTag::iCast iter = tag.m_cast.begin();iter != tag.m_cast.end();++iter)
  {
    ...
    if (!iter->thumb.empty())
    {
      string thumbFile(GetSafeFile(strPath, iter->strName));
      CTextureCache::Get().Export(iter->thumb, thumbFile, overwrite);
    }
  }
}

bool CVideoDatabase::ExportSkipEntry(const string &nfoFile)
{
  string strParent;
...
void CVideoDatabase::ImportFromXML(const string &path)
{
  CGUIDialogProgress *progress=NULL;
  try
  {
    ...
    string actorsDir(URIUtils::AddFileToFolder(path, "actors"));
    string moviesDir(URIUtils::AddFileToFolder(path, "movies"));
    string musicvideosDir(URIUtils::AddFileToFolder(path, "musicvideos"));
    string tvshowsDir(URIUtils::AddFileToFolder(path, "tvshows"));
    ...
    while (movie)
    {
      ...
      if (strnicmp(movie->Value(), "movie", 5) == 0)
      {
        ...
        string filename = info.m_strTitle;
        if (info.m_iYear > 0)
      ...
      else if (strnicmp(movie->Value(), "musicvideo", 10) == 0)
      {
        ...
        CFileItem item(info);
        bool useFolders = info.m_basePath.empty() ? LookupByFolders(item.GetPath()) : false;
        string filename = StringUtils::Join(info.m_artist, g_advancedSettings.m_videoItemSeparator) + "." + info.m_strTitle;
        if (info.m_iYear > 0)
          filename += StringUtils::Format("_%i", info.m_iYear);
        CFileItem artItem(item);
      ...
      else if (strnicmp(movie->Value(), "tvshow", 6) == 0)
      {
        ...
        string artPath(GetSafeFile(tvshowsDir, info.m_strTitle));
        ...
        while (episode)
        {
          ...
          info.Load(episode);
          CFileItem item(info);
          string filename = StringUtils::Format("s%02ie%02i.avi", info.m_iSeason, info.m_iEpisode);
          CFileItem artItem(item);
...
void CVideoDatabase::ConstructPath(string& strDest, const string& strPath, const string& strFileName)
{
...
void CVideoDatabase::SplitPath(const string& strFileNameAndPath, string& strPath, string& strFileName)
{
...
void CVideoDatabase::InvalidatePathHash(const string& strPath)
{
  SScanSettings settings;
  bool foundDirectly;
...
string CVideoDatabase::GetSafeFile(const string &dir, const string &name) const
{
  string safeThumb(name);
...
bool CVideoDatabase::GetItemsForPath(const string &content, const string &strPath, CFileItemList &items)
{
  string path(strPath);
