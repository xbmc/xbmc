...
void CVideoDatabase::UpdateTables(int iVersion)
{
  ...
  { // update old art URLs
    ...
    std::vector<std::pair<int, std::string> > art;
    while (!m_pDS->eof())
    {
      art.push_back(std::make_pair(m_pDS->fv(0).get_asInt(), CURL(m_pDS->fv(1).get_asString()).Get()));
      m_pDS->next();
    }
    m_pDS->close();
    for (std::vector<std::pair<int, std::string>>::iterator i = art.begin(); i != art.end(); ++i)
  ...
  { // update URL encoded paths
    ...
    while (!m_pDS->eof())
    {
      files.push_back(std::make_pair(m_pDS->fv(0).get_asInt(), m_pDS->fv(1).get_asString()));
      m_pDS->next();
    }
    m_pDS->close();

    for (std::vector<std::pair<int, std::string> >::iterator i = files.begin(); i != files.end(); ++i)
    {
  ...
  { // Update thumb to poster or banner as applicable
    ...
    if (db.Open())
    {
      ...
      m_pDS->close();
      for (std::vector<CArtItem>::iterator i = art.begin(); i != art.end(); ++i)
      {
  ...
  { // update the runtime columns
    ...
    tables.push_back(std::make_pair("movie", VIDEODB_ID_RUNTIME));
    tables.push_back(std::make_pair("episode", VIDEODB_ID_EPISODE_RUNTIME));
    tables.push_back(std::make_pair("mvideo", VIDEODB_ID_MUSICVIDEO_RUNTIME));
    for (std::vector<std::pair<std::string, int> >::iterator i = tables.begin(); i != tables.end(); ++i)
    {
      ...
      while (!m_pDS->eof())
      {
        ...
        if (duration)
          videos.push_back(std::make_pair(m_pDS->fv(0).get_asInt(), duration));
        m_pDS->next();
      }
      m_pDS->close();
      for (std::vector<std::pair<int, int> >::iterator j = videos.begin(); j != videos.end(); ++j)
