...
int CVideoDatabase::AddActor(const CStdString& name, const CStdString& thumbURLs, const CStdString &thumb)
{
  try
  {
    ...
    if (NULL == m_pDS.get()) return -1;
    int idActor = -1;
    CStdString strSQL=PrepareSQL("select actor_id from actor where name like '%s'", name.c_str());
    m_pDS->query(strSQL.c_str());
    if (m_pDS->num_rows() == 0)
    {
      m_pDS->close();
...
//********************************************************************************************************************************
void CVideoDatabase::GetMoviesByActor(const CStdString& name, CFileItemList& items)
{
  Filter filter;
  filter.join  = "LEFT JOIN actor_link ON actor_link.media_id=movieview.idMovie AND actor_link.media_type='movie' "
                 "LEFT JOIN actor a ON a.actor_id=actor_link.actor_id "
                 "LEFT JOIN director_link ON director_link.media_id=movieview.idMovie AND director_link.media_type='movie' "
...
void CVideoDatabase::GetTvShowsByActor(const CStdString& name, CFileItemList& items)
{
  Filter filter;
  filter.join  = "LEFT JOIN actor_link ON actor_link.media_id=tvshowview.idShow AND actor_link.media_type='tvshow' "
                 "LEFT JOIN actor a ON a.actor_id=actor_link.actor_id "
                 "LEFT JOIN director_link ON director_link.media_id=tvshowview.idShow AND director_link.media_type='tvshow' "
                 "LEFT JOIN actor d ON d.actor_id=director_link.actor_id";
  filter.where = PrepareSQL("a.name='%s' OR d.name='%s'", name.c_str(), name.c_str());
...
void CVideoDatabase::GetEpisodesByActor(const CStdString& name, CFileItemList& items)
{
  Filter filter;
  filter.join  = "LEFT JOIN actor_link ON actor_link.media_id=episodeview.idEpisode AND actor_link.media_type='episode' "
                 "LEFT JOIN actor a ON a.actor_id=actor_link.actor_id "
                 "LEFT JOIN director_link ON director_link.media_id=episodeview.idEpisode AND director_link.media_type='episode' "
                 "LEFT JOIN actor d ON d.actor_id=director_link.actor_id";
  filter.where = PrepareSQL("a.name='%s' OR d.name='%s'", name.c_str(), name.c_str());
...
void CVideoDatabase::GetMusicVideosByArtist(const CStdString& strArtist, CFileItemList& items)
{
  try
  {
    ...
    if (strArtist.empty())  // TODO: SMARTPLAYLISTS what is this here for???
      strSQL=PrepareSQL("select distinct * from musicvideoview join actor_link on actor_link.media_id=musicvideoview.idMVideo AND actor_link.media_type='musicvideo' join actor on actor.actor_id=actor_link.actor_id");
    else
      strSQL=PrepareSQL("select * from musicvideoview join actor_link on actor_link.media_id=musicvideoview.idMVideo AND actor_link.media_type='musicvideo' join actor on actor.actor_id=actor_link.actor_id where actor.name='%s'", strArtist.c_str());
    m_pDS->query( strSQL.c_str() );
...
int CVideoDatabase::UpdateDetailsForMovie(int idMovie, const CVideoInfoTag& details, const std::map<std::string, std::string> &artwork, const std::set<std::string> &updatedDetails)
{
  ...
  try
  {
    ...
    if (updatedDetails.find("tag") != updatedDetails.end())
      UpdateLinksToItem(idMovie, "movie", "tag", details.m_tags);
    ...
    if (updatedDetails.find("writer") != updatedDetails.end())
      UpdateActorLinksToItem(idMovie, "movie", "writer", details.m_writingCredits);
...
CVideoInfoTag CVideoDatabase::GetDetailsForEpisode(const dbiplus::sql_record* const record, bool getDetails /* = false */)
{
  ...
  if (getDetails)
  {
    GetCast(details.m_iDbId, "episode", details.m_cast);
    GetCast(details.m_iIdShow, "tvshow", details.m_cast);
...
bool CVideoDatabase::GetNavCommon(const CStdString& strBaseDir, CFileItemList& items, const char *type, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    if (CProfilesManager::Get().GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      ...
      extFilter.AppendJoin(PrepareSQL("JOIN %sview ON %s_link.media_id = %sview.%s AND %s_link.media_type='%s'", view.c_str(), type, view.c_str(), view_id.c_str(), type, media_type.c_str()));
      extFilter.AppendJoin(PrepareSQL("JOIN files ON files.idFile = %sview.idFile", view.c_str()));
    ...
    else
    {
      ...
      if (idContent == VIDEODB_CONTENT_MOVIES)
      {
        ...
        extraJoin  = PrepareSQL("JOIN files ON files.idFile = %sview.idFile", view.c_str());
      }
      ...
      extFilter.AppendField(extraField);
      extFilter.AppendJoin(PrepareSQL("JOIN %s_link ON %s.%s_id = %s_link.%s_id", type, type, type, type, type));
      extFilter.AppendJoin(PrepareSQL("JOIN %sview ON %s_link.media_id = %sview.%s AND %s_link.media_type='%s'",
...
bool CVideoDatabase::GetMusicVideoAlbumsNav(const CStdString& strBaseDir, CFileItemList& items, int idArtist /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  try
  {
    ...
    Filter extFilter = filter;
    extFilter.fields = PrepareSQL("musicvideoview.c%02d, musicvideoview.idMVideo, actor.name", VIDEODB_ID_MUSICVIDEO_ALBUM);
    extFilter.AppendJoin(PrepareSQL("JOIN actor_link ON actor_link.media_id=musicvideoview.idMVideo AND actor_link.media_type='musicvideo'"));
...
bool CVideoDatabase::GetPeopleNav(const CStdString& strBaseDir, CFileItemList& items, const char *type, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  if (NULL == m_pDB.get()) return false;
  if (NULL == m_pDS.get()) return false;
  ...
  try
  {
    ...
    if (CProfilesManager::Get().GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      ...
      for (it=mapActors.begin();it != mapActors.end();++it)
      {
        ...
        CVideoDbUrl itemUrl = videoUrl;
        CStdString path = StringUtils::Format("%d/", it->first);
        itemUrl.AppendPath(path);
        pItem->SetPath(itemUrl.ToString());
    ...
    else
    {
      while (!m_pDS->eof())
      {
        try
        {
          ...
          CVideoDbUrl itemUrl = videoUrl;
          CStdString path = StringUtils::Format("%d/", m_pDS->fv(0).get_asInt());
          itemUrl.AppendPath(path);
          pItem->SetPath(itemUrl.ToString());
...
void CVideoDatabase::GetMovieGenresByName(const CStdString& strSearch, CFileItemList& items)
{
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      CFileItemPtr pItem(new CFileItem(m_pDS->fv(1).get_asString()));
      CStdString strDir = StringUtils::Format("%d/", m_pDS->fv(0).get_asInt());
...
void CVideoDatabase::GetTvShowGenresByName(const CStdString& strSearch, CFileItemList& items)
{
  ...
  try
  {
    ...
    if (CProfilesManager::Get().GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
      strSQL=PrepareSQL("select genre.genre_id,genre.name,path.strPath from genre,genre_link,tvshow,path,tvshowlinkpath where genre.genre_id=genre_link.genre_id and genre_link.media_id=tvshow.idShow AND genre_link.media_type='tvshow' and path.idPath=tvshowlinkpath.idPath and tvshowlinkpath.idShow=tvshow.idShow and genre.name like '%%%s%%'",strSearch.c_str());
    else
      strSQL=PrepareSQL("select distinct genre.genre_id,genre.name from genre,genre_link where genre.genre_id=genre_link.genre_id AND genre_link.media_type='tvshow' and name like '%%%s%%'", strSearch.c_str());
    m_pDS->query( strSQL.c_str() );
...
void CVideoDatabase::GetTvShowsActorsByName(const CStdString& strSearch, CFileItemList& items)
{
  ...
  try
  {
    ...
    if (CProfilesManager::Get().GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
      strSQL=PrepareSQL("select actor.actor_id,actor.name,path.strPath from actor_link,actor,tvshow,path,tvshowlinkpath where actor.actor_id=actor_link.actor_id and actor_link.media_id=tvshow.idShow and actor_link.media_type='tvshow' and tvshowlinkpath.idPath=tvshow.idShow and tvshowlinkpath.idPath=path.idPath and actor.name like '%%%s%%'",strSearch.c_str());
    else
      strSQL=PrepareSQL("select distinct actor.actor_id,actor.name from actor_link,actor,tvshow where actor.actor_id=actor_link.actor_id and actor_link.media_id=tvshow.idShow and actor_link.media_type='tvshow' and actor.name like '%%%s%%'",strSearch.c_str());
    m_pDS->query( strSQL.c_str() );
...
void CVideoDatabase::GetTvShowsDirectorsByName(const CStdString& strSearch, CFileItemList& items)
{
  ...
  try
  {
    ...
    if (CProfilesManager::Get().GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
      strSQL = PrepareSQL("select distinct director_link.actor_id,actor.name,path.strPath from tvshow,path,actor,director_link,tvshowlinkpath where tvshowlinkpath.idPath=path.idPath and tvshowlinkpath.idShow=tvshow.idShow and director_link.media_id=tvshow.idShow AND director_link.media_type='tvshow' and director_link.actor_id=actor.actor_id and actor.name like '%%%s%%'",strSearch.c_str());
    else
      strSQL = PrepareSQL("select distinct director_link.actor_id,actor.name from tvshow,actor,director_link where director_link.media_id=tvshow.idShow AND director_link.media_type='tvshow' and director_link.actor_id=actor.actor_id and actor.name like '%%%s%%'",strSearch.c_str());
...
void CVideoDatabase::AppendIdFilter(const char* field, const char *table, const char *mediaType, const char *view, const char *viewKey, const CUrlOptions::UrlOptions& options, Filter &filter)
{
  CUrlOptions::UrlOptions::const_iterator option = options.find((std::string)field + "id");
  if (option != options.end())
  {
    filter.AppendJoin(PrepareSQL("JOIN %s_link ON %s_link.media_id=%sview.%s AND %s_link.media_type='%s'", field, field, view, viewKey, field, mediaType));
    filter.AppendWhere(PrepareSQL("%s_link.%s_id = %i", field, table, (int)option->second.asInteger()));
  }
}

void CVideoDatabase::AppendFilter(const char* field, const char *table, const char *mediaType, const char *view, const char *viewKey, const CUrlOptions::UrlOptions& options, Filter &filter)
{
...
bool CVideoDatabase::GetFilter(CDbUrl &videoUrl, Filter &filter, SortDescription &sorting)
{
  ...
  if (type == "movies")
  {
    AppendIdFilter("genre", "genre", "movie", "movie", "idMovie", options, filter);
    AppendFilter("genre", "genre", "movie", "movie", "idMovie", options, filter);

    AppendIdFilter("country", "country", "movie", "movie", "idMovie", options, filter);
    AppendFilter("country", "country", "movie", "movie", "idMovie", options, filter);

    AppendIdFilter("studio", "studio", "movie", "movie", "idMovie", options, filter);
    AppendFilter("studio", "studio", "movie", "movie", "idMovie", options, filter);

    AppendIdFilter("director", "actor", "movie", "movie", "idMovie", options, filter);
    AppendFilter("director", "actor", "movie", "movie", "idMovie", options, filter);

    option = options.find("year");
    if (option != options.end())
      filter.AppendWhere(PrepareSQL("movieview.c%02d = '%i'", VIDEODB_ID_YEAR, (int)option->second.asInteger()));

    AppendIdFilter("actor", "actor", "movie", "movie", "idMovie", options, filter);
    AppendFilter("actor", "actor", "movie", "movie", "idMovie", options, filter);
  ...
  else if (type == "tvshows")
  {
    if (itemType == "tvshows")
    {
      AppendIdFilter("genre", "genre", "tvshow", "tvshow", "idShow", options, filter);
      AppendFilter("genre", "genre", "tvshow", "tvshow", "idShow", options, filter);

      AppendIdFilter("studio", "studio", "tvshow", "tvshow", "idShow", options, filter);
      AppendFilter("studio", "studio", "tvshow", "tvshow", "idShow", options, filter);

      AppendIdFilter("director", "actor", "tvshow", "tvshow", "idShow", options, filter);

      option = options.find("year");
      if (option != options.end())
        filter.AppendWhere(PrepareSQL("tvshowview.c%02d like '%%%i%%'", VIDEODB_ID_TV_PREMIERED, (int)option->second.asInteger()));

      AppendIdFilter("actor", "actor", "tvshow", "tvshow", "idShow", options, filter);
      AppendFilter("actor", "actor", "tvshow", "tvshow", "idShow", options, filter);

      AppendIdFilter("tag", "tag", "tvshow", "tvshow", "idShow", options, filter);
      AppendFilter("tag", "tag", "tvshow", "tvshow", "idShow", options, filter);
    }
    else if (itemType == "seasons")
    {
      ...
      AppendIdFilter("genre", "genre", "tvshow", "season", "idShow", options, filter);

      AppendIdFilter("director", "actor", "tvshow", "season", "idShow", options, filter);

      option = options.find("year");
      if (option != options.end())
        filter.AppendWhere(PrepareSQL("seasonview.premiered like '%%%i%%'", (int)option->second.asInteger()));

      AppendIdFilter("actor", "actor", "tvshow", "season", "idShow", options, filter);
    }
    else if (itemType == "episodes")
    {
      ...
      if (idShow > -1)
      {
        ...
        AppendIdFilter("genre", "genre", "tvshow", "episode", "idShow", options, filter);
        AppendFilter("genre", "genre", "tvshow", "episode", "idShow", options, filter);

        AppendIdFilter("director", "actor", "tvshow", "episode", "idShow", options, filter);
        AppendFilter("director", "actor", "tvshow", "episode", "idShow", options, filter);
        ...
        AppendIdFilter("actor", "actor", "tvshow", "episode", "idShow", options, filter);
        AppendFilter("actor", "actor", "tvshow", "episode", "idShow", options, filter);
      ...
      else
      {
        ...
        AppendIdFilter("director", "actor", "episode", "episode", "idEpisode", options, filter);
        AppendFilter("director", "actor", "episode", "episode", "idEpisode", options, filter);
      }
    }
  }
  else if (type == "musicvideos")
  {
    AppendIdFilter("genre", "genre", "musicvideo", "musicvideo", "idMVideo", options, filter);
    AppendFilter("genre", "genre", "musicvideo", "musicvideo", "idMVideo", options, filter);

    AppendIdFilter("studio", "studio", "musicvideo", "musicvideo", "idMVideo", options, filter);
    AppendFilter("studio", "studio", "musicvideo", "musicvideo", "idMVideo", options, filter);

    AppendIdFilter("director", "actor", "musicvideo", "musicvideo", "idMVideo", options, filter);
    AppendFilter("director", "actor", "musicvideo", "musicvideo", "idMVideo", options, filter);
    ...
    {
      if (itemType != "albums")
        filter.AppendJoin(PrepareSQL("JOIN actor_link ON actor_link.media_id=musicvideoview.idMVideo AND actor_link.media_type='musicvideo'"));
    ...
    AppendIdFilter("tag", "tag", "musicvideo", "musicvideo", "idMVideo", options, filter);
    AppendFilter("tag", "tag", "musicvideo", "musicvideo", "idMVideo", options, filter);
  }
  else
    return false;
