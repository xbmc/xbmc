...
bool CVideoDatabase::GetFileInfo(const std::string& strFilenameAndPath, CVideoInfoTag& details, int idFile /* = -1 */)
{
  try
  {
    ...
    // each FileItem has the "path" and "filename path" that point to the playlist file
    // already set to VideoInfoTag so dont replace it
    const bool isPlaylist = URIUtils::HasExtension(details.m_strFileNameAndPath, ".strm|.m3u|.m3u8");
