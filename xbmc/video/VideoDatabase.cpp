                               ...
                               AllowNonFileNameMatch allowNonFileNameMatch)
{
  try
  {
    ...
    std::string strSQL;
    if (idFile == -1 && strPath != strFilenameAndPath && URIUtils::IsBluray(strFilenameAndPath))
      // see if path is basepath for bluray://
      strSQL = PrepareSQL("SELECT idMovie FROM movie "
                          "WHERE c%02d='%s'",
                          VIDEODB_ID_BASEPATH, strPath.c_str());
    else if (idFile == -1 && strPath != strFilenameAndPath &&
...
int CVideoDatabase::AddMovieVersion(CFileItem& item, int idMovie, const ArtMap& art)
{
  ...
  if (tag.m_iTrack > -1)
    path = URIUtils::GetBlurayPlaylistPath(path, tag.m_iTrack);
...
void CVideoDatabase::ExportToXML(const std::string &path, bool singleFile /* = true */, bool images /* = false */, bool actorThumbs /* false */, bool overwrite /*=false*/)
{
  ...
  try
  {
    ...
    if (!singleFile)
    {
      ...
      // If they are NOT blurays then we need to export them as separate files
      ...
      for (unsigned int fileId :
           fileIdMap | std::views::filter([](const auto& p) { return p.second > 1; }) |
               std::views::keys)
      {
        for (auto range{fileMap.equal_range(fileId)};
             auto& fileInformation :
             std::ranges::subrange(range.first, range.second) | std::views::values)
        {
          multiMovieMap[fileInformation.vvId] = URIUtils::IsBlurayPath(fileInformation.path) ||
  ...
  try
  {
    ...
    while (movie)
    {
      ...
      if (StringUtils::CompareNoCase(movie->Value(), MediaTypeMovie, 5) == 0)
      {
        ...
        if (URIUtils::GetDirectory(lastPath) == URIUtils::GetDirectory(item.GetPath()) &&
