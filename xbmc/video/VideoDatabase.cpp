...
bool CVideoDatabase::GetRandomMusicVideo(CFileItem* item, int& idSong, const CStdString& strWhere)
{
  try
  {
    ...
    *item->GetVideoInfoTag() = GetDetailsForMusicVideo(m_pDS);
    ...
    CStdString strExt = URIUtils::GetExtension(strFileName);
    CStdString path; path.Format("videodb://musicvideos/titles/%ld%s",item->GetVideoInfoTag()->m_iDbId,strExt.c_str());
    item->SetPath(path);
...
void CVideoDatabase::GetMoviesByName(const CStdString& strSearch, CFileItemList& items)
{
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      CStdString strExt = URIUtils::GetExtension(strFileName);
      if (setId <= 0)
        path.Format("videodb://movies/titles/%i%s", movieId, strExt.c_str());
      else
        path.Format("videodb://movies/sets/%i/%i%s", setId, movieId, strExt.c_str());
      pItem->SetPath(path);
      pItem->m_bIsFolder=false;
      items.Add(pItem);
      m_pDS->next();
...
void CVideoDatabase::GetEpisodesByName(const CStdString& strSearch, CFileItemList& items)
{
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      CStdString strFileName = m_pDS->fv("files.strFilename").get_asString();
      CStdString strExt = URIUtils::GetExtension(strFileName);
      CStdString path; path.Format("videodb://tvshows/titles/%ld/%ld/%ld%s",m_pDS->fv("episode.idShow").get_asInt(),m_pDS->fv(2).get_asInt(),m_pDS->fv(0).get_asInt(),strExt.c_str());
      pItem->SetPath(path);
      pItem->m_bIsFolder=false;
      items.Add(pItem);
      m_pDS->next();
...
void CVideoDatabase::GetMoviesByPlot(const CStdString& strSearch, CFileItemList& items)
{
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      CStdString strFileName = m_pDS->fv("files.strFilename").get_asString();
      CStdString strExt = URIUtils::GetExtension(strFileName);
      CStdString path; path.Format("videodb://movies/titles/%ld%s", m_pDS->fv(0).get_asInt(),strExt.c_str());
      pItem->SetPath(path);
      pItem->m_bIsFolder=false;
