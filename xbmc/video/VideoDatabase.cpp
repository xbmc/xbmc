                                       ...
                                       const int oldIdFile)
{
  ...
  {
    BeginTransaction();
    std::string sql = PrepareSQL("UPDATE episode SET idFile=%i, c%02d=%i WHERE idEpisode=%i",
                                 idFile, VIDEODB_ID_EPISODE_ORIGINALIDFILE, oldIdFile, idEpisode);
    m_pDS->exec(sql);
  ...
  try
  {
    ...
    if (m_pDS2->num_rows() > 0)
    {
      ...
      while (!m_pDS2->eof())
      {
        ...
        if (URIUtils::IsProtocol(fullPath, "dvd"))
          fullPath = URIUtils::GetDVDPath(fullPath);
                                   ...
                                   const std::string& strFileName)
{
  if (URIUtils::IsStack(strFileName) || URIUtils::IsInArchive(strFileName) ||
      ...
      URIUtils::IsProtocol(strFileName, "dvd"))
    strDest = strFileName;
  else
...
void CVideoDatabase::SplitPath(const std::string& strFileNameAndPath, std::string& strPath, std::string& strFileName)
{
  if (URIUtils::IsStack(strFileNameAndPath) || StringUtils::StartsWithNoCase(strFileNameAndPath, "rar://") || StringUtils::StartsWithNoCase(strFileNameAndPath, "zip://"))
  {
    ...
    strFileName = strFileNameAndPath;
           ...
           URIUtils::IsProtocol(strFileNameAndPath, "dvd"))
  {
