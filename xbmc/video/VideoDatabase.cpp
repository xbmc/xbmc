...
void CVideoDatabase::DeleteFile(int idFile)
{
  ...
  if (m_pDS->eof())
  {
    ...
    if (path >= 0 && (URIUtils::IsBlurayPath(strPath) || URIUtils::IsDVDPath(strPath)))
    {
...
bool CVideoDatabase::GetStreamDetails(CVideoInfoTag& tag)
{
  ...
  int fileId{-1};
  if (URIUtils::IsBlurayPath(path) || URIUtils::IsDVDPath(path))
...
{
  if (URIUtils::IsBlurayPath(item.GetDynPath()) || URIUtils::IsDVDPath(item.GetDynPath()))
...
CDateTime CVideoDatabase::SetPlayCount(const CFileItem& item, int count, const CDateTime& date)
{
  ...
  else if (URIUtils::IsBlurayPath(item.GetDynPath()) || URIUtils::IsDVDPath(item.GetDynPath()))
...
std::string CVideoDatabase::GetPlotByShowId(const int idShow) const
{
  std::string strSQL;
  ...
  try
  {
    ...
    if (m_pDS2->num_rows() > 0)
    {
      ...
      while (!m_pDS2->eof())
      {
        ...
        // if bluray:// or dvd:// get actual path
        if (URIUtils::IsDVDPath(fullPath))
          fullPath = URIUtils::GetDVDFile(fullPath);
  ...
  try
  {
    ...
    while (!m_pDS->eof())
    {
      ...
      if (!singleFile && CUtil::SupportsWriteFileOperations(movie.m_strFileNameAndPath))
      {
        ...
        else
        {
          ...
          else if (URIUtils::IsDVDPath(item.GetDynPath()))
            nfoFile =
                URIUtils::ReplaceExtension(URIUtils::GetDVDFile(item.GetDynPath()), ".nfo");
    ...
    while (!pDS2->eof())
    {
      ...
      while (!pDS->eof())
      {
        ...
        else if (URIUtils::IsDVDPath(file))
          file = URIUtils::GetDVDFile(file);
        ...
        if (!singleFile && CUtil::SupportsWriteFileOperations(episode.m_strFileNameAndPath))
        {
          ...
          else
          {
            ...
            if (URIUtils::IsBlurayPath(item.GetDynPath()))
              ...
              nfoFile = URIUtils::ReplaceExtension(URIUtils::GetDVDFile(item.GetDynPath()), ".nfo");
