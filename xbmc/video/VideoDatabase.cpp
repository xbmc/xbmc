...
bool CVideoDatabase::GetPaths(std::set<std::string> &paths)
{
  try
  {
    ...
    typedef odb::query<CODBPath> query;
    query query1 = (query::content == "movies" || query::content == "musicvideos" || query::content == "tvshows")
            && !query::path.like("multipath://%")
            && query::noUpdate == false;
    query1 += "ORDER BY " + query::path;

    odb::result<CODBPath> res = m_cdb.getDB()->query<CODBPath>(query1);
    for (CODBPath& path: res)
    {
      paths.insert(path.m_path);
    }

    odb::query<ODBView_TVShow_Path> query2 = odb::query<ODBView_TVShow_Path>::path::content == "tvshows"
                                             && !odb::query<ODBView_TVShow_Path>::path::path.like("multipath://%")
                                             && odb::query<ODBView_TVShow_Path>::path::noUpdate == false;

    query2 += "ORDER BY " + odb::query<ODBView_TVShow_Path>::path::path;

    odb::result<ODBView_TVShow_Path> tvshowPathsRes = m_cdb.getDB()->query<ODBView_TVShow_Path>(query2);
    ...
    odb::query<ODBView_Movie_File_Path> query3 = odb::query<ODBView_Movie_File_Path>::CODBFile::filename != "video_ts.ifo"
                                                 && odb::query<ODBView_Movie_File_Path>::CODBFile::filename != "index.bdmv"
                                                 && odb::query<ODBView_Movie_File_Path>::CODBPath::noUpdate == false;

    query3 += "ORDER BY " + odb::query<ODBView_Movie_File_Path>::CODBPath::path;
    odb::result<ODBView_Movie_File_Path> movieFilesRes = m_cdb.getDB()->query<ODBView_Movie_File_Path>(query3);
...
int CVideoDatabase::AddFile(const std::string& strFileNameAndPath)
{
  ...
  try
  {
    ...
    odb::result<CODBFile> res = m_cdb.getDB()->query<CODBFile>(query::filename == strFileName && query::path->idPath == idPath);
    CODBFile file;
...
int CVideoDatabase::GetTvShowId(const std::string& strPath)
{
  try
  {
    ...
    {
      odb::result<ODBView_TVShow_Path> res = m_cdb.getDB()->query<ODBView_TVShow_Path>(query::path::path == strParent);
      if (res.begin() != res.end())
      {
        int idShow = res.begin()->show->m_idTVShow;
        if (idShow != -1)
          iFound = 2;
      }
      strPath1 = strParent;
...
bool CVideoDatabase::GetSeasonInfo(int idSeason, CVideoInfoTag& details, bool allDetails /* = true */)
{
  ...
  try
  {
    ...
    const int season = objTVShowSeason.season->m_season;
    std::string name = objTVShowSeason.season->m_name;
    ...
    details.m_iDbId = objTVShowSeason.season->m_idSeason;
    details.m_iIdSeason = details.m_iDbId;
    details.m_type = MediaTypeSeason;
    details.m_iUserRating = objTVShowSeason.season->m_userrating;
    details.m_iIdShow = objTVShowSeason.show->m_idTVShow;
    ...
    else
    {
      ...
      for (auto i: res)
      {
        ...
        if (type == CBookmark::EPISODE)
        {
          ...
          {
            odb::result<CODBSeason> resSeason(m_cdb.getDB()->query<CODBSeason>(odb::query<CODBSeason>::idSeason == episode.m_idSeason));
            if (resSeason.begin() != resSeason.end())
            {
              CODBSeason objSeason;
              resSeason.begin().load(objSeason);
              bookmark.seasonNumber = objSeason.m_season;
            }
            
            odb::result<CODBEpisode> resEpisode(m_cdb.getDB()->query<CODBEpisode>(odb::query<CODBEpisode>::idEpisode == episode.m_idEpisode));
            if (resEpisode.begin() != resEpisode.end())
            {
              CODBEpisode objEpisode;
              resEpisode.begin().load(objEpisode);
              bookmark.episodeNumber = objEpisode.m_episode;
            }
          }
        }
...
bool CVideoDatabase::GetBookMarkForEpisode(const CVideoInfoTag& tag, CBookmark& bookmark)
{
  try
  {
    ...
    ODBView_Season_Episodes episode;
    if (m_cdb.getDB()->query_one<ODBView_Season_Episodes>(query::CODBEpisode::idEpisode == tag.m_iDbId, episode))
    {
      odb::result<CODBEpisode> resEpisode(m_cdb.getDB()->query<CODBEpisode>(odb::query<CODBEpisode>::idEpisode == episode.m_idEpisode));
      if (resEpisode.begin() != resEpisode.end())
      {
...
bool CVideoDatabase::GetActorsNav(const std::string& strBaseDir, CFileItemList& items, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  try
  {
    ...
    for (const auto &i : mapItems)
    {
      ...
      if (m_cdb.getDB()->query_one<ODBView_Person_Art>(odb::query<ODBView_Person_Art>::CODBPerson::idPerson == i.first, objArt))
        pItem->GetVideoInfoTag()->m_strPictureURL.ParseString("<thumb>"+objArt.art->m_url+"</thumb>");
      else
        pItem->GetVideoInfoTag()->m_strPictureURL.ParseString("<thumb></thumb>");*/
...
int CVideoDatabase::GetSeasonForEpisode(int idEpisode)
{
  try
  {
    ...
    odb::result<ODBView_Season_Episodes> res(m_cdb.getDB()->query<ODBView_Season_Episodes>(odb::query<ODBView_Season_Episodes>::CODBEpisode::idEpisode == idEpisode));
    if (res.begin() == res.end())
      return -1;
  ...
  try
  {
    ...
    { // this path is not saved in db
      ...
      while (URIUtils::GetParentPath(strPath1, strParent))
      {
        ...
        CONTENT_TYPE content = CONTENT_NONE;
        odb::result<CODBPath> res(m_cdb.getDB()->query<CODBPath>(query::path == strParent));
        if (res.begin() != res.end())
        {
          CODBPath path;
          res.begin().load(path);
          ...
          AddonPtr addon;
          if (content != CONTENT_NONE &&
              CServiceBroker::GetAddonMgr().GetAddon(path.m_scraper, addon))
          {
            scraper = std::dynamic_pointer_cast<CScraper>(addon);
            scraper->SetPathSettings(content, path.m_settings);
            settings.parent_name = path.m_useFolderNames;
