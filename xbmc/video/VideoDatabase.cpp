...
void CVideoDatabase::CreateTables()
{
  ...
  m_pDS->exec("CREATE TABLE streamdetails (idFile integer, iStreamType integer, "
              ...
              "strHdrType text, strAudioChannelMask bigint)");
  ...
  try
  {
    ...
    for (int i=1; i<=details.GetAudioStreamCount(); i++)
    {
      m_pDS->exec(PrepareSQL("INSERT INTO streamdetails "
                             ...
                             "strAudioLanguage, strAudioChannelMask) "
...
void CVideoDatabase::UpdateTables(int iVersion)
{
  ...
  if (iVersion < 136)
  {
    m_pDS->exec(PrepareSQL("ALTER TABLE streamdetails ADD strAudioChannelMask bigint"));
    ...
    for (const auto& mapping : mappings)
      m_pDS->exec(PrepareSQL("UPDATE streamdetails set strAudioChannelMask = %lli "
                             "WHERE iStreamType = 1 AND iAudioChannels = %i",
                             static_cast<int64_t>(mapping.m_mask), mapping.m_channels));
  }
}
