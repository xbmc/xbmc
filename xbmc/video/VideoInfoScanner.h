...
typedef struct SScanSettings
{
  ...
  bool
      parent_name_root; /* use the name of directory where scan started as name for files in that dir */
...
protected:
  ...
  INFO_RET RetrieveInfoForTvShow(CFileItem* pItem,
                                 bool bDirNames,
  ...
  INFO_RET RetrieveInfoForMovie(CFileItem* pItem,
                                bool bDirNames,
                                ADDON::ScraperPtr& scraper,
                                bool useLocal,
                                CScraperUrl* pURL,
                                CGUIDialogProgress* pDlgProgress);
  INFO_RET RetrieveInfoForMusicVideo(CFileItem* pItem,
                                     bool bDirNames,
                                     ADDON::ScraperPtr& scraper,
                                     bool useLocal,
                                     CScraperUrl* pURL,
  ...
  /*! \brief Retrieve a "fast" hash of the given directory (if available)
     ...
     */
  std::string GetFastHash(const std::string& directory,
  ...
  /*! \brief Retrieve a "fast" hash of the given directory recursively (if available)
     ...
     \return the md5 hash of the folder
     */
  std::string GetRecursiveFastHash(const std::string& directory,
                                   const std::vector<std::string>& excludes) const;
