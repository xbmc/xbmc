...
void CVideoDatabase::UpdateTables(int iVersion)
{
  ...
  if (iVersion < 146)
  {
    // Datafix for scrapers that provided the episode duration as streamdetails.iVideoDuration of a
    // single invalid streamdetails record missing all other non-key fields such as width/height.
    // Missing episode durations (episode.c09) will be copied from streamdetails when available
    // and the invalid streamdetails records will be deleted.

    constexpr int LOCAL_VIDEODB_ID_EPISODE_RUNTIME = 9;

    // Indices haven't been created yet; those are needed to speed up the queries below
    ...
    // Temporary table to store the episodes of interest and avoid MySQL/MariaDB/Sqlite incompatible
    // differences in the later UPDATE and DELETE queries
    ...
    // We don't look for height/width = 0 here as the file may have been played and proper streamdetails derived (bluray)
    m_pDS->exec(PrepareSQL(
        "INSERT INTO single_video_streams (idEpisode, idFile, iVideoDuration, iTotalStreams) "
        "SELECT e.idEpisode, e.idFile, MAX(s.iVideoDuration), "
    ...
    m_pDS->exec(PrepareSQL("UPDATE episode "
                           "SET c%02d = CAST((SELECT iVideoDuration FROM single_video_streams AS s "
                           "            WHERE s.idEpisode = episode.idEpisode) AS TEXT) "
    ...
    m_pDS->exec("DROP TABLE single_video_streams");

    // Remove indices
    m_pDS->exec("DROP INDEX ix_episode_file_1 ON episode");
    m_pDS->exec("DROP INDEX id_episode_file_2 ON episode");
    m_pDS->exec("DROP INDEX ix_streamdetails ON streamdetails");
  }
}
