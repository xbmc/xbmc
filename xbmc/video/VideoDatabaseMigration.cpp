...
void CVideoDatabase::UpdateTables(int iVersion)
{
  ...
  if (iVersion < 145)
  {
    ...
    std::string sql{PrepareSQL("UPDATE episode "
                               "SET c%02d = (SELECT MAX(iVideoDuration) FROM streamdetails AS s "
                               "WHERE s.idFile = episode.idFile "
                               " AND s.iStreamType = 0 "
                               " AND s.iVideoDuration > 0) "
                               "WHERE episode.c%02d = 0 "
                               " AND EXISTS (SELECT 1 FROM streamdetails AS s "
                               "              WHERE s.idFile = episode.idFile "
                               "               AND s.iStreamType = 0 "
                               "               AND s.iVideoDuration > 0)",
                               LOCAL_VIDEODB_ID_EPISODE_RUNTIME, LOCAL_VIDEODB_ID_EPISODE_RUNTIME)};
    ...
    sql = PrepareSQL("DELETE FROM streamdetails "
                     ...
                     " AND idFile IN "
                     "  (SELECT idFile FROM "
                     "    (SELECT idFile FROM streamdetails GROUP BY idFile HAVING COUNT(*) = 1"
                     "    ) AS t) "
