...
/*!
 * \brief Measure the content geometry of every library file that does not have one.
 *
 * The work list is recomputed from the database on each run, so the job is resumable. A first
 * pass over a large library is hours of network traffic.
 *
 * Playback and any other video library job suspend it between files, and the file being measured
 * when playback starts is abandoned mid-sample.
 */
class CVideoLibraryContentGeometryJob : public CVideoLibraryProgressJob
{
public:
  /*!
   * \param retryFailed also measure files a previous run tried and could not read. Off for the
   *                    automatic sweeps; the only way to retry a file that has not changed since
   *                    it failed.
   */
