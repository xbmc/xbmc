...
void CWSDiscoveryListenerUDP::Process()
{
  ...
  while (!m_bStop)
  {
    ...
    // Did thread get stopped during timeout
    // Thread stopped, pointer may be invalidated, so cant call
    // WSD_ACT_BYE addcommand currently
    if (m_bStop)
      break;
    //      return;
                                               ...
                                               const std::string extraparameter)
{
  ...
  // how are fmt exception/failures handled with libfmt?
