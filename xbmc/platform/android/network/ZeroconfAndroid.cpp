...
{
  CLog::Log(LOGDEBUG, "ZeroconfAndroid: identifier: {} type: {} name:{} port:{}",
  ...
  {
    //    CLog::Log(LOGDEBUG, "ZeroconfAndroid: key:{}, value:{}", it.first.c_str(),it.second.c_str());
...
bool CZeroconfAndroid::doRemoveService(const std::string& fcr_ident)
{
  ...
  if(it != m_services.end())
  {
    m_manager.unregisterService(it->second.registrationListener);
    m_services.erase(it);
    CLog::Log(LOGDEBUG, "CZeroconfAndroid: Removed service {}", fcr_ident.c_str());
    return true;
  }
  else
    return false;
...
{
  {
    ...
    {
      m_manager.unregisterService(it.second.registrationListener);
      CLog::Log(LOGDEBUG, "CZeroconfAndroid: Removed service {}", it.first.c_str());
    }
    m_services.clear();
  }
}
