...
public:
  ...
  static void _onPictureInPictureModeChanged(JNIEnv* env, jobject context, jboolean isInPipMode);
  static void _onPipPlayPause(JNIEnv* env, jobject context);
  ...
  void enterPip(int num, int den, const std::string& pauseLabel);
  void updatePipParams(bool autoEnter, int num, int den);
  void updatePipActions(bool playing, const std::string& label);
  bool supportsPip();
...
protected:
  ...
  virtual void onPipModeChanged(bool isInPipMode) = 0;
  virtual void onPipPlayPause() = 0;
