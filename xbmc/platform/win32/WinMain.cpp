...
{
  const std::string file = CWIN32Util::GetProfilePath(usePlatformDirectories) + "\\.running";

  constexpr DWORD NO_DESIRED_ACCESS = 0;
  constexpr DWORD NO_SHARING = 0;

  using KODI::PLATFORM::WINDOWS::ToW;
  handle = CreateFile(ToW(file).c_str(), NO_DESIRED_ACCESS, NO_SHARING, nullptr, CREATE_NEW,
...
{
  using KODI::PLATFORM::WINDOWS::ToW;
  handle = CreateMutex(nullptr, FALSE,
                       ToW(std::string(CCompileInfo::GetAppName()) + " Media Center").c_str());
