...
enum KD_TEX_FMT
{
  ...
  KD_TEX_FMT_SDR_RGB5_A1        = 0x1400, // 16bpp, 5 bit per color channel, pt-alpha
  KD_TEX_FMT_SDR_RGBA4          = 0x1500, // 16bpp, 4 bit per channel
  KD_TEX_FMT_SDR_RGB8           = 0x1300, // 24bpp, 8 bit per channel, no alpha (unsuitable for GPUs!)
  ...
  // The decode mode depends on KD_TEX_DEC. When optimized for size, a suitable
  // 32bit/texel format is tried (RGB9_E5, RGBA8), otherwise RGBA16f.
  // SRGB will decode to RGBA8.
  ...
  // The decode mode depends on KD_TEX_DEC. When optimized for size, the
  // 32bit/texel format is tried (RGB9_E5), otherwise RGBA16f.
  // Bitrate varies from 8bpp (4x4 tile) to 0.89bpp (12x12 tile).
...
enum KD_TEX_SWIZ
{
  ...
  KD_TEX_SWIZ_SDF               = 0x600000, // Red channel contains a SDF
  KD_TEX_SWIZ_RGB_SDF           = 0x700000, // RGB8 texture with a SDF packed in the alpha channel
  KD_TEX_SWIZ_MSDF              = 0x800000, // Encoded MSDF in the color channels, Alpha is ignored
...
enum KD_TEX_COL
{
  KD_TEX_COL_UNDEFINED          = 0x0000000,
  KD_TEX_COL_LINEAR             = 0x1000000, // Flag if linear
  KD_TEX_COL_LIMITED            = 0x2000000, // Flag if limited
  KD_TEX_COL_SRGB               = 0x4000000, // SRGB color space
  KD_TEX_COL_BT709              = 0x8000000, // BT709 color space

  KD_TEX_COL_MASK               = 0xF000000,
};

// Texture decode mode
// If a texture is not supported by the hardware, the producer can suggest an
// acceptable mode which might be converted to in software. ASTC hardware
// decoders might offer a related mode setting.
enum KD_TEX_DEC
{
  KD_TEX_DEC_AUTO           = 0x00000000,
  KD_TEX_DEC_SIZE           = 0x10000000, // Might sacrifice quality (i.e. downgrade to RGB9_E5/RGBA8 for ASTC)
  KD_TEX_DEC_QUALITY        = 0x20000000, // Tries to convert unsupported format into an equal or better format (ASTC: RGBA16f)
  KD_TEX_DEC_8bpp           = 0x30000000, // Forces an equivalent 8bbp format (i.e. RGBA8 for ASTC LDR, fallback RGBA16f)
  KD_TEX_DEC_RGB9_E5        = 0x40000000, // Forces RGB9_E5 format (suitable for ASTC LDR/HDR, fallback RGBA16f)
  KD_TEX_DEC_R5G6B5         = 0x50000000, // Forces an conversion to the R5G6B5 format (BC1, ETC1)

  KD_TEX_DEC_MASK           = 0xF0000000,
};
