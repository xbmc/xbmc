...
std::string GetSoundSkinPath()
{
  ...
  {
    CLog::Log(LOGINFO, "Unknown sounds addon '{}'. Setting default sounds.", value.c_str());
    setting->Reset();
  }
...
bool CGUIAudioManager::Load()
{
  ...
  {
    CLog::Log(LOGINFO, "{}, Line {}\n{}", strSoundsXml.c_str(), xmlDoc.ErrorRow(),
              xmlDoc.ErrorDesc());
    return false;
  }
  ...
  if ( strValue != "sounds")
  {
    CLog::Log(LOGINFO, "{} Doesn't contain <sounds>", strSoundsXml.c_str());
    return false;
  }
