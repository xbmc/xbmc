...
namespace
{
// Cap outgoing fade clones so a fast scroll can't pile them up (PR #25565's "fade between two
// textures": the live texture plus one outgoing clone).
constexpr size_t MAX_FADING_TEXTURES = 1;
} // namespace
...
{
  m_texture->SetScalingMethod(scalingMethod);
}

void CGUIImage::SetDiffuseScalingMethod(TEXTURE_SCALING scalingMethod)
{
  m_texture->SetDiffuseScalingMethod(scalingMethod);
}
...
void CGUIImage::SetFileName(const std::string& strFileName, bool setConstant, const bool useCache)
{
  ...
  if (m_crossFadeTime)
  {
    ...
    // scrolling back to the image still fading out: swap it back to live and let the crossfade
    // run in reverse, instead of reloading it and starting a fresh fade
    if (!m_fadingTextures.empty())
    {
      CFadingTexture* cover = m_fadingTextures.back();
      if (cover->m_texture->GetFileName() == strFileName && cover->m_texture->ReadyToRender())
      {
        std::swap(m_texture, cover->m_texture);
        std::swap(m_currentFadeTime, cover->m_fadeTime);
    ...
    { // save the current image
      ...
      // drop the least-visible clone, not the oldest: a transient empty/fallback intermediate
      // fades almost instantly while the real previous image must keep crossfading out
      while (m_fadingTextures.size() > MAX_FADING_TEXTURES)
      {
        size_t least = 0;
        for (size_t i = 1; i < m_fadingTextures.size(); ++i)
          if (m_fadingTextures[i]->m_fadeTime < m_fadingTextures[least]->m_fadeTime)
            least = i;
        delete m_fadingTextures[least];
        m_fadingTextures.erase(m_fadingTextures.begin() + least);
      }
