...
void GUIFontManager::LoadFonts(const std::string& fontSet)
{
  ...
  {
    CLog::Log(LOGERROR, "file {} doesn't start with <fonts>", strPath.c_str());
    return;
  }
  ...
  if (!firstFont.empty())
  {
    ...
    CLog::Log(LOGERROR, "file '{}' doesn't have a valid <fontset>", strPath.c_str());
}
