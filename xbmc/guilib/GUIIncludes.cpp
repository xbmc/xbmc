...
static bool Contains(auto&& container, std::string_view item)
{
  return std::ranges::binary_search(container, item);
}
...
void CGUIIncludes::LoadExpressions(const TiXmlElement *node)
{
  ...
  while (child)
  {
    ...
    if (tagName && child->FirstChild())
      m_expressions.emplace(tagName, "[" + child->FirstChild()->ValueStr() + "]");
...
void CGUIIncludes::LoadConstants(const TiXmlElement *node)
{
  ...
  while (child)
  {
    const char *tagName = child->Attribute("name");
    if (tagName && child->FirstChild())
      m_constants.emplace(tagName, child->FirstChild()->ValueStr());
...
void CGUIIncludes::LoadVariables(const TiXmlElement *node)
{
  ...
  while (child)
  {
    const char *tagName = child->Attribute("name");
    if (tagName && child->FirstChild())
      m_skinvariables.emplace(tagName, *child);
  ...
  while (child)
  {
    ...
    if (tagName && child->FirstChild())
    {
      ...
      if (haveParamTags && !definitionTag)
        ...
        m_includes.emplace(tagName, std::make_pair(*includeBody, std::move(defaultParams)));
    }
...
void CGUIIncludes::ResolveConstants(TiXmlElement *node)
{
  ...
  if (child && child->Type() == TiXmlNode::TINYXML_TEXT &&
      Contains(CONSTANT_NODES, node->ValueStr()))
  {
  ...
  else
  {
    ...
    while (attribute)
    {
      if (Contains(CONSTANT_ATTRIBUTES, attribute->Name()))
...
void CGUIIncludes::ResolveExpressions(TiXmlElement *node)
{
  ...
  TiXmlNode *child = node->FirstChild();
  if (child && child->Type() == TiXmlNode::TINYXML_TEXT &&
      Contains(EXPRESSION_NODES, node->ValueStr()))
  {
