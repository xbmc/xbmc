...
{
  return (RENDER_STEREO_MODE) CSettings::Get().GetInt(CSettings::SETTING_VIDEOSCREEN_STEREOSCOPICMODE);
}
...
void CStereoscopicsManager::SetStereoMode(const RENDER_STEREO_MODE &mode)
{
  ...
  if (applyMode != currentMode && applyMode >= RENDER_STEREO_MODE_OFF)
  {
    if (!g_Windowing.SupportsStereo(applyMode))
      return;
    CSettings::Get().SetInt(CSettings::SETTING_VIDEOSCREEN_STEREOSCOPICMODE, applyMode);
  }
}
...
{
  return (RENDER_STEREO_MODE) CSettings::Get().GetInt(CSettings::SETTING_VIDEOSCREEN_PREFEREDSTEREOSCOPICMODE);
}
...
void CStereoscopicsManager::OnPlaybackStarted(void)
{
  ...
  if (!g_infoManager.EvaluateBool("videoplayer.isstereoscopic"))
  {
    ...
    if (mode != RENDER_STEREO_MODE_OFF && CSettings::Get().GetBool(CSettings::SETTING_VIDEOPLAYER_QUITSTEREOMODEONSTOP))
      SetStereoMode(RENDER_STEREO_MODE_OFF);
    return;
  }
  ...
  if (mode != RENDER_STEREO_MODE_OFF)
  {
    ...
    if (CSettings::Get().GetBool(CSettings::SETTING_VIDEOPLAYER_QUITSTEREOMODEONSTOP) == false)
      return;
...
{
  RENDER_STEREO_MODE mode = GetStereoMode();
  if (CSettings::Get().GetBool(CSettings::SETTING_VIDEOPLAYER_QUITSTEREOMODEONSTOP) && mode != RENDER_STEREO_MODE_OFF)
    SetStereoMode(RENDER_STEREO_MODE_OFF);
