...
bool CGUIFontTTFBase::CacheCharacter(wchar_t letter, uint32_t style, Character *ch)
{
  ...
  {
    CLog::LogF(LOGDEBUG, L"Failed to load glyph %x", letter);
    return false;
  }
  ...
  {
    CLog::LogF(LOGDEBUG, L"Failed to get glyph %x", letter);
    return false;
  }
  if (m_stroker)
  ...
  {
    CLog::LogF(LOGDEBUG, L"Failed to render glyph %x to a bitmap", letter);
    return false;
  }
