  ...
  while (XPending(m_display))
  {
    ...
    switch (xevent.type)
    {
      ...
      case ConfigureNotify:
      {
        ...
        while(XCheckTypedWindowEvent(m_display, m_window, ConfigureNotify, &xevent))
          ;
        ...
        // THe EGL backend supports only fullscreen windows. Thus there's no need for a move.
#if 0
        /* check for move */
...
size_t CWinEventsX11::GetQueueSize()
{
  ...
  return XEventsQueued(m_display, QueuedAlready);
}
...
{
  if (m_lastKey.type == XBMC_KEYDOWN)
  {
