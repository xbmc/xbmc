...
// Direct-to-Plane: the video has its own DRM plane and the display hardware
// blends the GUI plane over it.
bool CWinSystemGbmGLESContext::VideoOnSeparatePlane() const
{
  return m_DRM && m_DRM->GetVideoPlane() != nullptr && m_DRM->GetGuiPlane() != nullptr;
}
...
{
  m_guiComposite.End(VideoOnSeparatePlane());
}

void CWinSystemGbmGLESContext::CompositeGui()
{
  m_guiComposite.Composite(VideoOnSeparatePlane(), GetGUIElementCount());
}
