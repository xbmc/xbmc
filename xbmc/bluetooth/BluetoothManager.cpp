...
void CBluetoothManager::OnDeviceConnected(IBluetoothDevice *device)
{
  if (device->IsPaired())
  {
    ...
    case IBluetoothDevice::DEVICE_TYPE_AUDIO:
      CLog::Log(LOGINFO, "CBluetoothManager::OnDeviceConnected - audio device %s", device->GetAddress());
      m_audioDeviceAddress = device->GetAddress();
      CAEFactory::OnSettingsChange("audiooutput.audiodevice");
    default:
      break;
    }
...
void CBluetoothManager::OnDeviceDisconnected(IBluetoothDevice *device)
{
  if (device->IsPaired())
  {
    if (m_audioDeviceAddress == device->GetAddress())
    {
      CLog::Log(LOGINFO, "CBluetoothManager::OnDeviceDisconnected - audio device %s", device->GetAddress());
      m_audioDeviceAddress = "";
      CAEFactory::OnSettingsChange("audiooutput.audiodevice");
    }
...
const char* CBluetoothManager::GetAudioDeviceAddress()
{
  if (m_audioDeviceAddress.length() > 0)
    return m_audioDeviceAddress.c_str();
  else
    return NULL;
}
