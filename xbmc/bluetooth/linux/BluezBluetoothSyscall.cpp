...
{
  CBluezBluetoothDevice device(args[0].asString().c_str());
  CStdString key;
  ...
  case IBluetoothDevice::DEVICE_TYPE_PHONE:
    ...
    break;
  default:
    if (CGUIKeyboardFactory::ShowAndGetInput(key, g_localizeStrings.Get(16506), true))
      Reply(key);
    else
...
std::vector<boost::shared_ptr<IBluetoothDevice> > CBluezBluetoothSyscall::GetDevices()
{
  ...
  if (m_hasAdapter)
  {
    ...
    {
      CLog::Log(LOGDEBUG, "Bluetooth: found known device %s", devices[i].asString().c_str());
      v.push_back(GetDevice(devices[i].asString().c_str()));
    }
  }
  return v;
}
  ...
  if (m_hasAdapter && m_connection)
  {
    ...
    if (msg)
    {
      ...
      if (dbus_message_is_signal(msg, "org.bluez.Adapter", "DeviceFound"))
      {
        ...
        CBluezBluetoothDevice device(address, properties["Name"].asString().c_str(),
