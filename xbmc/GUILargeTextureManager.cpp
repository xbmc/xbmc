...
CImageLoader::CImageLoader(const string &path, const bool useCache)
{
  m_path = path;
  m_texture = NULL;
...
bool CImageLoader::DoWork()
{
  ...
  string texturePath = g_TextureManager.GetTexturePath(m_path);
  if (m_use_cache)
...
CGUILargeTextureManager::CLargeTexture::CLargeTexture(const string &path)
{
  m_path = path;
  m_refCount = 1;
  m_timeToDelete = 0;
...
bool CGUILargeTextureManager::GetImage(const string &path, CTextureArray &texture, bool firstRequest, const bool useCache)
{
  CSingleLock lock(m_listSection);
...
void CGUILargeTextureManager::ReleaseImage(const string &path, bool immediately)
{
  CSingleLock lock(m_listSection);
  for (listIterator it = m_allocated.begin(); it != m_allocated.end(); ++it)
  {
...
void CGUILargeTextureManager::QueueImage(const string &path, bool useCache)
{
  CSingleLock lock(m_listSection);
  for (queueIterator it = m_queued.begin(); it != m_queued.end(); ++it)
  {
