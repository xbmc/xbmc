...
bool CTextureCache::IsCachedImage(const string &url) const
{
...
bool CTextureCache::HasCachedImage(const string &url)
{
  CTextureDetails details;
  string cachedImage(GetCachedImage(url, details));
  return (!cachedImage.empty() && cachedImage != url);
}

string CTextureCache::GetCachedImage(const string &image, CTextureDetails &details, bool trackUsage)
{
  string url = CTextureUtils::UnwrapImageURL(image);
...
string CTextureCache::CheckCachedImage(const string &url, bool returnDDS, bool &needsRecaching)
{
  CTextureDetails details;
  string path(GetCachedImage(url, details, true));
  ...
  if (!path.empty())
  {
    ...
    { // check for dds version
      string ddsPath = URIUtils::ReplaceExtension(path, ".dds");
...
void CTextureCache::BackgroundCacheImage(const string &url)
{
  CTextureDetails details;
  string path(GetCachedImage(url, details));
...
bool CTextureCache::CacheImage(const string &image, CTextureDetails &details)
{
  string path = GetCachedImage(image, details);
...
string CTextureCache::CacheImage(const string &image, CBaseTexture **texture, CTextureDetails *details)
{
  string url = CTextureUtils::UnwrapImageURL(image);
...
void CTextureCache::ClearCachedImage(const string &url, bool deleteSource /*= false */)
{
  // TODO: This can be removed when the texture cache covers everything.
  string path = deleteSource ? url : "";
  string cachedFile;
...
bool CTextureCache::GetCachedTexture(const string &url, CTextureDetails &details)
{
  CSingleLock lock(m_databaseSection);
  return m_database.GetCachedTexture(url, details);
}

bool CTextureCache::AddCachedTexture(const string &url, const CTextureDetails &details)
{
  CSingleLock lock(m_databaseSection);
...
bool CTextureCache::SetCachedTextureValid(const string &url, bool updateable)
{
  CSingleLock lock(m_databaseSection);
  return m_database.SetCachedTextureValid(url, updateable);
}

bool CTextureCache::ClearCachedTexture(const string &url, string &cachedURL)
{
  CSingleLock lock(m_databaseSection);
  return m_database.ClearCachedTexture(url, cachedURL);
}

bool CTextureCache::ClearCachedTexture(int id, string &cachedURL)
{
  CSingleLock lock(m_databaseSection);
  return m_database.ClearCachedTexture(id, cachedURL);
}

string CTextureCache::GetCacheFile(const string &url)
{
  Crc32 crc;
  crc.ComputeFromLowerCase(url);
  string hex = StringUtils::Format("%08x", (unsigned int)crc);
  string hash = StringUtils::Format("%c/%s", hex[0], hex.c_str());
  return hash;
}

string CTextureCache::GetCachedPath(const string &file)
{
...
void CTextureCache::OnCachingComplete(bool success, CTextureCacheJob *job)
{
  ...
  { // remove from our processing list
    CSingleLock lock(m_processingSection);
    std::set<string>::iterator i = m_processinglist.find(job->m_url);
  ...
  { // check our processing list
    {
      ...
      std::set<string>::iterator i = m_processinglist.find(cacheJob->m_url);
      if (i == m_processinglist.end())
      {
...
bool CTextureCache::Export(const string &image, const string &destination, bool overwrite)
{
  CTextureDetails details;
  string cachedImage(GetCachedImage(image, details));
  if (!cachedImage.empty())
  {
    string dest = destination + URIUtils::GetExtension(cachedImage);
...
bool CTextureCache::Export(const string &image, const string &destination)
{
  CTextureDetails details;
  string cachedImage(GetCachedImage(image, details));
  if (!cachedImage.empty())
