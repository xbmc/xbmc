...
void CLinuxJoystick::Initialize(JoystickArray &joysticks)
{
  ...
  while ((pDirent = readdir(pd))!= NULL)
  {
    if (strncmp(pDirent->d_name, "js", 2) == 0)
    {
      ...
      CLog::Log(LOGNOTICE, "CLinuxJoystick::Initialize: Enabled Joystick: \"%s\" (Linux Joystick API)", name);
      CLog::Log(LOGNOTICE, "CLinuxJoystick::Initialize: driver version is %d.%d.%d",
      ...
      {
        CLog::Log(LOGERROR, "CLinuxJoystick::Initialize: can't get button or axis map");
      ...
      if (!buttonMapOK)
      {
        ...
        CLog::Log(LOGERROR, "CLinuxJoystick::Initialize: XBMC is not fully compatible with your kernel. Unable to retrieve button map!");
