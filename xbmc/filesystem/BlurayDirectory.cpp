                                       ...
                                       std::map<unsigned int, std::string>& playlist_langs)
{
  ...
  std::sort(clips.begin(), clips.end(),
            [](std::vector<unsigned int> i, std::vector<unsigned int> j) { return (i[0] < j[0]); });
                                        ...
                                        std::map<unsigned int, std::string>& playlist_langs)
{
  ...
  if (numEpisodes == 1 || (!foundEpisode && !allEpisodes))
  {
    ...
    else if (numEpisodes == 1 && existingCandidates > 0 && candidatePlaylists.size() > 1)
    {
      ...
      // Loop backwards as desired length preferred to longest length
      int keepPlaylist = -1;
      for (unsigned int i = candidatePlaylists.size() - 1; i >= 0; --i)
      {
        if (langs[i] > 1)
        {
          // If more than one language then keep
          keepPlaylist = candidatePlaylists[i].first;
          break;
        }
      }
      if (keepPlaylist != -1)
        // Remove other playlists (keep the first one with more than one languages)
        candidatePlaylists.erase(
            std::remove_if(candidatePlaylists.begin(), candidatePlaylists.end(),
                           [&keepPlaylist](std::pair<unsigned int, unsigned int>& p)
  ...
  for (const auto& playlist : candidatePlaylists)
  {
    ...
    std::string clips;
    for (unsigned int i = CLIP_PLAYLIST_OFFSET; i < it->size(); ++i)
    {
      clips += std::to_string(it->at(i)) + ',';
    }
    if (!clips.empty())
      clips.pop_back();
