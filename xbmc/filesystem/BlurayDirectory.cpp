                                        ...
                                        std::map<unsigned int, std::string>& playlist_langs) const
{
  ...
  if (!foundEpisode)
  {
    ...
    const unsigned int long_playlists = std::count_if(
        playlists_length.begin(), playlists_length.end(),
        [](const std::vector<unsigned int>& x) { return x[1] >= MIN_EPISODE_LENGTH; });
    bool foundCommonPlaylist{false};
    for (const std::vector<unsigned int>& playlist : playlists)
      if (std::any_of(commonStartingPlaylists.begin(), commonStartingPlaylists.end(),
