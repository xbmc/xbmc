                                            ...
                                            const PlaylistMap& playlists)
{
  ...
  for (unsigned int i = 0; i < episodesOnDisc.size(); ++i)
  {
    ...
    else if (episodesOnDisc[i].m_iSeason == 0)
    {
      ...
      {
        // Special found
        specialIndex = static_cast<unsigned int>(specialsOnDisc.size() - 1);
        if (!allEpisodes)
          isSpecial = true;
      }
  ...
  const unsigned int numSpecials{static_cast<unsigned int>(specialsOnDisc.size())};
  ...
  {
    CLog::LogF(LOGDEBUG, "Expected on disc - special - season {} episode {} duration {}",
    ...
    for (unsigned int i{0}; const auto& clip : playlistInformation.clips)
    {
      if (allEpisodes || i == episodeIndex)
      {
        ...
        const auto& singleEpisodePlaylists =
            playAllPlaylistsMap[playAllPlaylist].find(clip)->second;
  ...
  if (isSpecial || allEpisodes)
  {
    for (const auto& playlist : candidateSpecials)
    {
      ...
      if (isSpecial && candidateSpecials.size() == 1)
        tag = specialsOnDisc[specialIndex];
                                        ...
                                        bool isSpecial /* = false */)
{
  ...
  // Get languages
  std::string langs{it->second.languages};
