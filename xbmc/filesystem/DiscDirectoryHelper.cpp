    ...
    std::map<unsigned int, unsigned int>& candidatePlaylists)
{
  ...
  if (allEpisodes == AllEpisodes::ALL)
  ...
  if (playAllPlaylists.size() == 1)
  {
    ...
    {
      if (allEpisodes == AllEpisodes::ALL ||
          i == episodeIndex - numSpecials) // Specials before episodes in episodesOnDisc
      {
        ...
        for (const auto& singleEpisodePlaylist : singleEpisodePlaylists)
        {
          ...
          candidatePlaylists.insert(
              {singleEpisodePlaylist, i + numSpecials}); // Also save episodeIndex for all episodes
        }
      }
      ++i;
    }
      ...
      const bool foundCommonPlaylist{std::ranges::any_of(
          ...
          [&commonStartingPlaylists, &commonPlaylist](const PlaylistMapEntry& p)
          {
            ...
            return std::ranges::any_of(commonStartingPlaylists,
                                       [playlist, &commonPlaylist](unsigned int c)
                                       {
                                         if (c == playlist)
                                         {
      ...
      if (!groups.empty())
      {
        for (const auto& group : groups)
        {
          for (unsigned int i = 0; i < numEpisodes; ++i)
          {
            ...
            {
              candidatePlaylists.insert(
                  {group[i], i + numSpecials}); // Also save episodeIndex for all episodes
  ...
  if (candidatePlaylists.size() > 1 && isSpecial == IsSpecial::EPISODE)
  {
    ...
    for (unsigned int currentEpisodeIndex : indexes)
    {
      ...
      std::vector<std::tuple<unsigned int, unsigned int, unsigned int>> candidatePlaylistsDuration;
      ...
      for (const auto& [playlist, index] : oldCandidatePlaylists)
        if (index == currentEpisodeIndex)
        {
          ...
          candidatePlaylistsDuration.emplace_back(
              playlist, abs(static_cast<int>(playlistInformation.duration - duration)),
      ...
      std::ranges::sort(candidatePlaylistsDuration,
                        ...
                        {
                          const auto& [p_playlist, p_delta, p_chapters] = p;
                          const auto& [q_playlist, q_delta, q_chapters] = q;
                          if (p_chapters == q_chapters)
                            return p_delta < q_delta;
                          return p_chapters > q_chapters;
                        });
...
void FindSpecials(unsigned int numSpecials,
                  unsigned int numEpisodes,
                  const PlaylistMap& playlists,
                  const std::map<unsigned int, unsigned int>& candidatePlaylists,
                  const std::vector<unsigned int>& playAllPlaylists,
...
{
  // Get clips
  assert(playlists.contains(playlist));
  ...
  itemTag->m_iTrack = static_cast<int>(playlist);
...
void PopulateFileItems(const CURL& url,
                       ...
                       int episodeIndex,
                       const std::vector<CVideoInfoTag>& episodesOnDisc,
                       const std::vector<unsigned int>& candidateSpecials,
                       const std::map<unsigned int, unsigned int>& candidatePlaylists,
                       const PlaylistMap& playlists)
{
  ...
  if (isSpecial == IsSpecial::EPISODE)
  {
  ...
  if (isSpecial == IsSpecial::SPECIAL || allEpisodes == AllEpisodes::ALL)
  {
    for (const auto& playlist : candidateSpecials)
    {
      const auto newItem{std::make_shared<CFileItem>("", false)};
      CVideoInfoTag tag;
      if (isSpecial == IsSpecial::SPECIAL && candidateSpecials.size() == 1)
                                               ...
                                               const PlaylistMap& playlists)
{
  ...
  AllEpisodes allEpisodes{episodeIndex == -1 ? AllEpisodes::ALL : AllEpisodes::SINGLE};
  IsSpecial isSpecial{allEpisodes == AllEpisodes::SINGLE &&
                              episodesOnDisc[episodeIndex].m_iSeason == 0
                          ? IsSpecial::SPECIAL
                          : IsSpecial::EPISODE};
  ...
  if (isSpecial == IsSpecial::SPECIAL && numEpisodes > 0)
    allEpisodes = AllEpisodes::ALL;

  CLog::LogF(LOGDEBUG, "*** Episode Search Start ***");

  if (allEpisodes == AllEpisodes::SINGLE)
  {
  ...
  FindCandidatePlaylists(numEpisodes, episodesOnDisc, episodeIndex, numSpecials, allEpisodes,
                         isSpecial, playlists, playAllPlaylists, playAllPlaylistsMap, groups,
                         candidatePlaylists);

  std::vector<unsigned int> candidateSpecials;
  if (numSpecials > 0)
    FindSpecials(numSpecials, numEpisodes, playlists, candidatePlaylists, playAllPlaylists,
                 candidateSpecials);

  CLog::LogF(LOGDEBUG, "*** Episode Search End ***");

  PopulateFileItems(url, items, allEpisodes, isSpecial, episodeIndex, episodesOnDisc,
                    candidateSpecials, candidatePlaylists, playlists);
