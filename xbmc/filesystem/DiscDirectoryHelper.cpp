                                               ...
                                               const PlaylistMap& playlists)
{
          ...
          for (unsigned int i = 0; i < numEpisodes; ++i)
          {
            if (allEpisodes ||
                i == static_cast<unsigned int>(
                         episodeIndex - numSpecials)) // Specials before episodes in episodesOnDisc
            {
              candidatePlaylists.insert(
  ...
  if (candidatePlaylists.size() > 1 && !isSpecial)
  {
...
std::string CDiscDirectoryHelper::HexToString(int num, int count)
{
  std::stringstream ss;
...
std::string CDiscDirectoryHelper::GetString(const std::vector<char>& bytes,
                                            unsigned int offset,
                                            unsigned int length)
{
