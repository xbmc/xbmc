...
void CDiscDirectoryHelper::FindSpecials(const PlaylistMap& playlists)
{
  ...
  {
    CLog::LogF(LOGDEBUG, "Disc holds {} specials and nothing to tell them apart - offering none",
               m_numSpecials);
    return;
  }
                                             ...
                                             const PlaylistNames& names)
{
  ...
  // Laid out as the episode listing is - the title and what the disc calls it lead the description
  std::string label2{StringUtils::Format(
      "{} - {}", GetTitleWithName(playlist, names),
      StringUtils::Format(
                                                     ...
                                                     const PlaylistMap& playlistMap) const
{
  ...
  const std::vector<PlaylistInformation> selected{SelectMoviePlaylists(
      featurePlaylists, clips, mainPlaylist, GetTitle::ALL, Duplicates::REMOVE)};
  if (selected.empty())
  {
    CLog::LogF(LOGDEBUG,
  ...
  if (job == GetTitle::MAIN)
  {
    ...
    for (const auto& item : items)
    {
      ...
      if (const auto hint{hints.find(playlist)}; hint != hints.end() &&
                                                 hint->second.role != PlaylistRole::FEATURE &&
