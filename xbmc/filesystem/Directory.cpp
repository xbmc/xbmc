    ...
    else
    {
      ...
      while (!result && !cancel)
      {
        ...
        if (!result)
        {
          ...
          CLog::Log(LOGERROR, "{} - Error getting {}", __FUNCTION__, url.GetRedacted().c_str());
          return false;
        }
      }
...
bool CDirectory::Create(const CURL& url)
{
  ...
  CLog::Log(LOGERROR, "{} - Error creating {}", __FUNCTION__, url.GetRedacted().c_str());
  return false;
}
...
bool CDirectory::Exists(const CURL& url, bool bUseCache /* = true */)
{
  ...
  CLog::Log(LOGERROR, "{} - Error checking for {}", __FUNCTION__, url.GetRedacted().c_str());
  return false;
}
...
bool CDirectory::Remove(const CURL& url)
{
  ...
  CLog::Log(LOGERROR, "{} - Error removing {}", __FUNCTION__, url.GetRedacted().c_str());
  return false;
}
