...
bool CPVRFile::Open(const CURL& url)
{
  ...
  if (StringUtils::StartsWith(strURL, "pvr://channels/tv/", true) ||
      StringUtils::StartsWith(strURL, "pvr://channels/radio/", true))
  {
  ...
  else if (StringUtils::StartsWith(strURL, "pvr://recordings/", true))
  {
...
bool CPVRFile::Delete(const CURL& url)
{
  ...
  if (StringUtils::StartsWith(path, "recordings/", true) && path[path.size()-1] != '/')
  {
    CStdString strURL = url.Get();
    CFileItemPtr tag = g_PVRRecordings->GetByPath(strURL);
    if (tag && tag->HasPVRRecordingInfoTag())
