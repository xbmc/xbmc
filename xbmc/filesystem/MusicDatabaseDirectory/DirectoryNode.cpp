...
CDirectoryNode::CDirectoryNode(NODE_TYPE Type, const string& strName, CDirectoryNode* pParent)
{
  m_Type=Type;
  m_strName=strName;
  m_pParent=pParent;
...
CDirectoryNode* CDirectoryNode::ParseURL(const string& strPath)
{
  CURL url(strPath);
...
void CDirectoryNode::GetDatabaseInfo(const string& strPath, CQueryParams& params)
{
...
CDirectoryNode* CDirectoryNode::CreateNode(NODE_TYPE Type, const string& strName, CDirectoryNode* pParent)
{
  switch (Type)
  {
  case NODE_TYPE_ROOT:
...
//  Current node name
const string& CDirectoryNode::GetName() const
{
  return m_strName;
}
...
string CDirectoryNode::GetLocalizedName() const
{
  return "";
}
...
string CDirectoryNode::BuildPath() const
{
  ...
  while (pParent!=NULL)
  {
    const string& strNodeName=pParent->GetName();
...
void CDirectoryNode::AddOptions(const string &options)
{
  if (options.empty())
    return;
