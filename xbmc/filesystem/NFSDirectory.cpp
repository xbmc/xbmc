...
bool CNFSDirectory::ResolveSymlink( const std::string &dirName, struct nfsdirent *dirent, CURL &resolvedUrl)
{
  ...
  if(ret == 0)
  {
    ...
    if (ret != 0)
    {
      CLog::Log(LOGERROR, "NFS: Failed to stat({}) on link resolve {}", fullpath.c_str(),
  ...
  {
    CLog::Log(LOGERROR, "Failed to readlink({}) {}", fullpath.c_str(),
              nfs_get_error(gNfsConnection.GetNfsContext()));
    retVal = false;
  }
  return retVal;
...
bool CNFSDirectory::GetDirectory(const CURL& url, CFileItemList &items)
{
  ...
  if(ret != 0)
  {
    CLog::Log(LOGERROR, "Failed to open({}) {}", strDirName.c_str(),
              nfs_get_error(gNfsConnection.GetNfsContext()));
    return false;
  }
  lock.Leave();
...
bool CNFSDirectory::Create(const CURL& url2)
{
  ...
  if(!success)
    CLog::Log(LOGERROR, "NFS: Failed to create({}) {}", folderName.c_str(),
              nfs_get_error(gNfsConnection.GetNfsContext()));
  return success;
}
