...
//! The disc's playlists and their durations, which a record has to agree with to be believed
using DiscPlaylistDurations = std::map<unsigned int, std::chrono::milliseconds>;
...
public:
  ...
  /*!
   \brief Parse the contents of a project file.
   \return READ if it names any of the disc's playlists, ABSENT if it names none
   */
  static ProjectReadResult ParseProject(std::span<const std::byte> buffer,
                                        const DiscPlaylistDurations& discPlaylists,
                                        ProjectInformation& projectInformation);
