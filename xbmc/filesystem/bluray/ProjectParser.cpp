                                                     ...
                                                     const std::set<unsigned int>& discPlaylists)
{
  ...
  if (!discPlaylists.contains(information.playlist))
    return std::nullopt;
...
bool ParseProject(const std::span<const std::byte> buffer,
                  const std::set<unsigned int>& discPlaylists,
                              ...
                              ProjectInformation& projectInformation)
{
  ...
  try
  {
    ...
    return ParseProject(buffer, discPlaylists, projectInformation) ? ProjectReadResult::READ
