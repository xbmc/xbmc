    ...
    for (const StreamInformation& stream : b.clips[0].programs[0].streams)
    {
      ...
      switch (stream.coding)
      {
        ...
        case VIDEO_HEVC:
        {
          ...
          if (bs != s.end())
          {
            const auto& bsvi{dynamic_cast<TSVideoStreamInfo*>(bs->second.get())};
            vsi = PopulateVideoStreamInfo(stream, bsvi); // Handles bsvi nullptr
          }
          else
            vsi = PopulateVideoStreamInfo(stream, nullptr);
          p.videoStreams.emplace_back(std::move(vsi));
          break;
        }
        case AUDIO_LPCM:
        case AUDIO_AC3:
        ...
        case AUDIO_DTSHD_SECONDARY:
        {
          ...
          if (bs != s.end())
          {
            const auto& bsai{dynamic_cast<TSAudioStreamInfo*>(bs->second.get())};
            asi = PopulateAudioStreamInfo(stream, bsai);
          }
          else
            asi = PopulateAudioStreamInfo(stream, nullptr);
