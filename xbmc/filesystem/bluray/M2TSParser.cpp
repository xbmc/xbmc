                             ...
                             StreamMap& streams)
{
  try
  {
    ...
    std::string clipFile{
        URIUtils::AddFileToFolder(path, "BDMV", "STREAM", fmt::format("{:05}.m2ts", clip))};
    CFile file;
    if (!file.Open(clipFile))
    {
      clipFile = URIUtils::AddFileToFolder(path, "BDMV", "STREAM", fmt::format("{:05}.fmts", clip));
      if (!file.Open(clipFile))
        return false;
    }
    ...
    const auto videoStreamsVector{
        [&]
        {
          std::vector<TSVideoStreamInfo*> vec;
          for (const auto& streamInfo : streams | std::views::values)
          {
            if (streamInfo && IsVideoStream(streamInfo->streamType))
              if (auto* videoInfo{dynamic_cast<TSVideoStreamInfo*>(streamInfo.get())})
                vec.push_back(videoInfo);
          }
          return vec;
        }()};

    if (videoStreamsVector.size() == 2 && videoStreamsVector[1]->dolbyVision)
    {
      // If two video streams and second one is DV, set DV flag for first video stream
      videoStreamsVector[0]->dolbyVision = true;

      // Remove the second video stream
      auto it2 = std::ranges::find_if(streams, [ptr = videoStreamsVector[1]](const auto& pair)
