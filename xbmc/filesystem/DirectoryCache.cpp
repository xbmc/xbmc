...
bool CDirectoryCache::GetDirectory(const string& strPath, CFileItemList &items, bool retrieveAll)
{
  CSingleLock lock (m_cs);

  string storedPath = strPath;
...
void CDirectoryCache::SetDirectory(const string& strPath, const CFileItemList &items, DIR_CACHE_TYPE cacheType)
{
  ...
  dir->SetLastAccess(m_accessCounter);
  m_cache.insert(pair<string, CDir*>(storedPath, dir));
}

void CDirectoryCache::ClearFile(const string& strFile)
{
  ClearDirectory(URIUtils::GetDirectory(strFile));
}

void CDirectoryCache::ClearDirectory(const string& strPath)
{
  CSingleLock lock (m_cs);
...
void CDirectoryCache::ClearSubPaths(const string& strPath)
{
  CSingleLock lock (m_cs);
...
void CDirectoryCache::AddFile(const string& strFile)
{
  CSingleLock lock (m_cs);
...
bool CDirectoryCache::FileExists(const string& strFile, bool& bInCache)
{
  CSingleLock lock (m_cs);
  bInCache = false;
  ...
  URIUtils::RemoveSlashAtEnd(strPath);
  string storedPath = URIUtils::GetDirectory(strPath);
  URIUtils::RemoveSlashAtEnd(storedPath);
...
void CDirectoryCache::InitCache(set<string>& dirs)
{
...
void CDirectoryCache::ClearCache(set<string>& dirs)
{
  iCache i = m_cache.begin();
  while (i != m_cache.end())
  {
