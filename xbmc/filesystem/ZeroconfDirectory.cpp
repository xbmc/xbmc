...
namespace
{
  string GetHumanReadableProtocol(std::string const& fcr_service_type)
  {
  ...
  bool GetXBMCProtocol(std::string const& fcr_service_type, string& fr_protocol)
  {
    if(fcr_service_type == "_smb._tcp.")
      fr_protocol = "smb";
    else if(fcr_service_type == "_ftp._tcp.")
...
bool CZeroconfDirectory::GetDirectory(const string& strPath, CFileItemList &items)
{
  assert(strPath.substr(0, 11) == "zeroconf://");
  string path = strPath.substr(11, strPath.length());
  URIUtils::RemoveSlashAtEnd(path);
  if(path.empty())
  {
    ...
    for(std::vector<CZeroconfBrowser::ZeroconfService>::iterator it = found_services.begin(); it != found_services.end(); ++it)
    {
      ...
      if(GetXBMCProtocol(it->GetType(), tmp))
      {
        ...
        string service_path(CURL::Encode(CZeroconfBrowser::ZeroconfService::toPath(*it)));
        url.SetFileName(service_path);
        item->SetPath(url.Get());

        //now do the formatting
        string protocol = GetHumanReadableProtocol(it->GetType());
  ...
  else
  {
    ...
    string decoded(CURL::Decode(path));
    try
    {
