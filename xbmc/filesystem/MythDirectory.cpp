...
bool CMythDirectory::IsMovie(const cmyth_proginfo_t program)
{
  ...
  if (iMovieLength > 0) // Use hack to identify movie based on length (used if EPG is dubious).
    return StringUtils::StartsWith(GetValue(m_dll->proginfo_programid(program)), "MV", true)
...
bool CMythDirectory::SupportsWriteFileOperations(const CStdString& strPath)
{
   ...
   * operations.
   */
  return StringUtils::StartsWith(filename, "recordings/", true) ||
         StringUtils::StartsWith(filename, "movies/", true) ||
        (StringUtils::StartsWith(filename, "tvshows/", true) && URIUtils::HasExtension(filename));
}

bool CMythDirectory::IsLiveTV(const CStdString& strPath)
{
  CURL url(strPath);
  return StringUtils::StartsWith(url.GetFileName(), "channels/", true);
}
