...
CSFTPSession::CSFTPSession(const string &host, unsigned int port, const string &username, const string &password)
{
...
sftp_file CSFTPSession::CreateFileHande(const string &file)
{
  if (m_connected)
  {
    CSingleLock lock(m_critSect);
...
bool CSFTPSession::GetDirectory(const string &base, const string &folder, CFileItemList &items)
{
  ...
  if (m_connected)
  {
    ...
    else
    {
      ...
      while (read)
      {
        ...
        if (attributes)
        {
          string itemName = attributes->name;
...
bool CSFTPSession::Connect(const string &host, unsigned int port, const string &username, const string &password)
{
...
CSFTPSessionPtr CSFTPSessionManager::CreateSession(const string &host, unsigned int port, const string &username, const string &password)
{
  // Convert port number to string
  stringstream itoa;
  itoa << port;
  string portstr = itoa.str();

  CSingleLock lock(m_critSect);
  string key = username + ":" + password + "@" + host + ":" + portstr;
...
{
  CSingleLock lock(m_critSect);
  for(map<string, CSFTPSessionPtr>::iterator iter = sessions.begin(); iter != sessions.end();)
  {
