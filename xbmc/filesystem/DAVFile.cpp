...
bool CDAVFile::Delete(const CURL& url)
{
  ...
  CLog::Log(LOGDEBUG, "CDAVFile::Delete - Execute DELETE ({})", url.GetRedacted().c_str());
  if (!dav.Execute(url))
  {
    CLog::Log(LOGERROR, "CDAVFile::Delete - Unable to delete dav resource ({})",
...
bool CDAVFile::Rename(const CURL& url, const CURL& urlnew)
{
  ...
  CLog::Log(LOGDEBUG, "CDAVFile::Rename - Execute MOVE ({} -> {})", url.GetRedacted().c_str(),
            url2.GetRedacted().c_str());
  if (!dav.Execute(url))
  {
    CLog::Log(LOGERROR, "CDAVFile::Rename - Unable to rename dav resource ({} -> {})",
              url.GetRedacted().c_str(), url2.GetRedacted().c_str());
    return false;
  }
