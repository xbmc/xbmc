...
bool CImageFile::Open(const CURL& url)
{
  ...
  string cachedFile = CTextureCache::Get().CheckCachedImage(file, false, needsRecaching);
  if (cachedFile.empty())
...
{
  bool needsRecaching = false;
  string cachedFile = CTextureCache::Get().CheckCachedImage(url.Get(), false, needsRecaching);
  if (!cachedFile.empty())
