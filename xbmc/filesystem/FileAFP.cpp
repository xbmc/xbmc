...
CAfpConnection::afpConnnectError CAfpConnection::Connect(const CURL& url)
{
  ...
  {
    serverChanged=true;//no reusing of old server object possible with libafpclient it seems...
    Disconnect();
  }
  ...
  {
		*m_pAfpUrl=tmpurl;//this is our current url object whe are connected to (at least we try)
  ...
  else//if one of password or username is not set - prompt the user
    if((nonConstUrl.GetPassWord().IsEmpty() || nonConstUrl.GetUserName().IsEmpty()) && serverChanged)
    {
        return AfpAuth;
    }
  ...
  if(!m_pAfpServer || serverChanged)
  {
    ...
    CLog::Log(LOGDEBUG,"AFP: Connected to server %s using UAM \"%s\"\n",
              ...
              m_pLibAfp->uam_bitmap_to_string(m_pAfpServer->using_uam));
    free(conn_req);//we don't need it after here ...
  }
...
{
  CSingleLock lock(gAfpConnection);  
  if (m_pFp == NULL || !gAfpConnection.GetVolume()) return 0;
...
#endif
  int eof = 0;
  int bytesRead = gAfpConnection.GetImpl()->afp_wrap_read(gAfpConnection.GetVolume(), name, (char *)lpBuf,(size_t)uiBufSize, m_fileOffset,m_pFp,&eof);
  if(bytesRead > 0)
...
{
  CSingleLock lock(gAfpConnection);
  if (m_pFp == NULL || !gAfpConnection.GetVolume()) return -1;
...
#endif  
  numberOfBytesWritten=gAfpConnection.GetImpl()->afp_wrap_write(gAfpConnection.GetVolume(), name, (const char *)lpBuf, (size_t)uiBufSize, m_fileOffset, m_pFp,uid,gid);
