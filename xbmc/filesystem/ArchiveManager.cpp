...
int64_t CArchiveEntry::get_file_length()
{
  return m_file_length;
}
void CArchiveEntry::set_file_length(const int64_t file_length)
{
...
bool CArchiveManager::CacheArchivedPath(const CStdString &strArchive,
                                        CArchiveEntry &item)
{
  CStdString strCachedPath, strPathInArchive,
    strDir(CSpecialProtocol::TranslatePath("special://temp/"));
                                      ...
                                      deque<CArchiveEntry> &items)
{
  ...
  while (m_dllLibArchive.archive_read_next_header(a, &entry) == ARCHIVE_OK) {
    ...
    e.set_file_length(m_dllLibArchive.archive_entry_size(entry));
    items.push_back(e);
  }
