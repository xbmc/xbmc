...
bool IDirectory::IsAllowed(const CStdString& strFile) const
{
  ...
  if (URIUtils::HasExtension(strFile, ".ifo"))
  {
    ...
    (fileName.length() == 12 &&
     StringUtils::StartsWith(fileName, "vts_") &&
     StringUtils::EndsWith(fileName, "_0.ifo"));
  }
  ...
  if (URIUtils::HasExtension(strFile, ".dat"))
  {
    ...
    return (fileName.length() == 11 || fileName.length() == 12) &&
          (StringUtils::StartsWith(fileName, "AVSEQ") ||
           StringUtils::StartsWith(fileName, "MUSIC") ||
           StringUtils::StartsWith(fileName, "ITEM"));
  }
