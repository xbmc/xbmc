...
bool CMusicDatabaseDirectory::GetDirectory(const string& strPath, CFileItemList &items)
{
  string path = CLegacyPathTranslation::TranslateMusicDbPath(strPath);
  items.SetPath(path);
  ...
  for (int i=0;i<items.Size();++i)
  {
    ...
    {
      string strImage = GetIcon(item->GetPath());
...
NODE_TYPE CMusicDatabaseDirectory::GetDirectoryChildType(const string& strPath)
{
  string path = CLegacyPathTranslation::TranslateMusicDbPath(strPath);
  auto_ptr<CDirectoryNode> pNode(CDirectoryNode::ParseURL(path));
...
NODE_TYPE CMusicDatabaseDirectory::GetDirectoryType(const string& strPath)
{
  string path = CLegacyPathTranslation::TranslateMusicDbPath(strPath);
  auto_ptr<CDirectoryNode> pNode(CDirectoryNode::ParseURL(path));
...
NODE_TYPE CMusicDatabaseDirectory::GetDirectoryParentType(const string& strPath)
{
  string path = CLegacyPathTranslation::TranslateMusicDbPath(strPath);
  auto_ptr<CDirectoryNode> pNode(CDirectoryNode::ParseURL(path));
...
bool CMusicDatabaseDirectory::IsArtistDir(const string& strDirectory)
{
  NODE_TYPE node=GetDirectoryType(strDirectory);
  return (node==NODE_TYPE_ARTIST);
}

bool CMusicDatabaseDirectory::HasAlbumInfo(const string& strDirectory)
{
  NODE_TYPE node=GetDirectoryType(strDirectory);
...
void CMusicDatabaseDirectory::ClearDirectoryCache(const string& strDirectory)
{
  string path = CLegacyPathTranslation::TranslateMusicDbPath(strDirectory);
  URIUtils::RemoveSlashAtEnd(path);

  Crc32 crc;
  crc.ComputeFromLowerCase(path);

  string strFileName = StringUtils::Format("special://temp/%08x.fi", (unsigned __int32) crc);
  CFile::Delete(strFileName);
}

bool CMusicDatabaseDirectory::IsAllItem(const string& strDirectory)
{
...
bool CMusicDatabaseDirectory::GetLabel(const string& strDirectory, string& strLabel)
{
  strLabel = "";
...
bool CMusicDatabaseDirectory::ContainsSongs(const string &path)
{
...
bool CMusicDatabaseDirectory::CanCache(const string& strPath)
{
  string path = CLegacyPathTranslation::TranslateMusicDbPath(strPath);
  auto_ptr<CDirectoryNode> pNode(CDirectoryNode::ParseURL(path));
  if (!pNode.get())
...
string CMusicDatabaseDirectory::GetIcon(const string &strDirectory)
{
