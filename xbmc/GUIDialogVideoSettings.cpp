...
void CGUIDialogVideoSettings::CreateSettings()
{
  ...
  { // Audio and Subtitle settings
    ...
    AddAudioStreams(3);
    AddBool(4, 13397, &g_stSettings.m_currentVideoSettings.m_SubtitleOn);
    AddSlider(5, 303, &g_stSettings.m_currentVideoSettings.m_SubtitleDelay, -g_stSettings.m_fSubsDelayRange, 0.1f, g_stSettings.m_fSubsDelayRange, "%2.1fs");
    AddSubtitleStreams(6);
  }
}

void CGUIDialogVideoSettings::AddAudioStreams(int id)
{
  VideoSetting setting;
  setting.id = id;
...
void CGUIDialogVideoSettings::AddSubtitleStreams(int id)
{
  VideoSetting setting;
  setting.id = id;
  ...
  setting.type = VideoSetting::SPIN;
  setting.min = 0;
  setting.data = &g_stSettings.m_currentVideoSettings.m_SubtitleStream;
...
{
  return (dwID >= WINDOW_DIALOG_VIDEO_OSD_SETTINGS && dwID <= WINDOW_DIALOG_VIDEO_OSD_SETTINGS + 1);
}

void CGUIDialogVideoSettings::Render()
{
  if (m_iScreen == WINDOW_DIALOG_VIDEO_OSD_SETTINGS + 1)
  {
    m_volume = g_stSettings.m_nVolumeLevel * 0.01f;
    UpdateSetting(1);
    if (g_application.m_pPlayer)
