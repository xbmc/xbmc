...
void CGUIDialogHost::OnInitWindow()
{
	...
	SET_CONTROL_FOCUS(GetID(), CTL_CHECKMARK_HOSTING_TYPE, 0);
...
void CGUIDialogHost::Update() const
{
	CONTROL_ENABLE_ON_CONDITION(GetID(), CTL_LABEL_PLAYER_LIMIT,!m_bPrivate);
	CONTROL_ENABLE_ON_CONDITION(GetID(), CTL_SPIN_PLAYER_LIMIT, m_bPrivate);
	CONTROL_ENABLE_ON_CONDITION(GetID(), CTL_LABEL_PASSWORD,	!m_bPrivate);
	CONTROL_ENABLE_ON_CONDITION(GetID(), CTL_BUTTON_PASSWORD,	m_bPrivate);
	CONTROL_ENABLE_ON_CONDITION(GetID(), CTL_LABEL_DESCRIPTION,	!m_bPrivate);
	CONTROL_ENABLE_ON_CONDITION(GetID(), CTL_BUTTON_DESCRIPTION,m_bPrivate);

	SET_CONTROL_LABEL(GetID(), CTL_BUTTON_DESCRIPTION, m_strDescription);

	unsigned int len = m_strPassword.length();
	CStdString strPassword;
	for (unsigned int i = 0; i < len; i++) strPassword += '*';
	SET_CONTROL_LABEL(GetID(), CTL_BUTTON_PASSWORD, strPassword);
}
