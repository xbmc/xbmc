...
# define DB GLX_DOUBLEBUFFER
		...
		bool have_net_wm_state                = false;
		bool have_net_wm_state_maximized_vert = false;
		bool have_net_wm_state_maximized_horz = false;
		bool have_net_wm_state_fullscreen     = false;

		Atom net_supported    = XInternAtom(display, "_NET_SUPPORTED", True);
		...
		if (net_supported != None) {
			...
			if (XGetWindowProperty(display, RootWindow(display, screen),
				net_supported, 0, 8192, False, XA_ATOM, &actual_type, &actual_format,
				&count, &remaining, &supported) == Success) {
				for (unsigned long i = 0; i < count; ++i) {
					if (((Atom *)supported)[i] == net_wm_state)
						have_net_wm_state = true;
					else if (((Atom *)supported)[i] == net_wm_state_maximized_vert)
						have_net_wm_state_maximized_vert = true;
					else if (((Atom *)supported)[i] == net_wm_state_maximized_horz)
						have_net_wm_state_maximized_horz = true;
					else if (((Atom *)supported)[i] == net_wm_state_fullscreen)
						have_net_wm_state_fullscreen = true;
				}
				XFree(supported);
			}
		}

		enum { USE_RESIZE, USE_MAXIMIZE, USE_FULLSCREEN } fullScreenMethod = USE_RESIZE;
		if (have_net_wm_state) {
			if (have_net_wm_state_maximized_vert && have_net_wm_state_maximized_horz)
				fullScreenMethod = USE_MAXIMIZE;
			if (have_net_wm_state_fullscreen)
				fullScreenMethod = USE_FULLSCREEN;
		}

		if (_fullScreen) {
			if (fullScreenMethod == USE_RESIZE) {
				_w = fullWidth;
				_h = fullHeight;
				_x = _y = 0;
				_useOffset = true;
