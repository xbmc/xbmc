...
bool CGUIWindowPictures::OnClick(int iItem)
{
  ...
  if (pItem->IsCBT() || pItem->IsCBZ() || pItem->IsCBR())
  {
...
void CGUIWindowPictures::GetContextButtons(int itemNumber, CContextButtons &buttons)
{
  ...
  if (item && !item->GetPropertyBOOL("pluginreplacecontextitems"))
  {
    ...
    else
    {
      if (item)
      {
        if (!(item->m_bIsFolder || item->IsArchive() || item->IsZIP() || item->IsRAR() || item->IsCBT() || item->IsCBZ() || item->IsCBR()))
...
void CGUIWindowPictures::LoadPlayList(const CStdString& strPlayList)
{
  ...
  if (playlist.size() > 0)
  {
    ...
    for (int i = 0; i < (int)playlist.size(); ++i)
    {
      ...
      if (pItem->IsPicture() && !(pItem->IsArchive() || pItem->IsZIP() || pItem->IsRAR() || pItem->IsCBT() || pItem->IsCBZ() || pItem->IsCBR()))
        pSlideShow->Add(pItem.get());
    }
...
{
  CFileItemPtr item = (itemNumber >= 0 && itemNumber < m_vecItems->Size()) ? m_vecItems->Get(itemNumber) : CFileItemPtr();
  if (!item || item->m_bIsFolder || item->IsArchive() || item->IsZIP() || item->IsRAR() || item->IsCBT() || item->IsCBZ() || item->IsCBR() || !item->IsPicture())
    return;
