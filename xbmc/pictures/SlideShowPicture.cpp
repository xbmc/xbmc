...
void CSlideShowPic::SetTexture(int iSlideNumber, CBaseTexture* pTexture, DISPLAY_EFFECT dispEffect, TRANSISTION_EFFECT transEffect)
{
  ...
  if (m_displayEffect == EFFECT_FLOAT)
  {
    ...
    m_fPosX = cos(angle) * g_advancedSettings.PictureSettings->SlideshowPanAmount() * m_iTotalFrames * 0.00005f;
    m_fPosY = sin(angle) * g_advancedSettings.PictureSettings->SlideshowPanAmount() * m_iTotalFrames * 0.00005f;
  ...
  {
    m_fPosZ = 1.0f;
    m_fVelocityZ = 0.0001f * g_advancedSettings.PictureSettings->SlideshowZoomAmount();
  }
...
void CSlideShowPic::Process()
{
  ...
  if (!m_bNoEffect && !bPaused)
  {
    if (m_displayEffect == EFFECT_FLOAT)
    {
      ...
      float fMoveAmount = g_advancedSettings.PictureSettings->SlideshowPanAmount() * m_iTotalFrames * 0.0001f;
...
void CSlideShowPic::Render()
{
  ...
  float fComp = 1.0f + 0.01f * g_advancedSettings.PictureSettings->SlideshowBlackBarCompensation();
  ...
  if (m_displayEffect == EFFECT_FLOAT)
    fScale *= (1.0f + g_advancedSettings.PictureSettings->SlideshowPanAmount() * m_iTotalFrames * 0.0001f);
