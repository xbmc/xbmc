...
namespace
{
//! The words a filename can carry to state a stream's flags
constexpr auto ExternalStreamFlagWords = std::array{
...
{
  const auto it = std::ranges::find_if(ExternalStreamFlagWords, [&token](const auto& word)
                                       { return StringUtils::EqualsNoCase(token, word.first); });
  if (it == ExternalStreamFlagWords.end())
    return std::nullopt;
