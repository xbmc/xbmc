                                        ...
                                        std::vector<std::string>& associatedFiles)
{
  for (int i = items.Size() - 1; i >= 0; --i)
  {
        ...
        (StringUtils::StartsWithNoCase(strCandidate, CURL::Decode(videoName))))
    {
      ...
      {
        // Remove the archive itself from associated files
        items.Remove(i);
      }
    }
    else
                                                     ...
                                                     std::vector<std::string>& associatedFiles)
{
  ...
  {
    return ScanArchiveForAssociatedItems(path, videoName, item_exts, associatedFiles);
  }
  return NOT_ARCHIVE;
}
