...
bool CGUIWindowTestPattern::OnAction(const CAction &action)
{
  ...
  case ACTION_MOVE_LEFT:
     m_pattern = m_pattern > 0 ? m_pattern - 1 : NUM_PATTERNS - 1;
     break;
...
void CGUIWindowTestPattern::DrawBouncingRectangle(int top, int left, int bottom, int right)
{
  m_bounceX += m_bounceDirectionX;
  m_bounceY += m_bounceDirectionY;

  if ((m_bounceDirectionX == 1 && m_bounceX + BOUNCE_SQUARE_SIZE == right) || (m_bounceDirectionX == -1 && m_bounceX == left))
    m_bounceDirectionX = -m_bounceDirectionX;

  if ((m_bounceDirectionY == 1 && m_bounceY + BOUNCE_SQUARE_SIZE == bottom) || (m_bounceDirectionY == -1 && m_bounceY == top))
    m_bounceDirectionY = -m_bounceDirectionY;

  glColor3f(1, 1, 1);
  glRecti(m_bounceX, m_bounceY, m_bounceX + BOUNCE_SQUARE_SIZE, m_bounceY + BOUNCE_SQUARE_SIZE);
}

void CGUIWindowTestPattern::DrawContrastBrightnessPattern(int top, int left, int bottom, int right)
{
  ...
  m_blinkFrame = (m_blinkFrame + 1) % BLINK_CYCLE;
...
void CGUIWindowTestPattern::DrawCircle(int originX, int originY, int radius)
{
  float angle;
  int vectorX;
  int vectorY;
