...
bool CGUIWindowFileManager::HaveBookmarkPermissions(CFileItem* pItem, const CStdString &strType)
{
...
void CGUIWindowFileManager::OnPopupMenu(int list, int item)
{
	...
	if (m_strDirectory[list] == "")
	{
		...
		if (CGUIDialogContextMenu::BookmarksMenu("files", m_vecItems[list][item]->GetLabel(), m_vecItems[list][item]->m_strPath, m_vecItems[list][item]->m_iLockMode, !(m_vecItems[list][item]->m_iBadPwdCount < g_stSettings.m_iMasterLockMaxRetry), iPosX, iPosY))
		{
			m_rootDir.SetShares(g_settings.m_vecMyFilesShares);
