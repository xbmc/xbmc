...
bool CDirectoryTuxBox::GetDirectory(const CStdString& strPath, CFileItemList &items)
{
  ...
  strLine1.Format(g_localizeStrings.Get(20335).c_str(), g_localizeStrings.Get(20336).c_str());
  ...
  while (iTryConnect <= 1 && !dlgProgress->IsCanceled())
  {
    ...
    if(http.Open(url, false, iWaitTimer)) 
    {
      ...
      //Update Progressbar
      iProgressPercent=iProgressPercent+5;
      UpdateProgress(dlgProgress, strLine1, g_localizeStrings.Get(20336).c_str(), iProgressPercent, false);
