  ...
  if (!m_context)
  {
    ...
    if (f != NULL)
    {
      ...
      if ( g_guiSettings.GetString("smb.winsserver").length() > 0 && !g_guiSettings.GetString("smb.winsserver").Equals("0.0.0.0") ) 
      {
        ...
        fprintf(f, "  name resolve order = host wins bcast\n");
      }
      else
        fprintf(f, "name resolve order = host bcast\n");
    ...
    if (smbc_init_context(m_context))
    {
      ...
      if ( g_guiSettings.GetString("smb.winsserver").length() > 0 && !g_guiSettings.GetString("smb.winsserver").Equals("0.0.0.0") )
      {
        ...
        lp_do_parameter( -1, "name resolve order", "host wins bcast");
      }
      else 
        lp_do_parameter( -1, "name resolve order", "host bcast");
