    ...
    lt::disk_job_flags_t)
{
  ...
  post(m_ioc, [handler, error, b, this] {
    handler(lt::disk_buffer_holder(*this, const_cast<char*>(b.data()), static_cast<int>(b.size())),
            error);
  });
}
    ...
    std::function<void(lt::piece_index_t, const lt::sha1_hash&, const lt::storage_error&)> handler)
{
  ...
  const lt::sha1_hash hash = m_torrents[storage]->Hash(piece, blockHashes, error);
