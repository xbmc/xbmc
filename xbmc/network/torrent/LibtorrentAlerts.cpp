...
namespace
{
std::string PeerEndpointToString(const lt::peer_endpoint_t& peerEndpoint)
{
  if (const auto* endpoint =
          std::get_if<lt::aux::noexcept_movable<lt::tcp::endpoint>>(&peerEndpoint))
  {
  ...
  std::ostringstream ss;
  ss << std::get<lt::sha256_hash>(peerEndpoint);
  return ss.str();
}
} // namespace
...
void CLibtorrentAlerts::Initialize(std::shared_ptr<lt::session> session)
{
...
void CLibtorrentAlerts::Deinitialize()
{
  m_running.store(false);
  m_session.reset();
}
      ...
      switch (alert->type())
      {
        ...
        case lt::peer_ban_alert::alert_type:
        {
          ...
          CLog::Log(LOGDEBUG, LOGLIBTORRENT, "Peer banned: {} ({})",
                    HashToString(a->handle.info_hashes().v2), PeerEndpointToString(a->ep));
        ...
        case lt::lsd_peer_alert::alert_type:
        {
          ...
          CLog::Log(LOGDEBUG, LOGLIBTORRENT, "LSD peer: {} {}", PeerEndpointToString(a->ep),
                    a->pid.to_string());
