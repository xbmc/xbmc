...
public:
  ...
  bool WaitForMetadata(const lt::sha1_hash& infoHash);
  bool WaitForPiece(const lt::sha1_hash& infoHash, int pieceIndex);
  ...
  void OnTorrentError(const lt::sha1_hash& infoHash) override;
  void OnMetadataReceived(const lt::sha1_hash& infoHash) override;
  void OnMetadataFailed(const lt::sha1_hash& infoHash) override;
  void OnPieceFinished(const lt::sha1_hash& infoHash, int pieceIndex) override;
  void OnReadPieceFinished(const lt::sha1_hash& infoHash,
                           int pieceIndex,
                           const uint8_t* pieceBuffer,
                           size_t readSize) override;
  void OnReadPieceFailed(const lt::sha1_hash& infoHash, int pieceIndex) override;

private:
  // Private utility functions
  lt::torrent_handle AddTorrent(const lt::add_torrent_params& params, lt::error_code& ec);
  std::shared_ptr<CTorrent> GetTorrent(const lt::sha1_hash& infoHash);
  ...
  // Piece parameters
  std::map<std::pair<lt::sha1_hash, int>, std::shared_ptr<CTorrentPiece>> m_waitingPieces;
  std::mutex m_piecesMutex;

  // Read piece parameters
  std::map<std::pair<lt::sha1_hash, int>, std::vector<std::shared_ptr<CTorrentPiece>>>
      m_readingPieces;
