...
lt::torrent_handle CLibtorrent::AddTorrent(const std::string& magnetUri)
{
  ...
  else
  {
    ...
    if (errorCode)
    {
      ...
      // Dowload metadata
      const lt::sha1_hash infoHash = addTorrentParams.info_hashes.get_best();
...
bool CLibtorrent::WaitForPiece(const lt::sha1_hash& infoHash, int pieceIndex)
{
  if (!m_session)
    return false;

  // Check m_session if piece is already available
  lt::torrent_handle torrentHandle = m_session->find_torrent(infoHash);
  if (torrentHandle.have_piece(static_cast<lt::piece_index_t>(pieceIndex)))
    return true;

  const std::pair<lt::sha1_hash, int> key = std::make_pair(infoHash, pieceIndex);
...

  {
    ...
    if (it == m_waitingPieces.end())
      it =
          m_waitingPieces.emplace(key, std::make_shared<CTorrentPiece>(infoHash, pieceIndex)).first;
                               ...
                               size_t bufferSize)
{
  ...
  const lt::sha1_hash infoHash = torrentHandle.info_hash();

  // Create torrent piece with the read buffer
  const std::shared_ptr<CTorrentPiece> torrentPiece =
      std::make_shared<CTorrentPiece>(infoHash, pieceIndex);
...
void CLibtorrent::OnTorrentError(const lt::sha1_hash& infoHash)
{
  OnMetadataFailed(infoHash);
}

void CLibtorrent::OnMetadataReceived(const lt::sha1_hash& infoHash)
{
...
void CLibtorrent::OnMetadataFailed(const lt::sha1_hash& infoHash)
{
  std::lock_guard<std::mutex> lock(m_torrentsMutex);
...
void CLibtorrent::OnPieceFinished(const lt::sha1_hash& infoHash, int pieceIndex)
{
  const std::pair<lt::sha1_hash, int> key = std::make_pair(infoHash, pieceIndex);
...
void CLibtorrent::OnReadPieceFinished(const lt::sha1_hash& infoHash,
                                      int pieceIndex,
...
void CLibtorrent::OnReadPieceFailed(const lt::sha1_hash& infoHash, int pieceIndex)
{
  const std::pair<lt::sha1_hash, int> key = std::make_pair(infoHash, pieceIndex);
...
lt::torrent_handle CLibtorrent::AddTorrent(const lt::add_torrent_params& params, lt::error_code& ec)
{
  if (m_session)
  {
    ...
    std::lock_guard<std::mutex> lock(m_torrentsMutex);
    if (m_torrents.find(params.info_hashes.get_best()) == m_torrents.end())
      m_torrents.emplace(params.info_hashes.get_best(), std::make_shared<CTorrent>(torrentHandle));
...
std::shared_ptr<CTorrent> CLibtorrent::GetTorrent(const lt::sha1_hash& infoHash)
{
  std::lock_guard<std::mutex> lock(m_torrentsMutex);
