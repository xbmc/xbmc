...
lt::torrent_handle CLibtorrent::AddTorrent(const std::string& magnetUri)
{
  ...
  auto applyTorrentDefaults = [this](lt::add_torrent_params& params)
  {
    params.save_path = m_cache->GetCachePath();
    params.flags &= ~lt::torrent_flags::auto_managed;
    params.flags &= ~lt::torrent_flags::paused;
  };
  applyTorrentDefaults(addTorrentParams);
  ...
  if (errorCode)
  {
    addTorrentParams = lt::load_torrent_file(magnetUri, errorCode, {});
    if (errorCode)
    {
  ...
  else
  {
    ...
    lt::add_torrent_params cachedTorrentParams = lt::load_torrent_file(metadataPath, errorCode, {});
    ...
    if (errorCode)
    {
      ...
      addTorrentParams.ti = torrentHandle.torrent_file();
      m_cache->SaveMetadata(addTorrentParams);

      return torrentHandle;
    }

    addTorrentParams = std::move(cachedTorrentParams);
    applyTorrentDefaults(addTorrentParams);
  }
