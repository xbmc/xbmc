...
CNetworkInterfaceLinux::CNetworkInterfaceLinux(CNetworkLinux* network, string interfaceName, char interfaceMacAddrRaw[6])
...
string& CNetworkInterfaceLinux::GetName(void)
{
...
string CNetworkInterfaceLinux::GetMacAddress()
{
...
string CNetworkInterfaceLinux::GetCurrentIPAddress(void)
{
   string result = "";
...
string CNetworkInterfaceLinux::GetCurrentNetmask(void)
{
   string result = "";
...
string CNetworkInterfaceLinux::GetCurrentWirelessEssId(void)
{
   string result = "";
...
string CNetworkInterfaceLinux::GetCurrentDefaultGateway(void)
{
   string result = "";
...
void CNetworkLinux::GetMacAddress(string interfaceName, char rawMac[6])
{
  memset(rawMac, 0, 6);
#if defined(TARGET_DARWIN) || defined(TARGET_FREEBSD)
...
std::vector<string> CNetworkLinux::GetNameServers(void)
{
  ...
  if (!result.size())
   ...
   {
      string ns = inet_ntoa(((struct sockaddr_in *)&_res.nsaddr_list[i])->sin_addr);
      result.push_back(ns);
   }
#endif
   return result;
...
#if defined(TARGET_DARWIN) || defined(TARGET_FREEBSD)
bool CNetworkInterfaceLinux::GetHostMacAddress(unsigned long host_ip, string& mac)
{
  bool ret = false;
  size_t needed;
  char *buf, *next;
...
void CNetworkInterfaceLinux::GetSettings(NetworkAssignment& assignment, string& ipAddress, string& networkMask, string& defaultGateway, string& essId, string& key, EncMode& encryptionMode)
{
   ipAddress = "0.0.0.0";
   networkMask = "0.0.0.0";
...
void CNetworkInterfaceLinux::SetSettings(NetworkAssignment& assignment, string& ipAddress, string& networkMask, string& defaultGateway, string& essId, string& key, EncMode& encryptionMode)
{
#if defined(TARGET_LINUX)
...
void CNetworkInterfaceLinux::WriteSettings(FILE* fw, NetworkAssignment assignment, string& ipAddress, string& networkMask, string& defaultGateway, string& essId, string& key, EncMode& encryptionMode)
{
