...
std::vector<NetworkAccessPoint> CNetworkInterfaceLinux::GetAccessPoints(void)
{
   ...
   char* pos = (char *) res_buf;             // pointer to the current event (about 10 per wireless network)
   ...
   while (pos + IW_EV_LCP_LEN <= end)
   {
      ...
      switch (iwe->cmd)
      {
         ...
         case SIOCGIWFREQ: // This gets called twice per network, what's the difference between the two?
         {
