...
bool CPeripheralCecAdapter::InitialiseFeature(const PeripheralFeature feature)
{
  if (feature == FEATURE_CEC && !m_bStarted && GetSettingBool("enabled"))
  {
    ...
    if (m_configuration.serverVersion < CEC_LIB_SUPPORTED_VERSION)
    {
      ...
      string strMessage = StringUtils::Format(g_localizeStrings.Get(36040).c_str(), m_cecAdapter ? m_configuration.serverVersion : -1, CEC_LIB_SUPPORTED_VERSION);
...
bool CPeripheralCecAdapter::OpenConnection(void)
{
  ...
  string strMessage = StringUtils::Format(g_localizeStrings.Get(21336).c_str(), g_localizeStrings.Get(36000).c_str());
...
int CPeripheralCecAdapter::CecAlert(void *cbParam, const libcec_alert alert, const libcec_parameter data)
{
  ...
  if (iAlertString)
  {
    string strLog(g_localizeStrings.Get(iAlertString));
...
void CPeripheralCecAdapter::OnSettingChanged(const string &strChangedSetting)
{
...
void CPeripheralCecAdapter::SetConfigurationFromSettings(void)
{
  ...
  string strPhysicalAddress = GetSettingString("physical_address");
  ...
  string strWakeDevices = GetSettingString("wake_devices_advanced");
  ...
  string strStandbyDevices = GetSettingString("standby_devices_advanced");
...
void CPeripheralCecAdapter::ReadLogicalAddresses(const string &strString, cec_logical_addresses &addresses)
{
  for (size_t iPtr = 0; iPtr < strString.size(); iPtr++)
  {
    string strDevice = strString.substr(iPtr, 1);
...
string CPeripheralCecAdapterUpdateThread::UpdateAudioSystemStatus(void)
{
  string strAmpName;
...
bool CPeripheralCecAdapterUpdateThread::SetInitialConfiguration(void)
{
  ...
  string strAmpName = UpdateAudioSystemStatus();
  if (!strAmpName.empty())
