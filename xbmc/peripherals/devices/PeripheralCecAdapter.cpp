...
CPeripheralCecAdapter::CPeripheralCecAdapter(CPeripherals& manager,
                                             ...
                                             CPeripheralBus* bus)
  : CPeripheralHID(manager, scanResult, bus), CThread("CECAdapter"), m_cecAdapter(NULL), m_cecCommandState(CecCommandState::PROCESSED)
{
  ResetMembers();
                                              ...
                                              bool forceType /*= false */)
{
  ...
  else if (cecStatus == CecPowerState::STAND_BY && (mode == STATE_SWITCH_TOGGLE || mode == STATE_ACTIVATE_SOURCE))
  {
...
CPeripheralCecAdapter::CecPowerState CPeripheralCecAdapter::GetConnectedDevicesPowerState() const
{
  ...
  for (uint8_t i = 0 ; i < 16; ++i)
  {
    ...
    {
      const cec_power_status devicePowerStatus = m_cecAdapter->GetDevicePowerStatus(static_cast<cec_logical_address>(i));
      switch (devicePowerStatus) {
        case CEC::CEC_POWER_STATUS_ON: ++activeDevices; break;
        case CEC::CEC_POWER_STATUS_STANDBY: ++standingByDevices; break;
        case CEC::CEC_POWER_STATUS_IN_TRANSITION_ON_TO_STANDBY:
        case CEC::CEC_POWER_STATUS_IN_TRANSITION_STANDBY_TO_ON: hasChangingPowerDevices = true; break;
        default: break;
      }
    }
  }
