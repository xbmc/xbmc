...
int SqliteDatabase::connect(bool create) {
  ...
  //CLog::Log(LOGDEBUG, "Connecting to sqlite:{}:{}", host.c_str(), db.c_str());
  ...
  try
  {
    ...
    {
      CLog::Log(LOGFATAL, "SqliteDatabase: can't open {}", db_fullpath.c_str());
    ...
    else if (errorCode == SQLITE_OK)
    {
      ...
      {
        CLog::Log(LOGFATAL, "SqliteDatabase: {} is read only", db_fullpath.c_str());
...
int SqliteDatabase::copy(const char *backup_name) {
  ...
  CLog::Log(LOGDEBUG, "Copying from {} to {} at {}", db.c_str(), backup_name, host.c_str());
...
int SqliteDatabase::drop_analytics(void) {
  ...
  CLog::Log(LOGDEBUG, "Cleaning indexes from database {} at {}", db.c_str(), host.c_str());
  ...
  CLog::Log(LOGDEBUG, "Cleaning views from database {} at {}", db.c_str(), host.c_str());
  ...
  CLog::Log(LOGDEBUG, "Cleaning triggers from database {} at {}", db.c_str(), host.c_str());
