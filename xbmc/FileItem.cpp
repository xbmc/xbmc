...
std::string CFileItem::GetMovieName(bool bUseFolderNames /* = false */) const
{
  ...
  if (URIUtils::IsStack(m_strPath))
  {
    ...
    if (URIUtils::GetFileName(strMovieName).empty())
    {
      CFileItem item(strMovieName, true);
      return item.GetMovieName(true);
    }
    else
    {
      CFileItem item(strMovieName, false);
      return item.GetMovieName();
    }
  }
  else
...
std::string CFileItem::GetBaseMoviePath(bool bUseFolderNames) const
{
  ...
  if (URIUtils::IsInArchive(strMovieName) || URIUtils::IsArchive(url))
  {
    ...
    if (name.empty())
      name = strMovieName;
    if (bUseFolderNames && !URIUtils::GetParentPath(name, strMovieName))
      strMovieName = name;
    else
      strMovieName = name;
  }
