...
CFileItem& CFileItem::operator=(const CFileItem& item)
{
  ...
  m_allEpisodes = item.m_allEpisodes;
  return *this;
}
                           ...
                           bool replaceEpisodes /* false */)
{
  ...
  if (replaceLabels)
  {
        ...
        (item.IsDiscImage() || item.IsDVDFile() || item.IsBluray()))
    {
      ...
      {
        bool specials = false;
        const int startEpisode = (*std::min_element(episodes.begin(), episodes.end(),
                                                    [&](CVideoInfoTag& i, CVideoInfoTag& j)
                                                    {
                                     ...
                                     .m_iEpisode;
        const int endEpisode = (*std::max_element(episodes.begin(), episodes.end(),
                                                  [&](CVideoInfoTag& i, CVideoInfoTag& j)
                                                  {
                                                    if (i.m_iSeason == 0 && j.m_iSeason != 0)
                                                      return true;
        ...
        if (std::count_if(episodes.begin(), episodes.end(),
                          [&](CVideoInfoTag& i) { return i.m_iSeason == 0; }) > 0)
          specials = true;
        ...
        else
        {
          ...
          CFileItemPtr match = dbItems.Get(details.m_basePath);
