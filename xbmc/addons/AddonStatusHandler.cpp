...
void CAddonStatusHandler::Process()
{
  ...
  std::string heading = StringUtils::Format("%s: %s", AddonProps::TranslateType(m_addon->Type(), true).c_str(), m_addon->Name().c_str());
  ...
  else if ((m_status == ADDON_STATUS_NEED_SETTINGS) || (m_status == ADDON_STATUS_NEED_SAVEDSETTINGS))
  {
    ...
    AddonPropsPtr addonProps = CAddonMgr::GetInstance().GetInstalledAddonInfo(m_addon->ID());
    if (CGUIDialogAddonSettings::ShowAndGetInput(addonProps))
    {
