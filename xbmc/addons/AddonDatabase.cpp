...
bool CAddonDatabase::FindByAddonId(const std::string& addonId, ADDON::AddonInfos& result)
{
  try
  {
    if (nullptr == m_pDB.get())
...
bool CAddonDatabase::GetAddonInfo(const std::string& addonID, const AddonVersion& version, const std::string& repoId, AddonInfoPtr& info)
{
  try
  {
    ...
    return GetAddonInfo(m_pDS->fv(0).get_asInt(), info);
  }
  catch (...)
  {
    CLog::Log(LOGERROR, "%s failed on addon %s", __FUNCTION__, addonID.c_str());
...
bool CAddonDatabase::GetAddonInfo(const std::string& id, AddonInfoPtr& info)
{
  try
  {
    ...
    return GetAddonInfo(maxid, info);
  }
  catch (...)
  {
    CLog::Log(LOGERROR, "%s failed on addon %s", __FUNCTION__, id.c_str());
...
bool CAddonDatabase::GetAddonInfo(int id, AddonInfoPtr &info)
{
  try
  {
    ...
    AddonInfoPtr usedInfo = std::make_shared<CAddonInfo>(
                                ...
                                m_pDS2->fv(5).get_asString(),  /* summary */
                                m_pDS2->fv(6).get_asString(),  /* description */
                                m_pDS2->fv(1).get_asString(),  /* metadata */
                                "",                            /* changelog */
                                "");                           /* origin */
    if (usedInfo->IsUsable())
    {
      info = std::move(usedInfo);
      return true;
    }
    else
      CLog::Log(LOGERROR, "CAddonDatabase: failed to build %s", usedInfo->ID().c_str());
  }
  catch (...)
  {
...
bool CAddonDatabase::GetRepositoryContent(AddonInfos& addons)
{
  return GetRepositoryContent("", addons);
}

bool CAddonDatabase::GetRepositoryContent(const std::string& id, AddonInfos& addons)
{
  try
  {
    if (nullptr == m_pDB.get())
