...
//! \brief The ratio a layout's aspect attribute declares: a plain ratio canonicalised through
//! the vocabulary, or a deprecated width:height fraction. Zero when nothing usable is stated.
                                 ...
                                 const std::string& skinId)
{
  ...
  if (strAspect.find(':') != std::string::npos)
  {
    ...
    // Warned about only where it changes what the layout does; a fraction agreeing with its own
    // pixels is a no-op either way, and is what every shipped skin declares.
    const bool matchesPixels =
        height > 0 && std::fabs(aspect - (static_cast<float>(width) / height)) < 0.005f;

    CLog::Log(matchesPixels ? LOGDEBUG : LOGWARNING,
  ...
  else if (!strAspect.empty())
  {
    ...
    // A declaration naming a ratio Kodi knows resolves to that entry's own value, so a layout,
    // a stated screen shape and a measurement cannot each mean a different "1.78". One outside
    // the vocabulary stands as declared.
...
CSkinInfo::CSkinInfo(const AddonInfoPtr& addonInfo) : CAddon(addonInfo, AddonType::SKIN)
{
  ...
  {
    // What to draw in the screen area the layout leaves uncovered when it is held at its own
    // ratio.
    if (name == "surround")
    {
...
void CSkinInfo::Start()
{
  ...
  if (!m_resolutions.empty())
  {
    // find the closest resolution to the raster, which is the area being drawn into
                                   ...
                                   std::string_view strBaseDir /* = "" */) const
{
  ...
  // find the closest resolution to the raster, the same target Start() selects against
