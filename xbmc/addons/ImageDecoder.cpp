...
CImageDecoder::CImageDecoder(ADDON::AddonInfoPtr addonInfo)
  : CAddonDll(addonInfo)
{
  m_mimetype = Type(ADDON_IMAGEDECODER)->GetValue("@mimetype").asString();
  m_extension = Type(ADDON_IMAGEDECODER)->GetValue("@extension").asString();
}
...
bool CImageDecoder::Create(const std::string& mimetype)
{
  ...
  return CAddonDll::Create(ADDON_IMAGEDECODER, &m_struct, &m_info) == ADDON_STATUS_OK;
}
