...
bool CAddonInstallJob::DoWork()
{
  ...
  {
    std::string details = StringUtils::Format(
        CServiceBroker::GetResourcesComponent()->GetLocalizeStrings().Get(24142), failedDep.first,
        failedDep.second);
  ...
  else if (m_addon->LifecycleState() == AddonLifecycleState::DEPRECATED)
  {
    ...
    std::string text = StringUtils::Format(
        CServiceBroker::GetResourcesComponent()->GetLocalizeStrings().Get(24168),
