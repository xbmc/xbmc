...
std::unique_ptr<CPluginSource> CPluginSource::FromExtension(CAddonInfo addonInfo, const cp_extension_t* ext)
{
  ...
  if (!provides.empty())
    addonInfo.m_extrainfo.insert(make_pair("provides", provides));
...
{
  std::string provides;
  InfoMap::const_iterator i = m_addonInfo.m_extrainfo.find("provides");
  if (i != m_addonInfo.m_extrainfo.end())
    provides = i->second;
