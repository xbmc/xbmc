    ...
    const std::vector<std::pair<CAddonVersion, std::string>>& versions) const
{
  ...
  {
    CFileItem item(StringUtils::Format(
        CServiceBroker::GetResourcesComponent()->GetLocalizeStrings().Get(21339),
        version.asString()));
...
void CGUIDialogAddonInfo::OnInstall()
{
          ...
          AddonRepoUpdateMode::ANY_REPOSITORY)
  {
    ...
    const std::string text = StringUtils::Format(
        CServiceBroker::GetResourcesComponent()->GetLocalizeStrings().Get(39028),
...
bool CGUIDialogAddonInfo::PromptIfDependency(int heading, int line2) const
{
  ...
  if (!deps.empty())
  {
    std::string line0 = StringUtils::Format(
        CServiceBroker::GetResourcesComponent()->GetLocalizeStrings().Get(24046),
        m_localAddon->Name());
