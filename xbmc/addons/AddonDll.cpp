...
bool CAddonDll::LoadDll()
{
  ...
  else
  {
    ...
    if (!XFILE::CFile::Exists(libPath))
    {
      ...
      if (!XFILE::CFile::Exists(libPath))
      {
        CLog::Log(LOGERROR, "ADDON: Could not locate %s", m_addonInfo.MainLibName().c_str());
        return false;
      }
    }
  ...
  if (!XFILE::CFile::Exists(strFileName))
  {
    ...
    strFileName = tempbin + "/" + m_addonInfo.MainLibName();
  }
#endif
  if (!XFILE::CFile::Exists(strFileName))
  {
    ...
    if (!altbin.empty())
    {
      strAltFileName = altbin + m_addonInfo.MainLibName();
