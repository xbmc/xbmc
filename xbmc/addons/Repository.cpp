  ...
  : CAddon(ext)
{
  ...
  if (ext)
  {
    ...
    for (size_t i = 0; i < ext->configuration->num_children; ++i)
    {
         ...
         strcmp(ext->configuration->children[i].name, "dir") == 0)
      {
        ...
        if (min_version <= version)
        {
          ...
          dir.compressed = StringUtils::EqualsNoCase(CAddonMgr::Get().GetExtValue(&ext->configuration->children[i], "info@compressed"), "true");
          ...
          dir.zipped     = StringUtils::EqualsNoCase(CAddonMgr::Get().GetExtValue(&ext->configuration->children[i], "datadir@zip"), "true");
          dir.hashes     = StringUtils::EqualsNoCase(CAddonMgr::Get().GetExtValue(&ext->configuration->children[i], "hashes"), "true");
    ...
    if (!CAddonMgr::Get().GetExtValue(ext->configuration, "info").empty())
    {
      ...
      info.compressed = StringUtils::EqualsNoCase(CAddonMgr::Get().GetExtValue(ext->configuration, "info@compressed"), "true");
      ...
      info.zipped     = StringUtils::EqualsNoCase(CAddonMgr::Get().GetExtValue(ext->configuration, "datadir@zip"), "true");
      info.hashes     = StringUtils::EqualsNoCase(CAddonMgr::Get().GetExtValue(ext->configuration, "hashes"), "true");
