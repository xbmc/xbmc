                             ...
                             std::vector<AddonInfoPtr>& addons) noexcept
{
  ...
  {
    CLog::Log(LOGERROR, "CRepository: failed to read {}", repo.info.c_str());
    return false;
  }
  ...
  {
    CLog::Log(LOGDEBUG, "CRepository '{}' is gzip. decompressing", repo.info.c_str());
    std::string buffer;
    if (!CZipFile::DecompressGzip(response, buffer))
    {
      CLog::Log(LOGERROR, "CRepository: failed to decompress gzip from '{}'", repo.info.c_str());
      return false;
    }
  ...
  for (const auto& dir : m_dirs)
  {
    if (!dir.checksum.empty())
    {
      ...
      if (!FetchChecksum(dir.checksum, part, recheckAfterThisDir))
      {
        ...
        CLog::Log(LOGERROR, "CRepository: failed read '{}'", dir.checksum.c_str());
        return STATUS_ERROR;
      }
...
{
  CLog::Log(LOGDEBUG, "CRepositoryUpdateJob[{}] checking for updates.", m_repo->ID().c_str());
  ...
  {
    CLog::Log(LOGDEBUG, "CRepositoryUpdateJob[{}] checksum not changed.", m_repo->ID().c_str());
    return true;
  }
    ...
    for (const auto& addon : addons)
    {
      ...
      if (database.GetAddon(addon->ID(), oldAddon) && addon->Version() > oldAddon->Version())
      {
        if (!oldAddon->Icon().empty() || !oldAddon->Art().empty() || !oldAddon->Screenshots().empty())
          CLog::Log(LOGDEBUG, "CRepository: invalidating cached art for '{}'", addon->ID().c_str());
