...
#include "kodi/api2/player/InfoTagVideo.hpp"
...
namespace Player
{
  ...
  CInfoTagVideo::CInfoTagVideo(CPlayer* player)
  {
    ...
    if (g_interProcess.AddonInfoTagVideo_GetFromPlayer(player->m_ControlHandle, &infoTag))
    {
      TransferInfoTag(infoTag);
      g_interProcess.AddonInfoTagVideo_Release(&infoTag);
    }
  }
