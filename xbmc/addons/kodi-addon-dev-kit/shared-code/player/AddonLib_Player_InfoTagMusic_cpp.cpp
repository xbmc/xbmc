...
#include "kodi/api2/player/InfoTagMusic.hpp"
...
namespace Player
{
  ...
  CInfoTagMusic::CInfoTagMusic(CPlayer* player)
  {
    ...
    if (g_interProcess.AddonInfoTagMusic_GetFromPlayer(player->m_ControlHandle, &infoTag))
    {
      TransferInfoTag(infoTag);
      g_interProcess.AddonInfoTagMusic_Release(&infoTag);
    }
  }
