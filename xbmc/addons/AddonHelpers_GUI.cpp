...
bool CGUIAddonWindow::LoadXML(const CStdString &strPath, const CStdString &strLowerPath)
{
  // load our window
  XFILE::CFile file;
  if (!file.Open(strPath) && !file.Open(CStdString(strPath).ToLower()) && !file.Open(strLowerPath))
  {
    ...
    CLog::Log(LOGERROR, "Window_Show: %s/%s - Unable to load skin file %s", TranslateType(m_addon->Type()).c_str(), m_addon->Name().c_str(), strPath.c_str());
    return false;
  }
  // load the strings in
  unsigned int offset = LoadAddonStrings();
...
unsigned int CGUIAddonWindow::LoadAddonStrings()
{
  // Path where the language strings reside
  CStdString pathToLanguageFile = m_addon->Path();
  CStdString pathToFallbackLanguageFile = m_addon->Path();
  ...
  return g_localizeStrings.LoadBlock(m_addon->Path(), pathToLanguageFile, pathToFallbackLanguageFile);
}
