    ...
    std::vector<CContextMenuItem>& items)
{
  ...
  if (StringUtils::IsNaturalNumber(menuLabel))
    menuLabel = g_localizeStrings.GetAddonString(addonInfo.m_id, atoi(menuLabel.c_str()));

  if (menuId.empty())
  {
    //anonymous group. create a new unique internal id.
    std::stringstream ss;
    ss << addonInfo.m_id << ++anonGroupCount;
    menuId = ss.str();
  }

  items.push_back(CContextMenuItem::CreateGroup(menuLabel, parent, menuId, addonInfo.m_id));
  ...
  if (CAddonMgr::GetInstance().GetExtElements(elem, "item", elems))
  {
    for (const auto& elem : elems)
    {
      ...
      if (StringUtils::IsNaturalNumber(label))
        label = g_localizeStrings.GetAddonString(addonInfo.m_id, atoi(label.c_str()));

      if (!label.empty() && !library.empty() && !visCondition.empty())
      {
        auto menu = CContextMenuItem::CreateItem(label, menuId,
            URIUtils::AddFileToFolder(addonInfo.m_path, library), visCondition, addonInfo.m_id);
  ...
  else
  {
    ...
    if (CAddonMgr::GetInstance().GetExtElements(ext->configuration, "item", elems))
    {
      ...
      CContextMenuItem menuItem = CContextMenuItem::CreateItem(label, parent,
          URIUtils::AddFileToFolder(addonInfo.m_path, addonInfo.m_libname), visCondition, addonInfo.m_id);
