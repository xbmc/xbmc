...
GUIHANDLE CAddonCallbacksGUI::Window_New(void *addonData, const char *xmlFilename, const char *defaultSkin, bool forceFallback, bool asDialog)
{
  ...
  if (gui->GetWindowManager().GetWindow(WINDOW_ADDON_END))
  {
    Unlock();
  ...
  while(id < WINDOW_ADDON_END && gui->GetWindowManager().GetWindow(id) != NULL) id++;
  Unlock();
  ...
  Lock();
  gui->GetWindowManager().Add(window);
  Unlock();
...
void CAddonCallbacksGUI::Window_Delete(void *addonData, GUIHANDLE handle)
{
  ...
  if (gui->GetWindowManager().GetActiveWindow() == pAddonWindow->m_iWindowId && !g_application.m_bStop)
  {
    if(gui->GetWindowManager().GetWindow(pAddonWindow->m_iOldWindowId))
      gui->GetWindowManager().ActivateWindow(pAddonWindow->m_iOldWindowId);
  ...
  gui->GetWindowManager().Remove(pAddonWindow->GetID());
  delete pAddonWindow;
  Unlock();
}
...
bool CAddonCallbacksGUI::Window_Show(void *addonData, GUIHANDLE handle)
{
  ...
  if (pAddonWindow->IsDialog())
    ...
    gui->GetWindowManager().ActivateWindow(pAddonWindow->m_iWindowId);
  Unlock();
...
bool CAddonCallbacksGUI::Window_DoModal(void *addonData, GUIHANDLE handle)
{
  ...
  if (pAddonWindow->m_iWindowId != gui->GetWindowManager().GetActiveWindow())
...
void CAddonCallbacksGUI::Dialog_TextViewer(const char *heading, const char *text)
{
  ...
  CGUIDialogTextViewer* pDialog = gui->GetWindowManager().GetWindow<CGUIDialogTextViewer>(WINDOW_DIALOG_TEXT_VIEWER);
...
int CAddonCallbacksGUI::Dialog_Select(const char *heading, const char *entries[], unsigned int size, int selected)
{
  ...
  CGUIDialogSelect* pDialog = gui->GetWindowManager().GetWindow<CGUIDialogSelect>(WINDOW_DIALOG_SELECT);
  pDialog->Reset();
