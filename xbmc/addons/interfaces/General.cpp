...
char* Interface_General::get_addon_info(void* kodiBase, const char* id)
{
  ...
  {
    CLog::Log(LOGERROR, "Interface_General::%s - invalid data (addon='%p', id='%p')", __FUNCTION__, static_cast<void*>(addon), id);
    return nullptr;
  }
...
bool Interface_General::open_settings_dialog(void* kodiBase)
{
  ...
  if (addon == nullptr)
  {
    CLog::Log(LOGERROR, "Interface_General::%s - invalid data (addon='%p')", __FUNCTION__, static_cast<void*>(addon));
    return false;
  }
...
char* Interface_General::unknown_to_utf8(void* kodiBase, const char* source, bool* ret, bool failOnBadChar)
{
  ...
  if (!addon || !source || !ret)
  {
    CLog::Log(LOGERROR, "Interface_General::%s - invalid data (addon='%p', source='%p', ret='%p')", __FUNCTION__, static_cast<void*>(addon), source, static_cast<void*>(ret));
    return nullptr;
  }
                                           ...
                                           unsigned int messageTime)
{
  ...
  {
    CLog::Log(LOGERROR, "Interface_General::%s - invalid data (addon='%p', message='%p')", __FUNCTION__, static_cast<void*>(addon), message);
    return false;
  }
...
void Interface_General::get_md5(void* kodiBase, const char* text, char* md5)
{
  ...
  {
    CLog::Log(LOGERROR, "Interface_General::%s - invalid data (addon='%p', text='%p')", __FUNCTION__, static_cast<void*>(addon), text);
    return;
  }
...
void Interface_General::get_free_mem(void* kodiBase, long* free, long* total, bool as_bytes)
{
  ...
  if (addon == nullptr || free == nullptr || total == nullptr)
  {
    CLog::Log(LOGERROR, "Interface_General::%s - invalid data (addon='%p', free='%p', total='%p')",
      __FUNCTION__, static_cast<void*>(addon), static_cast<void*>(free), static_cast<void*>(total));
    return;
  }
...
void Interface_General::kodi_version(void* kodiBase, char** compile_name, int* major, int* minor, char** revision, char** tag, char** tagversion)
{
     ...
     revision == nullptr || tag == nullptr || tagversion == nullptr)
  {
    CLog::Log(LOGERROR, "Interface_General::%s - invalid data (addon='%p', compile_name='%p', major='%p', minor='%p', revision='%p', tag='%p', tagversion='%p')",
      __FUNCTION__, static_cast<void*>(addon), static_cast<void*>(compile_name), static_cast<void*>(major), static_cast<void*>(minor), static_cast<void*>(revision),
      static_cast<void*>(tag), static_cast<void*>(tagversion));
    return;
  }
