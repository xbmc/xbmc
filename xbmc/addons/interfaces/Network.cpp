...
bool Interface_Network::wake_on_lan(void* kodiBase, const char* mac)
{
  ...
  {
    CLog::Log(LOGERROR, "Interface_Network::%s - invalid data (addon='%p', mac='%p')", __FUNCTION__, static_cast<void*>(addon), mac);
    return false;
  }
...
char* Interface_Network::get_ip_address(void* kodiBase)
{
  ...
  if (addon == nullptr)
  {
    CLog::Log(LOGERROR, "Interface_Network::%s - invalid data (addon='%p')", __FUNCTION__, static_cast<void*>(addon));
    return nullptr;
  }
...
char* Interface_Network::dns_lookup(void* kodiBase, const char* url, bool* ret)
{
  ...
  {
    CLog::Log(LOGERROR, "Interface_Network::%s - invalid data (addon='%p', url='%p', ret='%p')", __FUNCTION__, static_cast<void*>(addon), url, static_cast<void*>(ret));
    return nullptr;
  }
...
char* Interface_Network::url_encode(void* kodiBase, const char* url)
{
  ...
  {
    CLog::Log(LOGERROR, "Interface_Network::%s - invalid data (addon='%p', url='%p')", __FUNCTION__, static_cast<void*>(addon), url);
    return nullptr;
  }
