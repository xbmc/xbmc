...
void CServiceAddonManager::Start(const AddonPtr& addon)
{
  ...
  {
    CLog::Log(LOGDEBUG, "CServiceAddonManager: {} already started.", addon->ID().c_str());
    return;
  }

  if (StringUtils::EndsWith(addon->LibPath(), ".py"))
  {
    CLog::Log(LOGDEBUG, "CServiceAddonManager: starting {}", addon->ID().c_str());
    auto handle = CScriptInvocationManager::GetInstance().ExecuteAsync(addon->LibPath(), addon);
    if (handle == -1)
    {
      CLog::Log(LOGERROR, "CServiceAddonManager: {} failed to start", addon->ID().c_str());
      return;
    }
...
{
  CLog::Log(LOGDEBUG, "CServiceAddonManager: stopping {}.", service.first.c_str());
