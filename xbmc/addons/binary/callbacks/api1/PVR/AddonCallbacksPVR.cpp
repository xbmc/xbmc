...
void CAddonCallbacksPVR::PVRTransferEpgEntry(void *addonData, const ADDON_HANDLE handle, const EPG_TAG *epgentry)
{
  ...
  /* transfer this entry to the epg and force use of API level 2 (no structure change, only position)*/
  xbmcEpg->UpdateEntry((V2::KodiAPI::EPG_TAG*)epgentry, handle->dataIdentifier == 1 /* update db */);
}
