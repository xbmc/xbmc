...
void CAddOnControl_Label::Init(::V2::KodiAPI::CB_AddOnLib *callbacks)
{
  callbacks->GUI.Control.Label.SetVisible = CAddOnControl_Label::SetVisible;
  callbacks->GUI.Control.Label.SetLabel   = CAddOnControl_Label::SetLabel;
  callbacks->GUI.Control.Label.GetLabel   = CAddOnControl_Label::GetLabel;
}

void CAddOnControl_Label::SetVisible(void *addonData, GUIHANDLE handle, bool visible)
{
  try
  {
    if (!handle)
...
void CAddOnControl_Label::SetLabel(void *addonData, GUIHANDLE handle, const char *label)
{
  try
  {
    if (!handle)
...
void CAddOnControl_Label::GetLabel(void *addonData, GUIHANDLE handle, char &label, unsigned int &iMaxStringSize)
{
  try
  {
    if (!handle)
