...
#include "AudioEngineCB_General.h"
...
CAddOnAEGeneral::CAddOnAEGeneral()
{

}

void CAddOnAEGeneral::Init(CB_AddOnLib *callbacks)
{
  callbacks->AudioEngine.add_dsp_menu_hook        = CAddOnAEGeneral::add_dsp_menu_hook;
  callbacks->AudioEngine.remove_dsp_menu_hook     = CAddOnAEGeneral::remove_dsp_menu_hook;

  callbacks->AudioEngine.register_dsp_mode        = CAddOnAEGeneral::register_dsp_mode;
  callbacks->AudioEngine.unregister_dsp_Mode      = CAddOnAEGeneral::unregister_dsp_mode;

  callbacks->AudioEngine.get_current_sink_format  = CAddOnAEGeneral::get_current_sink_format;

  callbacks->AudioEngine.make_stream              = CAddOnAEGeneral::make_stream;
  callbacks->AudioEngine.free_stream              = CAddOnAEGeneral::free_stream;
}
...
CActiveAEDSPAddon *CAddOnAEGeneral::GetAudioDSPAddon(void *hdl)
{
  try
  {
    ...
    if (addon && addon->AudioEngineLib_GetHelper())
      return dynamic_cast<CActiveAEDSPAddon*>(static_cast<CAddonCallbacksAddon*>(addon->AddOnLib_GetHelper())->GetAddon());
...
/*\_____________________________________________________________________________
\*/
void CAddOnAEGeneral::register_dsp_mode(void* hdl, AE_DSP_MODES::AE_DSP_MODE* mode)
{
  try
  {
    CActiveAEDSPAddon *addon = GetAudioDSPAddon(hdl);
    ...
    {
      CLog::Log(LOGDEBUG, "CAddOnAEGeneral - %s - successfull registered mode %s of %s adsp-addon", __FUNCTION__, mode->strModeName, addon->Name().c_str());
    }
    else
    {
      CLog::Log(LOGERROR, "CAddOnAEGeneral - %s - failed to register mode %s of %s adsp-addon", __FUNCTION__, mode->strModeName, addon->Name().c_str());
    }
  }
  HANDLE_ADDON_EXCEPTION
}

void CAddOnAEGeneral::unregister_dsp_mode(void* hdl, AE_DSP_MODES::AE_DSP_MODE* mode)
{
  try
  {
    CActiveAEDSPAddon *addon = GetAudioDSPAddon(hdl);
...
AEStreamHandle* CAddOnAEGeneral::make_stream(void* hdl, AudioEngineFormat Format, unsigned int Options)
{
  try
  {
...
void CAddOnAEGeneral::free_stream(void* hdl, AEStreamHandle *StreamHandle)
{
  try
  {
    if (!StreamHandle)
