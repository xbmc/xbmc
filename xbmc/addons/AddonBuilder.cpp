...
std::shared_ptr<CAddon> CAddonBuilder::Build()
{
   ...
   */
  //AddonInfo props(m_addonInfo.Path());
...
std::shared_ptr<CAddon> CAddonBuilder::Build2()
{
  ...
  fprintf(stderr, "Build2() - m_addonInfo.path: %s\n", m_addonInfo.Path().c_str());
  /*
   * Startup of reworked addon interfaces, switch currently two times until
   * rework is finished. On end is only for all binary addons one
   * "return std::make_shared<CAddonDll>(std::move(m_addonInfo));" needed.
   ...
   * becomes later done on other place
   */
  CAddonInfo props(m_addonInfo.Path());
...
AddonPtr CAddonBuilder::FromProps(CAddonInfo addonProps)
{
  fprintf(stderr, "Build() - m_addonInfo.path: %s\n", addonProps.Path().c_str());
