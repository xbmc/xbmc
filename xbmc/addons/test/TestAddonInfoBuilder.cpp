...
{
  AddonInfoPtr addon = CAddonInfoBuilder::Generate("", AddonType::ADDON_UNKNOWN);
  EXPECT_EQ(nullptr, addon);
}

TEST_F(TestAddonInfoBuilder, TestGenerate_Id_Type)
{
  AddonInfoPtr addon = CAddonInfoBuilder::Generate("foo.baz", AddonType::ADDON_VIZ);
  EXPECT_NE(nullptr, addon);
  EXPECT_EQ(addon->ID(), "foo.baz");
  EXPECT_EQ(addon->MainType(), AddonType::ADDON_VIZ);
  EXPECT_TRUE(addon->HasType(AddonType::ADDON_VIZ));
  EXPECT_FALSE(addon->HasType(AddonType::ADDON_SCREENSAVER));
}
...
TEST_F(TestAddonInfoBuilder, TestGenerate_Repo)
{
  ...
  EXPECT_EQ(addon->MainType(), AddonType::ADDON_SCRAPER_MOVIES);
  EXPECT_TRUE(addon->HasType(AddonType::ADDON_SCRAPER_MOVIES));
  EXPECT_EQ(addon->Type(AddonType::ADDON_SCRAPER_MOVIES)->LibName(), "blablabla.xml");
  EXPECT_EQ(addon->Type(AddonType::ADDON_SCRAPER_MOVIES)->GetValue("@language").asString(), "en");

  EXPECT_TRUE(addon->HasType(AddonType::ADDON_SCRIPT_MODULE));
  EXPECT_EQ(addon->Type(AddonType::ADDON_SCRIPT_MODULE)->LibName(), "lib.so");
  EXPECT_FALSE(addon->HasType(AddonType::ADDON_SCRAPER_ARTISTS));
...
TEST_F(TestAddonInfoBuilder, TestGenerate_DBEntry)
{
  ...
  EXPECT_EQ(addon->MainType(), AddonType::ADDON_PLUGIN);
  EXPECT_TRUE(addon->HasType(AddonType::ADDON_PLUGIN));
  EXPECT_TRUE(addon->HasType(AddonType::ADDON_VIDEO));
  EXPECT_TRUE(addon->HasType(AddonType::ADDON_AUDIO));
  EXPECT_FALSE(addon->HasType(AddonType::ADDON_GAME));
