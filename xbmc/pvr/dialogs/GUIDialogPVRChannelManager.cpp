...
bool CGUIDialogPVRChannelManager::OnContextButton(int itemNumber, CONTEXT_BUTTON button)
{
  ...
  else if (button == CONTEXT_BUTTON_DELETE)
  {
    ...
    if (pDialog->IsConfirmed())
    {
      if (g_PVRClients->DeleteChannel(*pItem->GetPVRChannelInfoTag()))
      {
...
void CGUIDialogPVRChannelManager::Update()
{
  ...
  CGUIButtonControl *btnShowNewChannel = (CGUIButtonControl*) GetControl(BUTTON_NEW_CHANNEL);
  if (btnShowNewChannel)
    btnShowNewChannel->SetVisible(m_bAllowNewChannel);
