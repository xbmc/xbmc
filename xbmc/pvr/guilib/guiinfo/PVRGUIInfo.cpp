                                            ...
                                            std::string& strValue) const
{
  ...
  if (timer)
  {
    switch (info.m_info)
    {
      ...
      {
        strValue = CServiceBroker::GetPVRManager().Clients()->GetCreatedClient(timer->ClientID())->GetInstanceName();
        return true;
      }
      default:
        return false;
  ...
  if (recording)
  {
    ...
    switch (info.m_info)
    {
      ...
      case LISTITEM_BACKEND_INSTANCE_NAME:
      {
        strValue = CServiceBroker::GetPVRManager().Clients()->GetCreatedClient(recording->ClientID())->GetInstanceName();
        return true;
      }
    }
    return false;
  ...
  if (epgTag)
  {
    switch (info.m_info)
    {
      ...
      case LISTITEM_BACKEND_INSTANCE_NAME:
      {
        strValue = CServiceBroker::GetPVRManager().Clients()->GetCreatedClient(epgTag->ClientID())->GetInstanceName();
        return true;
      }
      case VIDEOPLAYER_PREMIERED:
      case LISTITEM_PREMIERED:
