...
bool CDirectoryProvider::Update(bool refresh)
{
  ...
  // update our the URL and fire off a new job if needed
  ...
  if (value != m_currentUrl)
  {
    ...
    if (refresh)
    { // if refreshing, read the directory synchronously
      CDirectoryJob job(m_currentUrl, m_parentID);
      if (job.DoWork())
      {
        m_items = job.GetItems();
        m_currentTarget = job.GetTarget();
        changed = true;
      }
    }
    else
...
bool CDirectoryProvider::OnClick(const CGUIListItemPtr &item)
{
  ...
  if (!execute.empty())
  {
    CApplicationMessenger::Get().ExecBuiltIn(execute);
    return true;
  }
  return false;
}
