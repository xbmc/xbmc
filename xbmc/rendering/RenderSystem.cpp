...
void CRenderSystemBase::OnAdvancedSettingsLoaded()
{
  ...
  m_guiGeometryClear =
      advSettings->m_guiGeometryClear ? CLEAR_FUNCTION::GEOMETRY : CLEAR_FUNCTION::FIXED_FUNCTION;
}
...
CRenderSystemBase::~CRenderSystemBase()
{
  ...
  if (m_settingsCallbackHandle.has_value())
...
CLEAR_FUNCTION CRenderSystemBase::GetClearFunction()
{
  std::unique_lock lock(m_settingsSection);
