...
public:
   ...
   *        The first matching subtag is returned when multiple subtags have a matching decription.
...
protected:
  ...
  void Reset();
  std::size_t Size() { return this->m_subTags.size(); }
  std::size_t SizeDesc() { return this->m_descToSubTags.size(); }
...
template<std::derived_from<BaseSubTag> T>
std::optional<T> CSubTagRegistry<T>::Lookup(const std::string& name) const
{
...
template<std::derived_from<BaseSubTag> T>
std::optional<T> CSubTagRegistry<T>::LookupByDescription(const std::string& description) const
{
...
template<std::derived_from<BaseSubTag> T>
bool CSubTagRegistry<T>::EmplaceRecords(const std::vector<RegistryFileRecord>& records)
{
  bool error{false};
...
template<std::derived_from<BaseSubTag> T>
bool CSubTagRegistry<T>::EmplaceRecord(const RegistryFileRecord& record)
{
...
template<std::derived_from<BaseSubTag> T>
std::vector<std::string> CSubTagRegistry<T>::SplitDescription(std::string&& description)
{
...
template<>
inline std::vector<std::string> CSubTagRegistry<GrandfatheredTag>::SplitDescription(
...
template<std::derived_from<BaseSubTag> T>
void CSubTagRegistry<T>::InsertDescLookupElem(std::string&& description, const T& subTag)
{
