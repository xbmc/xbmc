...
bool CSubTagRegistryManager::Initialize(std::unique_ptr<IRegistryRecordProvider> provider)
{
  ...
  m_initialized = Load(provider.get());
  return m_initialized;
}

bool CSubTagRegistryManager::Load(IRegistryRecordProvider* provider)
{
  CLog::Log(LOGINFO, "IANA Language Subtag Registry: loading...");
  ...
  {
    if (ImportRecord(record)) [[likely]]
      count++;
    else if (!error)
      error = true;
  }

  CLog::Log(LOGINFO, "IANA Language Subtag Registry: {} records loaded", count);
...
void CSubTagRegistryManager::Deinitialize()
{
  if (m_initialized)
  {
    ...
    m_initialized = false;
    CLog::LogF(LOGDEBUG, "registry unloaded");
  }
}
