...
template<std::derived_from<BaseSubTag> T>
bool CSubTagsCollection<T>::EmplaceRecords(
    const std::vector<RegistryFileRecord> &records) {
  bool error{false};
...
bool CSubTagsCollection<T>::EmplaceRecord(const RegistryFileRecord &record) {
  const auto [newElemIt, inserted] = m_subTags.emplace(record.m_subTag, T());
  if (!inserted) [[unlikely]]
  {
    CLog::LogF(LOGERROR, "redefinition of subtag [{}] ignored.", record.m_subTag);
  }
  ...
  for (std::string description : record.m_description)
  {
