...
TEST(TestIso639_3, LookupByCode1)
{
  ...
  result = CIso639_3::GetInstance().LookupByCode(longCode);
...
TEST(TestIso639_3, LookupByCode2)
{
  ...
  result = CIso639_3::GetInstance().LookupByCode("aaa");
  EXPECT_TRUE(result.has_value());
  EXPECT_EQ(*result, "Ghotuo");

  result = CIso639_3::GetInstance().LookupByCode("boh");
  EXPECT_TRUE(result.has_value());
  EXPECT_EQ(*result, "Boma");

  // value in the range reserved for private use - will never match
  result = CIso639_3::GetInstance().LookupByCode("qaa");
  EXPECT_FALSE(result.has_value());
}
...
TEST(TestIso639_3, LookupByName)
{
  ...
  result = CIso639_3::GetInstance().LookupByName("Puyuma");
  EXPECT_TRUE(result.has_value());
  EXPECT_EQ(*result, "pyu");

  result = CIso639_3::GetInstance().LookupByName("not a language");
  EXPECT_FALSE(result.has_value());
}
