...
template<class T>
class CTestSubTagRegistry : public CSubTagRegistry<T>
{
public:
...
TEST_P(DescGrandfatheredTests, TestValue)
{
  ...
  CTestSubTagRegistry<GrandfatheredTag> registry;
  ...
  for (const auto& desc : GetParam().m_positiveDescriptions)
  {
    ...
    if (l.has_value())
    {
      ...
      EXPECT_EQ(l->m_description, std::vector<std::string>{GetParam().m_positiveDescription});
    }
  }
...
INSTANTIATE_TEST_SUITE_P(TestI18nRegistry,
...
{
  auto param = GetParam().m_record;
  CTestSubTagRegistry<LanguageSubTag> registry;
  EXPECT_TRUE(registry.CallEmplaceRecords(param));
...
using RegistryTypes = ::testing::Types<LanguageSubTag,
                                       ...
                                       GrandfatheredTag,
                                       RedundantTag>;
TYPED_TEST_SUITE(TestI18nRegistryTyped, RegistryTypes);

template<typename T>
class TestI18nRegistryTyped : public testing::Test
{
};

TYPED_TEST(TestI18nRegistryTyped, EmplaceRecords)
{
  ...
  CTestSubTagRegistry<TypeParam> registry;
  EXPECT_TRUE(registry.CallEmplaceRecords(param));
  ...
  if (l.has_value())
  {
    ...
    EXPECT_EQ(l->m_description, std::vector<std::string>{"bar"});
  }
