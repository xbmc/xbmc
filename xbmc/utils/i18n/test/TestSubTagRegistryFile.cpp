...
TEST(TestI18nRegistryFile, Type)
{
  ...
  EXPECT_EQ(result[1].m_type, SubTagType::ExtLang);
  EXPECT_EQ(result[2].m_type, SubTagType::Script);
  EXPECT_EQ(result[3].m_type, SubTagType::Region);
  EXPECT_EQ(result[4].m_type, SubTagType::Variant);
  EXPECT_EQ(result[5].m_type, SubTagType::Grandfathered);
  EXPECT_EQ(result[6].m_type, SubTagType::Redundant);
}
...
TEST(TestI18nRegistryFile, Scope)
{
  ...
  EXPECT_EQ(result[0].m_scope, SubTagScope::Any);
  EXPECT_EQ(result[1].m_scope, SubTagScope::MacroLanguage);
  EXPECT_EQ(result[2].m_scope, SubTagScope::Collection);
  EXPECT_EQ(result[3].m_scope, SubTagScope::Special);
  EXPECT_EQ(result[4].m_scope, SubTagScope::PrivateUse);
}

TEST(TestI18nRegistryFile, Date)
{
  CRegistryFile f(
      XBMC_REF_FILE_PATH("xbmc/utils/i18n/test/test-language-subtag-registry-date.txt"));
  EXPECT_TRUE(f.Load());
  auto result = f.GetRecords();

  EXPECT_EQ(result.size(), 2);
  EXPECT_EQ(result[0].m_added, "2005-10-16");
  EXPECT_EQ(result[1].m_deprecated, "1989-01-01");
}
...
std::vector<std::vector<std::string>> ReadRecords(std::string filePath)
{
  ...
  while (r = f.ReadRecord())
    records.emplace_back(std::move(r.value()));
