...
void CScreenShot::TakeScreenshot(const std::string& filename, bool sync)
{
  ...
  if (!surface->Capture())
  {
    CLog::Log(LOGERROR, "Screenshot {} failed", CURL::GetRedacted(filename).c_str());
    return;
  }

  surface->CaptureVideo(true);

  CLog::Log(LOGDEBUG, "Saving screenshot {}", CURL::GetRedacted(filename).c_str());
  ...
  if (sync)
  {
    if (!CPicture::CreateThumbnailFromSurface(surface->GetBuffer(), surface->GetWidth(), surface->GetHeight(), surface->GetStride(), filename))
      CLog::Log(LOGERROR, "Unable to write screenshot {}", CURL::GetRedacted(filename).c_str());
  ...
  else
  {
    ...
    if (file.OpenForWrite(filename))
      ...
      CLog::Log(LOGERROR, "Unable to create file {}", CURL::GetRedacted(filename).c_str());
