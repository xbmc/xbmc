...
// Lookup ISO-3166-1 table and return codes or name.
bool CLangCodeExpander::LookupISO31661(std::string_view searchString,
                                       std::string& returned,
                                       bool returnCode)
{
  returned = "";

  if (searchString.size() == 2) // Lookup based on ISO-3166-1 Alpha 2
  {
    for (const ISO3166_1A2& codes : RegionNames)
    {
      if (searchString == codes.alpha2UC)
      {
        if (!returnCode)
        {
          returned = codes.englishname;
        }
        else
        {
          // Because the search was on an alpha-2 code, return the alpha-3 code.
          returned = codes.alpha3UC;
        }
        return true;
      }
    }
  ...
  if (searchString.size() == 3) // Lookup based on ISO-3166-1 Alpha 3
  {
    for (const ISO3166_1A2& codes : RegionNames)
    {
      if (searchString == codes.alpha3UC)
      {
        ...
        {
          // Because the search was on an alpha-3 code, return the alpha-3 code.
          returned = codes.alpha2UC;
        }
        return true;
      }
    }
  ...
  if (searchString.size() > 3) // Lookup based on ISO-3166-1 Name
  {
    for (const ISO3166_1A2& codes : RegionNames)
    {
      ...
      if (StringUtils::ToLower(searchString.data()) == StringUtils::ToLower(codes.englishname))
      {
        if (!returnCode)
        {
          // Because the search was on the name, return the alpha-3 code
          // if the code is NOT requested.
          returned = codes.alpha3UC;
        }
        else
        {
          // Because the search was on the name, return the alpha-2 code.
          returned = codes.alpha2UC;
        }
        return true;
      }
  ...
  return false; // The code or name was not found.
}
