...
/*!
 \brief Geolocation by scanning for nearby wifi networks and using Google Gears to get lat,lon
 */
bool CLocationJob::GetLocationByWifi()
{
  ...
  {
    network_list.AppendFormat(network_list.IsEmpty() ? "%s" : ",%s", GetJson(*it).c_str());
  }
  ...
  CVariant root = CJSONVariantParser::Parse((unsigned char *)json.c_str(), json.length());
  if (!root.isObject() || root.size() <= 0)
  {
    CLog::Log(LOGERROR, "LOCATION: Google geolocation failed: Invalid response");
    return false;
  }

  // Extract data from response
  CVariant location = root["location"];
  if (location.isObject() && location.size() > 0)
  {
    m_info.latitude.Format("%.6f", location["latitude"].asDouble());
    m_info.longitude.Format("%.6f", location["longitude"].asDouble());
    CVariant address = location["address"];
    if (address.isObject() && address.size() > 0)
    {
...
/*!
 \brief Geolocation by using IPInfoDB with the current external IP address
 */
...
bool CLocationJob::ReverseGeocode(CStdString lat, CStdString lon)
{
  ...
  if (!root.isObject() || root.size() <= 0)
  {
    CLog::Log(LOGERROR, "LOCATION: Reverse geocoding failed: Invalid response");
    return false;
  }
  if (strcmp(root["status"].asString(), "OK") == 0)
  {
    CLog::Log(LOGERROR, "LOCATION: Reverse geocoding failed: %s", root["status"].asString());
    return false;
  }
  ...
  {
    CVariant result = root["results"][i];
    for (unsigned int j = 0; infoNeeded && j < result.size(); ++j)
    {
      CVariant address_component = result["address_components"][j];
      for (unsigned int k = 0; k < address_component.size(); ++k)
      {
        CVariant type = address_component["types"][k];
        if ((infoNeeded & COUNTRYCODE) && strcmp(type.asString(), "country") == 0)
        {
          m_info.countryCode = address_component["short_name"].asString();
          infoNeeded &= ~COUNTRYCODE;
          // No break
        }
        if ((infoNeeded & COUNTRYNAME) && strcmp(type.asString(), "country") == 0)
        {
          m_info.countryName = address_component["long_name"].asString();
          infoNeeded &= ~COUNTRYNAME;
          break;
        }
        else if ((infoNeeded & REGIONNAME) && strcmp(type.asString(), "administrative_area_level_1") == 0)
        {
          m_info.regionName = address_component["short_name"].asString(); // Abbreviated state
          infoNeeded &= ~REGIONNAME;
          break;
        }
        else if ((infoNeeded & CITY) && (strcmp(type.asString(), "locality") == 0 || strcmp(type.asString(), "sublocality") == 0))
        {
          m_info.city = address_component["long_name"].asString();
          infoNeeded &= ~CITY;
          break;
        }
        else if ((infoNeeded & POSTALCODE) && strcmp(type.asString(), "postal_code") == 0)
        {
...
/*!
 \brief Encodes an network access point into a Google Gears specific JSON string
 */
CStdString CLocationJob::GetJson(const NetworkAccessPoint &ap)
{
  CStdString jsonBuffer;
  if (ap.getMacAddress().IsEmpty())
    return "{}";
  jsonBuffer.Format("{\"mac_address\":\"%s\"", ap.getMacAddress().c_str());
  if (!ap.getEssId().IsEmpty())
    jsonBuffer.AppendFormat(",\"ssid\":\"%s\"", ap.getEssId().c_str());
  if (ap.getSignalStrength() < 0)
    jsonBuffer.AppendFormat(",\"signal_strength\":%d", ap.getSignalStrength());
  if (ap.getChannel() != 0)
    jsonBuffer.AppendFormat(",\"channel\":%d", ap.getChannel());
  jsonBuffer.append("}");
  return jsonBuffer;
}
...
/*!
 \brief Wrapper for XMLUtils::GetString(). Instead of returning a missing xml tag,
        location services may return the tag containing only a single dash. This
...
CLocation::CLocation(void) : CInfoLoader(7 * 24 * 60 * 60 * 1000) // 1 week
{
  Reset();
}
...
/*!
  \brief  Translates location info ID into a value
  \param  info
  \return the info, or "" if unknown, or string 503 (Busy) if busy
 */
...
/*!
  \brief Callback for when the location job is completed
  \param jobId   unused
