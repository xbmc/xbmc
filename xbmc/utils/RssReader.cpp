  ...
  while (GetQueueSize())
  {
    ...
    else
    {
      ...
      while (!m_bStop && nRetries > 0)
      {
        ...
        {
          CLog::Log(LOGERROR, "Timeout while retrieving rss feed: {}", strUrl.c_str());
          break;
        }
        nRetries--;
        ...
        else
        {
          if (http.Get(strUrl, strXML))
          {
            ...
            CLog::Log(LOGDEBUG, "Got rss feed: {}", strUrl.c_str());
            break;
          }
          else if (nRetries > 0)
            CThread::Sleep(5000); // Network problems? Retry, but not immediately.
          else
            CLog::Log(LOGERROR, "Unable to obtain rss feed: {}", strUrl.c_str());
        }
      }
      http.Cancel();
    }
    if (!strXML.empty() && m_pObserver)
    {
      ...
      if (Parse(strXML, iFeed, fileCharset))
        CLog::Log(LOGDEBUG, "Parsed rss feed: {}", strUrl.c_str());
    }
  }
  UpdateObserver();
}
...
bool CRssReader::Parse(const std::string& data, int iFeed, const std::string& charset)
{
  ...
  CLog::Log(LOGDEBUG, "RSS feed encoding: {}", m_xml.GetUsedCharset().c_str());
