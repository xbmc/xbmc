 ...
 * ```
 *
 * The elements in CSet need to be totally ordered, this allows for log(n)
...
public:
  ...
  consteval CSet(Iter begin, Iter end)
  {
    ...
    assert(std::ranges::adjacent_find(m_keys) == m_keys.end());
  }
  ...
  constexpr auto begin() const { return m_keys.begin(); }
  constexpr auto end() const { return m_keys.end(); }
