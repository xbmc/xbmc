  ...
  if(GetHttpXML(urlx,"boxstatus"))
  {
    if(sBoxStatus.recording.Equals("1"))
    {
      ...
      if (dialog)
      {
        ...
        dialog->SetHeading(20330);
        dialog->SetLine( 0, 20331);
        dialog->SetLine( 1, 20334);
        dialog->SetLine( 2, "" );
        dialog->DoModal();
      }
...
bool CTuxBoxUtil::GetZapUrl(const CStdString& strPath, CFileItem &items )
{
  ...
  if (ipoint >=0)
  {
    if(ZapToUrl(url, strOptions, ipoint))
    {
      ...
      {
        strVideoStream.Format("0,%s,%s,%s,,,%s",sStrmInfo.pmt.Left(4).c_str(), sStrmInfo.vpid.Left(4).c_str(), strAudioChannelPid.Right(4).c_str(),sStrmInfo.pcrpid.Left(4).c_str());
      }
      else
      {
