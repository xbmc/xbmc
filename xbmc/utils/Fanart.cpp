  ...
  { // need conversion
    ...
    { // split up each color
      ...
      { // convert
        if (colorsOut.size())
          colorsOut += ",";
        colorsOut += StringUtils::Format("FF%2x%2x%2x", (unsigned int) atol(strTriplets[0].c_str()), (unsigned int) atol(strTriplets[1].c_str()), (unsigned int) atol(strTriplets[2].c_str()));
      }
    }
  }
  else
