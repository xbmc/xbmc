...
class CTuxBoxUtil
{
  public:
    ...
    bool ParseBouquets(TiXmlElement *root, CFileItemList &items, CURL &url, std::string strFilter, std::string strChild);
    bool ParseBouquetsEnigma2(TiXmlElement *root, CFileItemList &items, CURL &url, std::string& strFilter, std::string& strChild);
    bool ParseChannels(TiXmlElement *root, CFileItemList &items, CURL &url, std::string strFilter, std::string strChild);
    bool ParseChannelsEnigma2(TiXmlElement *root, CFileItemList &items, CURL &url, std::string& strFilter, std::string& strChild);
    ...
    bool InitZapstream(const std::string& strPath);
    bool SetAudioChannel(const std::string& strPath, const AUDIOCHANNEL& sAC);

    std::string GetPicon(std::string strServiceName);
    std::string GetSubMode(int iMode, std::string& strXMLRootString, std::string& strXMLChildString);
    std::string DetectSubMode(std::string strSubMode, std::string& strXMLRootString, std::string& strXMLChildString);
};
