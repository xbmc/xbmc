...
void CEdenVideoArtUpdater::Process()
{
  ...
  for (int i = 0; i < items.Size(); i++)
  {
    ...
    {
      string art = CVideoInfoScanner::GetImage(item.get(), true, item->GetVideoInfoTag()->m_basePath != item->GetPath(), "thumb");
      std::string type;
  ...
  for (int i = 0; i < items.Size(); i++)
  {
    ...
    if (!db.GetArtForItem(item->GetVideoInfoTag()->m_iDbId, item->GetVideoInfoTag()->m_type, artwork)
        || (artwork.size() == 1 && artwork.find("thumb") != artwork.end()))
    {
      string art = CVideoInfoScanner::GetImage(item.get(), true, false, "thumb");
      std::string type;
      if (CacheTexture(art, cachedThumb, item->GetLabel(), type))
        artwork.insert(make_pair(type, art));
    ...
    for (int j = 0; j < items2.Size(); j++)
    {
      ...
      if (!db.GetArtForItem(episode->GetVideoInfoTag()->m_iDbId, episode->GetVideoInfoTag()->m_type, artwork)
          || (artwork.size() == 1 && artwork.find("thumb") != artwork.end()))
      {
        string art = CVideoInfoScanner::GetImage(episode.get(), true, episode->GetVideoInfoTag()->m_basePath != episode->GetPath(), "thumb");
...
string CEdenVideoArtUpdater::GetCachedActorThumb(const CFileItem &item)
{
  return GetThumb("actor" + item.GetLabel(), CProfilesManager::Get().GetVideoThumbFolder(), true);
}

string CEdenVideoArtUpdater::GetCachedSeasonThumb(int season, const string &path)
{
  string label;
  if (season == -1)
...
string CEdenVideoArtUpdater::GetCachedEpisodeThumb(const CFileItem &item)
{
  // get the locally cached thumb
  string strCRC = StringUtils::Format("%sepisode%i",
...
string CEdenVideoArtUpdater::GetCachedVideoThumb(const CFileItem &item)
{
  ...
  {
    string path = item.GetVideoInfoTag()->m_strFileNameAndPath;
...
string CEdenVideoArtUpdater::GetCachedFanart(const CFileItem &item)
{
  ...
  string path = item.GetVideoInfoTag()->GetPath();
  if (path.empty())
    return "";
  return GetThumb(path, URIUtils::AddFileToFolder(CProfilesManager::Get().GetVideoThumbFolder(), "Fanart/"), false);
}

string CEdenVideoArtUpdater::GetThumb(const string &path, const string &path2, bool split)
{
  // get the locally cached thumb
  Crc32 crc;
  ...
  if (split)
  {
    string hex = StringUtils::Format("%08x", (__int32)crc);
