...
void CHTMLRow::Parse(const string& strTable)
{
  CHTMLUtil util;
  string strTag;
  ...
  do
  {
    ...
    if (iTableRowStart >= 0)
    {
      ...
      string strRow = strTable.substr(iTableRowStart, 1 + iTableRowEnd - iTableRowStart);
...
void CHTMLTable::Parse(const string& strHTML)
{
  ...
  if (iPosStart >= 0)
  {
    ...
    string strTable = strHTML.substr(iPosStart, 1 + iPosEnd - iPosStart);
    int iTableRowStart = 0;
    do
    {
