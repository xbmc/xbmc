...
void CLog::Close()
{
  ...
  s_globals.m_platfIF.CloseLogFile();
...
bool CLog::Init(const char* path)
{
  ...
  return s_globals.m_platfIF.OpenLogFile(strLogPath + "xbmc.log", strLogPath + "xbmc.old.log");
}
...
{
#if defined(_DEBUG) || defined(PROFILE)
  s_globals.m_platfIF.PrintDebugString(line);
...
bool CLog::WriteLogString(int logLevel, const std::string& logString)
{
  ...
  s_globals.m_platfIF.GetCurrentLocalTime(hour, minute, second);
  ...
  return s_globals.m_platfIF.WriteStringToLog(strData);
}
