  ...
  if (!progressTrackingFile.empty())
  {
        ...
        && UPNP::CUPnP::SaveFileState(item, bookmark, updatePlayCount))
    {
      ...
      if (updatedItem.HasProperty("original_listitem_url"))
        ...
        updatedItem.SetPath(progressTrackingFile); // fallback to progressTrackingFile which should be the upnp path

      CGUIMessage message(GUI_MSG_NOTIFY_ALL, 0, 0, GUI_MSG_UPDATE_ITEM, 0, std::make_shared<CFileItem>(updatedItem));
