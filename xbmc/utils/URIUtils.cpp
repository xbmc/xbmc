                  ...
                  FinalDot finalDotAction = FinalDot::IGNORE_FINAL_DOT)
{
  ...
  // If filename starts with a dot then ignore that
  size_t trim;
  if (period == 0 || (period == separator + 1))
    trim = period + 1;
  else
    trim = separator + 1;

  // Extract filename
  const std::string filename{StringUtils::ToLower(path.substr(trim))};
  if (filename.find_last_of('.') == std::string::npos)
    return NO_EXTENSION; // No extension (a leading dot only)

  // Check for known compound (double dot) extensions first
  // Or check against a given list of extensions
  std::string exts{!extensions.empty() ? extensions : ".tar.bz2|.tar.gz|.tar.xz"};

  for (auto chunk : exts | std::views::split('|'))
  {
    const std::string ext(chunk.begin(), chunk.end());
...
void URIUtils::RemoveExtension(std::string& path)
{
  ...
  static const std::string_view extensions{
      []
      {
...
#ifdef TARGET_DARWIN
        mask += "|.app|.applescript|.workflow";
#endif
        return std::string_view{mask};
      }()};
