...
int CGFFPatch::examinePatch(BYTE* pbuffer, UINT location)
{
  ...
  if (findConditionalJump(&pbuffer[0], tmp, 0x20) == FALSE)
    return 0;
  if ( (pbuffer[location-2] == 0x6a) && (pbuffer[location-1] >= 0 ) && (pbuffer[location-1] < 6 ) && 
       ((pbuffer[location-3] == 0) && (pbuffer[location-4] == 0x6a) || ((pbuffer[location-3] & 0x50) == 0x50)))
    return 1;
  if (((pbuffer[location-1] & 0x50) == 0x50) && (pbuffer[location-2] == 0) && (pbuffer[location-3] == 0x6a))
    return 2;

  if (((pbuffer[location-1] & 0x50) == 0x50) && ((pbuffer[location-2] & 0x50) == 0x50) && (pbuffer[location-3] == 0x00) )
    return 3;

  if (((pbuffer[location-1] & 0x50) == 0x50) && ((pbuffer[location-2] & 0x50) == 0x50) && (pbuffer[location-5] == 0x00))
    return 4;
...
UINT CGFFPatch::searchData(BYTE* pBuffer, UINT startPos, UINT size)
{
  ...
  {
    if ((pBuffer[i] == 0x6a) && (pBuffer[i+1] == 0x0b) && ((pBuffer[i+2] & 0x50) == 0x50) &&(pBuffer[i+3] == 0xff))
      return i;
  }
