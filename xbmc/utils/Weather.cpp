...
void CWeatherJob::LoadLocalizedToken()
{
  ...
  if (PODoc.LoadFile("special://xbmc/language/English/strings.po"))
  {
    ...
    {
      if (PODoc.GetEntryType() != ID_FOUND) continue;
      int id = PODoc.GetEntryID();
      PODoc.ParseEntry();
      ...
      {
        CStdString Label = PODoc.GetMsgid(0);
        if (!Label.IsEmpty())
        {
          m_localizedTokens.insert(make_pair(Label, id));
          counter++;
        }
      }
    }
  ...
  CLog::Log(LOGDEBUG, "Weather: no PO string file available, to load English tokens, fallback to strings.xml file");
