...
void CFileOperationJob::SetFileOperation(FileAction action, CFileItemList &items, const string &strDestFile)
{
  m_action = action;
...
bool CFileOperationJob::DoProcessFile(FileAction action, const string& strFileA, const string& strFileB, FileOperationList &fileOperations, double &totalTime)
{
  int64_t time = 1;
...
bool CFileOperationJob::DoProcessFolder(FileAction action, const string& strPath, const string& strDestFile, FileOperationList &fileOperations, double &totalTime)
{
...
bool CFileOperationJob::DoProcess(FileAction action, CFileItemList & items, const string& strDestFile, FileOperationList &fileOperations, double &totalTime)
{
  for (int iItem = 0; iItem < items.Size(); ++iItem)
  {
    ...
    if (pItem->IsSelected())
    {
      string strNoSlash = pItem->GetPath();
      URIUtils::RemoveSlashAtEnd(strNoSlash);
      string strFileName = URIUtils::GetFileName(strNoSlash);
...
CFileOperationJob::CFileOperation::CFileOperation(FileAction action, const string &strFileA, const string &strFileB, int64_t time) : m_action(action), m_strFileA(strFileA), m_strFileB(strFileB), m_time(time)
{
}
...
string CFileOperationJob::GetActionString(FileAction action)
{
  string result;
  switch (action)
  {
...
inline bool CFileOperationJob::CanBeRenamed(const string &strFileA, const string &strFileB)
{
#ifndef TARGET_POSIX
