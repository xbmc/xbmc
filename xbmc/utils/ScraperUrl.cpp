...
CScraperUrl::CScraperUrl(const tinyxml2::XMLElement* element) : CScraperUrl()
{
...
bool CScraperUrl::ParseAndAppendUrl(const tinyxml2::XMLElement* element)
{
  ...
  SUrlEntry url(element->FirstChild()->Value());
...
bool CScraperUrl::ParseAndAppendUrlsFromEpisodeGuide(const std::string& episodeGuide)
{
  ...
  auto link = docHandle.FirstChildElement("episodeguide").ToElement();
                               ...
                               int season)
{
  ...
  CXBMCTinyXML2 doc;
  auto thumb = doc.NewElement("thumb");
  thumb->SetAttribute("spoof", referrer.c_str());
  thumb->SetAttribute("cache", cache.c_str());
  if (post)
  ...
  if (season >= 0)
  {
    thumb->SetAttribute("season", std::to_string(season).c_str());
    thumb->SetAttribute("type", "season");
  }
  thumb->SetAttribute("aspect", aspect.c_str());
  thumb->SetAttribute("preview", preview.c_str());
  auto text = doc.NewText(url.c_str());
