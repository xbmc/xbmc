  ...
  if (pExpression)
  {
    ...
    while (i > -1 && (i < (int)curInput.size() || curInput.size() == 0))
    {
      ...
      if (iOptional > -1) // check that required param is there
      {
        ...
        std::string szParam = reg.GetReplaceStdString(temp);
        CRegExp reg2;
        reg2.RegComp("(.*)(\\\\\\(.*\\\\2.*)\\\\\\)(.*)");
        int i2=reg2.RegFind(strCurOutput.c_str());
        while (i2 > -1)
        {
          std::string szRemove = reg2.GetReplaceStdString("\\2");
      ...
      std::string result = reg.GetReplaceStdString(strCurOutput.c_str());
      if (!result.empty())
      {
...
void CScraperParser::ConvertJSON(CStdString &string)
{
  ...
  while (reg.RegFind(string.c_str()) > -1)
  {
    ...
    std::string szReplace = reg.GetReplaceStdString("\\1");
  ...
  while (reg2.RegFind(string.c_str()) > -1)
  {
    ...
    std::string szHexValue = reg2.GetReplaceStdString("\\1");
