...
TEST(TestGroupUtils, SetsSortBySortTitle)
{
  CFileItemList items;
  items.Add(MakeMovieInSet("Avengers Assemble", 4, "Avengers Collection", "Set 02"));
  items.Add(MakeMovieInSet("Iron Man", 5, "Iron Man Collection", "Set 01"));

  CFileItemList sets;
  ASSERT_TRUE(GroupUtils::Group(GroupBySet, "videodb://movies/sets/", items, sets));
  ASSERT_EQ(sets.Size(), 2);

  sets.Sort(SortBy::SORT_TITLE, SortOrder::ASCENDING);

  EXPECT_EQ(sets[0]->GetVideoInfoTag()->m_strTitle, "Iron Man Collection");
  EXPECT_EQ(sets[1]->GetVideoInfoTag()->m_strTitle, "Avengers Collection");
}
