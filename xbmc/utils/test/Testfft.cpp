...
TEST(Testfft, fft)
{
  int numelements = (sizeof(refdata)/sizeof(float)), i;
  float vardata[numelements];
  CStdString refstr, varstr;

  memcpy(vardata, refdata, sizeof(refdata));
  fft(vardata, numelements/2, 1);
  for (i = 0; i < numelements; i++)
  {
...
TEST(Testfft, twochannelrfft)
{
  ...
  memcpy(vardata, refdata, sizeof(refdata));
  twochannelrfft(vardata, numelements/2);
  for (i = 0; i < numelements; i++)
  {
...
TEST(Testfft, twochanwithwindow)
{
  ...
  memcpy(vardata, refdata, sizeof(refdata));
  twochanwithwindow(vardata, numelements/2);
  for (i = 0; i < numelements; i++)
  {
