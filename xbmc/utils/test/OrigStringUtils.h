...
template<typename T_Orig, std::enable_if_t<!std::is_enum<T_Orig>::value, int> = 0>
constexpr auto&& EnumToInt_Orig(T_Orig&& arg) noexcept
{
  return arg;
}
template<typename T_Orig, std::enable_if_t<std::is_enum<T_Orig>::value, int> = 0>
...
public:
  ...
  */
  ...
  static std::string Format(const std::string& fmt, Args&&... args)
  {
    ...
    return ::fmt::format(fmt, EnumToInt_Orig(std::forward<Args>(args))...);
  }
  template<typename... Args>
  ...
  // Added to simplify use of new API tests with this
  ...
  // Added to simplify use of new API tests with this
  static bool EqualsNoCase(const std::string_view sv1, const std::string_view sv2)
  {
    std::string str1{sv1};
    std::string str2{sv2};
  ...
  // Added to simplify use of new API tests

  static int CompareNoCase(const std::string_view sv1, const std::string_view sv2, size_t n = 0)
  {
    std::string str1{sv1};
    std::string str2{sv2};
    return CompareNoCase(str1, str2, n);
  }
  ...
  static OutputIt SplitTo(OutputIt d_first, const std::string& input, const std::vector<std::string> &delimiters)
  {
    ...
    for (size_t di = 1; di < delimiters.size(); di++)
      OrigStringUtils::Replace(str, delimiters[di], delimiters[0]);
...
struct origsortstringbyname
{
  bool operator()(const std::string& strItem1, const std::string& strItem2) const
  {
    return OrigStringUtils::CompareNoCase(strItem1, strItem2) < 0;
  }
};
