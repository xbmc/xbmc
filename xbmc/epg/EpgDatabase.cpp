  ...
  if (ResultQuery(strQuery))
  {
    ...
    try
    {
      while (!m_pDS->eof())
      {
        ...
        newTag->m_strYear            = m_pDS->fv("sYear").get_asString().c_str();
...
int CEpgDatabase::Persist(const CEpgInfoTag &tag, bool bSingleUpdate /* = true */)
{
  ...
  if (iBroadcastId < 0)
  {
    strQuery = PrepareSQL("REPLACE INTO epgtags (idEpg, iStartTime, "
        "iEndTime, sTitle, sPlotOutline, sIMDBNumber, sCast, sDirector, sWriter, sOriginalTitle, sYear, sPlot, sIconPath, iGenreType, iGenreSubType, sGenre, "
        ...
        tag.Title(true).c_str(), tag.PlotOutline(true).c_str(), tag.IMDBNumber(true).c_str(), tag.Cast(true).c_str(), tag.Director(true).c_str(), tag.Writer(true).c_str(),
		tag.OriginalTitle(true).c_str(), tag.Year(true).c_str(),tag.Plot(true).c_str(),tag.Icon().c_str(), tag.GenreType(), tag.GenreSubType(), strGenre.c_str(),
  ...
  else
  {
    strQuery = PrepareSQL("REPLACE INTO epgtags (idEpg, iStartTime, "
        ...
        tag.EpgID(), iStartTime, iEndTime,
        tag.Title(true).c_str(), tag.PlotOutline(true).c_str(), tag.IMDBNumber(true).c_str(), tag.Cast(true).c_str(), tag.Director(true).c_str(), tag.Writer(true).c_str(),tag.OriginalTitle(true).c_str(), tag.Year(true).c_str(), tag.Plot(true).c_str(), tag.Icon().c_str(), tag.GenreType(), tag.GenreSubType(), strGenre.c_str(),
        iFirstAired, tag.ParentalRating(), tag.StarRating(), tag.Notify(),
        tag.SeriesNum(), tag.EpisodeNum(), tag.EpisodePart(), tag.EpisodeName().c_str(),
