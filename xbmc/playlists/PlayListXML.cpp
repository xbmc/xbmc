...
bool CPlayListXML::Load( const std::string& strFileName )
{
  ...
  {
    CLog::Log(LOGERROR, "Playlist {} has invalid format/malformed xml", strFileName.c_str());
    return false;
  }
  ...
  {
    CLog::Log(LOGERROR, "Playlist {} has no <streams> root", strFileName.c_str());
    return false;
  }
  ...
  while ( pSet )
  {
    ...
    if ( !url.empty() )
    {
      ...
      CLog::Log(LOGERROR, "Playlist entry {} in file {} has missing <url> tag", name.c_str(),
                strFileName.c_str());
