...
{
  if (std::any_of(str.begin(), str.end(), [](char c) { return !isdigit(c); }))
    return {};
...
bool CSmartPlaylistRule::ValidateDate(const std::string& input, void* data)
{
  ...
  const auto year{StrToInt(input.substr(0, 4))};
  if (!year.has_value())
    return false;

  const auto month{StrToInt(input.substr(5, 2))};
  if (!month.has_value())
    return false;

  const auto day{StrToInt(input.substr(8, 2))};
  if (!day.has_value())
    return false;
