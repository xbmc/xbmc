...
void CLangInfo::GetAddonsLanguageCodes(std::map<std::string, std::string>& languages)
{
  ...
  CServiceBroker::GetAddonMgr().GetAddons(addons, ADDON::AddonType::ADDON_RESOURCE_LANGUAGE);
...
LanguageResourcePtr CLangInfo::GetLanguageAddon(const std::string& locale /* = "" */) const
{
  ...
  if (CServiceBroker::GetAddonMgr().GetAddon(addonId, addon,
                                             ADDON::AddonType::ADDON_RESOURCE_LANGUAGE,
...
bool CLangInfo::SetLanguage(std::string language /* = "" */, bool reloadServices /* = true */)
{
  ...
  if (!addonMgr.GetAddon(language, addon, ADDON::AddonType::ADDON_RESOURCE_LANGUAGE,
                         ADDON::OnlyEnabled::CHOICE_YES))
  {
                                                  ...
                                                  void* data)
{
  // find languages...
  ADDON::VECADDONS addons;
  if (!CServiceBroker::GetAddonMgr().GetAddons(addons, ADDON::AddonType::ADDON_RESOURCE_LANGUAGE))
    return;
