...
JSONRPC_STATUS CAddonsOperations::GetAddons(const string &method, ITransportLayer *transport, IClient *client, const CVariant &parameterObject, CVariant &result)
{
...
JSONRPC_STATUS CAddonsOperations::GetAddonDetails(const string &method, ITransportLayer *transport, IClient *client, const CVariant &parameterObject, CVariant &result)
{
...
JSONRPC_STATUS CAddonsOperations::SetAddonEnabled(const string &method, ITransportLayer *transport, IClient *client, const CVariant &parameterObject, CVariant &result)
{
  string id = parameterObject["addonid"].asString();
  bool disabled = false;
...
JSONRPC_STATUS CAddonsOperations::ExecuteAddon(const string &method, ITransportLayer *transport, IClient *client, const CVariant &parameterObject, CVariant &result)
{
  string id = parameterObject["addonid"].asString();
  AddonPtr addon;
...
void CAddonsOperations::FillDetails(AddonPtr addon, const CVariant& fields, CVariant &result, CAddonDatabase &addondb, bool append /* = false */)
{
  ...
  for (unsigned int index = 0; index < fields.size(); index++)
  {
    ...
    {
      string url = addonInfo[field].asString();
      ...
      bool needsRecaching;
      string image = CTextureCache::Get().CheckCachedImage(url, false, needsRecaching);
