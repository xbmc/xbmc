...
static int SetAddon(const std::vector<std::string>& params)
{
  ...
  {
    ADDON::TYPE type = AddonProps::TranslateType(params[i]);
...
static int SetFile(const std::vector<std::string>& params)
{
  ...
  ADDON::TYPE type;
  if ((type = AddonProps::TranslateType(strMask)) != ADDON_UNKNOWN)
  {
    CURL url;
    ...
    if (type == ADDON_SCRIPT)
      strMask = ".py";
    std::string replace;
    if (CGUIDialogFileBrowser::ShowAndGetFile(url.Get(), strMask, AddonProps::TranslateType(type, true), replace, true, true, true))
    {
