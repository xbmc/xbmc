...
namespace XBMCAddon
{
  namespace xbmcgui
  {
    ...
    void ListItem::setProperty(const char * key, const String& value)
    {
      ...
      if (lowerKey.compare("startoffset") == 0)
      ...
      else if (lowerKey.compare("fanart_image") == 0)
        item->SetArt("fanart", value);
      else
        item->SetProperty(lowerKey, value.c_str());
    }
