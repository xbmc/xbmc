...
namespace XBMCAddon
{
  namespace xbmcgui
  {
    ...
    /// against the global fonts: those of the **active skin**, plus any declared
    /// by an enabled `resource.font` add-on. A name none of them defines falls
    /// back to `font13`, so the same add-on renders differently from one skin to
    /// the next.
    ///
    ...
    /// `<font>` name against this file first, then against the global fonts,
    /// then `font13`. Nothing outside the window sees your fonts, and you may
    /// safely reuse a name the skin also defines.
    ///
    ...
    ///
    /// \note A `<font>` name that neither your Font.xml nor the global fonts
    /// define still falls back to `font13`, and now logs one warning per window
    /// load.
    ///
