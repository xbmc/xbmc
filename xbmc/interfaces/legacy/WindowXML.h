...
namespace XBMCAddon
{
  namespace xbmcgui
  {
    ...
    ///
    /// <b><c>xbmcgui.WindowXML(xmlFilename, scriptPath[, defaultSkin, defaultRes])</c></b>
    ///
    /// Creates a new xml file based window class.
    ///
    /// \note This class include also all calls from <b><c>\ref python_xbmcgui_window</c></b>.
    ///
    /// @param[in] xmlFilename          string - the name of the xml file to
    ///                                 look for.
    /// @param[in] scriptPath           string - path to script. used to
    ///                                 fallback to if the xml doesn't exist in
    ///                                 the current skin. (eg xbmcaddon.Addon().getAddonInfo('path').decode('utf-8'))
    /// @param[in] defaultSkin          [opt] string - name of the folder in the
    ///                                 skins path to look in for the xml.
    ///                                 (default='Default')
    /// @param[in] defaultRes           [opt] string - default skins resolution.
    ///                                 (default='720p')
    ...
    protected:
     ...
     public:
      ...
      void addItem(const Alternative<String, const ListItem*>& item, int position = INT_MAX);
      ...
      SWIGHIDDENVIRTUAL
#endif /* DOXYGEN_SHOULD_SKIP_THIS */
      void setCurrentListPosition(int position);
      ...
      SWIGHIDDENVIRTUAL
#endif /* DOXYGEN_SHOULD_SKIP_THIS */
      void setProperty(const String &strProperty, const String &strValue);
#ifndef SWIG
      // CGUIWindow
...
//      SWIGHIDDENVIRTUAL void    OnDeinitWindow(int nextWindowID);
    ...
    /// \ingroup python_xbmcgui_window_xml
    /// @{
    /// @brief __GUI xml window dialog class.__
    ///
    /// <b><c>xbmcgui.WindowXMLDialog(xmlFilename, scriptPath[, defaultSkin, defaultRes])</c></b>
    ///
