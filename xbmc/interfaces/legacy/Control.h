...
namespace XBMCAddon
{
  namespace xbmcgui
  {
    ...
    ///
    /// \python_class{ ControlCheckMark(x, y, width, height, label[, focusTexture, noFocusTexture,
    ...
    /// @param focusTexture         [opt] string - filename for focus texture.
    /// @param noFocusTexture       [opt] string - filename for no focus texture.
    /// @param checkWidth           [opt] integer - width of checkmark.
    /// @param checkHeight          [opt] integer - height of checkmark.
    /// @param alignment            [opt] integer - alignment of label
    /// - \ref kodi_gui_font_alignment "Flags for alignment" used as bits to have several together:
    /// | Defination name   |   Bitflag  | Description                         |
    /// |-------------------|:----------:|:------------------------------------|
    ...
    /// | XBFONT_JUSTIFIED  | 0x00000010 | Justify text
    /// @param font                 [opt] string - font used for label text. (e.g. 'font13')
    /// @param textColor            [opt] hexstring - color of enabled checkmark's label. (e.g. '0xFFFFFFFF')
    /// @param disabledColor        [opt] hexstring - color of disabled checkmark's label. (e.g. '0xFFFF3300')
    ///
    ///
    /// @note You can use the above as keywords for arguments and skip certain
    ///       optional arguments.\n
    ...
    public:
      ...
      ///
      /// \ingroup python_xbmcgui_control_checkmark
      /// @brief \python_func{ setSelected(isOn) }
      ///-----------------------------------------------------------------------
      /// Sets this checkmark status to on or off.
      ///
      /// @param isOn               bool - True=selected (on) / False=not
      ///                               selected (off)
      ///
      ///
      ///-----------------------------------------------------------------------
      ...
      ///
      /// \ingroup python_xbmcgui_control_checkmark
      /// @brief \python_func{ setLabel(label[, font, textColor, disabledColor]) }
      ///-----------------------------------------------------------------------
      ...
      ///
      /// @param disabledColor      hexstring - color of disabled checkmark's
