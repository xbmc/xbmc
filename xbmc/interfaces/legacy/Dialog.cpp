      ...
      if (!heading.empty())
      {
        if (inputtype == 1)
        {
          ...
          if (CGUIDialogNumeric::ShowAndGetDate(timedate, heading))
            value = StringUtils::Format("{:2}/{:2}/{:4}", timedate.GetDay(), timedate.GetMonth(), timedate.GetYear());
          else
            return emptyString;
        }
        else if (inputtype == 2)
        ...
        case INPUT_DATE:
          {
            ...
            if (CGUIDialogNumeric::ShowAndGetDate(timedate, heading))
              value =
                  StringUtils::Format("{:2}/{:2}/{:4}", timedate.GetDay(), timedate.GetMonth(), timedate.GetYear());
            else
              value = emptyString;
          }
          break;
