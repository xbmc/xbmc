...
string CTextureRule::TranslateField(int field) const
{
  for (unsigned int i = 0; i < NUM_FIELDS; i++)
...
string CTextureRule::GetField(int field, const string &type) const
{
...
string CTextureRule::FormatParameter(const string &operatorString, const string &param, const CDatabase &db, const string &strType) const
{
...
string CTextureUtils::GetWrappedImageURL(const string &image, const string &type, const string &options)
{
...
string CTextureUtils::GetWrappedThumbURL(const string &image)
{
  return GetWrappedImageURL(image, "", "size=thumb");
}

string CTextureUtils::UnwrapImageURL(const string &image)
{
  if (StringUtils::StartsWith(image, "image://"))
  {
    CURL url(image);
...
{
  string sql = PrepareSQL("UPDATE sizes SET usecount=usecount+1, lastusetime=CURRENT_TIMESTAMP WHERE idtexture=%u AND width=%u AND height=%u", details.id, details.width, details.height);
  return ExecuteQuery(sql);
}

bool CTextureDatabase::GetCachedTexture(const string &url, CTextureDetails &details)
{
  try
  {
    ...
    string sql = PrepareSQL("SELECT id, cachedurl, lasthashcheck, imagehash, width, height FROM texture JOIN sizes ON (texture.id=sizes.idtexture AND sizes.size=1) WHERE url='%s'", url.c_str());
...
bool CTextureDatabase::GetTextures(CVariant &items, const Filter &filter)
{
  try
  {
    ...
    string sql = "SELECT %s FROM texture JOIN sizes ON (texture.id=sizes.idtexture AND sizes.size=1)";
    string sqlFilter;
...
bool CTextureDatabase::SetCachedTextureValid(const string &url, bool updateable)
{
  string date = updateable ? CDateTime::GetCurrentDateTime().GetAsDBDateTime() : "";
  string sql = PrepareSQL("UPDATE texture SET lasthashcheck='%s' WHERE url='%s'", date.c_str(), url.c_str());
  return ExecuteQuery(sql);
}

bool CTextureDatabase::AddCachedTexture(const string &url, const CTextureDetails &details)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string sql = PrepareSQL("DELETE FROM texture WHERE url='%s'", url.c_str());
    m_pDS->exec(sql.c_str());

    string date = details.updateable ? CDateTime::GetCurrentDateTime().GetAsDBDateTime() : "";
...
bool CTextureDatabase::ClearCachedTexture(const string &url, string &cacheFile)
{
...
bool CTextureDatabase::ClearCachedTexture(int id, string &cacheFile)
{
  try
  {
    if (NULL == m_pDB.get()) return false;
    ...
    string sql = PrepareSQL("select cachedurl from texture where id=%u", id);
    m_pDS->query(sql.c_str());
...
bool CTextureDatabase::InvalidateCachedTexture(const string &url)
{
  string date = (CDateTime::GetCurrentDateTime() - CDateTimeSpan(2, 0, 0, 0)).GetAsDBDateTime();
  string sql = PrepareSQL("UPDATE texture SET lasthashcheck='%s' WHERE url='%s'", date.c_str(), url.c_str());
  return ExecuteQuery(sql);
}

string CTextureDatabase::GetTextureForPath(const string &url, const string &type)
{
  try
  {
    ...
    string sql = PrepareSQL("select texture from path where url='%s' and type='%s'", url.c_str(), type.c_str());
    m_pDS->query(sql.c_str());

    if (!m_pDS->eof())
    { // have some information
      string texture = m_pDS->fv(0).get_asString();
      m_pDS->close();
      return texture;
    }
    m_pDS->close();
...
void CTextureDatabase::SetTextureForPath(const string &url, const string &type, const string &texture)
{
  try
  {
    if (NULL == m_pDB.get()) return;
    ...
    string sql = PrepareSQL("select id from path where url='%s' and type='%s'", url.c_str(), type.c_str());
    m_pDS->query(sql.c_str());
    if (!m_pDS->eof())
    { // update
...
void CTextureDatabase::ClearTextureForPath(const string &url, const string &type)
{
  try
  {
    if (NULL == m_pDB.get()) return;
    ...
    string sql = PrepareSQL("DELETE FROM path WHERE url='%s' and type='%s'", url.c_str(), type.c_str());
    m_pDS->exec(sql.c_str());
  }
  catch (...)
  {
