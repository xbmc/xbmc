...
VECCREGEXP CompileRegexesFromXML(TiXmlElement* pFolderStacking)
{
  std::vector<std::string> patterns;
  CAdvancedSettings::GetCustomRegexps(pFolderStacking, patterns);
...
void CAdvancedSettings::ParseSettingsFile(const std::string &file)
{
  ...
  if (pVideoStacking)
  {
    auto regexes = CompileRegexesFromXML(pVideoStacking);
    regexes.erase(std::remove_if(regexes.begin(), regexes.end(), validateVideoStackRegex),
                  regexes.end());
    std::move(regexes.begin(), regexes.end(), std::back_inserter(m_videoStackRegExps));
  }
  ...
  if (pFolderStacking)
  {
    auto regexes = CompileRegexesFromXML(pFolderStacking);
    std::move(regexes.begin(), regexes.end(), std::back_inserter(m_folderStackRegExps));
  }
