...
class QueueMediaAsync : public CJob
{
  public:
    ...
    bool DoWork()
    {
      ...
      CPlayList& Playlist = CServiceBroker::GetPlaylistPlayer().GetPlaylist(m_iPlaylist);
      ...
      for (int i = 0; i < m_vecItems->Size(); i++)
      {
        ...
        // Perform Playlist refresh every 50 items
        if ( i % 50 == 0 )
        {
        ...
        {
          if (Playlist.GetItem(j)->IsSamePath(nItem.get()))
          {
...
bool CGUIMediaWindow::OnPlayAndQueueMedia(const CFileItemPtr &item, std::string player)
{
  ...
  if (iPlaylist != PLAYLIST_NONE)
  {
    ...
    QueueMediaAsyncJobID = CJobManager::GetInstance().AddJob(job, this, CJob::PRIORITY_NORMAL);
  }
