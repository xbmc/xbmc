...
{
  m_iCurWeather = g_guiSettings.GetInt("weather.currentlocation") - 1; // weather.currentlocation is one-indexed
  ...
  case GUI_MSG_CLICKED:
    {
      ...
      else if (iControl == CONTROL_SELECTLOCATION)
      {
        ...
        g_guiSettings.SetInt("weather.currentlocation", m_iCurWeather + 1);
        g_settings.Save();
        Refresh();
      }
    }
