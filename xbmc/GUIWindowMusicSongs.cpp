...
#define SORT_LABEL			0x00001
#define SORT_DATE				0x00002
#define SORT_SIZE				0x00004
#define SORT_TRACK			0x00008
#define SORT_DURATION		0x00010
#define SORT_TITEL			0x00020
#define SORT_ARTIST			0x00040
#define SORT_ALBUM			0x00080
#define SORT_FILENAME		0x00100

class CSortMusicSongs
{
public:
	CSortMusicSongs(int iSortMethods)
	{
		m_iSortMethods=iSortMethods;
	}
	~CSortMusicSongs(){};
	bool operator()(CFileItem* pStart, CFileItem* pEnd)
	{
	   ...
	   if (rpStart.m_bIsFolder == rpEnd.m_bIsFolder)
		{
			...
			switch (m_iCurrentSortMethod) 
			{
				case SORT_LABEL:	//	Sort by Listlabel
					strcpy(szfilename1, rpStart.GetLabel().c_str());
					strcpy(szfilename2, rpEnd.GetLabel().c_str());
				break;

				case SORT_DATE: // Sort by Date
          if ( rpStart.m_stTime.wYear > rpEnd.m_stTime.wYear ) return bGreater;
					if ( rpStart.m_stTime.wYear < rpEnd.m_stTime.wYear ) return !bGreater;
        ...
        case SORT_SIZE:	//	Sort by Size
          if ( rpStart.m_dwSize > rpEnd.m_dwSize) return bGreater;
					if ( rpStart.m_dwSize < rpEnd.m_dwSize) return !bGreater;
					return true;
        break;

        case SORT_TRACK:	//	Sort by TrackNum
					if ( rpStart.m_musicInfoTag.GetTrackNumber() > rpEnd.m_musicInfoTag.GetTrackNumber()) return bGreater;
					if ( rpStart.m_musicInfoTag.GetTrackNumber() < rpEnd.m_musicInfoTag.GetTrackNumber()) return !bGreater;
					return true;
        break;

        case SORT_DURATION:	//	Sort by Duration
					if ( rpStart.m_musicInfoTag.GetDuration() > rpEnd.m_musicInfoTag.GetDuration()) return bGreater;
					if ( rpStart.m_musicInfoTag.GetDuration() < rpEnd.m_musicInfoTag.GetDuration()) return !bGreater;
					return true;
        break;

        case SORT_TITEL:	//	Sort by Title
 					strcpy(szfilename1, rpStart.m_musicInfoTag.GetTitle());
					strcpy(szfilename2, rpEnd.m_musicInfoTag.GetTitle());
        break;

				case SORT_ARTIST:	//	Sort by Artist
 					strcpy(szfilename1, rpStart.m_musicInfoTag.GetArtist());
					strcpy(szfilename2, rpEnd.m_musicInfoTag.GetArtist());
				break;

        case SORT_ALBUM:	//	Sort by Album
 					strcpy(szfilename1, rpStart.m_musicInfoTag.GetAlbum());
					strcpy(szfilename2, rpEnd.m_musicInfoTag.GetAlbum());
        break;

        case SORT_FILENAME:	//	Sort by FileName
 					strcpy(szfilename1, rpStart.m_strPath);
					strcpy(szfilename2, rpEnd.m_strPath);
        break;
	...
	void SetState()
	{
		//switch (m_iCurrentSortMethod) 
		//{
			m_iCurrentSortMethod=SORT_LABEL;	//	Sort by Listlabel
			//break;

			m_iCurrentSortMethod=SORT_DATE; // Sort by Date
			//break;

      m_iCurrentSortMethod=SORT_SIZE;	//	Sort by Size
      //break;

      m_iCurrentSortMethod=SORT_TRACK;	//	Sort by TrackNum
      //break;

      m_iCurrentSortMethod=SORT_DURATION;	//	Sort by Duration
      //break;

      m_iCurrentSortMethod=SORT_TITEL;	//	Sort by Title
      //break;

			m_iCurrentSortMethod=SORT_ARTIST;	//	Sort by Artist
			//break;

      m_iCurrentSortMethod=SORT_ALBUM;	//	Sort by Album
      //break;

      m_iCurrentSortMethod=SORT_FILENAME;	//	Sort by FileName
      //break;
...
protected:
	int m_iCurrentSortMethod;
	int m_iSortMethods;
	int m_bSortAscending;
	CStdString m_strDirectory;
	int		m_SortState;  // Status des Sorts mehrstufig, ersetzt m_strDirecoty.Empty()
};

class CSortState
{
public:
	CSortState(int iEnabledSortMethods)
	{
		m_iEnabledSortMethods=iEnabledSortMethods;
	}

	~CSortState(){}
	int m_iEnabledSortMethods;
	int m_iSortMethod;
	int m_bSortAscending;
};
