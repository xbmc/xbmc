...
+  // Actions execute in master-profile service context before settings->Load()
+  // switches the active profile's settings, language, and services.
...
|------|-----------|--------|------------|
| `FlushDeferredActionsNow()` called on non-login path by accident | Very Low | Low | The call site in `LoadProfile` is only reached after both the bounds-check guard (`if (index >= m_profiles.size()) return false`) and the duplicate-profile guard (`if (m_currentProfile == index) return true`) have passed — only reachable on valid, actual profile switches |
