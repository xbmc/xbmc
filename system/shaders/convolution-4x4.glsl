...
vec4 line (float ypos, float xf, vec4 linetaps)
{
  ...
  pixels  = pixel(-1.0 - xf, ypos) * linetaps.r;
  pixels += pixel(      -xf, ypos) * linetaps.g;
  pixels += pixel( 1.0 - xf, ypos) * linetaps.b;
  pixels += pixel( 2.0 - xf, ypos) * linetaps.a;
...
void main()
{
  ...
  gl_FragColor  = line(-1.0 - yf, xf, linetaps) * columntaps.r;
  gl_FragColor += line(      -yf, xf, linetaps) * columntaps.g;
  gl_FragColor += line( 1.0 - yf, xf, linetaps) * columntaps.b;
  gl_FragColor += line( 2.0 - yf, xf, linetaps) * columntaps.a;
