...
retrieveLatestNVIDIAURL()
{
	...
	driverPageURL=$(curl -f -s "http://www.nvidia.com/Download/processDriver.aspx?psid=63&pfid=431&rpf=1&osid=11&lid=1&lang=en-us&dtid=1")
	if [ "$?" -ne "0" ]; then
...
getInstallers()
{
	...
	rm Files/chroot_local-includes/root/NVIDIA*.run &> /dev/null
	...
	rm Files/chroot_local-includes/root/ati*.run  &> /dev/null
