...
def _wait_for_new_screenshot(kodi: KodiInstance, existing: set, timeout: float):
...
def _capture_non_blank_screenshot(kodi: KodiInstance, client: KodiJsonRpcClient, timeout: float):
    deadline = time.monotonic() + timeout
    last_failure = None
    while True:
...
def test_screenshot_renders_non_blank_frame(kodi: KodiInstance):
