...
class KodiProcess:
    ...
    def wait_for_exit(self, timeout: float = 30.0) -> int:
        ...
        If it is still up at the deadline it gets SIGTERM, then SIGKILL, so a hung
        process never leaks and hangs CI, and the reported code is the shell
        convention for the signal that ended it: 143 or 137. Kodi treats SIGTERM as a
        regular quit and exits 0, so the code the process itself returns would read
        as a clean shutdown.
        """
        ...
        try:
            ...
            try:
                ...
                self.process.kill()
                self.process.wait(timeout=15)
                returncode = 128 + 9  # SIGKILL; signal.SIGKILL is missing on Windows
