...
public:
   ...
   ~PLT_ActionDesc();
     ...
     */
   NPT_Array<PLT_ArgumentDesc*>& GetArgumentDescs() { return m_ArgumentDescs; }
...
{
public:
  PLT_GetSCPDXMLIterator(NPT_XmlElementNode* node) : m_Node(node) {}

  NPT_Result operator()(T* const& data) const { return data->GetSCPDXML(m_Node); }
...
public:
    // methods
  PLT_ActionDescNameFinder(const char* name) : m_Name(name) {}
