 ...
 *
 * $Id: searching.c,v 1.36 2005/10/03 21:07:14 mroi Exp $
 *
 */
...
dvdnav_status_t dvdnav_time_search(dvdnav_t *this,
  ...
  {
    printerr("seeking outside time map");
    pthread_mutex_unlock(&this->vm_lock);
    return DVDNAV_STATUS_ERR;
  }
  ...
  {
    const uint64_t target2 = tmap->map_ent[entry+1];
    const uint64_t timeunit = tmap->tmu*90000;
    if( !( target2 & 0x80000000) )
    {
      length = target2 - target;
      target += length * ( time - (entry+1)*timeunit ) / timeunit;
    }
  }

#ifdef LOG_DEBUG
  fprintf(MSG_OUT, "libdvdnav: time_search - looking for sector %x\n", target);
#endif

  /* okey, we now have a sector to find, time to figure out what cell that belongs too */  
  found = 0;
  for(cell_nr = first_cell_nr; cell_nr <= last_cell_nr ; cell_nr ++) {
    cell =  &(state->pgc->cell_playback[cell_nr-1]);    

#ifdef LOG_DEBUG
      fprintf(MSG_OUT, "libdvdnav: time_search - cell %i, start %x, stop %x\n", cell_nr, cell->first_sector, cell->last_sector );
#endif
...
timemapdone:
  ...
  if( !found ) {
    /* not found using time map. try to find using cell's playback time instead */
#ifdef LOG_DEBUG    
    fprintf(MSG_OUT, "libdvdnav: time_search - Time map seeking failed, reverting to cell interpolation");
#endif
