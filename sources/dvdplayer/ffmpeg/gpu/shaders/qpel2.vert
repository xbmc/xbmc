...
void main(void)
{
  ...
  if( (mv_x == 0.25) || (mv_y == 0.25 ) || (mv_x == 0.75) || (mv_y == 0.75) )
  {
  ...
  // Store the full portion in TexCoord 0, and the actual mv in TexCoord1
  // Store the width of a texel in TexCoord2
  gl_TexCoord[0] = (gl_Vertex + (floor(gl_MultiTexCoord0/mv_scale)))/scale;
  gl_TexCoord[1] = fract(gl_MultiTexCoord0/mv_scale);
