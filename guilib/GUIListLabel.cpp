...
void CGUIListLabel::Render()
{
  if (m_label.font && !m_text.IsEmpty())
  {
    ...
    if (m_scrolling && m_renderRect.x2 < m_textWidth + m_renderRect.x1 - std::numeric_limits<float>::epsilon())
      m_label.font->DrawScrollingText(m_renderRect.x1, m_renderRect.y1, m_label.angle, &color, 1,
...
{
  CStdStringW newText;
  g_charsetConverter.utf8ToW(label, newText);
  if (newText != m_text)
